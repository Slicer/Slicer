...
function(gp_resolve_item context item exepath dirs resolved_item_var)
  ...
  if(NOT resolved)
    if(item MATCHES "@rpath")
      ...
      set(ri "")
      if(DEFINED GP_RPATH_DIR
          AND EXISTS "${GP_RPATH_DIR}/${norpath_item}")
        set(ri "${GP_RPATH_DIR}/${norpath_item}")
      else()
        # Avoid find_file(): its macOS framework handling may return an
        # umbrella header with the same name as the framework binary.
        get_filename_component(norpath_item_name "${norpath_item}" NAME)
        set(_gp_search_dirs "${exepath};${dirs}")
        foreach(_gp_search_dir IN LISTS _gp_search_dirs)
          if(EXISTS "${_gp_search_dir}/${norpath_item}")
            set(ri "${_gp_search_dir}/${norpath_item}")
            break()
          endif()
          if(EXISTS "${_gp_search_dir}/${norpath_item_name}")
            set(ri "${_gp_search_dir}/${norpath_item_name}")
            break()
          endif()
        endforeach()
        unset(_gp_search_dir)
        unset(_gp_search_dirs)
      endif()
      if(ri)
        set(resolved 1)
        set(resolved_item "${ri}")
      endif()
      unset(norpath_item_name)
      unset(norpath_item)
      unset(ri)
