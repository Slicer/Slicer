 ...
 *
 * The nonparametric nonuniform intensity normalization (N3) algorithm is a
 * method for correcting nonuniformity associated with MR images. The
 * algorithm assumes a simple parametric model (Gaussian) for the bias field
 ...
 * susceptible to ill-conditioned matrix calculation as is the original
 * proposed fitting component.
 *
 * Notes for the user:
 ...
 *  4. The filter returns the corrected image.  If the bias field is wanted,
 *     one can reconstruct it using the class
 *     itkBSplineControlPointImageFilter. See the IJ article and the test file
 *     for an example.
...
private:
  ...
  N3MRIBiasFieldCorrectionImageFilter( const Self& );
   ...
   */
  typename BiasFieldControlPointLatticeType::Pointer
    m_BiasFieldControlPointLattice;
