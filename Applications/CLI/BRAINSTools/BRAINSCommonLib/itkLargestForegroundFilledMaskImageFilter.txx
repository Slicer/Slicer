...
::GenerateData()
{
    ...
    if ( this->m_OtsuPercentileLowerThreshold < 0.0 || this->m_OtsuPercentileLowerThreshold > 1.5 || this->m_OtsuPercentileUpperThreshold < 0 )
      {
  ...
  if(numNonZeroHistogramBins <= 2 )
    ...
    {
    //## The Otsu thresholding stuff below should not be part of the new class, it shout really be a separate function.
...
#if 1
    //HACK:  The printout below is not what is really being used to threshold.  it is really threshold_low_foreground, NumericTraits<typename OutputImageType::PixelType>::max()
