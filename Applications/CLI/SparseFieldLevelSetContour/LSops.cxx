...
int vprime_idx = -1;
for( int i = 0; i < meshdata->adj[L_z[0]].myNeighbs.size(); i++ ) {
int idx = meshdata->adjimm[L_z[0]].myNeighbs[i];
if( point_type[idx] == 2 ) {// found first neighbor not in L_z
point_type[idx] = -2;
phi[i] = -2.0;
vprime_idx = idx;
break;
...
int len_vis = visited.size();
for( int i = 0; i < visited.size(); i++ ) { // look at all neigbhors of all 
...
ii *= -1;
double cfl = 0.15;
double init_energy = energy->eval_energy( L_z ); // see whether we really decrease this
...
for( int i = 0; i < F.size(); i++ ) phi[ L_z[i] ] += cfl * F[i];
...
for( int k = 0; k < vals.size(); k++ ) { vals[k] = phi[ neigh[k] ];}
phi[idx] = vals.min() + 1.0;
double phi_ = phi[idx];
if( vals.min() > 0.5 ) { // i have no zero neighbors, 
...
double phi_ = phi[idx];
if( vals.max() < -0.5 ) { // i have no zero neighbors, 
