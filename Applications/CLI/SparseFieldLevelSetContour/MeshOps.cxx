...
double mink = meshdata->MeanCurv.min();
double maxk = meshdata->MeanCurv.max();
...
double sumK = 0.0;
int numneigh = meshdata->adjimm[k].myNeighbs.size();
...
{
cout<<"Smoothing gradient of curvature...\n";
int iterations = 3;
vtkCellArray* faces = meshdata->polydata->GetPolys();
vtkPoints*    verts = meshdata->polydata->GetPoints();
int numverts = verts->GetNumberOfPoints();
...
int numneigh = meshdata->adjimm[i].myNeighbs.size();
...
int num_neigh = meshdata->adj[k].myNeighbs.size();
double vec[3];
vector<double> vv(3);
vector<double> vv_(3);
...
vv[1] = vec[1] - thispt[1];
vv[2] = vec[2] - thispt[2];
double t1 = vv[0]*vv[0] + vv[1]*vv[1] + vv[2]*vv[2];
pkmult( vv, rotate1, vv_ );
pkmult( vv_, rotate2, vv );
double t2 = vv[0]*vv[0] + vv[1]*vv[1] + vv[2]*vv[2];
double err = abs(t2 - t1);
xdata[i] = vv[0];
ydata[i] = vv[1];
...
int numPts = pts.size();
if( numPts < 3 ) {
cout<<"Must have at least 3 pts to define closed contour! \n";
return vector<int>(0);
}
vector<int> C(0);
...
cout<<"Error, path finder stuck in a loop. Try another initialization. \n";
return vector<int>(0);
}
...
double v0[3];
double v1[3];
for( int i = 0; i < numfaces; i++ ) { // for every face
...
if( 0 == (i % 10000 ) )
cout<<"Storing immediate neighbors... "<<double(i)/numverts*100<<"% \n";
...
for( int its = 0; its < NUMADJ; its++ ) { // how many levels deep to append
cout<<" adding level "<<its<<" to adjacency...\n";
for( int i = 0; i < numverts; i++ ) { // for every vertex
if( 0 == (i % 10000 ) )
cout<<"Storing next level neighbors... "<<double(i)/numverts*100<<"% \n";
int len = meshdata->adj[i].myNeighbs.size(); // length of my neighbor array
for( int k = 0; k < len; k++ ) { // for every neigbhor index
int idx = meshdata->adj[i].myNeighbs[k];
vector<int>* others = &(meshdata->adj[idx].myNeighbs); // get neighbor's neighbor array
int otherlen = (others)->size(); // length of neighbor's neighbor array
for( int j = 0; j < otherlen; j++ ) { // for every element in neighbor's neigbhor array
int ptId = (*others)[j];
