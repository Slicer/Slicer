...
int vtkITKArchetypeImageSeriesScalarReader::RequestData(vtkInformation* vtkNotUsed(request),
...
# define vtkITKExecuteDataDeclareDICOMImageIO                                           \
   ...
   {                                                                                    \
     vtkErrorMacro(<< "vtkITKExecuteDataFromSeries: Unsupported DICOMImageIOApproach: " \
                   << this->GetDICOMImageIOApproach());                                 \
...
#define vtkITKExecuteDataFromSeries(typeN, type)                                                                   \
  ...
  {                                                                                                                \
    ...
    DownCast<type>(data->GetPointData()->GetScalars())                                                             \
      ->SetVoidArray(ptr, PixelContainer##typeN->Size(), 0, vtkAOSDataArrayTemplate<type>::VTK_DATA_ARRAY_DELETE); \
...
#define vtkITKExecuteDataFromFile(typeN, type)                                                                      \
  ...
  {                                                                                                                 \
    ...
    DownCast<type>(data->GetPointData()->GetScalars())                                                              \
      ->SetVoidArray(ptr, PixelContainer2##typeN->Size(), 0, vtkAOSDataArrayTemplate<type>::VTK_DATA_ARRAY_DELETE); \
  ...
  try
  {
    ...
    if (this->FileNames.size() == 1)
    {
      ...
      {
        vtkErrorMacro(<< "UpdateFromFile: Unsupported number of components (only 1 allowed): "
