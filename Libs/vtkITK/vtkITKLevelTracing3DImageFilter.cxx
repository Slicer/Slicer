                               ...
                               int seed[3])
{
  ...
  // Copy to the output
  memcpy(
    oscalars, tracing->GetOutput()->GetBufferPointer(), tracing->GetOutput()->GetBufferedRegion().GetNumberOfPixels());
}

//
// Contouring filter specialized for volumes and "short int" data values.
//
int vtkITKLevelTracing3DImageFilter::RequestData(vtkInformation* vtkNotUsed(request),
...
# define vtkTemplateMacroCase_ll(typeN, type, call)
  ...
  if (inScalars->GetNumberOfComponents() == 1)
  {
    ...
    {
      vtkTemplateMacro(vtkITKLevelTracing3DTrace(
        this, static_cast<VTK_TT*>(scalars), dims, extent, origin, spacing, (unsigned char*)os, this->Seed));
    } // switch
  }
...
int vtkITKLevelTracing3DImageFilter::RequestInformation(vtkInformation* request,
                                                        vtkInformationVector** inputVector,
                                                        vtkInformationVector* outputVector)
{
