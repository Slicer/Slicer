...
template <class T>
void vtkITKLevelTracingTrace(vtkITKLevelTracingImageFilter* vtkNotUsed(self),
                             T* scalars,
                             int dims[3],
                             int extent[6],
                             double origin[3],
...
//
int vtkITKLevelTracingImageFilter::RequestData(vtkInformation* vtkNotUsed(request), vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
...
# define vtkTemplateMacroCase_ll(typeN, type, call)
  ...
  else if (inScalars->GetNumberOfComponents() == 3)
  {
    ...
    vtkITKLevelTracingTrace(this, (unsigned char*)grayScalars->GetVoidPointer(0), dims, extent, origin, spacing, newPts, newPolys, this->Seed, this->Plane);
  }
  else
  {
...
void vtkITKLevelTracingImageFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  switch (Plane)
  {
    case 2: os << "IJ" << std::endl; break;
    case 1: os << "IK" << std::endl; break;
    case 0: os << "JK" << std::endl; break;
    default: os << "(unknown)" << std::endl; break;
  }
}
