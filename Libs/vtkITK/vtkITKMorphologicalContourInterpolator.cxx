...
template <class T>
void vtkITKMorphologicalContourInterpolatorExecute(vtkITKMorphologicalContourInterpolator* self, vtkImageData* input, vtkImageData* vtkNotUsed(output), T* inPtr, T* outPtr)
{
  ...
  // Copy to the output
  memcpy(outPtr, interpolatorFilter->GetOutput()->GetBufferPointer(), interpolatorFilter->GetOutput()->GetBufferedRegion().GetNumberOfPixels() * sizeof(T));
}
