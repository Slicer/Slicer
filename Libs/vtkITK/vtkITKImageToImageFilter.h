...
public:
  ...
  ///
  /// Pass DebugOn.
  void DebugOn() override { this->m_Process->DebugOn(); };

  ///
  /// Pass DebugOff.
  void DebugOff() override { this->m_Process->DebugOff(); };

  ///
  /// Pass SetNumberOfThreads.
  void SetNumberOfThreads(int val) { this->m_Process->SetNumberOfWorkUnits(val); };

  ///
  /// Pass SetNumberOfThreads.
  int GetNumberOfThreads() { return this->m_Process->GetNumberOfWorkUnits(); };
  ...
  virtual vtkImageData* GetOutput(int idx) { return (vtkImageData*)this->vtkImporter->GetOutput(idx); };

  ///
  /// Set the Input of the filter.
  virtual void SetInput(vtkImageData* Input) { this->vtkCast->SetInputData(Input); };

  void SetInputConnection(vtkAlgorithmOutput* input) override { this->vtkCast->SetInputConnection(input); };

  void SetInputConnection(int port, vtkAlgorithmOutput* input) override { this->vtkCast->SetInputConnection(port, input); };

  ///
  /// Return the input to the filter
  virtual vtkDataObject* GetInput() { return (vtkDataObject::SafeDownCast(this->vtkCast->GetInput())); };
  ...
  {
    return this->vtkCast->Update() && this->vtkImporter->Update(port);
  }
#else
  ...
  void HandleStartEvent() { this->InvokeEvent(vtkCommand::StartEvent, nullptr); };
  void HandleEndEvent() { this->InvokeEvent(vtkCommand::EndEvent, nullptr); };
  /// ETX

protected:
  /// BTX
  /// Dummy ExecuteData
  void ExecuteData(vtkDataObject*) override { vtkWarningMacro(<< "This filter does not respond to Update(). Doing a GetOutput->Update() instead."); }
  /// ETX
