...
void vtkTeemNRRDWriter::vtkImageDataInfoToNrrdInfo(vtkImageData* in,
                                                   int& kind,
                                                   size_t& numComp,
                                                   int& vtkType,
                                                   void** buffer)
...
void* vtkTeemNRRDWriter::MakeNRRD()
{
  ...
  if (nrrdWrap_nva(nrrd, const_cast<void*>(buffer), this->VTKToNrrdPixelType(vtkType), nrrdDim, size)
      || nrrdSpaceDimensionSet(nrrd, spaceDim) || nrrdSpaceOriginSet(nrrd, origin))
  {
  ...
  if (this->DiffusionWeightedData)
  {
    ...
    if (kind[0] == nrrdKindList && numGrad == size[0] && numBValues == size[0])
    {
      ...
      for (unsigned int ig = 0; ig < numGrad; ig++)
      {
        ...
        value_stream << std::setprecision(17) << DoubleConvert(grad[0] * factor) << "   "
                     << DoubleConvert(grad[1] * factor) << "   " << DoubleConvert(grad[2] * factor);
