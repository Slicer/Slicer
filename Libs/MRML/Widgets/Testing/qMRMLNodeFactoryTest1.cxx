...
int qMRMLNodeFactoryTest1(int argc, char* argv[])
{
  ...
  if (nodeFactory.mrmlScene() != nullptr || nodeFactory.createNode("vtkMRMLCameraNode") != nullptr)
  {
...

  {
    ...
    if (createdNode == nullptr || createdNode->IsA("vtkMRMLCameraNode") != 1 || createdNode->GetReferenceCount() != 2
...

  {
    ...
    if (createdSingletonNode == nullptr || createdSingletonNode->IsA("vtkMRMLInteractionNode") != 1
        || createdSingletonNode->GetReferenceCount() != 2 || scene->IsNodePresent(createdSingletonNode) == 0)
    {
    ...
    if (createdSingletonNode2 == nullptr || createdSingletonNode2 != createdSingletonNode
        || createdSingletonNode2->IsA("vtkMRMLInteractionNode") != 1 || createdSingletonNode2->GetReferenceCount() != 2
...

  {
    ...
    if (createdNodeStatic == nullptr || createdNodeStatic->IsA("vtkMRMLCameraNode") != 1
        || createdNodeStatic->GetReferenceCount() != 2 || scene->IsNodePresent(createdNodeStatic) == 0)
    {
      std::cerr << "Line " << __LINE__ << " - qMRMLNodeFactory::createNode() failed.\n"
...

  {
    ...
    if (nodeFactory.attribute("attribute1") != "value1" || (nodeFactory.attribute("attribute2").isNull() != true)
        || (nodeFactory.attribute("attribute0").isNull() != true))
    {
      std::cerr << "Line " << __LINE__ << " - qMRMLNodeFactory::addAttribute failed:"
                ...
                << " attribute0: " << qPrintable(nodeFactory.attribute("attribute0")) << std::endl;
      return EXIT_FAILURE;
    }
  }
