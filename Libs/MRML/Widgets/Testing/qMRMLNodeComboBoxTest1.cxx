...
int qMRMLNodeComboBoxTest1(int argc, char* argv[])
{
  ...
  {
    std::cerr << __LINE__ << " - qMRMLNodeSelector::count() failed: " << nodeSelector.nodeCount() << " nodes." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  */
  ...
  if (nodeSelector.nodeCount() != 1)
  {
    std::cerr << __LINE__ << " - qMRMLNodeSelector: attribute filtering failed, expected 1 node with attribute foo set to bar2. nodeCount = " << nodeSelector.nodeCount()
              << std::endl;
    return EXIT_FAILURE;
  }
