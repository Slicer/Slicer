...
public:
  ...
  vtkSmartPointer<vtkMRMLClipNode>       MRMLClipNode;
...
void qMRMLClipNodeWidget::updateWidgetFromMRML()
{
  ...
  for (QButtonGroup* clipButtonGroup : clipButtonGroups)
  {
    ...
    vtkMRMLNode* clipNode = this->mrmlClipNode()->GetNthClippingNode(nodeIndex);
    if (!clipNode)
    {
      continue;
    }
...
void qMRMLClipNodeWidget::updateClippingNodeFrame()
{
  ...
  {
    vtkMRMLNode* clipNode = nullptr;
    if (i < d->MRMLClipNode->GetNumberOfClippingNodes())
    {
      clipNode = d->MRMLClipNode->GetNthClippingNode(i);
    }
    ...
    for (QObject* object : objectsList)
    {
      ...
      if (widget)
      {
        widget->setEnabled(clipNode != nullptr);
      }

      if (clipNode)
      {
        object->setProperty("ID", QVariant(clipNode->GetID()));
