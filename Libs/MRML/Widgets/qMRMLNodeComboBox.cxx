...
void qMRMLNodeComboBoxPrivate::init(QAbstractItemModel* model)
{
  ...
  {
    QComboBox* const comboBox = this->ComboBox;
    this->ComboBox = nullptr;
    q->setComboBox(comboBox);
  }
...
//-----------------------------------------------------------------------------
void qMRMLNodeComboBoxPrivate::onMRMLSceneEvent(vtkObject* vtk_obj, unsigned long event, void* client_data, void* call_data)
{
  Q_UNUSED(vtk_obj);
  Q_UNUSED(call_data);
...
void qMRMLNodeComboBoxPrivate::setModel(QAbstractItemModel* model)
{
  ...
  q->connect(model, SIGNAL(rowsInserted(QModelIndex, int, int)), q, SLOT(emitNodesAdded(QModelIndex, int, int)));
  q->connect(model, SIGNAL(rowsAboutToBeRemoved(QModelIndex, int, int)), q, SLOT(emitNodesAboutToBeRemoved(QModelIndex, int, int)));
  q->connect(model, SIGNAL(rowsRemoved(QModelIndex, int, int)), q, SLOT(refreshIfCurrentNodeHidden()));
...
vtkMRMLNode* qMRMLNodeComboBoxPrivate::mrmlNode(int row) const
{
  ...
  {
    modelIndex = this->ComboBox->model()->index(row, this->ComboBox->modelColumn(), this->ComboBox->rootModelIndex());
  }
  else
  { // special case where the view can handle a tree... currentIndex could be
    ...
    modelIndex = this->ComboBox->model()->index(row, this->ComboBox->modelColumn(), modelIndex.parent());
  }
  /*
...
vtkMRMLNode* qMRMLNodeComboBoxPrivate::mrmlNodeFromIndex(const QModelIndex& index) const
{
  Q_Q(const qMRMLNodeComboBox);
  Q_ASSERT(q->model());
  const QString nodeId = this->ComboBox->model()->data(index, qMRMLSceneModel::UIDRole).toString();
  if (nodeId.isEmpty())
  {
    return nullptr;
  }
...
{
  return this->ComboBox->model()->match(this->ComboBox->model()->index(0, 0), qMRMLSceneModel::UIDRole, nodeID, 1, Qt::MatchRecursive | Qt::MatchExactly | Qt::MatchWrap);
}
...
void qMRMLNodeComboBoxPrivate::updateActionItems(bool resetRootIndex)
{
  ...
  const int currentIndex = this->ComboBox->currentIndex();
  this->MRMLSceneModel->setPostItems(extraItems, this->MRMLSceneModel->mrmlSceneItem());
  this->ComboBox->setCurrentIndex(currentIndex);

  QObject::connect(this->ComboBox->view(), SIGNAL(clicked(QModelIndex)), q, SLOT(activateExtraItem(QModelIndex)), Qt::UniqueConnection);
  /*
  if (resetRootIndex)
    {
    this->ComboBox->setRootModelIndex(q->model()->index(0, 0));
...
void qMRMLNodeComboBoxPrivate::updateDelegate(bool force)
{
  ...
  if (this->ComboBox->style()->styleHint(QStyle::SH_ComboBox_Popup, &opt, this->ComboBox))
  {
    if (force //
        || qobject_cast<qMRMLNodeComboBoxDelegate*>(this->ComboBox->itemDelegate()))
    {
      this->ComboBox->setItemDelegate(new qMRMLNodeComboBoxMenuDelegate(q->parent(), q->comboBox()));
    }
  }
  else
  {
    ...
    {
      this->ComboBox->setItemDelegate(new qMRMLNodeComboBoxDelegate(q->parent(), q->comboBox()));
    }
  }
}
...
void qMRMLNodeComboBox::activateExtraItem(const QModelIndex& index)
{
  ...
  const QString data = this->model()->data(index, Qt::DisplayRole).toString();
  if (data.startsWith(createNew))
  {
    const QString label = data.mid(createNew.length());
    QString nodeTypeName;
    const bool createAs = data.endsWith(asSuffix);
    ...
    if (!nodeTypeName.isEmpty())
    {
      ...
      {
        vtkMRMLNode* const newNode = this->addNode(nodeTypeName);
        if (newNode != nullptr)
        {
...
//-----------------------------------------------------------------------------
void qMRMLNodeComboBox::addAttribute(const QString& nodeType, const QString& attributeName, const QVariant& attributeValue)
{
  Q_D(qMRMLNodeComboBox);
...
//-----------------------------------------------------------------------------
void qMRMLNodeComboBox::removeAttribute(const QString& nodeType, const QString& attributeName)
{
  Q_D(qMRMLNodeComboBox);
...
{
  Q_D(const qMRMLNodeComboBox);
  vtkMRMLNode* const node = this->currentNode();
  emit this->nodeAboutToBeEdited(node);

  if (!d->InteractionNodeSingletonTag.isEmpty())
  {
    vtkMRMLInteractionNode* interactionNode =
      vtkMRMLInteractionNode::SafeDownCast(this->mrmlScene()->GetSingletonNode(d->InteractionNodeSingletonTag.toUtf8(), "vtkMRMLInteractionNode"));
    if (interactionNode)
    {
...
void qMRMLNodeComboBox::renameCurrentNode()
{
  ...
  bool ok = false;
  const QString currentName = node->GetName();
  const QString newName = getNameFromDialog(renameTitlePrefix + this->nodeTypeLabel(node->GetClassName()), currentName, &ok);
...
{
  const QString label = this->nodeTypeLabel(nodeTypeName);
  bool okPressed = false;
  const QString dialogTitle = createNew + label;
  const QString defaultName = "";
  const QString nodeName = this->getNameFromDialog(dialogTitle, defaultName, &okPressed);
...
void qMRMLNodeComboBox::emitCurrentNodeChanged()
{
  ...
  const int currentIndex = d->ComboBox->currentIndex();
  ...
  if (!node && ((!d->NoneEnabled && currentIndex != -1) || (d->NoneEnabled && currentIndex != 0)))
  {
    ...
    vtkMRMLNode* const newSelectedNode = this->nodeFromIndex(this->nodeCount() - 1);
    if (newSelectedNode)
    {
...
{
  Q_D(qMRMLNodeComboBox);
  vtkMRMLNode* const node = d->mrmlNode(currentIndex);
...
{
  Q_D(const qMRMLNodeComboBox);
  const int extraItemsCount = d->MRMLSceneModel->preItems(d->MRMLSceneModel->mrmlSceneItem()).count() + d->MRMLSceneModel->postItems(d->MRMLSceneModel->mrmlSceneItem()).count();
...
void qMRMLNodeComboBox::setMRMLScene(vtkMRMLScene* scene)
{
  ...
  const QString oldCurrentNode = d->ComboBox->itemData(d->ComboBox->currentIndex(), qMRMLSceneModel::UIDRole).toString();
  const bool previousSceneWasValid = (this->nodeCount() > 0);
...
void qMRMLNodeComboBoxPrivate::setCurrentNodeIDInternal(const QString& nodeID)
{
  ...
  if (indexes.size() == 0)
  {
    ...
    // why we set it here.
    const QModelIndex noneIndex = ctk::modelChildIndex(this->ComboBox->model(), sceneIndex, 0, this->ComboBox->modelColumn());
    this->ComboBox->view()->setCurrentIndex(this->NoneEnabled ? noneIndex : sceneIndex);
  ...
  const QModelIndex oldIndex = this->ComboBox->view()->currentIndex();
...
{
  qMRMLSortFilterProxyModel* const m = this->sortFilterProxyModel();
...
void qMRMLNodeComboBox::setAddEnabled(bool enable)
{
  ...
  {
    qDebug() << "setAddEnabled: An action starting with name " << createNew
             << " already exists. "
...
void qMRMLNodeComboBox::setRemoveEnabled(bool enable)
{
  ...
  {
    qDebug() << "setRemoveEnabled: An action starting with name " << tr("Delete current ")
             << " already exists. "
                "Not enabling this property.";
    return;
  }
...
void qMRMLNodeComboBox::setEditEnabled(bool enable)
{
  ...
  {
    qDebug() << "setEditEnabled: An action starting with name " << tr("Edit current ")
             << " already exists. "
                "Not enabling this property.";
    return;
  }
...
void qMRMLNodeComboBox::setRenameEnabled(bool enable)
{
  ...
  {
    qDebug() << "setRenameEnabled: An action starting with name " << tr("Rename current ")
             << " already exists. "
                "Not enabling this property.";
    return;
  }
...
QList<vtkMRMLNode*> qMRMLNodeComboBox::nodes() const
{
  ...
  {
    vtkMRMLNode* const node = this->nodeFromIndex(i);
    Q_ASSERT(node);
    if (node)
    {
      nodeList << node;
...
void qMRMLNodeComboBox::setComboBox(QComboBox* comboBox)
{
  ...
  QAbstractItemModel* const oldModel = this->model();
  QComboBox* const oldComboBox = d->ComboBox;
  ...
  if (d->ComboBox)
  {
    d->updateDelegate(d->ComboBox->view()->metaObject()->className() == QString("QComboBoxListView"));
  }
}
...
void qMRMLNodeComboBox::emitNodesAdded(const QModelIndex& parent, int start, int end)
{
  ...
  {
    vtkMRMLNode* const node = d->mrmlNodeFromIndex(this->model()->index(start, 0, parent));
    if (node)
    {
      emit nodeAdded(node);
    }
...
void qMRMLNodeComboBox::addMenuAction(QAction* newAction)
{
  ...
  for (QAction* const action : d->UserMenuActions)
  {
    ...
    {
      qDebug() << "addMenuAction: duplicate action text of " << newAction->text() << ", not adding this action";
      return;
    }
  }
  if ((d->AddEnabled && newAction->text().startsWith(createNew)) ||                //
      (d->RemoveEnabled && newAction->text().startsWith(tr("Delete current "))) || //
      (d->EditEnabled && newAction->text().startsWith(tr("Edit current "))) ||     //
      (d->RenameEnabled && newAction->text().startsWith(tr("Rename current "))))
  {
    qDebug() << "addMenuAction: warning: the text on this action, " << newAction->text()
