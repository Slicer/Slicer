...
QModelIndexList qMRMLSceneModelPrivate::indexes(const QString& nodeID) const
{
  ...
  const QModelIndex scene = q->mrmlSceneIndex();
  ...
  QModelIndexList nodeIndexes = q->match(scene, qMRMLSceneModel::UIDRole, nodeID, 1, Qt::MatchExactly | Qt::MatchRecursive);
  ...
  const QModelIndex nodeParentIndex = nodeIndexes[0].parent();
...
{
  Q_Q(qMRMLSceneModel);
  const QModelIndex sceneIndex = q->mrmlSceneIndex();
  const int count = q->rowCount(sceneIndex);
  for (int i = 0; i < count; ++i)
  {
    vtkMRMLNode* const node = q->mrmlNodeFromIndex(ctk::modelChildIndex(q, sceneIndex, i, 0));
...
void qMRMLSceneModelPrivate::removeAllExtraItems(QStandardItem* parent, const QString extraType)
{
  ...
  QModelIndexList indexes = q->match(start, qMRMLSceneModel::UIDRole, extraType, 1, Qt::MatchExactly);
  while (start != QModelIndex() && indexes.size())
  {
    const QModelIndex parentIndex = indexes[0].parent();
...
{
  const QString uid = item ? item->data(qMRMLSceneModel::UIDRole).toString() : QString();
  return uid == "preItem" || uid == "postItem";
}

//------------------------------------------------------------------------------
void qMRMLSceneModelPrivate::reparentItems(QList<QStandardItem*>& children, int newIndex, QStandardItem* newParentItem)
{
  Q_Q(qMRMLSceneModel);
  const int min = q->preItems(newParentItem).count();
  const int max = newParentItem->rowCount() - q->postItems(newParentItem).count();
  const int pos = qMin(min + newIndex, max);
...
void qMRMLSceneModel::setPreItems(const QStringList& extraItems, QStandardItem* parent)
{
  ...
  const QStringList currentExtraItems = d->extraItems(parent, "preItem");
...
void qMRMLSceneModel::setPostItems(const QStringList& extraItems, QStandardItem* parent)
{
  ...
  const QStringList currentExtraItems = d->extraItems(parent, "postItem");
  if (currentExtraItems == extraItems)
  {
    // no change
    return;
...
QStandardItem* qMRMLSceneModel::mrmlSceneItem() const
{
  ...
  for (int i = 0; i < count; ++i)
  {
    QStandardItem* const child = this->invisibleRootItem()->child(i);
    if (!child)
    {
      continue;
    }
    const QVariant uid = child->data(qMRMLSceneModel::UIDRole);
...
{
  QStandardItem* const scene = this->mrmlSceneItem();
  if (scene == nullptr)
  {
    return QModelIndex();
  }
...
vtkMRMLNode* qMRMLSceneModel::mrmlNodeFromItem(QStandardItem* nodeItem) const
{
  ...
  const QVariant nodePointer = nodeItem->data(qMRMLSceneModel::PointerRole);
  if (!nodePointer.isValid() || nodeItem->data(qMRMLSceneModel::UIDRole).toString() == "scene")
  {
    return nullptr;
  }
  return nodeItem ? d->MRMLScene->GetNodeByID(nodeItem->data(qMRMLSceneModel::UIDRole).toString().toUtf8()) : nullptr;
}
//------------------------------------------------------------------------------
QStandardItem* qMRMLSceneModel::itemFromNode(vtkMRMLNode* node, int column) const
{
  const QModelIndex nodeIndex = this->indexFromNode(node, column);
  QStandardItem* const nodeItem = this->itemFromIndex(nodeIndex);
  return nodeItem;
}
...
QModelIndex qMRMLSceneModel::indexFromNode(vtkMRMLNode* node, int column) const
{
  ...
  const QMap<vtkMRMLNode*, QPersistentModelIndex>::iterator rowCacheIt = d->RowCache.find(node);
  ...
  if (rowCacheIt.value().isValid())
  {
    ...
    // then we use it.
    QStandardItem* const nodeItem = this->itemFromIndex(rowCacheIt.value());
    if (nodeItem != nullptr)
    {
  ...
  const QModelIndex nodeParentIndex = nodeIndex.parent();
...
int qMRMLSceneModel::nodeIndex(vtkMRMLNode* node) const
{
  ...
  int index = -1;
  vtkMRMLNode* const parent = this->parentNode(node);
...
//------------------------------------------------------------------------------
bool qMRMLSceneModel::isAffiliatedNode(vtkMRMLNode* nodeA, vtkMRMLNode* nodeB) const
{
...
//------------------------------------------------------------------------------
bool qMRMLSceneModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent)
{
  Q_UNUSED(column);
...
void qMRMLSceneModel::updateScene()
{
  ...
  // could be called ...)
  qvtkDisconnect(nullptr, vtkCommand::ModifiedEvent, this, SLOT(onMRMLNodeModified(vtkObject*)));
  qvtkDisconnect(nullptr, vtkMRMLNode::IDChangedEvent, this, SLOT(onMRMLNodeIDChanged(vtkObject*, void*)));
  ...
  else if (!d->MRMLScene)
  {
    ...
    this->removeRows(preSceneItemCount, this->rowCount() - preSceneItemCount - postSceneItemCount);
  ...
  this->mrmlSceneItem()->setData(QVariant::fromValue(reinterpret_cast<long long>(d->MRMLScene.GetPointer())), qMRMLSceneModel::PointerRole);
  ...
  this->mrmlSceneItem()->removeRows(preNodesItemCount, this->mrmlSceneItem()->rowCount() - preNodesItemCount - postNodesItemCount);
...
QStandardItem* qMRMLSceneModelPrivate::insertNode(vtkMRMLNode* node, int nodeIndex)
{
  ...
  vtkMRMLNode* const parentNode = q->parentNode(node);
  QStandardItem* parentItem = parentNode ? q->itemFromNode(parentNode) : q->mrmlSceneItem();
  if (!parentItem)
  {
    Q_ASSERT(parentNode);
  ...
  const int min = q->preItems(parentItem).count();
  const int max = parentItem->rowCount() - q->postItems(parentItem).count();
...
QStandardItem* qMRMLSceneModel::insertNode(vtkMRMLNode* node, QStandardItem* parent, int row)
{
  ...
  {
    QStandardItem* const newNodeItem = new QStandardItem();
...
{
  qvtkConnect(node, vtkCommand::ModifiedEvent, this, SLOT(onMRMLNodeModified(vtkObject*)));
  qvtkConnect(node, vtkMRMLNode::IDChangedEvent, this, SLOT(onMRMLNodeIDChanged(vtkObject*, void*)));
}
...
void qMRMLSceneModel::updateItemFromNode(QStandardItem* item, vtkMRMLNode* node, int column)
{
  ...
  if (this->canBeAChild(node) && !d->DraggedNodes.contains(node))
  {
    ...
    {
      const int newIndex = this->nodeIndex(node);
...
//------------------------------------------------------------------------------
void qMRMLSceneModel::updateItemDataFromNode(QStandardItem* item, vtkMRMLNode* node, int column)
{
  Q_D(qMRMLSceneModel);
  if (column == this->nameColumn())
  {
  ...
  if (column == this->visibilityColumn())
  {
        ...
        item->data(VisibilityRole).toInt() != visible)
    {
      ...
      switch (visible)
      {
        case 0: item->setIcon(d->HiddenIcon); break;
        case 1: item->setIcon(d->VisibleIcon); break;
        case 2: item->setIcon(d->PartiallyVisibleIcon); break;
        default:
...
{
  const int wasModifying = node->StartModify();
  ...
  vtkMRMLNode* const parent = this->mrmlNodeFromItem(parentItem);
  ...
  {
    QStandardItem* const parentItem = item->parent();
    if (parentItem && desiredNodeIndex < (parentItem->rowCount() - this->postItems(parentItem).count()))
    {
      this->updateItemFromNode(item, node, item->column());
    }
  }
...
//-----------------------------------------------------------------------------
void qMRMLSceneModel::onMRMLSceneEvent(vtkObject* vtk_obj, unsigned long event, void* client_data, void* call_data)
{
  ...
  switch (event)
  {
    ...
    case vtkMRMLScene::NodeRemovedEvent:
      ...
      break;
    case vtkCommand::DeleteEvent: sceneModel->onMRMLSceneDeleted(scene); break;
    case vtkMRMLScene::StartCloseEvent: sceneModel->onMRMLSceneAboutToBeClosed(scene); break;
    case vtkMRMLScene::EndCloseEvent: sceneModel->onMRMLSceneClosed(scene); break;
    case vtkMRMLScene::StartImportEvent: sceneModel->onMRMLSceneAboutToBeImported(scene); break;
...
void qMRMLSceneModel::onMRMLSceneNodeAboutToBeRemoved(vtkMRMLScene* scene, vtkMRMLNode* node)
{
  ...
  const int connectionsRemoved = qvtkDisconnect(node, vtkCommand::ModifiedEvent, this, SLOT(onMRMLNodeModified(vtkObject*)));
...
void qMRMLSceneModel::onMRMLSceneNodeRemoved(vtkMRMLScene* scene, vtkMRMLNode* node)
{
  ...
  for (QList<QStandardItem*> orphans : d->Orphans)
  {
    ...
    vtkMRMLNode* const node = this->mrmlNodeFromItem(orphan);
    const int newIndex = this->nodeIndex(node);
    QStandardItem* newParentItem = this->itemFromNode(this->parentNode(node));
    if (newParentItem == nullptr)
    {
...
void printStandardItem(QStandardItem* item, const QString& offset)
{
  ...
  qDebug() << offset << item << item->index() << item->text() << item->data(qMRMLSceneModel::UIDRole).toString() << item->row() << item->column() << item->rowCount()
           << item->columnCount();
...
{
  char* const oldID = reinterpret_cast<char*>(callData);
...
void qMRMLSceneModel::updateNodeItems(vtkMRMLNode* node, const QString& nodeUID)
{
  ...
  {
    const QModelIndex index = nodeIndexes[i];
    ...
    {
      const int oldSize = nodeIndexes.size();
      nodeIndexes = this->indexes(node);
      const int newSize = nodeIndexes.size();
...
void qMRMLSceneModel::onItemChanged(QStandardItem* item)
{
  ...
  vtkMRMLNode* const mrmlNode = this->mrmlNodeFromItem(item);
  Q_ASSERT(mrmlNode);
  if (mrmlNode == nullptr)
  {
