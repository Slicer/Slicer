...
QStandardItem* qMRMLSceneCategoryModel::itemFromCategory(const QString& category) const
{
  ...
  const int rowCount = this->mrmlSceneItem()->rowCount();
  for (int i = 0; i < rowCount; ++i)
  {
    QStandardItem* const child = this->mrmlSceneItem()->child(i, 0);
    if (child &&                                                          //
...
{
  return this
    ->match(ctk::modelChildIndex(const_cast<qMRMLSceneCategoryModel*>(this), this->mrmlSceneIndex(), 0, 0), qMRMLSceneModel::UIDRole, QString("category"), -1, Qt::MatchExactly)
    .size();
}
...
QStandardItem* qMRMLSceneCategoryModel::insertNode(vtkMRMLNode* node)
{
  ...
  const QString category = QString(node->GetAttribute("Category"));
  ...
  const int max = parentItem->rowCount() - this->postItems(parentItem).count();
...
void qMRMLSceneCategoryModel::updateItemFromNode(QStandardItem* item, vtkMRMLNode* node, int column)
{
  ...
  if (parentItem != nullptr && (parentItem != newParentItem))
  {
    ...
    const int max = newParentItem->rowCount() - this->postItems(newParentItem).count();
    const int pos = max;
...
void qMRMLSceneCategoryModel::updateNodeFromItem(vtkMRMLNode* node, QStandardItem* item)
{
  ...
  const QString category = (parentItem != this->mrmlSceneItem()) ? parentItem->text() : QString();
