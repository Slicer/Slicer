...
void vtkMRMLApplicationLogic::PropagateTableSelection()
{
  ...
  for (int i = 0; i < nnodes; i++)
  {
    vtkMRMLTableViewNode* tnode =
      vtkMRMLTableViewNode::SafeDownCast(this->GetMRMLScene()->GetNthNodeByClass(i, "vtkMRMLTableViewNode"));
...
void vtkMRMLApplicationLogic::PropagatePlotChartSelection()
{
  ...
  for (int i = 0; i < nnodes; i++)
  {
    vtkMRMLPlotViewNode* pnode =
      vtkMRMLPlotViewNode::SafeDownCast(this->GetMRMLScene()->GetNthNodeByClass(i, "vtkMRMLPlotViewNode"));
...
//----------------------------------------------------------------------------
void vtkMRMLApplicationLogic::FitSliceToContent(bool all,
                                                ...
                                                bool resetOrientation /* =true */)
{
  ...
  for (this->Internal->SliceLogics->InitTraversal(it);
       (sliceLogic = vtkMRMLSliceLogic::SafeDownCast(this->Internal->SliceLogics->GetNextItemAsObject(it)));)
  {
...
//----------------------------------------------------------------------------
void vtkMRMLApplicationLogic::FitSliceToBackground(bool onlyIfPropagateVolumeSelectionAllowed /* =false */,
                                                   bool resetOrientation /* =true */)
{
  ...
  if (mrmlFile.empty())
  {
    if (userMessages)
    {
      userMessages->AddMessage(vtkCommand::ErrorEvent,
                               "Could not unpack file '" + std::string(sdbFilePath) + "' into '"
...
//----------------------------------------------------------------------------
std::string vtkMRMLApplicationLogic::CreateUniqueFileName(const std::string& filename,
...
int vtkMRMLApplicationLogic::LoadDefaultParameterSets(vtkMRMLScene* scene, const std::vector<std::string>& directories)
{
  ...
  for (unsigned int d = 0; d < directories.size(); d++)
  {
    ...
    // vtkDebugMacro("\nLoadDefaultParameterSets: checking for parameter sets in dir " << d << " = " <<
    // dirString.c_str());
    ...
    else
    {
      while ((dirp = readdir(dp)) != nullptr)
      {
        ...
        {
          // vtkDebugMacro("\nAdding " << fileToCheck.c_str() << " to list of potential parameter sets. Type = " <<
          // fileType);
          filesToLoad.push_back(fileToCheck);
        }
        else
        {
          // vtkDebugMacro("\nSkipping potential parameter set " << fileToCheck.c_str() << ", file type = " <<
          // fileType);
        }
  ...
  vtkMRMLApplicationLogic::IntersectingSlicesOperation operation)
{
  ...
  vtkMRMLSliceDisplayNode* sliceDisplayNode =
    vtkMRMLSliceDisplayNode::SafeDownCast(scene->GetFirstNodeByClass("vtkMRMLSliceDisplayNode"));
  if (!sliceDisplayNode)
  {
