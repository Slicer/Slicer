...
void vtkMRMLSliceLogic::UpdateSliceCompositeNode()
{
  ...
  if (!this->SliceCompositeNode)
  {
    if (!updatedSliceCompositeNode && !layoutName.empty())
    {
      ...
      updatedSliceCompositeNode = vtkSmartPointer<vtkMRMLSliceCompositeNode>::Take(
...
void vtkMRMLSliceLogic::OnMRMLNodeModified(vtkMRMLNode* node)
{
  ...
  if (node == this->SliceNode)
  {
    ...
    for (int additionalLayerIndex = 0; additionalLayerIndex < this->SliceCompositeNode->GetNumberOfAdditionalLayers();
...
void vtkMRMLSliceLogic::SetNthLayer(int layerIndex, vtkMRMLSliceLayerLogic* layer)
{
  ...
  if (layer)
  {
    ...
    vtkEventBroker::GetInstance()->AddObservation(
      layer, vtkCommand::ModifiedEvent, this, this->GetMRMLLogicsCallbackCommand());
  }
  this->Modified();
}
...
//----------------------------------------------------------------------------
bool vtkMRMLSliceLogic::UpdateFractions(BlendPipeline* pipeline,
                                        const std::vector<vtkAlgorithmOutput*>& imagePorts,
                                        const std::vector<double>& opacities)
{
  if (!pipeline)
  ...
  for (int index = 1, stageIndex = 0; index < static_cast<int>(imagePorts.size()); ++index, ++stageIndex)
  {
    ...
    if (!pipeline->FractionMaths[stageIndex].GetPointer())
    {
      vtkErrorWithObjectMacro(
        nullptr, << "UpdateFractions: Failed to get Nth FractionMath Pipeline filter (N=" << stageIndex << ").");
      continue;
    }
...
void vtkMRMLSliceLogic::UpdatePipeline()
{
  ...
  if (this->SliceNode && this->SliceCompositeNode)
  {
        ...
        || this->SliceCompositeNode->GetCompositing() == vtkMRMLSliceCompositeNode::Subtract)
    {
      ...
      if (vtkMRMLSliceLogic::UpdateAddSubOperation(this->Pipeline->AddSubMath.GetPointer(),
                                                   this->SliceCompositeNode->GetCompositing()))
      {
        modified = 1;
      }
      if (vtkMRMLSliceLogic::UpdateAddSubOperation(this->PipelineUVW->AddSubMath.GetPointer(),
                                                   this->SliceCompositeNode->GetCompositing()))
      {
        modified = 1;
      }
    ...
    if (vtkMRMLSliceLogic::UpdateBlendLayers(
          this->Pipeline->Blend.GetPointer(), layers, this->SliceCompositeNode->GetClipToBackgroundVolume()))
    {
      modified = 1;
    }
    if (vtkMRMLSliceLogic::UpdateBlendLayers(
          this->PipelineUVW->Blend.GetPointer(), layersUVW, this->SliceCompositeNode->GetClipToBackgroundVolume()))
    {
      modified = 1;
    }
...
void vtkMRMLSliceLogic::FitSliceToAll(int width, int height)
{
  ...
  vtkNew<vtkCollection> volumeNodes;
  for (int layer = 0; layer < vtkMRMLSliceLogic::Layer_Last + this->SliceCompositeNode->GetNumberOfAdditionalLayers();
       layer++)
  {
...
std::vector<vtkMRMLDisplayNode*> vtkMRMLSliceLogic::GetPolyDataDisplayNodes()
{
  ...
  if (this->SliceCompositeNode)
  {
    for (int additionalLayerIndex = 0; additionalLayerIndex < this->SliceCompositeNode->GetNumberOfAdditionalLayers();
         ++additionalLayerIndex)
    {
      layerLogics.push_back(
        this->GetNthLayer(vtkMRMLSliceLogic::Layer_Last + this->SliceCompositeNode->GetNumberOfAdditionalLayers()));
    }
  }
