...
public:
  ...
  bool OpenSlicerDataBundle(const char* sdbFilePath,
  ...
  /// Saves the provided image as screenshot of the scene (same filepath as the scene URL but extension is .png instead
  /// of .mrml). Uses current scene's URL property, so the URL must be up-to-date when calling this method.
  void SaveSceneScreenshot(vtkImageData* screenshot);

  /// Pauses rendering for all views in the current layout.
  /// It should be used in situations where multiple nodes are modified, and it is undesirable to display an
  /// intermediate state. The caller is responsible for making sure that each PauseRender() is paired with
  /// ResumeRender().
