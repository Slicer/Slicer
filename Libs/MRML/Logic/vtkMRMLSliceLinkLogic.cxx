...
void vtkMRMLSliceLinkLogic::OnMRMLSceneNodeAdded(vtkMRMLNode* node)
{
  ...
  if (node->IsA("vtkMRMLSliceCompositeNode") || node->IsA("vtkMRMLSliceNode"))
  {
    vtkEventBroker::GetInstance()->AddObservation(node, vtkCommand::ModifiedEvent, this, this->GetMRMLNodesCallbackCommand());
...
void vtkMRMLSliceLinkLogic::OnMRMLNodeModified(vtkMRMLNode* node)
{
      ...
      && this->GetMRMLScene() && !this->GetMRMLScene()->IsBatchProcessing())
  {
    ...
    if (!sliceNode->GetInteracting() || !sliceNode->GetInteractionFlags())
    {
      ...
      if (it != this->SliceNodeInteractionStatus.end() && it->second.Interacting)
      {
        ...
        this->SliceNodeInteractionStatus.find(sliceNode->GetID())->second.Interacting = sliceNode->GetInteracting();
      }
      return;
    }
  ...
  if (compositeNode && this->GetMRMLScene() && !this->GetMRMLScene()->IsBatchProcessing())
  {
    ...
    // if this is not the node that we are interacting with, short circuit
    if (!compositeNode->GetInteracting() || !compositeNode->GetInteractionFlags())
    {
      return;
    }
...
//----------------------------------------------------------------------------
bool vtkMRMLSliceLinkLogic::IsOrientationMatching(vtkMRMLSliceNode* sliceNode1, vtkMRMLSliceNode* sliceNode2, double comparisonTolerance /* = 0.001 */)
{
  ...
  if (!this->GetBroadcastingEvents())
  {
    ...
    for (nodes->InitTraversal(it); (sNode = vtkMRMLSliceNode::SafeDownCast(nodes->GetNextItemAsObject(it)));)
    {
      ...
      if (vtkMRMLSliceLinkLogic::IsOrientationMatching(sliceNode, sNode))
      {
        ...
        if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::SliceToRASFlag)
        {
      ...
      // constant
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::FieldOfViewFlag)
      {
      ...
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::OrientationFlag)
      {
      ...
      if ((sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::ResetOrientationFlag))
      {
      ...
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::MultiplanarReformatFlag)
      {
        this->BroadcastLastRotation(sliceNode, sNode);
      }

      // Setting the label outline mode
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::LabelOutlineFlag)
      {
      ...
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::SliceSpacingFlag)
      {
      ...
      if (sliceNode->GetInteractionFlags() & sliceNode->GetInteractionFlagsModifier() & vtkMRMLSliceNode::UpdateSlabReconstructionThicknessFlag)
      {
...
void vtkMRMLSliceLinkLogic::BroadcastSliceCompositeNodeEvent(vtkMRMLSliceCompositeNode* sliceCompositeNode)
{
  ...
  if (!this->GetBroadcastingEvents() && sliceCompositeNode->GetInteracting())
  {
    ...
    for (nodes->InitTraversal(it); (cNode = vtkMRMLSliceCompositeNode::SafeDownCast(nodes->GetNextItemAsObject(it)));)
    {
      ...
      if (sliceCompositeNode->GetInteractionFlags() & sliceCompositeNode->GetInteractionFlagsModifier() & vtkMRMLSliceCompositeNode::ForegroundVolumeFlag)
      {
      ...
      if (sliceCompositeNode->GetInteractionFlags() & sliceCompositeNode->GetInteractionFlagsModifier() & vtkMRMLSliceCompositeNode::BackgroundVolumeFlag)
      {
        cNode->SetBackgroundVolumeID(sliceCompositeNode->GetBackgroundVolumeID());
      }

      // Labelmap selection
      if (sliceCompositeNode->GetInteractionFlags() & sliceCompositeNode->GetInteractionFlagsModifier() & vtkMRMLSliceCompositeNode::LabelVolumeFlag)
      {
        cNode->SetLabelVolumeID(sliceCompositeNode->GetLabelVolumeID());
      }

      // Foreground opacity
      if (sliceCompositeNode->GetInteractionFlags() & sliceCompositeNode->GetInteractionFlagsModifier() & vtkMRMLSliceCompositeNode::ForegroundOpacityFlag)
      {
        cNode->SetForegroundOpacity(sliceCompositeNode->GetForegroundOpacity());
      }

      // Labelmap opacity
      if (sliceCompositeNode->GetInteractionFlags() & sliceCompositeNode->GetInteractionFlagsModifier() & vtkMRMLSliceCompositeNode::LabelOpacityFlag)
      {
