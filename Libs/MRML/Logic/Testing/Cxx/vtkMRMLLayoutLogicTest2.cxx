...
int vtkMRMLLayoutLogicTest2(int, char*[])
{
  ...
  {
    std::cerr << __LINE__ << " Wrong number of views returned:" << logic->GetViewNodes()->GetNumberOfItems()
              << std::endl;
    return EXIT_FAILURE;
  }
      ...
      || logic->GetViewNodes()->GetItemAsObject(3) != yellowSliceNode.GetPointer())
  {
    std::cerr << __LINE__ << " Wrong nodes returned: "
              ...
              << vtkMRMLNode::SafeDownCast(logic->GetViewNodes()->GetItemAsObject(3))->GetID() << " " << std::endl
              << "  instead of :" << viewNode->GetID() << " " << redSliceNode->GetID() << " " << greenSliceNode->GetID()
              << " " << yellowSliceNode->GetID() << " "
              << "Pointers: " << logic->GetViewNodes()->GetItemAsObject(0) << " "
              << logic->GetViewNodes()->GetItemAsObject(1) << " " << logic->GetViewNodes()->GetItemAsObject(2) << " "
              << logic->GetViewNodes()->GetItemAsObject(3) << " " << std::endl
              << "  instead of :" << viewNode.GetPointer() << " " << redSliceNode.GetPointer() << " "
              << yellowSliceNode.GetPointer() << " " << greenSliceNode.GetPointer() << " " << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (logic->GetViewNodes()->GetItemAsObject(0) != viewNode.GetPointer())
  {
    std::cerr << __LINE__ << " Wrong node returned:" << logic->GetViewNodes()->GetItemAsObject(0) << std::endl;
    return EXIT_FAILURE;
  }
