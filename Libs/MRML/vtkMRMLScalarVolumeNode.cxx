...
Program:   3D Slicer
Module:    $RCSfile: vtkMRMLScalarVolumeNode.cxx,v $
...
//----------------------------------------------------------------------------
const char* vtkMRMLScalarVolumeNode::GetFileScalarTypeAsString()
{
...
//----------------------------------------------------------------------------
void vtkMRMLScalarVolumeNode::ReadData()
{
  if (this->ImageData) {
    this->ImageData->Delete();
    this->ImageData = NULL;
  ...
  if (fullName == NULL) {
    vtkErrorMacro("vtkMRMLScalarVolumeNode: File name not specified");
  }
...
void vtkMRMLScalarVolumeNode::WriteData()
{
  vtkErrorMacro("NOT IMPLEMENTED YET");
}
...
//----------------------------------------------------------------------------
void vtkMRMLScalarVolumeNode::SetIjkToRasDirections(double dirs[9])
{
  for (int i=0; i<9; i++) {
...
void vtkMRMLScalarVolumeNode::SetIjkToRasDirections(double ir, double ia, double is,
...
void vtkMRMLScalarVolumeNode::SetIToRasDirection(double ir, double ia, double is)
{
  IjkToRasDirections[0] = ir;
  IjkToRasDirections[1] = ia;
  IjkToRasDirections[2] = is;
...
void vtkMRMLScalarVolumeNode::SetJToRasDirection(double jr, double ja, double js)
{
  IjkToRasDirections[3] = jr;
  IjkToRasDirections[4] = ja;
  IjkToRasDirections[5] = js;
...
void vtkMRMLScalarVolumeNode::SetKToRasDirection(double kr, double ka, double ks)
{
  IjkToRasDirections[6] = kr;
  IjkToRasDirections[7] = ka;
  IjkToRasDirections[8] = ks;
...
void vtkMRMLScalarVolumeNode::GetIjkToRasDirections(double dirs[9])
{
  for (int i=0; i<9; i++) {
...
void vtkMRMLScalarVolumeNode::GetIToRasDirection(double dirs[3])
{
  for (int i=0; i<3; i++) {
    dirs[i] = IjkToRasDirections[i];
  }
...
void vtkMRMLScalarVolumeNode::GetJToRasDirection(double dirs[3])
{
  for (int i=0; i<3; i++) {
...
void vtkMRMLScalarVolumeNode::GetKToRasDirection(double dirs[3])
{
  for (int i=0; i<3; i++) {
...
double* vtkMRMLScalarVolumeNode::GetIjkToRasDirections()
{
...
void vtkMRMLScalarVolumeNode::GetIjkToRasMatrix(vtkMatrix4x4* mat)
{
  mat->Identity();
  int i=0;
  for (int row=0; row<3; row++) {
...
void vtkMRMLScalarVolumeNode::SetIjkToRasMatrix(vtkMatrix4x4* mat)
{
  int i=0;
  for (int row=0; row<3; row++) {
    for (int col=0; col<3; col++) {
...
//----------------------------------------------------------------------------
void vtkMRMLScalarVolumeNode::ComputeIjkToRasFromScanOrder(char *order, vtkMatrix4x4 *IjkToRas)
{
...
//----------------------------------------------------------------------------
const char* vtkMRMLScalarVolumeNode::ComputeScanOrderFromIjkToRas(vtkMatrix4x4 *ijkToRas)
{
  ...
  case 2:
    ...
    break;
  default:
    cerr << "vtkMRMLScalarVolumeNode::ComputeScanOrderFromRasToIjk:\n\tMax components "<< max_comp << " not in valid range 0,1,2\n";
    return "";
  }        
