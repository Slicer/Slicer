...
void vtkDataFileFormatHelper::PopulateITKSupportedWriteFileTypes()
{
  ...
  for (int idx = 0; idx < numFiles; idx++)
  {
    ...
    {
      // vtkWarningMacro("PopulateITKSupportedWriteFileTypes: USE_TEMP_ITK_FILEFORMAT_TABLE adding a format " <<
      // structFileFormat.ClassName << ", " << structFileFormat.GenericName << ", " << structFileFormat.Extension);
      this->AddSupportedWriterFileFormat(structFileFormat);
    }
    else
    {
      // vtkWarningMacro("PopulateITKSupportedWriteFileTypes: NOT adding a format with classname " <<
      // structFileFormat.ClassName << " and ext " << structFileFormat.Extension << " as it's already in either
      // ITKIOclassNames or one of the SupportedWriteFile arrays. In SupportedWriteFileClassNames = " <<
...
{
  if (!this->SupportedWriteFileExtensions || !this->SupportedWriteFileGenericNames
...
{
  if (!this->SupportedWriteFileExtensions || !this->SupportedWriteFileClassNames)
  {
    this->InitializeITKSupportedFileFormats();
  }
  if (this->SupportedWriteFileExtensions->GetNumberOfValues() == 0
