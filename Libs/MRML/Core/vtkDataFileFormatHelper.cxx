...
ITKImageFileFormat FileFormatTable[] = { // --- Preferred extensions ---
...
void vtkDataFileFormatHelper::PopulateITKSupportedWriteFileTypes()
{
  ...
  while (itr != allobjects.end())
  {
    ...
    else
    {
      ...
      while (writeItr != writeExtensions.end())
      {
        ...
        ITKImageFileFormat structFileFormat = { io->GetNameOfClass(), io->GetNameOfClass(), io->GetNameOfClass(), writeItr->c_str() };
  ...
  for (int idx = 0; idx < numFiles; idx++)
  {
    ...
    {
      // vtkWarningMacro("PopulateITKSupportedWriteFileTypes: USE_TEMP_ITK_FILEFORMAT_TABLE adding a format " << structFileFormat.ClassName << ", " << structFileFormat.GenericName
      // << ", " << structFileFormat.Extension);
      this->AddSupportedWriterFileFormat(structFileFormat);
    }
    else
    {
      // vtkWarningMacro("PopulateITKSupportedWriteFileTypes: NOT adding a format with classname " << structFileFormat.ClassName << " and ext " << structFileFormat.Extension << "
      // as it's already in either ITKIOclassNames or one of the SupportedWriteFile arrays. In SupportedWriteFileClassNames = " <<
      // this->SupportedWriteFileClassNames->LookupValue(structFileFormat.ClassName) << ", in generic names = " <<
      // this->SupportedWriteFileGenericNames->LookupValue(structFileFormat.GenericName) << ", in extensions = " <<
      // this->SupportedWriteFileExtensions->LookupValue(structFileFormat.Extension));
    }
  }
#endif
}
...
//----------------------------------------------------------------------------
std::string vtkDataFileFormatHelper::GetFileExtensionFromFormatString(const char* format)
{
...
//----------------------------------------------------------------------------
const char* vtkDataFileFormatHelper::GetITKSupportedExtensionGenericNameByIndex(int idx)
{
  if (!this->SupportedWriteFileExtensions || //
      !this->SupportedWriteFileGenericNames)
  {
  ...
  if (idx < 0 || this->SupportedWriteFileGenericNames->GetNumberOfTuples() == 0 || //
      ...
      this->SupportedWriteFileExtensions->GetNumberOfTuples() != this->SupportedWriteFileGenericNames->GetNumberOfTuples())
  {
...
//----------------------------------------------------------------------------
const char* vtkDataFileFormatHelper::GetITKSupportedExtensionClassNameByIndex(int idx)
{
  if (!this->SupportedWriteFileExtensions || //
      !this->SupportedWriteFileClassNames)
  {
  ...
  if (idx < 0 || this->SupportedWriteFileClassNames->GetNumberOfTuples() == 0 || //
      ...
      this->SupportedWriteFileExtensions->GetNumberOfTuples() != this->SupportedWriteFileClassNames->GetNumberOfTuples())
  {
...
//----------------------------------------------------------------------------
void vtkDataFileFormatHelper::AddSupportedWriterFileFormat(ITKImageFileFormat& structFileFormat)
{
