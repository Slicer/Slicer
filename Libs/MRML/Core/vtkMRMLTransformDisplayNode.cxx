  ...
  : vtkMRMLDisplayNode()
{
  ...
  this->AddNodeReferenceRole(GlyphPointsReferenceRole,
                             GlyphPointsReferenceRole,
                             nullptr,
...
void vtkMRMLTransformDisplayNode::WriteXML(ostream& of, int nIndent)
{
  ...
  vtkMRMLWriteXMLVectorMacro(
    TranslationHandleComponentVisibilitySlice, TranslationHandleComponentVisibilitySlice, bool, 4);
...
void vtkMRMLTransformDisplayNode::ProcessMRMLEvents(vtkObject* caller, unsigned long event, void* callData)
{
  if (caller != nullptr
      ...
      && caller == GetRegionNode() && this->Visibility)
  {
  ...
  else if (caller != nullptr && caller == GetGlyphPointsNode() // event can be any content modified event
           && this->VisualizationMode == VIS_MODE_GLYPH && (this->Visibility || this->GetVisibility2D()))
  {
    // update visualization if glyph points are changed
    this->Modified();
  }
  else if (caller != nullptr && event == vtkCommand::ModifiedEvent && caller == GetColorNode())
  {
...
const char* vtkMRMLTransformDisplayNode::ConvertGlyphTypeToString(int modeIndex)
{
  switch (modeIndex)
  {
    case GLYPH_TYPE_ARROW:
      return "ARROW";
    case GLYPH_TYPE_CONE:
      return "CONE";
    case GLYPH_TYPE_SPHERE:
      return "SPHERE";
    default:
      return "";
  }
...
//----------------------------------------------------------------------------
bool vtkMRMLTransformDisplayNode::IsContourLevelEqual(const std::vector<double>& levels1,
...
vtkColorTransferFunction* vtkMRMLTransformDisplayNode::GetColorMap()
{
  ...
  if (colorNode == nullptr || colorNode->GetColorTransferFunction() == nullptr
