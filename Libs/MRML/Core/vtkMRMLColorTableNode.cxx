  ...
  while (*atts != nullptr)
  {
    ...
    else if (!strcmp(attName, "colors"))
    {
      ...
      for (int i = 0; i < this->LookupTable->GetNumberOfTableValues(); i++)
      {
        ...
        vtkDebugMacro("Adding color at index " << index << ", r = " << r << ", g = " << g << ", b = " << b << ", a = "
                                               << a << " and then setting name to " << name.c_str() << endl);
...
void vtkMRMLColorTableNode::SetType(int type)
{
  ...
  {
    vtkDebugMacro("vtkMRMLColorTableNode::SetType Creating a new lookup table (was null) of type "
                  << this->GetTypeAsString() << "\n");
  ...
  else if (this->Type == this->FMRIPA)
  {
    ...
    this->GetLookupTable()->ForceBuild();
    this->SetNamesFromColors();
    this->SetDescription("A small fMRI positive activation scale going from red to yellow from 0-19, useful for "
                         "displaying functional MRI volumes when don't need the blue of the fMRI scale.");
  }
...
void vtkMRMLColorTableNode::AddColor(const char* name, double r, double g, double b, double a)
{
  if (this->GetType() != this->User && this->GetType() != this->File)
  {
    vtkErrorMacro("vtkMRMLColorTableNode::AddColor: ERROR: can't add a color if not a user defined color table, reset "
                  "the type first to User or File\n");
    return;
  }
  this->LastAddedColor++;
...
int vtkMRMLColorTableNode::SetColor(int entry, const char* name, double r, double g, double b, double a)
{
  if (this->GetType() != this->User && this->GetType() != this->File)
  {
    vtkErrorMacro("vtkMRMLColorTableNode::SetColor: ERROR: can't set a color if not a user defined color table, reset "
                  "the type first to User or File\n");
    return 0;
  }
  if (entry < 0 || entry >= this->GetLookupTable()->GetNumberOfTableValues())
  {
    vtkErrorMacro("vtkMRMLColorTableNode::SetColor: requested entry "
                  << entry << " is out of table range: 0 - " << this->GetLookupTable()->GetNumberOfTableValues()
                  << ", call SetNumberOfColors" << endl);
    return 0;
  }
                                     ...
                                     double a)
{
  if (this->GetType() != this->User && this->GetType() != this->File)
  {
    vtkErrorMacro("vtkMRMLColorTableNode::SetColors: Cannot set a color if not a user defined color table, reset the "
                  "type first to User or File");
    return 0;
  }
...
bool vtkMRMLColorTableNode::GetColor(int entry, double color[4])
{
  ...
  {
    vtkErrorMacro("vtkMRMLColorTableNode::GetColor: requested entry "
                  << entry << " is out of table range: 0 - " << this->GetLookupTable()->GetNumberOfTableValues()
                  << ", call SetNumberOfColors" << endl);
    return false;
  }
