...
void vtkMRMLSequenceNode::WriteXML(ostream& of, int nIndent)
{
  ...
  for (std::deque<IndexEntryType>::iterator indexIt = this->IndexEntries.begin(); indexIt != this->IndexEntries.end();
       ++indexIt)
  {
    ...
    if (indexIt->DataNode == nullptr)
    {
      ...
      else
      {
        vtkErrorMacro("Error while writing node " << (this->GetID() ? this->GetID() : "(unknown)")
                                                  << " to XML: data node is invalid at index value "
                                                  << indexIt->IndexValue);
      }
    }
    else
  ...
  while (std::getline(ss, nodeId_indexValue, ';'))
  {
    ...
    if (indexValueSeparatorPos > 0 && indexValueSeparatorPos != std::string::npos)
    {
      ...
      std::string indexValue =
        nodeId_indexValue.substr(indexValueSeparatorPos + 1, nodeId_indexValue.size() - indexValueSeparatorPos - 1);

      IndexEntryType indexEntry;
      indexEntry.IndexValue = indexValue;
      // The nodes are not read yet, so we can only store the node ID and get the pointer to the node later (in
      // UpdateScene())
...
void vtkMRMLSequenceNode::Copy(vtkMRMLNode* anode)
{
  ...
  this->IndexEntries.clear();
  for (std::deque<IndexEntryType>::iterator sourceIndexIt = snode->IndexEntries.begin();
       sourceIndexIt != snode->IndexEntries.end();
       ++sourceIndexIt)
  {
    ...
    if (seqItem.DataNode == nullptr)
    {
      ...
      std::string targetDataNodeID =
        (mapDataNodeIds ? sourceToTargetDataNodeID[sourceIndexIt->DataNodeID] : sourceIndexIt->DataNodeID);
...
//----------------------------------------------------------------------------
bool vtkMRMLSequenceNode::UpdateDataNodeAtValue(vtkMRMLNode* node,
...
int vtkMRMLSequenceNode::GetInsertPosition(const std::string& indexValue)
{
  ...
  if (this->IndexType == vtkMRMLSequenceNode::NumericIndex && !this->IndexEntries.empty())
  {
    ...
    if (numericIndexValue < foundNumericIndexValue) // Deals with case of index value being smaller than any in the
                                                    // sequence and numeric tolerances
    {
...
bool vtkMRMLSequenceNode::UpdateIndexValue(const std::string& oldIndexValue, const std::string& newIndexValue)
{
  ...
  {
    vtkErrorMacro("vtkMRMLSequenceNode::UpdateIndexValue failed, no data node found with index value "
                  << oldIndexValue);
    return false;
  }
  if (this->GetItemNumberFromIndexValue(newIndexValue) >= 0)
  {
    vtkErrorMacro("vtkMRMLSequenceNode::UpdateIndexValue failed, data node is already defined at index value "
                  << newIndexValue);
    return false;
  }
...
vtkMRMLStorageNode* vtkMRMLSequenceNode::CreateDefaultStorageNode()
{
  ...
  return vtkMRMLStorageNode::SafeDownCast(scene->CreateNodeByClass(this->GetDefaultStorageNodeClassName().c_str()));
}
...
std::string vtkMRMLSequenceNode::GetIndexTypeAsString(int indexType)
{
  switch (indexType)
  {
    case vtkMRMLSequenceNode::NumericIndex:
      return "numeric";
    case vtkMRMLSequenceNode::TextIndex:
      return "text";
    default:
      return "";
  }
...
vtkMRMLNode* vtkMRMLSequenceNode::DeepCopyNodeToScene(vtkMRMLNode* source, vtkMRMLScene* scene)
{
  ...
  // We don't want to copy these tags as they would prevent multiple timepoints from being added to the sequences scene
  // for these nodes. Since the singleton tags are not copied with CopyContent, it is safe to disable them here.
