...
/// - xmlAttributeName: XML attribute name (without quotes), typically the same as the property name but starts with
/// lowercase
...
#define vtkMRMLWriteXMLStdStringMacro(xmlAttributeName, propertyName) \
  xmlWriteOutputStream << " " #xmlAttributeName "=\""                 \
                       << vtkMRMLNode::XMLAttributeEncodeString(Get##propertyName().c_str()) << "\"";
...
#define vtkMRMLPrintStringMacro(propertyName)                  \
  printOutputStream << printOutputIndent << #propertyName ": " \
                    << (this->Get##propertyName() != nullptr ? this->Get##propertyName() : "(none)") << "\n";
