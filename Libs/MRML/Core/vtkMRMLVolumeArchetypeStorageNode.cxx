...
std::string vtkMRMLVolumeArchetypeStorageNode::UpdateFileList(vtkMRMLNode* refNode, bool move)
{
  ...
  std::string tempDir;
  vtkNew<vtkStringArray> fullNames;
  fullNames->InsertNextValue(oldName);
  vtkNew<vtkStringArray> tempFiles;
  if (!this->CreateUniqueTempFilesForWrite(fullNames, tempDir, tempFiles))
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(),
  ...
  std::string tempName = tempFiles->GetValue(0);
  if (tempName.empty())
  {
    vtkErrorToMessageCollectionMacro(
...
void vtkMRMLVolumeArchetypeStorageNode::ConvertSpatialVectorVoxelsBetweenRasLps(vtkImageData* imageData)
{
  ...
  else
  {
    vtkWarningToMessageCollectionMacro(
      ...
      vtkMRMLTr("vtkMRMLVolumeArchetypeStorageNode", "Displacements are expected to be stored as double or float. Vector values will not be converted from LPS to RAS."));
  }
}
