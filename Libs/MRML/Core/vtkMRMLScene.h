...
public:
  ...
  /// \brief This property controls whether Commit() should save the scene
  /// with XML or JSON format.
  ///
  /// If true, the contents generated by Commit() will be in JSON format.
  /// If false, the contents will be saved in XML format
  /// \sa Commit()
  vtkSetMacro(SetUseJSONFormat, int);
  vtkGetMacro(SetUseJSONFormat, int);
...
protected:
  ...
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  bool SaveStorableNodeToSlicerDataBundleDirectory(vtkMRMLStorableNode* storableNode,
                                                   std::string& dataDir,
