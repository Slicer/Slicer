...
vtkMRMLStorageNode* vtkMRMLMarkupsROINode::CreateDefaultStorageNode()
{
  ...
  return vtkMRMLStorageNode::SafeDownCast(scene->CreateNodeByClass("vtkMRMLMarkupsROIJsonStorageNode"));
}
...
void vtkMRMLMarkupsROINode::CreateDefaultDisplayNodes()
{
  ...
  vtkMRMLMarkupsROIDisplayNode* dispNode = vtkMRMLMarkupsROIDisplayNode::SafeDownCast(this->GetScene()->AddNewNodeByClass("vtkMRMLMarkupsROIDisplayNode"));
  if (!dispNode)
  {
...
void vtkMRMLMarkupsROINode::ApplyTransform(vtkAbstractTransform* transform, bool applyToLockedControlPoints)
{
  ...
  const bool wasUpdatingControlPointsFromROI = this->IsUpdatingControlPointsFromROI;
  this->IsUpdatingControlPointsFromROI = true;

  const bool wasUpdatingROIFromControlPoints = this->IsUpdatingROIFromControlPoints;
  ...
  const vtkNew<vtkMatrix4x4> newObjectToNodeMatrix;
...
const char* vtkMRMLMarkupsROINode::GetROITypeAsString(int roiType)
{
  switch (roiType)
  {
    case vtkMRMLMarkupsROINode::ROITypeBox: return "Box";
    case vtkMRMLMarkupsROINode::ROITypeBoundingBox: return "BoundingBox";
    default: break;
  }
  return "";
}
...
void vtkMRMLMarkupsROINode::ProcessMRMLEvents(vtkObject* caller, unsigned long event, void* callData)
{
  ...
  else if (caller == this->InteractionHandleToWorldMatrix.GetPointer() && !this->IsUpdatingInteractionHandleToWorldMatrix)
  {
    ...
    double xAxis_World[3] = { this->InteractionHandleToWorldMatrix->GetElement(0, 0),
    ...
    double yAxis_World[3] = { this->InteractionHandleToWorldMatrix->GetElement(0, 1),
    ...
    double zAxis_World[3] = { this->InteractionHandleToWorldMatrix->GetElement(0, 2),
...
{
  vtkVector3d center(this->ObjectToNodeMatrix->GetElement(0, 3), //
...
{
  vtkVector3d centerWorld(this->ObjectToWorldMatrix->GetElement(0, 3), //
...
void vtkMRMLMarkupsROINode::GetCenter(double center_Node[3])
{
  ...
  const vtkVector3d center = this->GetCenter();
...
void vtkMRMLMarkupsROINode::GetCenterWorld(double center_World[3])
{
  ...
  const vtkVector3d center = this->GetCenterWorld();
...
void vtkMRMLMarkupsROINode::SetSizeWorld(double x_World, double y_World, double z_World)
{
  ...
  const double xAxis_Scale = vtkMath::Norm(xAxis_Node);
  const double yAxis_Scale = vtkMath::Norm(yAxis_Node);
  const double zAxis_Scale = vtkMath::Norm(zAxis_Node);
...
void vtkMRMLMarkupsROINode::UpdateROIFromControlPoints()
{
...

  {
    ...
    switch (this->ROIType)
    {
      case vtkMRMLMarkupsROINode::ROITypeBox: this->UpdateBoxROIFromControlPoints(); break;
      case vtkMRMLMarkupsROINode::ROITypeBoundingBox: this->UpdateBoundingBoxROIFromControlPoints(); break;
      default: break;
    }
...
void vtkMRMLMarkupsROINode::UpdateControlPointsFromROI()
{
...

  {
    ...
    switch (this->ROIType)
    {
      case vtkMRMLMarkupsROINode::ROITypeBox: this->UpdateControlPointsFromBoxROI(); break;
      case vtkMRMLMarkupsROINode::ROITypeBoundingBox: this->UpdateControlPointsFromBoundingBoxROI(); break;
      default: break;
    }
  }
...
void vtkMRMLMarkupsROINode::UpdateControlPointsFromBoundingBoxROI()
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    const double oldSize = bounds_Object[2 * i + 1] - bounds_Object[2 * i];
  ...
  const vtkNew<vtkGeneralTransform> nodeToWorldTransform;
...
{
  const bool wasUpdatingInteractionHandleToWorldMatrix = this->IsUpdatingInteractionHandleToWorldMatrix;
  ...
  for (int j = 0; j < 3; ++j)
  {
    ...
    axis[j] = 1.0;
    const double size = vtkMath::Norm(newInteractionHandleToWorld->MultiplyDoublePoint(axis));
    for (int i = 0; i < 3; ++i)
    {
      const double element = newInteractionHandleToWorld->GetElement(i, j);
...
void vtkMRMLMarkupsROINode::GetPlanes(vtkPlanes* planes, bool insideOut /*=false*/)
{
  ...
  if (insideOut)
  {
    ...
    {
      double* const normal = normals->GetTuple3(i);
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsROINode::GenerateOrthogonalMatrix(double xAxis[3],
                                                     double yAxis[3],
                                                     double zAxis[3],
                                                     double origin[3],
                                                     vtkMatrix4x4* outputMatrix,
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsROINode::WriteCLI(std::vector<std::string>& commandLine, std::string prefix, int coordinateSystem, int vtkNotUsed(multipleFlag))
{
...
{
  vtkMatrix4x4* const objectToNode = this->GetObjectToNodeMatrix();
