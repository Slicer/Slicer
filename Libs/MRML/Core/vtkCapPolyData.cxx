...
//----------------------------------------------------------------------------
void vtkCapPolyData::GetPlanes(vtkImplicitFunction* function,
                                  ...
                                  vtkPolyData* outputEndCap)
{
  ...
  for (int i = 0; i < planes->GetNumberOfItems(); ++i)
  {
    ...
    if (endCapPolys && endCapPolyData->GetNumberOfPolys() > 0)
    {
      ...
      // The normal of the triangles generated by vtkContourTriangulator are based on the clockwise/counter-clockwise
      // direction of the largest contour. This normal will not always line up with the desired normal as defined by the
      // plane, so if the normal generated by vtkContourTriangulator faces the wrong  direction, then we need to flip
      // the normals of the polys so that it matches the expected normal.
      vtkPolygon::ComputeNormal(
        endCapPolyData->GetPoints(), polyPointIds->GetNumberOfIds(), polyPointIds->GetPointer(0), polyNormal);
...
void vtkCapPolyData::UpdateCellTypeArray(vtkPolyData* polyData)
{
  ...
  vtkSmartPointer<vtkIdTypeArray> cellTypes =
    vtkIdTypeArray::SafeDownCast(polyData->GetCellData()->GetArray("CellType"));
  if (!cellTypes)
  {
