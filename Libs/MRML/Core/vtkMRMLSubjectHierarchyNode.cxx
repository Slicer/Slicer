...
//----------------------------------------------------------------------------
void vtkMRMLSubjectHierarchyNode::ReadJSONFromString(const char* json, bool overwriteReferences/*= true*/)
{
  Superclass::ReadJSONFromString(json, overwriteReferences);

  rapidjson::Document document;
  document.Parse(json);
  if (document.HasParseError())
  {
    vtkErrorMacro("ReadJSONFromString: Failed to parse JSON string");
    return;
  }

  std::function<void(const rapidjson::Value&)> processItem;
  processItem = [&](const rapidjson::Value& value) {
    for (rapidjson::Value::ConstMemberIterator itr = value.MemberBegin(); itr != value.MemberEnd(); ++itr)
    {
      if (itr->value.IsObject() && itr->value.HasMember("SubjectHierarchyItem"))
      {
        processItem(itr->value);
      }
      if (std::string(itr->name.GetString()) != "SubjectHierarchyItem")
      {
        continue;
      }

      std::vector<const char*> attributes;
      std::vector<const char*> filteredAttributes;
      char* ossStrCStr = nullptr;
      char* arrayStrCStr = nullptr;
      char* numberStrCStr = nullptr;
      vtkIdType itemID = -1;
      for (rapidjson::Value::ConstMemberIterator itemItr = itr->value.MemberBegin(); itemItr != itr->value.MemberEnd(); ++itemItr)
      {
        if (std::string(itemItr->name.GetString()) == "SubjectHierarchyItem")
        {
          continue;
        }
        else if (std::string(itemItr->name.GetString()) == "attributes")
        {
          const rapidjson::Value& jsonAttributes = itemItr->value;
          ...
          attributes.push_back(itemItr->name.GetString());
          filteredAttributes.push_back(itemItr->name.GetString());
          ...
          filteredAttributes.push_back(ossStrCStr);
        }
        else if (itemItr->value.IsArray())
        {
          std::ostringstream arrayStream;
          for (rapidjson::SizeType i = 0; i < itemItr->value.Size(); i++)
          {
            if (i > 0)
            {
              arrayStream << " ";
            }
            if (itemItr->value[i].IsNumber())
            {
              arrayStream << itemItr->value[i].GetFloat();
            }
            else if (itemItr->value[i].IsString())
            {
              arrayStream << itemItr->value[i].GetString();
            }
          }
          attributes.push_back(itemItr->name.GetString());
          filteredAttributes.push_back(itemItr->name.GetString());
          ...
          filteredAttributes.push_back(arrayStrCStr);
        }
        else
        {
          attributes.push_back(itemItr->name.GetString());
          if (std::string(itemItr->name.GetString()) != "id" &&
            std::string(itemItr->name.GetString()) != "parent" &&
            std::string(itemItr->name.GetString()) != "dataNode")
          {
            filteredAttributes.push_back(itemItr->name.GetString());
          }
          if (itemItr->value.IsString())
          {
            attributes.push_back(itemItr->value.GetString());
            if (std::string(itemItr->name.GetString()) != "id" &&
              std::string(itemItr->name.GetString()) != "parent" &&
              std::string(itemItr->name.GetString()) != "dataNode")
            {
              filteredAttributes.push_back(itemItr->value.GetString());
            }
          }
          else if (itemItr->value.IsBool())
          {
            attributes.push_back(itemItr->value.GetBool() ? "true" : "false");
            if (std::string(itemItr->name.GetString()) != "id" &&
              std::string(itemItr->name.GetString()) != "parent" &&
              std::string(itemItr->name.GetString()) != "dataNode")
            {
              filteredAttributes.push_back(itemItr->value.GetBool() ? "true" : "false");
            }
          }
          else if (itemItr->value.IsNumber())
          {
            if (std::string(itemItr->name.GetString()) == "id")
            {
              itemID = itemItr->value.GetInt();
            }

            std::string numberStr;
            if (itemItr->value.IsInt())
            {
              numberStr = std::to_string(itemItr->value.GetInt());
            }
            else
            {
              numberStr = std::to_string(itemItr->value.GetDouble());
            }
            ...
            if (std::string(itemItr->name.GetString()) != "id" &&
              ...
              std::string(itemItr->name.GetString()) != "dataNode")
            {
              filteredAttributes.push_back(numberStrCStr);
            }
          }
        }
      }
      ...
      if (itemID == -1)
      {
        vtkErrorMacro("ReadJSONFromString: Failed to read item ID");
      }
      else
      {
        ...
        if (!item)
        {
          atts = new const char *[attributes.size() + 1];
          std::copy(attributes.begin(), attributes.end(), atts);
          atts[attributes.size()] = nullptr;
          this->ReadItemFromXML(atts);
        }
        else
        {
          atts = new const char *[filteredAttributes.size() + 1];
          std::copy(filteredAttributes.begin(), filteredAttributes.end(), atts);
          atts[filteredAttributes.size()] = nullptr;
          item->ReadXMLAttributes(atts);
        }
      }
