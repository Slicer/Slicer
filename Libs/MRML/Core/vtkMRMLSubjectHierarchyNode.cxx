...
public:
  ...
  /// \param recursive Flag whether to find only direct children (false) or in the whole branch (true). True by default
  /// \return Item if found, nullptr otherwise
  void FindChildrenByName(std::string name, std::vector<vtkIdType>& foundItemIDs, bool contains = false, bool recursive = true);
...
std::map<vtkIdType, vtkWeakPointer<vtkSubjectHierarchyItem>> vtkSubjectHierarchyItem::ItemCache = std::map<vtkIdType, vtkWeakPointer<vtkSubjectHierarchyItem>>();
std::map<vtkMRMLNode*, vtkWeakPointer<vtkSubjectHierarchyItem>> vtkSubjectHierarchyItem::DataNodeCache = std::map<vtkMRMLNode*, vtkWeakPointer<vtkSubjectHierarchyItem>>();
...
vtkIdType vtkSubjectHierarchyItem::AddToTree(vtkSubjectHierarchyItem* parent, vtkMRMLNode* dataNode)
{
  ...
  {
    // Add under parent
    const vtkSmartPointer<vtkSubjectHierarchyItem> childPointer(this);
...
void vtkSubjectHierarchyItem::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Parent: " << (this->Parent ? this->Parent->ID : vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID) << (this->Parent && !this->Parent->Parent ? " (scene)" : "")
     << "\n";
  os << indent << "Children: ";
...
void vtkSubjectHierarchyItem::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != nullptr)
  {
    ...
    if (!strcmp(attName, "dataNode"))
    {
      if (!this->Name.empty())
      {
        vtkErrorMacro("ReadXMLAttributes: Item cannot have data node and name specified at the same time. " << "Data node is not set for item named " << this->Name);
        continue;
      }
    ...
    if (!strcmp(attName, "name"))
    {
      ...
      {
        vtkErrorMacro("ReadXMLAttributes: Item cannot have data node and name specified at the same time. " << "Name is not set for item for data node with ID "
                                                                                                            << this->TemporaryDataNodeID);
        continue;
      }
    ...
    else if (!strcmp(attName, "uids"))
    {
      ...
      {
        const std::string itemStr = valueStr.substr(0, itemSeparatorPosition);
        const size_t nameValueSeparatorPosition = itemStr.find(vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR);

        const std::string name = itemStr.substr(0, nameValueSeparatorPosition);
        const std::string value = itemStr.substr(nameValueSeparatorPosition + vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR.size());
...
void vtkSubjectHierarchyItem::WriteXML(ostream& of, int nIndent, vtkMRMLNode* xmlEncoderNode)
{
  ...
  if (this->UIDs.size() > 0)
  {
    ...
    for (std::map<std::string, std::string>::iterator uidIt = this->UIDs.begin(); uidIt != this->UIDs.end(); ++uidIt)
    {
      of << xmlEncoderNode->XMLAttributeEncodeString(uidIt->first) << vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR
         << xmlEncoderNode->XMLAttributeEncodeString(uidIt->second) << vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR;
    }
    of << "\"";
  }
  ...
  if (this->Attributes.size() > 0)
  {
    ...
    for (std::map<std::string, std::string>::iterator attIt = this->Attributes.begin(); attIt != this->Attributes.end(); ++attIt)
    {
      of << xmlEncoderNode->XMLAttributeEncodeString(attIt->first) << vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR
         << xmlEncoderNode->XMLAttributeEncodeString(attIt->second) << vtkMRMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR;
    }
    of << "\"";
  }
...
void vtkSubjectHierarchyItem::DeepCopy(vtkSubjectHierarchyItem* item, bool copyChildren /*=true*/)
{
  ...
  if (copyChildren)
  {
    ...
    {
      const vtkSmartPointer<vtkSubjectHierarchyItem> copiedChildItem = vtkSmartPointer<vtkSubjectHierarchyItem>::New();
...
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::FindChildByID(vtkIdType itemID, bool recursive /*=true*/)
{
  ...
  const std::map<vtkIdType, vtkWeakPointer<vtkSubjectHierarchyItem>>::iterator itemIt = vtkSubjectHierarchyItem::ItemCache.find(itemID);
  ...
  for (childIt = this->Children.begin(); childIt != this->Children.end(); ++childIt)
  {
    ...
    if (recursive)
    {
      vtkSubjectHierarchyItem* const foundItemInBranch = currentItem->FindChildByID(itemID);
...
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::FindChildByDataNode(vtkMRMLNode* dataNode, bool recursive /*=true*/)
{
  ...
  for (childIt = this->Children.begin(); childIt != this->Children.end(); ++childIt)
  {
    ...
    if (recursive)
    {
      vtkSubjectHierarchyItem* const foundItemInBranch = currentItem->FindChildByDataNode(dataNode);
      if (foundItemInBranch)
      {
...
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::FindChildByUID(std::string uidName, std::string uidValue, bool recursive /*=true*/)
{
  ...
  for (childIt = this->Children.begin(); childIt != this->Children.end(); ++childIt)
  {
    ...
    if (recursive)
    {
      vtkSubjectHierarchyItem* const foundItemInBranch = currentItem->FindChildByUID(uidName, uidValue);
      if (foundItemInBranch)
      {
        return foundItemInBranch;
      }
...
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::FindChildByUIDList(std::string uidName, std::string uidValue, bool recursive /*=true*/)
{
  ...
  for (childIt = this->Children.begin(); childIt != this->Children.end(); ++childIt)
  {
    ...
    if (recursive)
    {
      vtkSubjectHierarchyItem* const foundItemInBranch = currentItem->FindChildByUIDList(uidName, uidValue);
      if (foundItemInBranch)
      {
        return foundItemInBranch;
      }
...
bool vtkSubjectHierarchyItem::Reparent(vtkSubjectHierarchyItem* newParentItem)
{
  ...
  const vtkSmartPointer<vtkSubjectHierarchyItem> thisPointer = this;
...
bool vtkSubjectHierarchyItem::RemoveChild(vtkSubjectHierarchyItem* item)
{
  ...
  // Prevent deletion of the item from memory until the events are processed
  const vtkSmartPointer<vtkSubjectHierarchyItem> removedItem = (*childIt);
...
void vtkSubjectHierarchyItem::ReparentChildrenToParent()
{
  ...
  for (childIDIt = childIDs.begin(); childIDIt != childIDs.end(); ++childIDIt)
  {
    ...
    for (childIt = this->Children.begin(); childIt != this->Children.end(); ++childIt)
    {
      ...
      {
        // Prevent deletion of the item from memory until the events are processed
        const vtkSmartPointer<vtkSubjectHierarchyItem> childSmartPointer = childItem;
...
void vtkSubjectHierarchyItem::RemoveAllChildren()
{
  ...
  const MRMLNodeModifyBlocker blocker(this->DataNode);
...
void vtkSubjectHierarchyItem::SetUID(std::string uidName, std::string uidValue)
{
  ...
  if (it != this->UIDs.end())
  {
    ...
    {
      vtkWarningMacro("SetUID: UID with name '" << uidName << "' already exists in subject hierarchy item '" << this->GetName() << "' with value '" << it->second
...
std::string vtkSubjectHierarchyItem::GetAttributeFromAncestor(std::string attributeName, std::string level)
{
  ...
  while (currentItem && currentItem->Parent)
  {
    ...
    else if (!level.empty() && currentItem->GetAttribute(vtkMRMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare(level))
    {
      continue;
    }
...
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::GetAncestorAtLevel(std::string level)
{
  ...
  {
    currentItem = currentItem->Parent;
    if (currentItem && !currentItem->GetAttribute(vtkMRMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare(level))
    {
      // Level found
      return currentItem;
    }
...
bool vtkMRMLSubjectHierarchyNode::vtkInternal::ResolveUnresolvedItems()
{
  ...
  const bool wasResolving = this->IsResolving;
  ...
  for (vtkSubjectHierarchyItem::ChildVector::iterator itemIt = this->UnresolvedItems->Children.begin(); itemIt != this->UnresolvedItems->Children.end(); ++itemIt)
  {
  ...
  while (this->UnresolvedItems->Children.size())
  {
    ...
    for (vtkSubjectHierarchyItem::ChildVector::iterator itemIt = this->UnresolvedItems->Children.begin(); itemIt != this->UnresolvedItems->Children.end(); ++itemIt)
    {
      ...
      else
      {
        ...
        const std::map<vtkIdType, vtkIdType>::iterator parentIt = idMap.find(item->TemporaryParentItemID);
      ...
      if (!item->TemporaryDataNodeID.empty())
      {
        const char* const newDataNodeID = scene->GetChangedID(item->TemporaryDataNodeID.c_str());
        if (newDataNodeID)
        {
...
void vtkMRMLSubjectHierarchyNode::vtkInternal::CopyAsUnresolved(vtkMRMLSubjectHierarchyNode* otherShNode)
{
  ...
  {
    vtkSubjectHierarchyItem* const currentOtherItem =
      (((*otherItemIt) == otherShNode->Internal->SceneItemID) ? otherShNode->Internal->SceneItem : otherShNode->Internal->SceneItem->FindChildByID(*otherItemIt));
    const vtkSmartPointer<vtkSubjectHierarchyItem> copiedItem = vtkSmartPointer<vtkSubjectHierarchyItem>::New();
...
void vtkMRMLSubjectHierarchyNode::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "UnresolvedItemsID: " << (this->Internal->UnresolvedItems ? this->Internal->UnresolvedItems->ID : vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID) << "\n";
...
vtkMRMLSubjectHierarchyNode* vtkMRMLSubjectHierarchyNode::ResolveSubjectHierarchy(vtkMRMLScene* scene)
{
  ...
  {
    const vtkSmartPointer<vtkMRMLSubjectHierarchyNode> newShNode = vtkSmartPointer<vtkMRMLSubjectHierarchyNode>::New();
  ...
  if (!firstShNode)
  {
    vtkErrorWithObjectMacro(scene, "vtkMRMLSubjectHierarchyNode::ResolveSubjectHierarchy: Invalid first subject hierarchy node");
    return firstShNode;
  }
  ...
  const bool wasResolving = firstShNode->Internal->IsResolving;
  ...
  {
    vtkMRMLSubjectHierarchyNode* const currentShNode = vtkMRMLSubjectHierarchyNode::SafeDownCast(*shNodeIt);
    if (currentShNode == nullptr)
    {
      continue;
    }
    // Prevent events from being invoked from both subject hierarchy nodes
    const bool wereEventsDisabled = firstShNode->Internal->EventsDisabled;
    ...
    if (!firstShNode->Internal->ResolveUnresolvedItems())
    {
      ...
      {
        const char* const nodeID = (*allShNodeIt)->GetID();
        if (nodeID)
        {
      ...
      {
        vtkMRMLNode* const node = scene->GetNodeByID(*allShNodeIDIt);
        if (node)
        {
      ...
      vtkErrorWithObjectMacro(scene, "vtkMRMLSubjectHierarchyNode::ResolveSubjectHierarchy: Failed to merge subject hierarchy nodes, re-building subject hierarchy from scratch");
...
void vtkMRMLSubjectHierarchyNode::ReadItemFromXML(const char** atts)
{
  ...
  const vtkSmartPointer<vtkSubjectHierarchyItem> item = vtkSmartPointer<vtkSubjectHierarchyItem>::New();
...
void vtkMRMLSubjectHierarchyNode::Copy(vtkMRMLNode* anode)
{
  ...
  vtkMRMLSubjectHierarchyNode* const otherShNode = vtkMRMLSubjectHierarchyNode::SafeDownCast(anode);
  ...
  if (this->GetScene() && this->GetScene()->IsRestoring())
  {
    ...
    const bool wereEventsDisabled = this->Internal->EventsDisabled;
...
void vtkMRMLSubjectHierarchyNode::SetItemDataNode(vtkIdType itemID, vtkMRMLNode* dataNode)
{
  ...
  vtkSubjectHierarchyItem* const item = this->Internal->FindItemByID(itemID);
  if (!item)
  {
  ...
  if (item->DataNode)
  {
    vtkErrorMacro("SetItemDataNode: Data node is already associated to item with " << itemID << ". This function cannot be used to replace data nodes for items");
    return;
  }
...
bool vtkMRMLSubjectHierarchyNode::RemoveItemUID(vtkIdType itemID, std::string uidName)
{
  ...
  const bool result = item->RemoveUID(uidName); // Events are invoked within this call
  return result;
}
...
bool vtkMRMLSubjectHierarchyNode::RemoveItemAttribute(vtkIdType itemID, std::string attributeName)
{
  ...
  const bool result = item->RemoveAttribute(attributeName); // Events are invoked within this call
  return result;
}
...
vtkIdType vtkMRMLSubjectHierarchyNode::CreateItem(vtkIdType parentItemID, vtkMRMLNode* dataNode, const char* ownerPluginName)
{
  ...
  if (itemID != INVALID_ITEM_ID)
  {
    ...
    {
      vtkSubjectHierarchyItem* const parentItem = this->Internal->FindItemByID(parentItemID);
      if (!parentItem)
      {
...
vtkIdType vtkMRMLSubjectHierarchyNode::CreateHierarchyItem(vtkIdType parentItemID, std::string name, std::string level, int positionUnderParent /*=-1*/)
{
  ...
  // Add item to the tree
  const vtkIdType itemID = item->AddToTree(parentItem, name, level, positionUnderParent);
...
void vtkMRMLSubjectHierarchyNode::SetItemParent(vtkIdType itemID, vtkIdType parentItemID, bool enableCircularCheck /*=true*/)
{
  ...
  if (enableCircularCheck)
  {
    ...
    while ((currentParentItem = currentParentItem->Parent) != nullptr)
    {
      ...
      {
        vtkErrorMacro("SetItemParent: Circular parenthood detected, parenting aborted: given parent item " << parentItemID << " is a child of the reparented item " << itemID);
        return;
      }
      if (currentParentItem == parentItem)
      {
        vtkErrorMacro("SetItemParent: Circular parenthood detected, parenting aborted: given parent item " << parentItemID << " is a child of itself");
        return;
      }
    }
  }
...
bool vtkMRMLSubjectHierarchyNode::ReparentItemByDataNode(vtkIdType itemID, vtkMRMLNode* newParentNode)
{
  ...
  vtkSubjectHierarchyItem* const formerParent = item->Parent;
  if (!formerParent)
  {
  ...
  vtkSubjectHierarchyItem* const newParentItem = this->Internal->SceneItem->FindChildByDataNode(newParentNode);
  if (!newParentItem)
  {
...
bool vtkMRMLSubjectHierarchyNode::MoveItem(vtkIdType itemID, vtkIdType beforeItemID)
{
  ...
  vtkSubjectHierarchyItem* const beforeItem = this->Internal->SceneItem->FindChildByID(beforeItemID);
...
vtkIdType vtkMRMLSubjectHierarchyNode::GetItemByUID(const char* uidName, const char* uidValue)
{
  ...
  vtkSubjectHierarchyItem* const item = this->Internal->SceneItem->FindChildByUID(uidName, uidValue);
...
vtkIdType vtkMRMLSubjectHierarchyNode::GetItemByUIDList(const char* uidName, const char* uidValue)
{
  ...
  vtkSubjectHierarchyItem* const item = this->Internal->SceneItem->FindChildByUIDList(uidName, uidValue);
  return (item ? item->ID : INVALID_ITEM_ID);
}
...
vtkIdType vtkMRMLSubjectHierarchyNode::GetItemByDataNode(vtkMRMLNode* dataNode)
{
  ...
  vtkSubjectHierarchyItem* const item = this->Internal->SceneItem->FindChildByDataNode(dataNode);
  if (item)
  {
...
vtkIdType vtkMRMLSubjectHierarchyNode::GetItemChildWithName(vtkIdType parentItemID, std::string name, bool recursive /*=false*/)
{
  ...
  {
    vtkWarningMacro("GetItemChildWithName: Multiple subject hierarchy item found with name '" << name << "' under item with ID " << parentItemID << ". Returning first");
  }
  return foundItemIDs[0];
}
...
{
  const vtkIdType itemID = this->GetItemByDataNode(dataNode);
  if (itemID == INVALID_ITEM_ID)
  {
...
void vtkMRMLSubjectHierarchyNode::SetItemDisplayVisibility(vtkIdType itemID, int visible)
{
  ...
  {
    vtkErrorMacro("SetItemDisplayVisibility: Invalid visibility value to set: " << visible << ". Needs to be one of the following: 0:Hidden, 1:Visible");
    return;
  }

  vtkMRMLNode* const dataNode = this->GetItemDataNode(itemID);
...
vtkMRMLDisplayNode* vtkMRMLSubjectHierarchyNode::GetDisplayNodeForItem(vtkIdType itemID)
{
  vtkMRMLNode* const dataNode = this->GetItemDataNode(itemID);
  vtkMRMLDisplayableNode* displayableNode = vtkMRMLDisplayableNode::SafeDownCast(dataNode);
  vtkMRMLDisplayNode* const displayNode = vtkMRMLDisplayNode::SafeDownCast(dataNode);
  if (displayableNode)
  {
...
vtkIdType vtkMRMLSubjectHierarchyNode::GetItemAncestorAtLevel(vtkIdType itemID, std::string level)
{
  ...
  vtkSubjectHierarchyItem* const ancestorItem = item->GetAncestorAtLevel(level);
  if (ancestorItem)
  {
...
//---------------------------------------------------------------------------
bool vtkMRMLSubjectHierarchyNode::IsAnyNodeInBranchTransformed(vtkIdType itemID, bool includeParentItem /*=true*/, vtkMRMLTransformNode* exceptionNode /*=nullptr*/)
{
  // Check transformable node from the item itself if any
  vtkSubjectHierarchyItem* const item = this->Internal->SceneItem->FindChildByID(itemID);
  if (!item)
  {
  ...
  {
    vtkMRMLTransformableNode* transformableNode = vtkMRMLTransformableNode::SafeDownCast(childTransformableNodes->GetItemAsObject(childNodeIndex));
...
void vtkMRMLSubjectHierarchyNode::DeserializeUIDList(std::string uidListString, std::vector<std::string>& deserializedUIDList)
{
  ...
  {
    const std::string uid = uidListString.substr(0, separatorPosition);
...
std::vector<vtkIdType> vtkMRMLSubjectHierarchyNode::GetItemsReferencedFromItemByDICOM(vtkIdType itemID)
{
  ...
  const std::string referencedInstanceUIDsAttribute = item->GetAttribute(vtkMRMLSubjectHierarchyConstants::GetDICOMReferencedInstanceUIDsAttributeName());
  ...
  for (std::vector<std::string>::iterator uidIt = referencedSopInstanceUids.begin(); uidIt != referencedSopInstanceUids.end(); ++uidIt)
  {
    ...
    {
      vtkSubjectHierarchyItem* const referencedItem = this->Internal->SceneItem->FindChildByUIDList(vtkMRMLSubjectHierarchyConstants::GetDICOMInstanceUIDName(), (*uidIt));
      if (referencedItem)
      {
    ...
    else
    {
      ...
      for (std::vector<vtkSubjectHierarchyItem*>::iterator itemIt = referencedItems.begin(); itemIt != referencedItems.end(); ++itemIt)
      {
        ...
        const std::string uids = (*itemIt)->GetUID(vtkMRMLSubjectHierarchyConstants::GetDICOMInstanceUIDName());
...
std::vector<vtkIdType> vtkMRMLSubjectHierarchyNode::GetItemsReferencingItemByDICOM(vtkIdType itemID)
{
  ...
  const std::string uidsString = item->GetUID(vtkMRMLSubjectHierarchyConstants::GetDICOMInstanceUIDName());
  if (uidsString.empty())
  {
  ...
  for (std::vector<vtkIdType>::iterator itemIt = allItemIDs.begin(); itemIt != allItemIDs.end(); ++itemIt)
  {
    ...
    const std::string referencedUids = currentItem->GetAttribute(vtkMRMLSubjectHierarchyConstants::GetDICOMReferencedInstanceUIDsAttributeName());
...
std::string vtkMRMLSubjectHierarchyNode::GenerateUniqueItemName(std::string name)
{
  ...
  {
    // Get current item
    vtkSubjectHierarchyItem* const currentItem = this->Internal->SceneItem->FindChildByID(*itemIt);
    if (!currentItem)
    {
    ...
    const std::string currentPrefix = currentItem->Name.substr(0, name.size());
    ...
    const int currentPostfixNumber = vtkVariant(currentItem->Name.substr(name.size() + 1)).ToInt();
...
void vtkMRMLSubjectHierarchyNode::ItemEventCallback(vtkObject* caller, unsigned long eid, void* clientData, void* callData)
{
  ...
  switch (eid)
  {
    ...
    case vtkMRMLSubjectHierarchyNode::SubjectHierarchyItemChildrenReorderedEvent:
    {
      ...
      vtkSubjectHierarchyItem* const item = reinterpret_cast<vtkSubjectHierarchyItem*>(callData);
      if (item)
      {
    ...
    {
      vtkSubjectHierarchyItem* const item = vtkSubjectHierarchyItem::SafeDownCast(caller);
      vtkMRMLNode* const dataNode = vtkMRMLNode::SafeDownCast(caller);
      if (item)
      {
    ...
    default: vtkErrorWithObjectMacro(self, "vtkMRMLSubjectHierarchyNode::ItemEventCallback: Unknown event ID " << eid); return;
  }
}
...
bool vtkMRMLSubjectHierarchyNode::ReorderItemChildren(vtkIdType parentItemID, vtkIdList* childIDs)
{
  ...
  {
    const vtkIdType childID = childIDs->GetId(childIDIndex);
    vtkSubjectHierarchyItem* const childItem = this->Internal->SceneItem->FindChildByID(childID);
    if (!childItem)
    {
    ...
    // Prevent deletion of the item from memory until the events are processed
    const vtkSmartPointer<vtkSubjectHierarchyItem> thisPointer = childItem;
