...
int vtkMRMLModelStorageNode::ReadDataInternal(vtkMRMLNode* refNode)
{
  ...
  if (coordinateSystemInFileHeader >= 0)
  {
    // coordinate system specified in the file, use it (regardless of what was the preferred coordinate system in the
    // node)
  ...
  else
  {
    ...
    // Introduction of saving coordinate system information in the file header
    // (6036edbc4b67712822ccb4ee89139cab1cbce3f2, 2020-02-26) and saving of application revision information
    // (127c2681d0b1e9f1114d269e675b814607791126, 2020-03-28) happened around the same time. We do not need to log a
    // warning if coordinate system information is not found when loading a scene that was saved before coordinate
    // system information was saved in model files.
    ...
    if (this->GetScene() && this->GetScene()->IsImporting())
    {
      ...
      if (vtkMRMLScene::ParseVersion(this->GetScene()->GetLastLoadedVersion(),
                                     currentApplication,
                                     currentMajor,
                                     currentMinor,
                                     currentPatch,
    ...
    if (isImportingFromLegacyScene)
    {
      ...
      vtkDebugMacro("ReadDataInternal (" << (this->ID ? this->ID : "(unknown)") << "): File " << fullName.c_str()
                                         << " does not contain coordinate system information. Using "
    ...
    else
    {
      ...
      vtkWarningMacro("ReadDataInternal ("
                      << (this->ID ? this->ID : "(unknown)") << "): File " << fullName.c_str()
                      << " does not contain coordinate system information. Using "
                      << vtkMRMLStorageNode::GetCoordinateSystemTypeAsString(this->CoordinateSystem) << ".");
    }
  }
...
int vtkMRMLModelStorageNode::WriteDataInternal(vtkMRMLNode* refNode)
{
  ...
  if (extension == ".vtk" && (modelNode->GetMeshType() == vtkMRMLModelNode::PolyDataMeshType ||
