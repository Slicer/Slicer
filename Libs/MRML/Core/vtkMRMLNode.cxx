...
//----------------------------------------------------------------------------
const char *vtkMRMLNode::WriteJSONToString(int indent, vtkMRMLMessageCollection* userMessagesInput)
{
  ...
  std::function<void(vtkXMLDataElement*, rapidjson::Value&, rapidjson::Document::AllocatorType&)> processElement;
  processElement = [&](vtkXMLDataElement* element, rapidjson::Value& jsonNode, rapidjson::Document::AllocatorType &allocator) {
    int nAttributes = element->GetNumberOfAttributes();
    for (int attrIndex = 0; attrIndex < nAttributes; ++attrIndex)
    {
      const char *name = element->GetAttributeName(attrIndex);
      const char *value = element->GetAttributeValue(attrIndex);
      std::string valueStr = value ? std::string(value) : "";

      if (std::string(name) == "description")
      {
        jsonNode.AddMember(rapidjson::Value(name, allocator).Move(),
                           rapidjson::Value(value, allocator).Move(),
                           allocator);
      }
      else if (std::string(name) == "attributes")
      {
        rapidjson::Value jsonAttributes(rapidjson::kObjectType);
        const char* attributes = element->GetAttribute("attributes");
        if (attributes)
        {
          ...
          if (attributesStr.find('|') != std::string::npos)
          {
            delimiter = "|";
            valueDelimiter = "^";
          }
          while (std::getline(ss, attribute, delimiter[0]))
          {
            int colonIndex = attribute.find(valueDelimiter);
            std::string attributeName = attribute.substr(0, colonIndex);
            std::string attributeValue = attribute.substr(colonIndex + 1);
            vtksys::SystemTools::ReplaceString(attributeValue, "%", "%25");
            vtksys::SystemTools::ReplaceString(attributeValue, ";", "%3B");
            jsonAttributes.AddMember(rapidjson::Value(this->XMLAttributeEncodeString(attributeName).c_str(), allocator).Move(),
                                     rapidjson::Value(this->XMLAttributeEncodeString(attributeValue).c_str(), allocator).Move(),
                                     allocator);
          }
        }
        jsonNode.AddMember(rapidjson::Value(name, allocator).Move(), jsonAttributes, allocator);
      }
      else if (std::string(name) == "uids")
      {
        rapidjson::Value jsonAttributes(rapidjson::kObjectType);
        const char* uids = element->GetAttribute("uids");
        if (uids)
        {
          std::string uidsStr(uids);
          std::stringstream ss(uidsStr);
          std::string uid;
          std::string delimiter = ";";
          std::string valueDelimiter = ":";
          if (uidsStr.find('|') != std::string::npos)
          {
            delimiter = "|";
            valueDelimiter = "^";
          }
          while (std::getline(ss, uid, delimiter[0]))
          {
            int colonIndex = uid.find(valueDelimiter);
            std::string attributeName = uid.substr(0, colonIndex);
            std::string attributeValue = uid.substr(colonIndex + 1);
            vtksys::SystemTools::ReplaceString(attributeValue, "%", "%25");
            vtksys::SystemTools::ReplaceString(attributeValue, ";", "%3B");
            jsonAttributes.AddMember(rapidjson::Value(this->XMLAttributeEncodeString(attributeName).c_str(), allocator).Move(),
                                     rapidjson::Value(this->XMLAttributeEncodeString(attributeValue).c_str(), allocator).Move(),
      ...
      else if (std::string(name) == "references")
      {
        rapidjson::Value jsonReferences(rapidjson::kObjectType);
        for (const auto& it : this->NodeReferences)
        {
          const std::string& referenceRole = it.first;
          int numReferencedNodes = this->GetNumberOfNodeReferences(referenceRole.c_str());

          rapidjson::Value jsonRoleReferences(rapidjson::kArrayType);
          for (int n = 0; n < numReferencedNodes; ++n)
          {
            const char *id = this->GetNthNodeReferenceID(referenceRole.c_str(), n);
            if (!id)
            ...
            rapidjson::Value jsonReference(rapidjson::kObjectType);
            jsonReference.AddMember("id", rapidjson::Value(id, allocator).Move(), allocator);

            const vtkMRMLNode::ReferencePropertiesType* referenceProperties = this->GetNthNodeReferenceProperties(referenceRole.c_str(), n);
            if (referenceProperties->size() > 0)
            {
              rapidjson::Value jsonProperties(rapidjson::kObjectType);
              for (const auto& referenceProperty : *referenceProperties)
              {
                std::string encodedPropertyKey = this->NodeReferencePropertyEncodeString(referenceProperty.first);
                std::string encodedPropertyValue = this->NodeReferencePropertyEncodeString(referenceProperty.second);

                jsonProperties.AddMember(
                  rapidjson::Value(encodedPropertyKey.c_str(), allocator).Move(),
                  rapidjson::Value(encodedPropertyValue.c_str(), allocator).Move(),
                  allocator);
              }
              jsonReference.AddMember("properties", jsonProperties, allocator);
            }

            jsonRoleReferences.PushBack(jsonReference, allocator);
          }

          if (jsonRoleReferences.Size() > 0)
          {
            jsonReferences.AddMember(rapidjson::Value(referenceRole.c_str(), allocator).Move(), jsonRoleReferences, allocator);
          }
        }

        if (!jsonReferences.ObjectEmpty())
        {
          jsonNode.AddMember("references", jsonReferences, allocator);
        }
      }
      else if (valueStr.find(' ') != std::string::npos)
      {
        ...
        while (iss >> item)
        {
          if ((std::all_of(item.begin(), item.end(), [](char c){ return ::isdigit(c) || c == '.' || c == '-'; }) && item != "-"))
          {
            std::istringstream itemStream(item);
            itemStream >> std::scientific;
            double value;
            itemStream >> value;
            jsonArray.PushBack(rapidjson::Value(value).Move(), allocator);
          }
          else
          {
            jsonArray.PushBack(rapidjson::Value(item.c_str(), allocator).Move(), allocator);
          }
        }
        if (!jsonArray.Empty())
        {
          jsonNode.AddMember(rapidjson::Value(name, allocator).Move(), jsonArray, allocator);
        }
      }
      else
      {
        if (value && (std::isdigit(value[0]) || (value[0] == '-' && std::isdigit(value[1]))))
        {
          jsonNode.AddMember(rapidjson::Value(name, allocator).Move(),
                             rapidjson::Value(std::atoi(value)).Move(),
                             allocator);
        }
        else if (value && (std::strcmp(value, "true") == 0 || std::strcmp(value, "false") == 0))
        {
          jsonNode.AddMember(rapidjson::Value(name, allocator).Move(),
                             rapidjson::Value(std::strcmp(value, "true") == 0).Move(),
                             allocator);
        }
        else
        {
          jsonNode.AddMember(rapidjson::Value(name, allocator).Move(),
                             rapidjson::Value(value ? value : "", allocator).Move(),
                             allocator);
        }
      }
    }

    for (int childIndex = 0; childIndex < element->GetNumberOfNestedElements(); ++childIndex)
    {
      vtkXMLDataElement* childElement = element->GetNestedElement(childIndex);
      rapidjson::Value jsonChildNode(rapidjson::kObjectType);
      processElement(childElement, jsonChildNode, allocator);
      jsonNode.AddMember(rapidjson::Value(childElement->GetName(), allocator).Move(), jsonChildNode, allocator);
    }
  };

  rapidjson::Document::AllocatorType &allocator = jsonDocument.GetAllocator();
  rapidjson::Value jsonRootNode(rapidjson::kObjectType);
  processElement(xmlElement, jsonRootNode, allocator);

  jsonDocument.AddMember(rapidjson::Value(this->GetNodeTagName(), allocator).Move(),
                         jsonRootNode, allocator);

  rapidjson::StringBuffer buffer;
  rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
  jsonDocument.Accept(writer);

  size_t length = std::strlen(buffer.GetString());
  char* result = new char[length + 1]; // +1 for the null terminator
  std::strcpy(result, buffer.GetString());

  return result;
}

//----------------------------------------------------------------------------
void vtkMRMLNode::ReadJSONFromString(const char* json, bool overwriteReferences/*= true*/)
{
  ...
  if (!document.HasMember(this->GetNodeTagName()))
  {
    vtkErrorMacro( << "JSON string does not contain vtkMRMLNode: " << this->GetNodeTagName());
    return;
  }

  const rapidjson::Value& node = document[this->GetNodeTagName()];
  std::vector<const char*> attributes;
  char* ossStrCStr = nullptr;
  char* arrayStrCStr = nullptr;
  char* numberStrCStr = nullptr;
  ...
  for (rapidjson::Value::ConstMemberIterator itr = node.MemberBegin(); itr != node.MemberEnd(); ++itr)
  {
    ...
    {
      const rapidjson::Value& jsonAttributes = itr->value;
      std::ostringstream oss;
      for (rapidjson::Value::ConstMemberIterator attrItr = jsonAttributes.MemberBegin(); attrItr != jsonAttributes.MemberEnd(); ++attrItr)
      {
        if (attrItr != jsonAttributes.MemberBegin())
        {
          oss << ";";
        }
        oss << attrItr->name.GetString() << ":" << attrItr->value.GetString();
      }
      attributes.push_back(itr->name.GetString());
      std::string ossStr = oss.str();
      ossStrCStr = new char[ossStr.length() + 1];
      std::strcpy(ossStrCStr, ossStr.c_str());
      attributes.push_back(ossStrCStr);
    }
    else if (std::string(itr->name.GetString()) == "references")
    {
      const rapidjson::Value& jsonReferences = itr->value;
      for (rapidjson::Value::ConstMemberIterator roleItr = jsonReferences.MemberBegin(); roleItr != jsonReferences.MemberEnd(); ++roleItr)
      {
        const char* referenceRole = roleItr->name.GetString();
        const rapidjson::Value& jsonRoleReferences = roleItr->value;
        for (rapidjson::SizeType referenceIndex = 0; referenceIndex < jsonRoleReferences.Size(); ++referenceIndex)
        {
          const rapidjson::Value& jsonReference = jsonRoleReferences[referenceIndex];
          const char* referencedNodeID = jsonReference["id"].GetString();

          if (overwriteReferences)
          {
            // Remove only the NodeReferenceIDs that have the same class as the referencedNodeID in the same referenceRole
            std::string referencedNodeIDStr = referencedNodeID;
            // getting the class from the ID, since the node could be not added to the scene yet
            std::string referencedNodeClassName = referencedNodeIDStr.substr(0, referencedNodeIDStr.find_first_of("Node"));
            int numberOfReferences = this->GetNumberOfNodeReferences(referenceRole);
            for (int referenceIndex = numberOfReferences - 1; referenceIndex >= 0; --referenceIndex)
            {
              const char* existingReferencedNodeID = this->GetNthNodeReferenceID(referenceRole, referenceIndex);
              if (existingReferencedNodeID)
              {
                std::string existingReferencedNodeIDStr = existingReferencedNodeID;
                std::string existingReferencedNodeClassName = existingReferencedNodeIDStr.substr(0, existingReferencedNodeIDStr.find_first_of("Node"));
                if (existingReferencedNodeClassName == referencedNodeClassName)
                {
                  this->RemoveNthNodeReferenceID(referenceRole, referenceIndex);
                }
              }
            }
          }
          else
          {
            ...
            {
              const char* existingReferencedNodeID = this->GetNthNodeReferenceID(referenceRole, referenceIndex);
              if (existingReferencedNodeID && strcmp(existingReferencedNodeID, referencedNodeID) == 0)
              {
          ...
          this->AddNodeReferenceID(referenceRole, referencedNodeID);
          if (jsonReference.HasMember("properties"))
          {
            const rapidjson::Value& jsonProperties = jsonReference["properties"];
            for (rapidjson::Value::ConstMemberIterator propItr = jsonProperties.MemberBegin(); propItr != jsonProperties.MemberEnd(); ++propItr)
            {
              std::string propertyKey = this->NodeReferencePropertyDecodeString(propItr->name.GetString());
              std::string propertyValue = this->NodeReferencePropertyDecodeString(propItr->value.GetString());
              int index = this->GetNumberOfNodeReferences(referenceRole) - 1;
              this->SetNthNodeReferenceProperty(referenceRole, index, propertyKey, propertyValue);
            }
          }
        }
      }
    ...
    else if (itr->value.IsArray())
    {
      ...
      attributes.push_back(itr->name.GetString());
      std::string arrayStr = arrayStream.str();
      arrayStrCStr = new char[arrayStr.length() + 1];
      std::strcpy(arrayStrCStr, arrayStr.c_str());
      attributes.push_back(arrayStrCStr);
    }
    else if (itr->value.IsObject())
    {
      // Skip objects, e.g. SubjectHierarchyItems
      continue;
    }
    else
    {
      ...
      {
        attributes.push_back(itr->value.GetString());
      }
      else if (itr->value.IsBool())
      {
        attributes.push_back(itr->value.GetBool() ? "true" : "false");
      }
      else if (itr->value.IsNumber())
      {
        ...
        if (itr->value.IsInt())
        {
          numberStr = std::to_string(itr->value.GetInt());
        }
        else
        {
          numberStr = std::to_string(itr->value.GetDouble());
        }
        numberStrCStr = new char[numberStr.length() + 1];
        std::strcpy(numberStrCStr, numberStr.c_str());
        attributes.push_back(numberStrCStr);
      }
    }
  }

  const char **atts = new const char *[attributes.size() + 1];
  std::copy(attributes.begin(), attributes.end(), atts);
  atts[attributes.size()] = nullptr;

  this->ReadXMLAttributes(atts);
  delete[] atts;
  delete[] ossStrCStr;
  delete[] arrayStrCStr;
  delete[] numberStrCStr;
}
...
//----------------------------------------------------------------------------
const char* vtkMRMLNode::GetMRMLAttributeNameFromReferenceRole(const char* refRole)
{
  if (refRole == nullptr)
  {
    return nullptr;
