...
void vtkMRMLNode::CopyReferences(vtkMRMLNode* node)
{
  ...
  if (referenceRoles == referenceRolesInSource)
  {
    ...
    for (NodeReferencesType::iterator sourceNodeReferencesIt = node->NodeReferences.begin();
         sourceNodeReferencesIt != node->NodeReferences.end() && referencesAreEqual;
...
void vtkMRMLNode::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  if (!this->NodeReferences.empty())
  {
    ...
    for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
    {
      ...
      {
        int n = 0;
        for (std::vector<const char*>::iterator referencedNodeIdsIt = referencedNodeIds.begin();
             referencedNodeIdsIt != referencedNodeIds.end();
             ++referencedNodeIdsIt)
        {
          ...
          const vtkMRMLNode::ReferencePropertiesType* referenceProperties =
            this->GetNthNodeReferenceProperties(referenceRole.c_str(), n);
          if (referenceProperties->size() > 0)
          {
            os << ": {";
            for (auto referencePropertyIt = referenceProperties->begin();
                 referencePropertyIt != referenceProperties->end();
                 ++referencePropertyIt)
            {
...
void vtkMRMLNode::ReadXMLAttributes(const char** atts)
{
  ...
  while (*xmlReadAtts != nullptr)
  {
    attName = *(xmlReadAtts++);
    attValue = *(xmlReadAtts++);
    if (const char* referenceRole = this->GetReferenceRoleFromMRMLAttributeName(attName))
    {
  ...
  while (start != std::string::npos && sep != std::string::npos && start != end && start != sep)
  {
    ...
    if (references.find(role) == references.end())
    {
      ...
      while (!ss.eof())
      {
        ...
        // It is important that there is a space between the node ID and the properties so that it is recognized as a
        // separate reference. If there is no space, then the node reference ID would be considered invalid. Since the
        // property is separated by a space, it is considered an invalid node id and silently discarded by older
        // versions of Slicer.
        ...
        if (referencePropertyStart != std::string::npos && referencePropertyEnd != std::string::npos)
        {
          ...
          std::string referenceProperties =
            value.substr(referencePropertyStart + 1, referencePropertyEnd - referencePropertyStart - 1);
...
void vtkMRMLNode::WriteXML(ostream& of, int nIndent)
{
  ...
  for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
  {
    ...
    for (int n = 0; n < numReferencedNodes; n++)
    {
      ...
      ssRef << " {";
      for (auto referencePropertyIt = referenceProperties->begin(); referencePropertyIt != referenceProperties->end();
           ++referencePropertyIt)
      {
        if (referencePropertyIt != referenceProperties->begin())
        {
  ...
  for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
  {
    if (it->first.c_str())
    {
      ...
      for (unsigned int i = 0; i < references.size(); i++)
      {
        ...
        if (node != nullptr && node == vtkMRMLNode::SafeDownCast(caller) && event == vtkCommand::ModifiedEvent)
        {
...
//----------------------------------------------------------------------------
void vtkMRMLNode::AddNodeReferenceRole(const char* refRole,
                                       ...
                                       bool observeContentModifiedEvents /*=false*/)
{
  ...
  eventList.ObserveContentModifiedEvents =
    (observeContentModifiedEvents ? ContentModifiedObserveEnabled : ContentModifiedObserveDisabled);
  ...
  for (it = this->NodeReferenceMRMLAttributeNames.begin(); it != this->NodeReferenceMRMLAttributeNames.end(); ++it)
  {
    ...
    if (nodeMRMLAttributeName == attributeName)
    {
      return nodeReferenceRole.c_str();
             ...
             attributeName.compare(attributeName.length() - nodeMRMLAttributeName.length(),
                                   nodeMRMLAttributeName.length(),
                                   nodeMRMLAttributeName) == 0)
    {
...
void vtkMRMLNode::UpdateReferenceID(const char* oldID, const char* newID)
{
  ...
  for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
  {
    for (unsigned int i = 0; i < it->second.size(); i++)
    {
      ...
      {
        this->SetAndObserveNthNodeReferenceID(reference->GetReferenceRole(),
                                              i,
                                              newID,
                                              reference->GetStaticEvents(),
                                              reference->GetObserveContentModifiedEvents());
      }
    }
  }
  this->EndModify(wasModifying);
...
void vtkMRMLNode::SetAttribute(const char* name, const char* value)
{
  ...
  if ((!oldValue && !value) || (oldValue && value && !strcmp(oldValue, value)))
  {
    return;
  }
  if (value != nullptr)
...
const char* vtkMRMLNode::GetAttribute(const char* name)
{
  ...
  AttributesType::const_iterator iter = this->Attributes.find(std::string(name));
...
void vtkMRMLNode::UpdateReferences()
{
  ...
  for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
  {
    ...
    {
      vtkMRMLNodeReference* reference = it->second[i];
      if (reference->GetReferencedNodeID() && std::string(reference->GetReferencedNodeID()) != "" &&
...
{
  roles.clear();
  for (NodeReferencesType::iterator roleIt = this->NodeReferences.begin(); roleIt != this->NodeReferences.end();
       roleIt++)
  {
...
vtkMRMLNode* vtkMRMLNode::GetNthNodeReference(const char* referenceRole, int n)
{
  ...
  if ((!node || node->GetScene() != this->GetScene()) || (node && this->GetScene() == nullptr))
  {
...
void vtkMRMLNode::UpdateNthNodeReference(const char* referenceRole, int n)
{
  ...
  this->SetAndObserveNthNodeReferenceID(references[n]->GetReferenceRole(),
                                        n,
                                        references[n]->GetReferencedNodeID(),
                                        references[n]->GetStaticEvents(),
                                        references[n]->GetObserveContentModifiedEvents());
}
                                                          ...
                                                          ContentModifiedObserveType observeContentModifiedEvents)
{
  ...
  if (referencedNodeID && strlen(referencedNodeID) > 0)
  {
    ...
    this->UpdateNodeReferenceEventObserver(oldReferencedNode,
                                           referencedNode,
                                           staticEvents,
                                           observeContentModifiedEvents == ContentModifiedObserveEnabled,
                                           referenceIt->GetPointer());
                                                           ...
                                                           vtkMRMLNodeReference* referenceToIgnore)
{
  ...
  if (oldReferencedNode == newReferencedNode)
  {
    ...
    // The same events may be already observed by other nodes, so suppress the warning if there are no changes in the
    // event list.
    vtkSetAndObserveMRMLObjectEventsMacroNoWarning(
      newReferencedNode, newReferencedNode, newConsolidatedEventList.GetPointer()); // update the event list
  }
  else
  {
    ...
    if (oldReferencedNode != nullptr)
    {
      if (oldReferencedNodeUseCount == 0)
      {
        ...
        vtkMRMLNode* oldReferencedNodeCopy =
          oldReferencedNode; // make a copy of oldReferencedNode (vtkSetAndObserveMRMLObjectMacro would overwrite the
                             // value with nullptr)
      ...
      else
      {
        ...
        vtkSetAndObserveMRMLObjectEventsMacroNoWarning(
          oldReferencedNode, oldReferencedNode, oldConsolidatedEventList.GetPointer()); // update the event list
    ...
    if (newReferencedNode != nullptr)
    {
      ...
      // event list
      vtkSetAndObserveMRMLObjectEventsMacroNoWarning(
        dummyNullReferencedNode,
        newReferencedNode,
        newConsolidatedEventList.GetPointer()); // update the event list & register
    }
  }
  ...
  for (it = this->NodeReferences.begin(); it != this->NodeReferences.end(); it++)
  {
    ...
    for (it1 = it->second.begin(); it1 != it->second.end(); it1++)
    {
      ...
      if (reference->GetReferencedNode())
      {
        vtkMRMLNode* nodePtr =
          reference
            ->GetReferencedNode(); // vtkSetAndObserveMRMLObjectMacro overwrites the argument, so we need to make a copy
...
vtkMRMLNode* vtkMRMLNode::SetAndObserveNodeReferenceID(const char* referenceRole,
                                                       ...
                                                       ContentModifiedObserveType observeContentModifiedEvents)
{
  return this->SetAndObserveNthNodeReferenceID(
    referenceRole, 0, referencedNodeID, events, observeContentModifiedEvents);
}
...
vtkMRMLNode* vtkMRMLNode::AddAndObserveNodeReferenceID(const char* referenceRole,
                                                       ...
                                                       ContentModifiedObserveType observeContentModifiedEvents)
{
  return this->SetAndObserveNthNodeReferenceID(referenceRole,
                                               this->GetNumberOfNodeReferences(referenceRole),
                                               referencedNodeID,
                                               events,
...
vtkMRMLNode::vtkMRMLNodeReference::~vtkMRMLNodeReference()
{
  if (this->GetReferencedNode() != nullptr)
  {
    ...
    // We cannot properly clean up the referenced node in the destructor, because the referencing node may be already
    // nullptr and we don't know which event observers we would need to remove. Therefore, we just report the error to
    // allow easier debugging.
    const char* referencedNodeId = this->ReferencedNode->GetID() ? this->ReferencedNode->GetID() : "(unknown)";
    const char* referencingNodeId = (this->ReferencingNode.GetPointer() && this->ReferencingNode->GetID())
                                      ? this->ReferencingNode->GetID()
                                      : "(unknown)";
    vtkWarningMacro("While deleting a reference object an active node reference is found to node "
                    << referencedNodeId << " from node " << referencingNodeId
...
void vtkMRMLNode::vtkMRMLNodeReference::SetStaticEvents(vtkIntArray* events)
{
  ...
  // Make a copy of the events to make sure the current values are used (and not the values that are current at the time
  // of node reference update).
