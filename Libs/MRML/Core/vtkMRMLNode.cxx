  ...
  while (*xmlReadAtts != nullptr)
  {
    ...
    if (!strcmp(attName, "attributes"))
    {
      ...
      while (std::getline(attributes, attribute, ';'))
      {
        ...
        int propertyStartIndex = attribute.find('{');
        int propertyEndIndex = attribute.find('}');
  ...
  while (start != std::string::npos && sep != std::string::npos && start != end && start != sep)
  {
    ...
    if (references.find(role) == references.end())
    {
      ...
      while (!ss.eof())
      {
        ...
        std::size_t referencePropertyStart = id.find_first_of('{');
        std::size_t referencePropertyEnd = id.find_first_of('}');
        std::string referenceProperties;
        if (referencePropertyStart != std::string::npos && referencePropertyEnd != std::string::npos)
        {
          referenceProperties = id.substr(referencePropertyStart + 1, referencePropertyEnd - referencePropertyStart - 1);
        }
        else
        {
        ...
        if (!referenceProperties.empty())
        {
          ...
          while (std::getline(ssProperties, property, ','))
          {
            ...
            {
              std::string key = property.substr(0, propertySeparator);
              std::string value = property.substr(propertySeparator + 1);
              int index = this->GetNumberOfNodeReferences(role.c_str()) - 1;
              this->SetNthNodeReferenceProperty(role.c_str(), index, key.c_str(), value.c_str());
            }
          }
        }
      }
