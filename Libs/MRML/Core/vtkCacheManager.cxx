...
int vtkCacheManager::GetCachedFileList(const char* dirname)
{
  ...
  {
    vtkDebugMacro("vtkCacheManager::GetCachedFileList: Cache Directory " << this->GetRemoteCacheDirectory()
...
const char* vtkCacheManager::GetFilenameFromURI(const char* uri)
{
  ...
  if (this->GetRemoteCacheDirectory() == nullptr || strcmp(this->GetRemoteCacheDirectory(), "") == 0)
  {
...
float vtkCacheManager::ComputeCacheSize(const char* dirName, unsigned long sz)
{
  ...
  {
    vtkDebugMacro("vtkCacheManager::ComputeCacheSize: Cache Directory " << this->GetRemoteCacheDirectory()
                                                                        << " doesn't look like a directory. \n");
    return (-1);
  }
