  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    if (mrmlNode && mrmlNode->IsA("vtkMRMLModelHierarchyNode"))
    {
      ...
      std::cerr << "  " << i << ": Model Hierarchy node"
                << " named [" << (hnode->GetName() ? hnode->GetName() : "null") << "] with id ["
                << (hnode->GetID() ? hnode->GetID() : "null") << "] has parent node id of ["
                << (hnode->GetParentNodeID() ? hnode->GetParentNodeID() : "null") << "]" << std::endl;
    }
  }
}
...
int ImportIDModelHierarchyParentIDConflictTestXMLString()
{
  ...
  const char scene1XML[] =
    ...
    "  <ModelHierarchy id=\"vtkMRMLModelHierarchyNode2\" name=\"vtkMRMLModelHierarchyNode2\" "
    "parentNodeRef=\"vtkMRMLModelHierarchyNode1\"></ModelHierarchy>"
    "  <ModelHierarchy id=\"vtkMRMLModelHierarchyNode3\" name=\"vtkMRMLModelHierarchyNode2\" "
    "parentNodeRef=\"vtkMRMLModelHierarchyNode2\"></ModelHierarchy>"
    "  <ModelHierarchy id=\"vtkMRMLModelHierarchyNode4\" name=\"vtkMRMLModelHierarchyNode2\" "
    "parentNodeRef=\"vtkMRMLModelHierarchyNode3\"></ModelHierarchy>"
    "  <ModelHierarchy id=\"vtkMRMLModelHierarchyNode5\" name=\"vtkMRMLModelHierarchyNode2\" "
    "parentNodeRef=\"vtkMRMLModelHierarchyNode4\"></ModelHierarchy>"
    "</MRML>";
  ...
  CHECK_INT_ADD_REPORT(scene->GetNumberOfNodesByClass("vtkMRMLModelHierarchyNode"),
                       10,
  ...
  for (int index = 0; index < 10; ++index)
  {
    ...
    CHECK_STRING_ADD_REPORT(hierarchyNode->GetParentNodeID(),
                            (index == 0 || index == 5) ? nullptr : expectedParentID.c_str(),
                            PrintModelHierarchyNodes(__LINE__, scene.GetPointer()));
  }
...
int ImportIDModelHierarchyParentIDConflictTestFile()
{
  ...
  CHECK_STRING_ADD_REPORT(hierarchyNode2->GetParentNodeID(),
                          "vtkMRMLModelHierarchyNode6",
                          PrintModelHierarchyNodes(__LINE__, scene3.GetPointer()));
