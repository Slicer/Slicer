  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    if (mrmlNode && mrmlNode->IsA("vtkMRMLModelHierarchyNode"))
    {
      ...
      std::cerr << "  " << i << ": Model Hierarchy node"
                << " named [" << (hnode->GetName() ? hnode->GetName() : "null") << "] with id [" << (hnode->GetID() ? hnode->GetID() : "null") << "] has parent node id of ["
...
int ImportIDModelHierarchyParentIDConflictTestXMLString()
{
  ...
  for (int index = 0; index < 10; ++index)
  {
    ...
    CHECK_STRING_ADD_REPORT(
      hierarchyNode->GetParentNodeID(), (index == 0 || index == 5) ? nullptr : expectedParentID.c_str(), PrintModelHierarchyNodes(__LINE__, scene.GetPointer()));
  }
