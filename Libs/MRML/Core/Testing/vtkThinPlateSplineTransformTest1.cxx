...
//----------------------------------------------------------------------------
void CreateThinPlateSplineVtk(vtkThinPlateSplineTransform* tpsTransform,
...
//----------------------------------------------------------------------------
void AddLandmarkDisplacement(vtkPoints* landmarks,
                             double landmarkIndexX,
                             double landmarkIndexY,
                             double landmarkIndexZ,
                             double dx,
                             double dy,
                             double dz,
                             double dims[3])
...
double getTransformedPointDifferenceItkVtk(const double inputPoint[3],
                                           itkThinPlateSplineType::Pointer tpsItk,
                                           vtkThinPlateSplineTransform* tpsVtk,
                                           bool logDetails)
{
  // ITK
...
//----------------------------------------------------------------------------
// Compute transformed point differences between single-precision and double-precision VTK ThinPlateSpline
// implementations
...
int vtkThinPlateSplineTransformTest1(int, char*[])
{
  ...
  for (int z = 0; z < dims[2]; z++)
  {
    for (int y = 0; y < dims[1]; y++)
    {
      for (int x = 0; x < dims[0]; x++)
      {
        ...
        for (int i = 0; i < 3; i++)
        {
          landmarkPosition[i] = origin[i] + x * spacing[0] * direction[i][0] + y * spacing[1] * direction[i][1]
                                + z * spacing[2] * direction[i][2];
        }
  ...
  itkThinPlateSplineType::Pointer tpsItk =
    CreateThinPlateSplineItk(sourceLandmarks.GetPointer(), targetLandmarks.GetPointer());
    ...
    for (double j = startJ + incJ; j <= endJ - incJ; j += incJ)
    {
      for (double i = startI + incI; i <= endI - incI; i += incI)
      {
        ...
        double differenceSingleDoubleVtk =
          getTransformedPointDifferenceSingleDoubleVtk(inputPoint, tpsVtk.GetPointer(), false);
