...
//----------------------------------------------------------------------------
itkBSplineType::Pointer CreateBSplineItk(const double origin[3],
...
void SetBSplineNodeItk(itkBSplineType::Pointer bspline, int nodeIndex[3], double nodeValue[3])
{
  ...
  parameters.SetElement(numberOfNodes * 0 + nodeIndex[0] + nodeIndex[1] * dims[0] + nodeIndex[2] * dims[0] * dims[1],
                        DisplacementScale * nodeValue[0]);
  parameters.SetElement(numberOfNodes * 1 + nodeIndex[0] + nodeIndex[1] * dims[0] + nodeIndex[2] * dims[0] * dims[1],
                        DisplacementScale * nodeValue[1]);
  parameters.SetElement(numberOfNodes * 2 + nodeIndex[0] + nodeIndex[1] * dims[0] + nodeIndex[2] * dims[0] * dims[1],
                        DisplacementScale * nodeValue[2]);
...
double getTransformedPointDifferenceItkVtk(const double inputPoint[3],
                                           itkBSplineType::Pointer bsplineItk,
                                           ...
                                           bool logDetails)
{
  ...
  if (logDetails)
  {
    ...
    std::cout << " Output point (transformed by VTK): " << outputPoint[0] << " " << outputPoint[1] << " "
...
double getTransformedPointDifferenceSingleDoubleVtk(const double inputPoint[3],
                                                    vtkOrientedBSplineTransform* bsplineVtk,
                                                    bool logDetails)
{
  double outputPoint[3] = { 0 };
  ...
  if (logDetails)
  {
    ...
    std::cout << " Input point: " << inputPoint[0] << " " << inputPoint[1] << " " << inputPoint[2] << std::endl;
    std::cout << " Output point (transformed by VTK, double-precision): " << outputPoint[0] << " " << outputPoint[1]
              << " " << outputPoint[2] << std::endl;
    std::cout << " Output point (transformed by VTK, single-precision): " << floatOutputPoint[0] << " "
              << floatOutputPoint[1] << " " << floatOutputPoint[2] << std::endl;
    std::cout << " Difference between single-precision and double-precision VTK transform results: " << difference
              << std::endl;
  }
...
double getDerivativeErrorVtk(const double inputPoint[3], vtkOrientedBSplineTransform* bsplineVtk, bool logDetails)
{
  ...
  for (int row = 0; row < 3; row++)
  {
    for (int col = 0; col < 3; col++)
    {
      ...
      if (logDetails)
      {
        std::cout << " Element[" << row << "," << col << "]: Ground truth: " << jacobianEstimation[row][col]
                  << "  VTK: " << jacobianVtk[row][col] << "  Difference: " << difference << std::endl;
      }
    }
  }
...
double getInverseErrorVtk(const double inputPoint[3], vtkOrientedBSplineTransform* bsplineVtk, bool logDetails)
{
  ...
  if (logDetails)
  {
    ...
    std::cout << " Input point: " << inputPoint[0] << " " << inputPoint[1] << " " << inputPoint[2] << std::endl;
    std::cout << " Transformed point: " << outputPoint[0] << " " << outputPoint[1] << " " << outputPoint[2]
              << std::endl;
    std::cout << " Transformed point transformed by inverse: " << inversePoint[0] << " " << inversePoint[1] << " "
              << inversePoint[2] << std::endl;
    std::cout << " Difference between VTK transform inverse and the ground truth: " << errorOfInverseComputation
              << std::endl;
  }
...
int vtkOrientedBSplineTransformTest1(int, char*[])
{
  ...
  double direction[3][3] = { { 0.92128500, -0.36017075, -0.146666625 },
  ...
  for (double k = startK + incK; k <= endK - incK; k += incK)
  {
    for (double j = startJ + incJ; j <= endJ - incJ; j += incJ)
    {
      for (double i = startI + incI; i <= endI - incI; i += incI)
      {
        ...
        double inputPoint[3];
        inputPoint[0] = origin[0] + direction[0][0] * spacing[0] * i + direction[0][1] * spacing[1] * j
                        + direction[0][2] * spacing[2] * k;
        inputPoint[1] = origin[1] + direction[1][0] * spacing[0] * i + direction[1][1] * spacing[1] * j
                        + direction[1][2] * spacing[2] * k;
        inputPoint[2] = origin[2] + direction[2][0] * spacing[0] * i + direction[2][1] * spacing[1] * j
                        + direction[2][2] * spacing[2] * k;
        // Compare transformation results computed by ITK and VTK.
        double differenceItkVtk =
          getTransformedPointDifferenceItkVtk(inputPoint, bsplineItk, bsplineVtk.GetPointer(), false);
        if (differenceItkVtk > 1e-6)
        {
          getTransformedPointDifferenceItkVtk(inputPoint, bsplineItk, bsplineVtk.GetPointer(), true);
          std::cout << "ERROR: Point transform result mismatch between ITK and VTK at grid point (" << i << "," << j
                    << "," << k << ")" << std::endl;
          numberOfItkVtkPointMismatches++;
        }
        double differenceSingleDoubleVtk =
          getTransformedPointDifferenceSingleDoubleVtk(inputPoint, bsplineVtk.GetPointer(), false);
        if (differenceSingleDoubleVtk > 1e-4)
        {
          getTransformedPointDifferenceSingleDoubleVtk(inputPoint, bsplineVtk.GetPointer(), true);
          std::cout << "ERROR: Point transform result mismatch between single-precision and double-precision VTK "
        ...
        if (derivativeError > 1e-6)
        {
          ...
          std::cout
            << "ERROR: Transform derivative result mismatch between VTK and numerical approximation at grid point ("
            << i << "," << j << "," << k << ")" << std::endl;
        ...
        if (inverseError > 1e-3)
        {
          ...
          std::cout << "ERROR: Point transformed by forward and inverse transform does not match the original point"
                    << std::endl;
