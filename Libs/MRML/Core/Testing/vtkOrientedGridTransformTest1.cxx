...
//----------------------------------------------------------------------------
itkGridTransformType::Pointer CreateGridTransformItk(const double origin[3],
...
double getTransformedPointDifferenceItkVtk(const double inputPoint[3],
                                           itkGridTransformType::Pointer gridItk,
                                           vtkOrientedGridTransform* gridVtk,
                                           bool logDetails)
{
  // ITK
...
//----------------------------------------------------------------------------
// Compute transformed point differences between single-precision and double-precision VTK grid transform
// implementations
      ...
      for (double i = startI + incI; i <= endI - incI; i += incI)
      {
        ...
        if (differenceItkVtk > 1e-2)
        {
          ...
          std::cout << "ERROR: Point transform result mismatch between ITK and VTK at grid point (" << i << "," << j
                    << "," << k << ") with linear interpolation" << std::endl;
  ...
  for (double k = startK + incK; k <= endK - incK; k += incK)
  {
    for (double j = startJ + incJ; j <= endJ - incJ; j += incJ)
    {
      for (double i = startI + incI; i <= endI - incI; i += incI)
      {
        ...
        // Compare transformation results computed by ITK and VTK.
        double differenceItkVtk = getTransformedPointDifferenceItkVtk(inputPoint, gridItk, gridVtk.GetPointer(), false);
        // the larger the distance between the grid points, the larger difference is expected between ITK's linear and
        // VTK's cubic interpolation, therefore make the threshold the 20% of the spacing
        if (differenceItkVtk > averageSpacing * 0.20)
        {
          getTransformedPointDifferenceItkVtk(inputPoint, gridItk, gridVtk.GetPointer(), true);
          std::cout << "ERROR: Point transform result mismatch between ITK and VTK at grid point (" << i << "," << j
                    << "," << k << ") with cubic interpolation" << std::endl;
          numberOfItkVtkPointMismatches++;
        }
        // Verify single/double-precision computation difference
        double differenceSingleDoubleVtk =
          getTransformedPointDifferenceSingleDoubleVtk(inputPoint, gridVtk.GetPointer(), false);
