...
public:
  ...
  vtkMRMLNode* CreateNodeInstance() override { return vtkMRMLVolumeNodeTestHelper1::New(); }
  const char* GetNodeTagName() override { return "vtkMRMLVolumeNodeTestHelper1"; }
...
int vtkMRMLVolumeNodeTest1(int, char*[])
{
  ...
  {
    std::cerr << "Failed to compute scan order from identity matrix: '" << (scanOrder ? scanOrder : "null") << "'"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (std::vector<std::string>::iterator it = scanOrders.begin(); it != scanOrders.end(); ++it)
  {
    ijkToRAS->Identity();
    vtkMRMLVolumeNode::ComputeIJKToRASFromScanOrder(
      it->c_str(), spacing, dimensions, /* centerImage= */ false, ijkToRAS.GetPointer());

    const char* computedScanOrder = vtkMRMLVolumeNode::ComputeScanOrderFromIJKToRAS(ijkToRAS.GetPointer());

    if (!computedScanOrder || strcmp(computedScanOrder, "") == 0 || *it != computedScanOrder)
    {
      std::cerr << "Failed to compute scan order from '" << *it << "' IJKToRAS matrix: '"
                << (computedScanOrder ? computedScanOrder : "null") << "'" << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      ...
      {
        std::cerr << "GetIJKToRASDirections at " << i << ", " << j << ": " << getDirs[i][j]
                  << " not equal to set value: " << dirs[i][j] << std::endl;
        return EXIT_FAILURE;
      }
    }
  }
  ...
  node1->SetIJKToRASDirections(
    -0.03164, -0.0606374, 5.92996, -0.448103, 0.00428152, -0.418707, 0.0, -0.445087, -0.811908);
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      ...
      {
        std::cerr << "Error setting and getting IJKToRASDirectionMatrix, element " << i << ", " << j
                  << " does not match: " << retIJKToRASDir->GetElement(i, j) << " != " << ijkToRASDir->GetElement(i, j)
  ...
  std::cout << "RAS: [" << ras[0] << "," << ras[1] << "," << ras[2] << "] -> IJK: [" << ijk[0] << "," << ijk[1] << ","
            << ijk[2] << "]" << std::endl;
  ...
  std::cout << "IJK: [" << ijk[0] << "," << ijk[1] << "," << ijk[2] << "] -> RAS: [" << ras2[0] << "," << ras2[1] << ","
            << ras2[2] << "]" << std::endl;
  ...
  node1->GetIJKToRASMatrix(ijkToRAS.GetPointer());
  ijkToRAS->MultiplyPoint(ijk, ras2);
  std::cout << "IJK: [" << ijk[0] << "," << ijk[1] << "," << ijk[2] << "] -> RAS: [" << ras[0] << "," << ras[1] << ","
            << ras[2] << "]" << std::endl;
