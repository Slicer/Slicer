...
int TestSetMesh(bool observeMeshBeforeObserveDisplay, bool observeDisplayBeforeAddToScene, bool meshTypeIsPolyData)
{
  ...
  {
    std::cerr << __LINE__ << ": vtkMRMLModelNode::SetAndobserveMesh failed when " << (meshTypeIsPolyData ? "surface" : "volumetric") << " mesh is set "
              << (observeMeshBeforeObserveDisplay ? "before" : "after") << " the display node is observed and when the display node is "
