...
int vtkMRMLInteractionNodeTest1(int, char*[])
{
  ...
  {
    std::cerr << "Error in SwitchToPersistentPlaceMode, got persistence = " << node1->GetPlaceModePersistence()
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (node1->GetPlaceModePersistence())
  {
    std::cerr << "Error in SwitchToSinglePlaceMode, got persistence = " << node1->GetPlaceModePersistence()
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  {
    std::cerr << "Error in SwitchToViewTransformMode, got persistence = " << node1->GetTransformModePersistence()
              << std::endl;
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
