...
int vtkImageMapToWindowLevelAddon::RequestData(vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (this->LookupTable == nullptr && !this->GetLogCompressWindow() && (inData->GetScalarType() == VTK_UNSIGNED_CHAR && this->Window == 255 && this->Level == 127.5))
  {
 ...
 */
template <class T>
void vtkImageMapToWindowLevelClamps(vtkImageData* data, double w, double l, T& lower, T& upper, unsigned char& lower_val, unsigned char& upper_val)
{
...
void vtkImageMapToWindowLevelAddonExecute(vtkImageMapToWindowLevelAddon* self, vtkImageData* inData, T* inPtr, vtkImageData* outData, unsigned char* outPtr, int outExt[6], int id)
{
  ...
  double window_scale = 255.0 / self->GetWindow();
  double log_scale = 9.0 / self->GetWindow();
  double log_shift = 1.0;
  double log_tmp_val;

  T lower, upper;
  unsigned char lower_val, upper_val, result_val;
  unsigned short ushort_val;
  vtkImageMapToWindowLevelClamps(inData, self->GetWindow(), self->GetLevel(), lower, upper, lower_val, upper_val);
    ...
    for (idxY = 0; !self->AbortExecute && idxY < extY; idxY++)
    {
      ...
      if (lookupTable)
      {
        ...
        for (idxX = 0; idxX < extX; idxX++)
        {
          ...
          else if (self->GetLogCompressWindow())
          {
            log_tmp_val = static_cast<double>(((*iptr + shift) * log_scale) + log_shift);
            ushort_val = static_cast<unsigned char>(255.0 * std::log10((log_tmp_val < 1.0) ? 1.0 : (log_tmp_val > 10.0) ? 10.0 : log_tmp_val));
          }
          else
          {
            ushort_val = static_cast<unsigned char>((*iptr + shift) * window_scale);
          }
      ...
      else
      {
        for (idxX = 0; idxX < extX; idxX++)
        {
          ...
          {
            log_tmp_val = static_cast<double>(((*iptr + shift) * log_scale) + log_shift);
            result_val = static_cast<unsigned char>(255.0 * std::log10((log_tmp_val < 1.0) ? 1.0 : (log_tmp_val > 10.0) ? 10.0 : log_tmp_val));
          }
          else
          {
            result_val = static_cast<unsigned char>((*iptr + shift) * window_scale);
          }
          *optr = result_val;
          switch (outputFormat)
          {
...
void vtkImageMapToWindowLevelAddon::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "LogCompressWindow: " << this->LogCompressWindow << endl;
}
