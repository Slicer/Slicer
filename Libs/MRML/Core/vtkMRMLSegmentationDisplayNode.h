...
public:
  ...
  struct SegmentDisplayProperties
  {
    ...
    /// By default it's invalid (-1,-1,-1), and only valid if different than color stored in segment. Its purpose is to
    /// enable showing per-view colors
    double OverrideColor[3];
    /// Visibility
    bool Visible{ true }; // Turns visibility on/off in general. Useful for allowing the user to show/hide a segment
                          // without changing any detailed visibility options.
    bool Visible3D{ true };
    bool Visible2DFill{
      true
    }; // This one is used for labelmap volume related operations (color table, merged labelmap)
    ...
    /// Pickable by interactions flag. If true, then the point under the mouse on the segment surfaces in the 3D views
    /// is computed, otherwise skipped.
    bool Pickable{ true };
    ...
    bool operator==(const SegmentDisplayProperties& rhs) const
    {
      ...
      return Visible == rhs.Visible && Visible3D == rhs.Visible3D && Visible2DFill == rhs.Visible2DFill &&
             Visible2DOutline == rhs.Visible2DOutline && Opacity3D == rhs.Opacity3D &&
             Opacity2DFill == rhs.Opacity2DFill && Opacity2DOutline == rhs.Opacity2DOutline && Pickable == rhs.Pickable;
    }
  };
