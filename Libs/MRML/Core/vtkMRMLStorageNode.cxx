  ...
  if (this->FileName != nullptr)
  {
    ...
    */
    if (this->GetScene() && this->IsFilePathRelative(this->FileName))
    {
      ...
      {
        vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "WriteXML", "Unable to convert relative file path to absolute, still using " << this->FileName);
      }
    }
  }
...
void vtkMRMLStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "URI: " << (this->URI ? this->URI : "(none)") << "\n";
  for (int i = 0; i < this->GetNumberOfURIs(); i++)
  {
  ...
  {
    os << indent << "FileType: " << this->SupportedWriteFileTypes->GetValue(i) << "\n";
  }
  os << indent << "WriteFileFormat: " << (this->WriteFileFormat ? this->WriteFileFormat : "(none)") << "\n";
  os << indent << "DefaultWriteFileExtension: " << (this->GetDefaultWriteFileExtension() ? this->GetDefaultWriteFileExtension() : "(none)") << "\n";
...
void vtkMRMLStorageNode::StageReadData(vtkMRMLNode* refNode)
{
    ...
    if (this->GetReadState() == this->Idle)
    {
      ...
      else
      {
        vtkErrorToMessageCollectionMacro(
          this->GetUserMessages(), "vtkMRMLStorageNode::StageReadData", "Unable to get a URI handler for " << this->URI << ", resetting stage to idle");
        this->SetReadStateIdle();
        return;
      }
  ...
  {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::StageReadData", "No IO Manager on the scene");
  }
...
void vtkMRMLStorageNode::StageWriteData(vtkMRMLNode* refNode)
{
  ...
  if (iomanager != nullptr)
  {
    if (this->GetWriteState() == this->Idle)
    {
      ...
      {
        vtkErrorToMessageCollectionMacro(
          this->GetUserMessages(), "vtkMRMLStorageNode::StageWriteData", "Unable to get a URI handler for " << this->URI << ", resetting stage to idle");
        return;
      }
  ...
  {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::StageWriteData", "StageWriteData: No IO Manager on the scene");
  }
}
...
std::string vtkMRMLStorageNode::GetFullNameFromNthFileName(int n)
{
  ...
  else
  {
    ...
    {
      vtkDebugMacro("GetFullNameFromNthFileName: scene root dir = " << (this->Scene->GetRootDirectory() != nullptr ? this->Scene->GetRootDirectory() : "null")
...
int vtkMRMLStorageNode::FileNameIsInList(const char* fileName)
{
  ...
  const std::string relativeFileName = fileNameIsRelative ? fileNameString : vtksys::SystemTools::RelativePath(rootDir, fileName);

  for (std::vector<std::string>::const_iterator it = this->FileNameList.begin(); it != this->FileNameList.end(); ++it)
  {
    ...
    {
      vtkDebugMacro("WARNING: trying to determine if file " << fileName << " is already in the list and comparing against " << it->c_str()
                                                            << ", they have mismatched absolute/relative paths. "
                                                            << "Using scene root dir to disambiguate: " << rootDir);
      std::string thisRelativeFileName = thisFileIsRelative ? *it : vtksys::SystemTools::RelativePath(rootDir, it->c_str());
      vtkDebugMacro("\tComparing " << relativeFileName << " and " << thisRelativeFileName);
...
unsigned int vtkMRMLStorageNode::AddFileName(const char* filename)
{
  if (!filename)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::AddFileName", "Cannot add a null file name");
    return 0;
  }
...
void vtkMRMLStorageNode::ResetNthFileName(int n, const char* fileName)
{
  if (fileName == nullptr)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ResetNthFileName", "ResetNthFileName: given file name is null (n = " << n << ").");
    return;
  }
  if (n < 0)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ResetNthFileName", "ResetNthFileName: invalid file name number (n = " << n << ").");
    return;
  }
  else if (n >= this->GetNumberOfFileNames())
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ResetNthFileName", "ResetNthFileName: file name number " << n << " not already set.");
    return;
  }
...
unsigned int vtkMRMLStorageNode::AddURI(const char* uri)
{
  if (uri == nullptr)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::AddURI", "Cannot add a null URI.");
    return 0;
  }
...
void vtkMRMLStorageNode::ResetNthURI(int n, const char* uri)
{
  if (uri == nullptr)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ResetNthURI", "ResetNthURI: given URI is null (n = " << n << ").");
    return;
  }
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ResetNthURI", "ResetNthURI: URI number " << n << " not already set.");
  }
}
...
void vtkMRMLStorageNode::SetDataDirectory(const char* dataDirName)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::SetDataDirectory", "Input directory name is null.");
    return;
  }
  if (this->GetFileName() == nullptr)
  {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::SetDataDirectory", "File name is null, no reason to reset data directory.");
    return;
  }
  // reset the filename
...
void vtkMRMLStorageNode::SetURIPrefix(const char* uriPrefix)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::SetURIPrefix", "Input URI prefix is null.");
    return;
  }
  vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::SetURIPrefix", "Setting URI prefix is not implemented yet.");
  // reset the uri
...
const char* vtkMRMLStorageNode::GetAbsoluteFilePath(const char* inputPath)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLStorageNode::GetAbsoluteFilePath", "Relative path " << inputPath << " is specified but no scene or root directory to find it from.");
    return nullptr;
  }
...
int vtkMRMLStorageNode::ReadData(vtkMRMLNode* refNode, bool temporary)
{
  if (refNode == nullptr)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ReadData", "Cannot read data into a null node.");
    return 0;
  }

  if (!this->CanReadInReferenceNode(refNode))
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ReadData", "Cannot read data into reference node of class " << refNode->GetClassName() << ".");
    return 0;
  }
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ReadData", "Both filename and uri are null.");
    return 0;
  }
  ...
  if (this->GetReadState() != this->TransferDone)
  {
    ...
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::ReadData", "ReadData: read state is pending, remote download hasn't finished yet");
    return 0;
  }
  vtkDebugMacro("ReadData: read state is ready, " << "URI = " << (this->GetURI() == nullptr ? "null" : this->GetURI()) << ", "
  ...
  if (!success)
  {
    ...
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLStorageNode::ReadData", "Failed to read node " << storableNodeName << " (" << storableNodeID << ")" << location);
...
int vtkMRMLStorageNode::WriteData(vtkMRMLNode* refNode)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLStorageNode::WriteData", "Cannot write " << (this->GetID() ? this->GetID() : "(null)") << ": input node is null");
    return 0;
  }
...
std::string vtkMRMLStorageNode::GetLowercaseExtensionFromFileName(const std::string& filename)
{
  ...
  if (extension.compare(".gz") == 0)
  {
    ...
    // the full extension
    extension =
      vtksys::SystemTools::GetFilenameLastExtension(vtksys::SystemTools::GetFilenameWithoutLastExtension(filename)) + vtksys::SystemTools::GetFilenameLastExtension(filename);
  }
...
void vtkMRMLStorageNode::FixFileName()
{
  ...
  vtkDebugMacro("FixFileName: File list member " << collapsedFullPath << " is not found relative to the mrml root directory. " << collapsedFullPath
                                                 << " was not found. Trying to find it relative to the archetype: " << this->GetFileName());
  ...
  if (vtksys::SystemTools::FileExists(secondCollapsedFullPath.c_str(), true) == false)
  {
    if (logWarning)
    {
      vtkWarningMacro("FixFileName: file list member '" << collapsedFullPath << "' is not found. Path relative to the archetype '" << secondCollapsedFullPath
                                                        << "' is not found either.");
    }
  }
  else
  {
    if (logWarning)
    {
      vtkWarningMacro("FixFileName: file list member '" << collapsedFullPath << "' is not found. Using path relative to the archetype '" << secondCollapsedFullPath
                                                        << "' instead.");
    }
...
//----------------------------------------------------------------------------
bool vtkMRMLStorageNode::GenerateTempFilePathForWrite(const std::string& fullFileName, std::string& tempDir, std::string& tempFullFileName)
{
  if (fullFileName.empty())
  {
    return false;
  }
  if (!vtksys::SystemTools::FileIsFullPath(fullFileName))
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::GenerateTempFilePathForWrite", "File name '" << fullFileName << "' is not a full path.");
    return false;
  }

  // Use the directory of the first file as the target directory
  std::string targetDir = vtksys::SystemTools::GetFilenamePath(fullFileName);
  // Generate a unique temporary subdirectory name using process ID and current time
  std::string tempSubDir = std::string("TempWrite") + vtksys::SystemTools::GetFilenameWithoutExtension(fullFileName);
  ...
  // Generate 6 quasi-random numbers (it is not too long so that filepath still remains valid, but name clashes are practically impossible)
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::CreateUniqueTempFilesForWrite", "Failed to create folder: '" << tempDir << "'");
    return false;
  }

  std::string fileName = vtksys::SystemTools::GetFilenameName(fullFileName);
  pathComponents.push_back(fileName);
  tempFullFileName = vtksys::SystemTools::JoinPath(pathComponents);
...
bool vtkMRMLStorageNode::MoveFilesWithLocking(const std::string& sourceDir, const std::string& targetDir)
{
  ...
  // Use std::filesystem for recursive directory iteration
  try
  {
    // NOTE: some writers (e.g., closed surface segmentation - .seg.vtm) may write files in nested directories
    for (std::filesystem::recursive_directory_iterator it(sourceDir), end; it != end; ++it)
    {
      ...
      // Compute relative path from sourceDir to the file
      std::filesystem::path relativePath = std::filesystem::relative(entry.path(), sourceDir);
      // The target path should be targetDir + relativePath
      std::filesystem::path targetPath = std::filesystem::path(targetDir) / relativePath;

      targetFiles->InsertNextValue(targetPath.string());
      sourceFiles->InsertNextValue(filePath);
    }
  }
  catch (const std::filesystem::filesystem_error& ex)
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::MoveFilesWithLocking", "Filesystem error: " << ex.what());
    return false;
  }
  ...
  // Use a single lock file for all target files (use the first target file)
  std::string lockFile = targetFiles->GetValue(0) + ".lock";
  ...
  {
    // Use platform-specific atomic file creation
#ifdef _WIN32
    HANDLE hFile = CreateFileA(lockFile.c_str(), GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile != INVALID_HANDLE_VALUE)
    {
      lockAcquired = true;
      DWORD bytesWritten;
      const char* lockContent = "locked\n";
      WriteFile(hFile, lockContent, strlen(lockContent), &bytesWritten, NULL);
      CloseHandle(hFile);
      break;
    }
#else
    ...
    // Lock file already exists or creation failed, wait and retry
  ...
  {
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLStorageNode::MoveFilesWithLocking", "Timeout waiting for lock file. Another process may be writing to one or more files.");
    return false;
  }
  ...
  {
    std::string targetFile = targetFiles->GetValue(fileIndex);
    ...
    if (vtksys::SystemTools::FileExists(targetFile.c_str(), true))
    {
      ...
      {
        vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::MoveFilesWithLocking", "Unable to remove old version of file: " << targetFile);
        allSuccess = false;
        continue;
      }
    }

    // Ensure target directory exists for netsted files for example in .seg.vtm case
    std::string targetDir = vtksys::SystemTools::GetFilenamePath(targetFile);
    if (!vtksys::SystemTools::FileExists(targetDir.c_str()))
    {
      if (!vtksys::SystemTools::MakeDirectory(targetDir.c_str()))
      {
        vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLStorageNode::MoveFilesWithLocking", "Failed to create target directory: " << targetDir);
        allSuccess = false;
        continue;
      }
    }
    ...
    // rename does not remove the temp file if it fails; if it succeeds, the temp file is moved to targetFile path and no longer exists at sourceFile path
    if (renameReturn != 0)
    {
      vtkErrorToMessageCollectionMacro(this->GetUserMessages(),
                                       "vtkMRMLStorageNode::MoveFilesWithLocking",
                                       "Failed to move source file " << sourceFile << " to " << targetFile << ", rename returned code " << renameReturn);
      allSuccess = false;
      continue;
    }
  }
