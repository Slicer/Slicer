...
vtkLookupTable* vtkMRMLProceduralColorNode::GetLookupTable()
{
  ...
  {
    int baseIndex = i * 3;
    this->ConvertedCTFtoLUT->SetTableValue(
      i, bareTable[baseIndex], bareTable[baseIndex + 1], bareTable[baseIndex + 2], 1.0);
  }
...
bool vtkMRMLProceduralColorNode::GetColor(int entry, double color[4])
{
  if (entry < 0 || entry >= this->GetNumberOfColors())
  {
    vtkErrorMacro("vtkMRMLColorTableNode::SetColor: requested entry "
                  << entry << " is out of table range: 0 - " << this->GetNumberOfColors() << ", call SetNumberOfColors"
                  << endl);
    return false;
  }
  /*
