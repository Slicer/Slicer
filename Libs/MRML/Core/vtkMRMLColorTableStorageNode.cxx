...
int vtkMRMLColorTableStorageNode::ReadDataInternal(vtkMRMLNode* refNode)
{
  ...
  if (fullName.empty())
  {
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLColorTableStorageNode::ReadDataInternal", "Filename is not specified (" << (this->ID ? this->ID : "(unknown)") << ").");
    return 0;
  }
...
int vtkMRMLColorTableStorageNode::ReadCsvFile(std::string fullFileName, vtkMRMLColorTableNode* colorNode)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLColorTableStorageNode::ReadCsvFile", "Failed to read color table file: '" << fullFileName << "'.");
    return 0;
  }
  ...
  {
    vtkErrorToMessageCollectionMacro(
      this->GetUserMessages(), "vtkMRMLColorTableStorageNode::ReadCsvFile", "Failed to read color table file: '" << fullFileName << "' due to missing 'LabelValue' column.");
    return 0;
  }
  ...
  for (vtkIdType row = 0; row < numberOfTuples; ++row)
  {
    ...
    if (labelValueColumn->GetValue(row).empty())
    {
      vtkErrorToMessageCollectionMacro(
        this->GetUserMessages(),
        "vtkMRMLColorTableStorageNode::ReadCsvFile",
        vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "labelValue is not specified in color file in line %1. Skipping this line."), std::to_string(fileRow).c_str())
          .c_str());
      continue;
    }
    ...
    if (!valid)
    {
      vtkErrorToMessageCollectionMacro(this->GetUserMessages(),
                                       "vtkMRMLColorTableStorageNode::ReadCsvFile",
                                       vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "labelValue '%1' is invalid in color file in line %2. Skipping this line."),
    ...
    if (labelValue < 0)
    {
      vtkErrorToMessageCollectionMacro(
        this->GetUserMessages(),
        "vtkMRMLColorTableStorageNode::ReadCsvFile",
        vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "labelValue '%1' is invalid in color file in line %2: the value must be positive (>0). Skipping this line."),
                            labelValueColumn->GetValue(row).c_str(),
                            std::to_string(fileRow).c_str())
          .c_str());
      continue;
    ...
    if (maxLabelValue > this->MaximumColorID)
    {
      vtkErrorToMessageCollectionMacro(
        this->GetUserMessages(),
        "vtkMRMLColorTableStorageNode::ReadCsvFile",
        vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "labelValue '%1' is invalid in color file in line %2: the value must not go above be %3."),
                            labelValueColumn->GetValue(row).c_str(),
                            std::to_string(fileRow).c_str(),
  ...
  for (vtkIdType row = 0; row < numberOfTuples; ++row)
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      if (colorColumn && row < colorColumn->GetNumberOfTuples())
      {
        ...
        else
        {
          vtkErrorToMessageCollectionMacro(this->GetUserMessages(),
                                           "vtkMRMLColorTableStorageNode::ReadCsvFile",
                                           vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "Failed to read color from column %1 in line %2, value: '%3'"),
    ...
    if (colorNode->SetColor(validLabelValues[row], name.c_str(), rgba[0], rgba[1], rgba[2], rgba[3]) == 0)
    {
      vtkErrorToMessageCollectionMacro(
        this->GetUserMessages(),
        "vtkMRMLColorTableStorageNode::ReadCsvFile",
        vtkMRMLI18N::Format(vtkMRMLTr("vtkMRMLColorTableStorageNode", "Unable to set color '%1' with name '%2', stopped processing after '%3' lines in the file %4."),
...
int vtkMRMLColorTableStorageNode::ReadCtblFile(std::string fullFileName, vtkMRMLColorTableNode* colorNode)
{
  ...
  while (fstr.good())
  {
    ...
    if (line[0] == '#')
    {
      ...
      {
        vtkErrorMacro("ReadDataInternal:\nfound a comment that this file " << " is a procedural color file, returning:\n" << line);
        return 0;
      }
    }
    else
  ...
  {
    vtkErrorMacro("ReadData: maximum color id " << maxID << " is > " << this->MaximumColorID << ", invalid color file: " << fullFileName);
    colorNode->SetNumberOfColors(0);
    return 0;
  }
  ...
  for (unsigned int i = 0; i < lines.size(); i++)
  {
    ...
    {
      vtkWarningMacro("ReadData: unable to set color " << id << " with name " << name.c_str() << ", breaking the loop over " << lines.size() << " lines in the file "
                                                       << this->FileName);
      return 0;
    }
  }
...
int vtkMRMLColorTableStorageNode::WriteCsvFile(std::string fullFileName, vtkMRMLColorTableNode* colorNode)
{
  ...
  std::vector<GetEntryFuncPtr> terminologyGetEntryFuncVector = {
    ...
    &vtkMRMLColorNode::GetTerminologyRegionModifier, &vtkMRMLColorNode::GetTerminologyRegionModifier, &vtkMRMLColorNode::GetTerminologyRegionModifier
  };
  ...
  return vtkMRMLTableStorageNode::WriteTable(fullFileName, colorTable, this->GetFieldDelimiterCharacters(fullFileName), componentNamesMap);
}
