...
//------------------------------------------------------------------------------
int vtkCurveMeasurementsCalculator::RequestData(vtkInformation* vtkNotUsed(request), vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (!inputPolyData || inputPolyData->GetNumberOfPoints() == 0 || inputPolyData->GetNumberOfLines() == 0)
  {
    // Clear values
...
bool vtkCurveMeasurementsCalculator::CalculatePolyDataCurvature(vtkPolyData* polyData)
{
  ...
  vtkSmartPointer<vtkDoubleArray> curvatureValues = vtkDoubleArray::SafeDownCast(polyData->GetPointData()->GetArray(this->GetCurvatureArrayName()));
  ...
  else
  {
    ...
    curvatureValues->InsertValue(linePoints->GetId(numberOfPoints - 1), curvatureValues->GetValue(linePoints->GetId(numberOfPoints - 2)));
  }
...
bool vtkCurveMeasurementsCalculator::CalculatePolyDataTorsion(vtkPolyData* polyData)
{
  ...
  vtkDoubleArray* tangents = vtkDoubleArray::SafeDownCast(pointData->GetAbstractArray(curveCoordinateSystemGenerator->GetTangentsArrayName()));
  vtkDoubleArray* binormals = vtkDoubleArray::SafeDownCast(pointData->GetAbstractArray(curveCoordinateSystemGenerator->GetBinormalsArrayName()));
  // Sanity checks
  vtkPolyData* curvePoly = curveCoordinateSystemGenerator->GetOutput();
  if (curvePoly->GetNumberOfPoints() != polyData->GetNumberOfPoints())
  {
    // Sanity check
    vtkErrorMacro("Number of points does not match between given poly data and the coordinate system generator poly data (" << curvePoly->GetNumberOfPoints()
                                                                                                                            << " != " << polyData->GetNumberOfPoints() << ")");
    return false;
  }
  if (binormals->GetNumberOfTuples() != pointData->GetNumberOfTuples())
  {
    vtkErrorMacro("Number of data points does not match between coordinate system generator (" << binormals->GetNumberOfTuples() << ") and poly data point data ("
  ...
  vtkSmartPointer<vtkDoubleArray> torsionArray = vtkDoubleArray::SafeDownCast(polyData->GetPointData()->GetArray(this->GetTorsionArrayName()));
  ...
  if (!this->CurveIsClosed)
  {
    torsionArray->InsertValue(linePoints->GetId(numberOfPoints - 1), torsionArray->GetValue(linePoints->GetId(numberOfPoints - 2)));
  }
  else
  {
...
bool vtkCurveMeasurementsCalculator::InterpolateControlPointMeasurementToPolyData(vtkPolyData* outputPolyData)
{
  ...
  {
    vtkErrorMacro("InterpolateControlPointMeasurementToPolyData: Size mismatch between PedigreeIDs array (" << pedigreeIdsArray->GetNumberOfTuples() << ") and polydata points ("
                                                                                                            << numberOfPoints << ")");
    return false;
  }
...
//---------------------------------------------------------------------------
void vtkCurveMeasurementsCalculator::OnControlPointArrayModified(vtkObject* vtkNotUsed(caller), unsigned long vtkNotUsed(eid), void* clientData, void* vtkNotUsed(callData))
{
