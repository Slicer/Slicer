                              ...
                              vtkMRMLMessageCollection* userMessages /*=nullptr*/)
{
  ...
  tempDirStr << tempBaseDir << "/" << vtksys::SystemTools::GetCurrentDateTime("__BundleSaveTemp-%F_%H%M%S_")
             << (this->RandomGenerator() % 1000);
  ...
  if (!retval)
  {
    vtkErrorToMessageCollectionMacro(userMessages,
                                     ...
                                     "Failed to save " << filename
                                                       << ": Failed to save scene to data bundle directory");
    return false;
  }
  ...
  {
    vtkErrorToMessageCollectionMacro(
      userMessages, "vtkMRMLScene::WriteToMRB", "Failed to save " << filename << ": Could not compress bundle");
    return false;
  }
                               ...
                               vtkMRMLMessageCollection* userMessagesInput /*=nullptr*/)
{
  ...
  unpackDirStr << tempBaseDir << "/" << vtksys::SystemTools::GetCurrentDateTime("__BundleLoadTemp-%F_%H%M%S_")
               << (this->RandomGenerator() % 1000);
                                                        ...
                                                        vtkMRMLMessageCollection* userMessagesInput /*=nullptr*/)
{
  ...
  for (int i = 0; i < numNodes; ++i)
  {
    ...
    if (mrmlNode->IsA("vtkMRMLStorableNode"))
    {
      ...
      if (!this->SaveStorableNodeToSlicerDataBundleDirectory(
            storableNode, dataDir, originalStorageNodeFileNames, userMessages))
      {
        success = false;
      }
  ...
  for (int i = 0; i < numNodes; ++i)
  {
    ...
    for (sit = snodes.begin(); sit != snodes.end(); sit++)
    {
      ...
      std::map<std::string, vtkMRMLNode*>::iterator storableNodeIt =
        storableNodes.find(std::string(storableNode->GetID()));
      ...
      else
      {
        ...
        vtkMRMLStorageNode* storageNodeInMainScene =
          vtkMRMLStorableNode::SafeDownCast(storableNodeIt->second)->GetStorageNode();
        ...
        if (storageNode)
        {
          ...
          {
            originalStorageNodeFileNames[storageNode].push_back(
              storageNode->GetNthFileName(i) ? storageNode->GetNthFileName(i) : "");
          }
