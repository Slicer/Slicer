...
//---------------------------------------------------------------------------
vtkMRMLThreeDReformatDisplayableManager::vtkInternal::vtkInternal(vtkMRMLThreeDReformatDisplayableManager* _external)
{
...
//---------------------------------------------------------------------------
vtkMRMLViewNode* vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::GetViewNode()
{
  return this->External->GetMRMLViewNode();
}

//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::AddSliceNode(vtkMRMLSliceNode* sliceNode)
{
  if (!sliceNode || this->SliceNodes.find(vtkMRMLSliceNode::SafeDownCast(sliceNode)) != this->SliceNodes.end())
  {
    return;
  }
  ...
  sliceNode->AddObserver(vtkCommand::ModifiedEvent, this->External->GetMRMLNodesCallbackCommand());
...
//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::RemoveSliceNode(vtkMRMLSliceNode* sliceNode)
{
  if (!sliceNode)
  {
    return;
...
//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::RemoveSliceNode(SliceNodesLink::iterator it)
{
...
//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::RemoveAllSliceNodes()
{
...
//---------------------------------------------------------------------------
vtkMRMLSliceNode* vtkMRMLThreeDReformatDisplayableManager::vtkInternal::GetSliceNode(
...
//---------------------------------------------------------------------------
vtkImplicitPlaneWidget2* vtkMRMLThreeDReformatDisplayableManager::vtkInternal::NewImplicitPlaneWidget()
{
  ...
  planeWidget->AddObserver(vtkCommand::StartInteractionEvent, this->External->GetWidgetsCallbackCommand());
  planeWidget->AddObserver(vtkCommand::InteractionEvent, this->External->GetWidgetsCallbackCommand());
  planeWidget->AddObserver(vtkCommand::EndInteractionEvent, this->External->GetWidgetsCallbackCommand());
  planeWidget->AddObserver(vtkCommand::UpdateEvent, this->External->GetWidgetsCallbackCommand());

  return planeWidget;
}

//---------------------------------------------------------------------------
vtkImplicitPlaneWidget2* vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::GetWidget(vtkMRMLSliceNode* sliceNode)
{
  if (!sliceNode)
  {
    return nullptr;
...
//---------------------------------------------------------------------------
bool vtkMRMLThreeDReformatDisplayableManager::vtkInternal ::UpdateWidget(vtkMRMLSliceNode* sliceNode,
                                                                         vtkImplicitPlaneWidget2* planeWidget)
{
  ...
  // Update Bound size
  vtkMRMLSliceCompositeNode* sliceCompositeNode = vtkMRMLSliceLogic::GetSliceCompositeNode(sliceNode);
  ...
  vtkMRMLVolumeNode* volumeNode =
    vtkMRMLVolumeNode::SafeDownCast(this->External->GetMRMLScene()->GetNodeByID(volumeNodeID));
  if (volumeNode)
  {
    double dimensions[3], center[3];
    vtkMRMLSliceLogic::GetVolumeRASBox(volumeNode, dimensions, center);
    double bounds[6] = { bounds[0] = center[0] - dimensions[0] / 2, bounds[1] = center[0] + dimensions[0] / 2,
                         bounds[2] = center[1] - dimensions[1] / 2, bounds[3] = center[1] + dimensions[1] / 2,
                         bounds[4] = center[2] - dimensions[2] / 2, bounds[5] = center[2] + dimensions[2] / 2 };
    rep->SetPlaceFactor(1.);
    rep->PlaceWidget(bounds);
  }

  // Update normal
  rep->SetNormal(sliceToRAS->GetElement(0, 2), sliceToRAS->GetElement(1, 2), sliceToRAS->GetElement(2, 2));
  // Update origin position
  rep->SetOrigin(sliceToRAS->GetElement(0, 3), sliceToRAS->GetElement(1, 3), sliceToRAS->GetElement(2, 3));

  rep->SetDrawOutline(sliceNode->GetWidgetOutlineVisible());

  // Update the widget itself if necessary
  bool visible =
    sliceNode->IsDisplayableInThreeDView(this->External->GetMRMLViewNode()->GetID()) && sliceNode->GetWidgetVisible();

  // re-render if it was visible or now becomes visible
  bool renderingRequired = planeWidget->GetEnabled() || visible;

  if ((!planeWidget->GetEnabled() && visible) || (planeWidget->GetEnabled() && !visible)
...
//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager ::OnMRMLSceneNodeAdded(vtkMRMLNode* nodeAdded)
{
...
//---------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager ::OnMRMLSceneNodeRemoved(vtkMRMLNode* nodeRemoved)
{
...
//----------------------------------------------------------------------------
void vtkMRMLThreeDReformatDisplayableManager::ProcessWidgetsEvents(vtkObject* caller,
                                                                   unsigned long event,
                                                                   void* vtkNotUsed(callData))
{
  ...
  vtkImplicitPlaneRepresentation* rep = (planeWidget) ? planeWidget->GetImplicitPlaneRepresentation() : nullptr;
