...
public:
  ...
  {
    bool rotationHandle1Visible = visibility &&
    ...
    bool rotationHandle2Visible = visibility &&
                                  (this->HandlesVisibilityMode != vtkMRMLSliceDisplayNode::NeverVisible) &&
    ...
    bool translationHandleVisible =
      visibility && (this->HandlesVisibilityMode != vtkMRMLSliceDisplayNode::NeverVisible) &&
  ...
  SliceIntersectionInteractionDisplayPipeline* pipeline)
{
  ...
  else
  {
    ...
    else if (pipeline->SliceIntersectionMode == vtkMRMLSliceDisplayNode::SkipLineCrossings)
    {
      ...
      double cosineValue = dotProduct / (vtkMath::Norm(intersectionPointToOuterLineTip1) *
                                         vtkMath::Norm(intersectionPointToOuterLineTip2));
      if (cosineValue > 1.0)
      {
        cosineValue = 1.0;
      }
...
double vtkMRMLSliceIntersectionInteractionRepresentation::GetMaximumHandlePickingDistance2()
{
  double maximumHandlePickingDistance =
    this->InteractionSize / 2.0 + this->PickingTolerance * this->GetScreenScaleFactor();
       ...
       ++sliceIntersectionIt)
  {
    ...
    else
    {
      ...
      for (HandleInfo handleInfo : handleInfoList)
      {
        ...
        if (interactionEventData->IsDisplayPositionValid())
        {
          double pixelTolerance = this->InteractionSize / 2.0 /
                                    vtkMRMLAbstractThreeDViewDisplayableManager::GetViewScaleFactorAtPosition(
                                      this->Renderer, handleWorldPos) +
