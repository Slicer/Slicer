...
char vtkMRMLCameraDisplayableManagerTest1EventLog[] = "# StreamVersion 1\n"
...
{
public:
  static vtkRenderRequestCallback* New() { return new vtkRenderRequestCallback; }
  void SetRenderer(vtkRenderer* renderer) { this->Renderer = renderer; }
  int GetRenderRequestCount() { return this->RenderRequestCount; }
...
int vtkMRMLCameraDisplayableManagerTest1(int argc, char* argv[])
{
  ...
  if (displayableManagerGroup->GetInteractor() != ri.GetPointer())
  {
    ...
    std::cerr << "Current RenderWindowInteractor:" << displayableManagerGroup->GetInteractor() << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (record || retval == vtkRegressionTester::DO_INTERACTOR)
  {
    ...
    std::cout << "Current RenderRequestCount: " << renderRequestCallback->GetRenderRequestCount() << std::endl;
  }
