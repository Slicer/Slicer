...
//----------------------------------------------------------------------
void vtkMRMLAbstractWidget::SetKeyboardEventTranslation(int widgetState, int modifier, char keyCode, int repeatCount, const char* keySym, unsigned long widgetEvent)
{
  if (widgetState >= static_cast<int>(this->EventTranslators.size()))
  {
    this->EventTranslators.resize(widgetState + 1);
...
//----------------------------------------------------------------------
void vtkMRMLAbstractWidget::SetKeyboardEventTranslation(int modifier, char keyCode, int repeatCount, const char* keySym, unsigned long widgetEvent)
{
...
//-----------------------------------------------------------------------------
unsigned long vtkMRMLAbstractWidget::TranslateInteractionEventToWidgetEvent(vtkWidgetEventTranslator* translator, vtkMRMLInteractionEventData* eventData)
{
  unsigned long widgetEvent = WidgetEventNone;
  ...
  if (eventData->GetType() == vtkCommand::KeyPressEvent)
  {
    ...
    {
      widgetEvent = translator->GetTranslation(vtkCommand::KeyPressEvent, vtkEvent::NoModifier, keyCode, repeatCount, keySym);
    }
    if (widgetEvent == WidgetEventNone)
    {
