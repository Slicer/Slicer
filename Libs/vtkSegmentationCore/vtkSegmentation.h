...
public:
  ...
  /// \param segmentIDs List of IDs of segments to include in the merged labelmap. If empty or missing, then all
  /// segments are included
  /// \param computeEffectiveExtent Specifies if the extent of a segment is the whole extent or the effective extent
  /// (where voxel values >0 found)
  ...
  /// labelValues. Label value of background = 0.
  /// \param extentComputationMode Input that determines how to compute extents (EXTENT_REFERENCE_GEOMETRY,
  /// EXTENT_UNION_OF_SEGMENTS,
