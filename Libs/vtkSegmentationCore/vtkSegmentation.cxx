...
static void FindSegment(vtkSegmentation::SegmentMap& map, vtkSegment *segment, vtkSegmentation::SegmentMap::iterator& segmentLocation)
{
  for (segmentLocation = map.begin(); segmentLocation != map.end(); ++segmentLocation)
    {
    if (segmentLocation->second == segment)
      {
      return;
      }
    }
...
bool vtkSegmentation::AddSegment(vtkSegment* segment, std::string segmentId/*=""*/, std::string insertBeforeSegmentId/*=""*/)
{
  ...
  if (!representationsCreated)
    ...
    FindSegment(this->Segments, segment, segmentIt);
