  ...
  void Execute(vtkObject* vtkNotUsed(caller), unsigned long event, void* vtkNotUsed(v)) override
  {
    if (event == vtkCommand::StartEvent && this->Watcher)
    {
      ...
      else
      {
        ...
        std::cout << "<filter-name>" << (this->Watcher->GetProcess() ? this->Watcher->GetProcess()->GetClassName() : "None") << "</filter-name>" << std::endl;
...
public:
  ...
  void Execute(vtkObject* vtkNotUsed(caller), unsigned long event, void* vtkNotUsed(v)) override
  {
    if (event == vtkCommand::ProgressEvent && this->Watcher)
    {
      ...
      {
        strncpy(this->Watcher->GetProcessInformation()->ProgressMessage, this->Watcher->GetComment().c_str(), 1023);
        this->Watcher->GetProcessInformation()->Progress = (this->Watcher->GetProcess()->GetProgress() * this->Watcher->GetFraction() + this->Watcher->GetStart());
      ...
      else
      {
        if (!this->Quiet)
        {
          std::cout << "<filter-progress>" << (this->Watcher->GetProcess()->GetProgress() * this->Watcher->GetFraction()) + this->Watcher->GetStart() << "</filter-progress>"
                    << std::endl;
          if (this->Watcher->GetFraction() != 1.0)
          {
            std::cout << "<filter-stage-progress>" << this->Watcher->GetProcess()->GetProgress() << "</filter-stage-progress>" << std::endl;
          }
          std::cout << std::flush;
        }
      }
...
//-----------------------------------------------------------------------------
vtkPluginFilterWatcher::vtkPluginFilterWatcher(vtkAlgorithm* o, const char* comment, ModuleProcessInformation* inf, double fraction, double start)
{
  // Initialize state
  this->Process = o;
