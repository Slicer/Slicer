...
{
  if(!this->IsCreated() ||
     static_cast<unsigned int>(this->m_CurrentIndex) == (this->m_InternalWidgetParamList->size() - 1))
    {
    return NULL;
    }
...
const char *vtkSlicerParameterWidget::GetValueFromWidget(vtkKWWidget *widg)
{
  ...
  vtkKWRadioButton
    *rb0 = vtkKWRadioButton::SafeDownCast(widg);
  ...
  if (sb)
    ...
    {
    vtkKWRadioButtonSetWithLabel *rbs = vtkKWRadioButtonSetWithLabel::SafeDownCast(rb0->GetParent());
    if(rbs)
      {
      int num = rbs->GetWidget()->GetNumberOfWidgets();
      for (int i=0; i < num; ++i)
...
void vtkSlicerParameterWidget::SetValueForWidget(vtkKWCoreWidget *inputWidget, const char* value)
{
  ...
  vtkKWRadioButton
    *rb0 = vtkKWRadioButton::SafeDownCast(inputWidget);
  vtkKWRadioButtonSetWithLabel
