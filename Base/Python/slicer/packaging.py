    ...
    if skip_packages is not None:
        if show_progress:
            return _pip_install_with_skips_dialog(
                requirements, skip_packages, constraints, requester,
            )
        else:
...
def _pip_install_with_dialog(
    ...
    dialog = _PipProgressDialog.acquire(requester=requester)
...
class _PipProgressDialog:
    ...
    @classmethod
    def acquire(cls, requester: str | None = None) -> _PipProgressDialog:
    ...
    def __init__(self, requester: str | None = None) -> None:
        import ctk
        import qt
        import slicer

        # Parent the shared dialog to the main window. A single instance is cached in
        # _sharedInstance and reused across installs, so a transient caller-supplied
        # parent could be destroyed while the cached instance lives on -- taking this
        # QDialog with it and leaving acquire() to return a dialog whose C++ object is
        # gone. The main window lives for the whole session, matching the shared
        # lifetime; if there is no main window (e.g. --no-main-window), the dialog is
        # simply top-level.
        self._dialog = qt.QDialog(slicer.util.mainWindow())
