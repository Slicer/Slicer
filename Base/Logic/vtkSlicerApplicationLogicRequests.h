...
{
public:
  ReadDataBackgroundRequestFile(
    const std::string& refNodeID,
  ...
  std::string GetRefNodeID()
  {
    return this->m_RefNodeID;
  }

  std::string GetRefNodeClass()
  {
    return this->m_RefNodeClass;
  }
  ...
  void Execute(vtkSlicerApplicationLogic* appLogic) override
  {
    ...
    vtkSmartPointer<vtkMRMLStorageNode> storageNode =
      vtkSmartPointer<vtkMRMLStorageNode>::Take(storableNode->CreateDefaultStorageNode());
    if (!storageNode)
    {
    ...
    try
    {
      vtkDebugWithObjectMacro(appLogic, "ReadDataBackgroundRequestFile: about to call read data, " \
        "storage node's read state is " << storageNode->GetReadStateAsString());
      // If the node was previously empty then the write state may still be
      // "SkipNoData", which would prevent the file from loading.
      // Change the write state back to "Idle" to make sure the file is loaded.
      ...
      if (appLogic->GetMRMLScene()->GetCacheManager()->IsRemoteReference(this->m_Filename.c_str()))
      {
        ...
        vtkDebugWithObjectMacro(appLogic, "ReadDataBackgroundRequestFile: calling ReadData on the storage node " \
          << storageNode->GetID() << ", uri = " << storageNode->GetURI());
...
{
public:
  WriteDataBackgroundRequestFile(
    vtkMRMLNode* node,
    vtkMRMLScene* miniScene,
    const std::string& filename,
    int uid = 0)
    : DataBackgroundRequestFile(filename, uid)
  {
