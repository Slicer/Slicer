        ...
        if (smhnd && tmhnd)
        {
          ...
          for (int n = 0; n < miniscene->GetNumberOfNodesByClass("vtkMRMLModelHierarchyNode"); n++)
          {
            ...
            if (mhnd)
            {
              ...
              std::vector<std::string>::const_iterator ssit = std::find(m_SourceNodes.begin(), m_SourceNodes.end(), mhnd->GetID());
...
{
public:
  ReadDataBackgroundRequestFile(const std::string& refNodeID,
                                const std::string& refNodeClass,
                                const std::string& filename,
                                int displayData,
                                int deleteFile,
                                int uid = 0,
                                const std::string& refStorageNodeClass = "")
    : DataBackgroundRequestFile(filename, uid)
  {
    this->m_RefNodeID = refNodeID;
    this->m_RefNodeClass = refNodeClass;
    this->m_RefStorageNodeClass = refStorageNodeClass;
  ...
  std::string GetRefStorageNodeClass() { return this->m_RefStorageNodeClass; }
  ...
  void Execute(vtkSlicerApplicationLogic* appLogic) override
  {
    ...
    // Prefer the storage node class the requesting node already carries.
    //
    // The node read into here is freshly created and therefore empty, and for some node
    // classes the default storage node depends on content that is not there yet: a
    // vtkMRMLSequenceNode picks its storage node from its first data node, so an empty one
    // reports no default storage node class at all. CreateDefaultStorageNode() then returns
    // nullptr and the read silently does nothing. The requesting node knows the answer, so
    // it is passed in; CreateDefaultStorageNode() remains the fallback.
    vtkSmartPointer<vtkMRMLStorageNode> storageNode;
    if (!this->m_RefStorageNodeClass.empty())
    {
      storageNode = vtkSmartPointer<vtkMRMLStorageNode>::Take(vtkMRMLStorageNode::SafeDownCast(this->m_Scene->CreateNodeByClass(this->m_RefStorageNodeClass.c_str())));
      if (!storageNode)
      {
        vtkWarningWithObjectMacro(
          appLogic, "ReadDataBackgroundRequestFile: could not create storage node of class " << this->m_RefStorageNodeClass.c_str() << ", falling back to the default one");
      }
    }
    if (!storageNode)
    {
      storageNode = vtkSmartPointer<vtkMRMLStorageNode>::Take(storableNode->CreateDefaultStorageNode());
    }
    if (!storageNode)
    {
    ...
    storageNode->SetScene(this->m_Scene);
...
protected:
  ...
  std::string m_RefStorageNodeClass;
};
