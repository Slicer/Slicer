...
void vtkImageRectangularSource::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Size: (" << this->Size[0] << ", " << this->Size[1] << ", " << this->Size[2] << ")\n";
...
//----------------------------------------------------------------------------
void vtkImageRectangularSource::SetWholeExtent(int minX, int maxX, int minY, int maxY, int minZ, int maxZ)
{
  int extent[6];
...
//----------------------------------------------------------------------------
int vtkImageRectangularSource::RequestInformation(vtkInformation* vtkNotUsed(request),
...
template <class T>
void vtkImageRectangularSourceExecute(vtkImageRectangularSource* self, vtkImageData* data, int ext[6], T* ptr)
{
  int idx0, idx1, idx2;
                 ...
                 int IncResultY)
{
  ...
  for (int y = MinInY; y <= MaxInY; y++)
  {
    ...
    else
    {
      ...
      {
        DefineLine(
          Min(inMinX1, inMinX2), Max(inMaxX1, inMaxX2), LineLength, inVal, outVal, InsideGraySlopeFlag, Result);
      }
    }
    Result += RowLength + IncResultY;
  }
...
template <class T>
void vtkImageRectangularSource_GeneralExecute(vtkImageRectangularSource* self,
                                              vtkImageData* data,
                                              int ext[6],
                                              int** Corners,
                                              T* ptr)
{
  ...
  data->GetContinuousIncrements(ext, inc0, inc1, inc2);
  RectSource::DefineSlice(Corners,
                          ext[3] - ext[2] + 1,
                          ext[1] - ext[0] + 1,
                          (T)(self->GetInValue()),
                          (T)(self->GetOutValue()),
                          self->GetInsideGraySlopeFlag(),
                          ptr,
                          inc1);
}
