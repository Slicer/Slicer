...
template <class T>
static void vtkImageFillROIDrawPolygon(int nx, int ny, int nPts, int* xPts, int* yPts, T value, T* outPtr)
{
  int i, scan, done;
  Edge *active, *p, *q, *del;
  int x, x1, x2;
...
template <class T>
static void DrawLines(int nx, int ny, int z, int radius, int nPts, int* xPts, int* yPts, T value, vtkImageData* outData)
{
  int i, x, y, x1, y1, x2, y2, xx1, xx2, yy1, yy2;
  ...
  for (i = 0; i < n - 1; i++)
  {
    ...
    if (x1 - rad >= min0 && x1 + rad <= max0 && y1 - rad >= min1 && y2 + rad <= max1 && x2 - rad >= min0
        && x2 + rad <= max0)
    {
      dx = abs(x2 - x1);
      dy = abs(y2 - y1);
...
template <class T>
static void
DrawPoints(int nx, int ny, int z, int radius, int nPts, int* xPts, int* yPts, T value, vtkImageData* outData)
{
  int i, x, y, x1, y1;
  T* outPtr;
  int min0, max0, min1, max1, min2, max2;
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (x1 - r >= min0 && x1 + r <= max0 && y1 - r >= min1 && y1 + r <= max1)
    {
      for (y = y1 - r; y <= y1 + r; y++)
      {
...
static void vtkImageFillROIExecute(vtkImageFillROI* self, vtkImageData* outData, T* outPtr)
{
  ...
  for (i = 0, j = 0; i < nPts; i++)
  {
    ...
    x = (int)(pt[0]);
    y = (int)(pt[1]);
    if (x >= outExt[0] && x <= outExt[1] && y >= outExt[2] && y <= outExt[3])
    {
      xPts[j] = x;
      yPts[j] = y;
      j++;
  ...
  switch (self->GetShape())
  {
    case SHAPE_POLYGON:
      if (nPts >= 3)
      {
        vtkImageFillROIDrawPolygon(nx, ny, nPts, xPts, yPts, (T)value, (T*)outPtr);
...
//----------------------------------------------------------------------------
int vtkImageFillROI::RequestData(vtkInformation* vtkNotUsed(request),
                                 ...
                                 vtkInformationVector* outputVector)
{
  ...
  if (!input || input->GetDataObjectType() != VTK_IMAGE_DATA)
  {
