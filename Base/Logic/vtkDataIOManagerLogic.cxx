...
# define vtkSetAndObserveDataIOManagerEventsMacro(node, value, events)                                          \
   {                                                                                                            \
     ...
     this->GetDataIOObserverManager()->SetAndObserveObjectEvents(vtkObjectPointer(&(node)), (value), (events)); \
...
vtkDataIOManagerLogic::vtkDataIOManagerLogic()
{
  ...
  this->DataIOObserverManager->GetCallbackCommand()->SetCallback(vtkDataIOManagerLogic::DataIOManagerCallback);
}
...
//----------------------------------------------------------------------------
void vtkDataIOManagerLogic::DataIOManagerCallback(vtkObject* caller,
                                                  unsigned long eid,
                                                  void* clientData,
                                                  void* callData)
{
...
int vtkDataIOManagerLogic::QueueRead(vtkMRMLNode* node)
{
  ...
  vtkDebugMacro("QueueRead: there are " << numScheduledNodes << " storage nodes scheduled to be read on "
                                        << node->GetID());
  if (storageNodeIndex == -1)
  {
  ...
  else
  {
    ...
    vtkDebugMacro("QueueRead: setting " << storageNodeIndex << " storage node read state to working, uri = "
                                        << dnode->GetNthStorageNode(storageNodeIndex)->GetURI());
  ...
  //--- just return.
  if ((cm->CachedFileExists(dest)) && allCachedFilesExist && (!(cm->GetEnableForceRedownload())))
  {
    dnode->GetNthStorageNode(storageNodeIndex)->SetReadStateTransferDone();
  ...
  if (this->GetDataIOManager()->GetEnableAsynchronousIO())
  {
    ...
    task->SetTaskFunction(
      this, (vtkSlicerTask::TaskFunctionPointer)&vtkDataIOManagerLogic::ApplyTransfer, transfer0.GetPointer());
  ...
  else
  {
    ...
    vtkDebugMacro("QueueRead: setting storage node state to transferdone: "
                  << dnode->GetNthStorageNode(storageNodeIndex)->GetURI());
  ...
  for (int n = 0; n < dnode->GetNthStorageNode(storageNodeIndex)->GetNumberOfURIs(); n++)
  {
    ...
    if (this->GetDataIOManager()->GetEnableAsynchronousIO())
    {
      ...
      task->SetTaskFunction(
        this, (vtkSlicerTask::TaskFunctionPointer)&vtkDataIOManagerLogic::ApplyTransfer, transfer1.GetPointer());
  ...
  {
    vtkDebugMacro("QueueRead: setting storage node state to transferdone after synchronous transfer of all files: "
                  << dnode->GetNthStorageNode(storageNodeIndex)->GetURI());
    dnode->GetNthStorageNode(storageNodeIndex)->SetReadStateTransferDone();
  }
...
int vtkDataIOManagerLogic::QueueWrite(vtkMRMLNode* node)
{
  ...
  for (int n = 0; n < numFiles; n++)
  {
    ...
    if (this->GetDataIOManager()->GetEnableAsynchronousIO())
    {
      ...
      task->SetTaskFunction(
        this, (vtkSlicerTask::TaskFunctionPointer)&vtkDataIOManagerLogic::ApplyTransfer, transfer.GetPointer());
...
void vtkDataIOManagerLogic::ApplyTransfer(void* clientdata)
{
    ...
    if (handler != nullptr && source != nullptr && dest != nullptr)
    {
      if (asynchIO && dt->GetTransferStatus() == vtkDataTransfer::Pending)
      {
        ...
        for (int i = 0; i < storableNode->GetNumberOfStorageNodes(); i++)
        {
          ...
          {
            vtkDebugMacro("ApplyTransfer: found a working storage node who's uri matches source " << source << " at "
                                                                                                  << i);
            storageNodeIndex = i;
            break;
          }
        ...
        vtkDebugMacro("ApplyTransfer: setting storage node read state to transfer done for uri "
                      << storageNode->GetURI());
  ...
  else if (dt->GetTransferType() == vtkDataTransfer::RemoteUpload)
  {
    ...
    if (handler != nullptr && source != nullptr && dest != nullptr)
    {
      ...
      {
        vtkDebugMacro("ApplyTransfer: Upload: stage file write on the handler, source = " << source
                                                                                          << ", dest = " << dest);
        handler->StageFileWrite(source, dest);
      }
    }
