...
vtkMTimeType vtkSlicerApplicationLogic::RequestReadFile(const char *refNode,
  const char *filename, int displayData, int deleteFile, bool backgroundProcess)
{
  ...
  if (backgroundProcess)
  {
    ...
    ReadDataBackgroundRequestFile* readDataRequestFile =
      new ReadDataBackgroundRequestFile(refNode, node->GetClassName(), filename, displayData, deleteFile, uid);
    task->SetTaskFunction(this, (vtkSlicerTask::TaskFunctionPointer)
                          &vtkSlicerApplicationLogic::ExecuteDataBackgroundRequest,
                          readDataRequestFile);
...
vtkMTimeType vtkSlicerApplicationLogic::RequestWriteData(const char *refNode, const char *filename, bool backgroundProcess)
{
  ...
  if (backgroundProcess)
  {
    ...
    vtkNew<vtkSlicerTask> task;
    task->SetTypeToProcessing();
    WriteDataBackgroundRequestFile* writeDataRequestFile =
      new WriteDataBackgroundRequestFile(nodeCopy, miniScene, filename, uid);
    task->SetTaskFunction(this, (vtkSlicerTask::TaskFunctionPointer)
                          &vtkSlicerApplicationLogic::ExecuteDataBackgroundRequest,
...
{
  DataBackgroundRequestFile* request =
    static_cast<DataBackgroundRequestFile*>(clientData);
  if (!request)
  {
    return;
  }
...
void vtkSlicerApplicationLogic::ProcessBackgroundQueue()
{
  ...
  {
    DataBackgroundRequestFile* request =
      static_cast<DataBackgroundRequestFile*>(this->InternalProcessedBackgroundQueue->front());
