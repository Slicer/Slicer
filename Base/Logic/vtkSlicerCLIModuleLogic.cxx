...
public:
  ...
  {
    if (std::find(this->ThreadIDs.begin(), this->ThreadIDs.end(), vtkMultiThreader::GetCurrentThreadID()) != this->ThreadIDs.end())
    {
...
vtkSlicerCLIModuleLogic::vtkSlicerCLIModuleLogic()
{
  ...
  this->AddObserver(vtkSlicerCLIModuleLogic::RequestHierarchyEditEvent, this->Internal->OneShotCallbackCallback, 100000000.f);
}
...
{
  vtkMRMLCommandLineModuleNode* node = vtkMRMLCommandLineModuleNode::SafeDownCast(this->GetMRMLScene()->CreateNodeByClass("vtkMRMLCommandLineModuleNode"));
  node->SetName(this->GetMRMLScene()->GetUniqueNameByString(this->Internal->DefaultModuleDescription.GetTitle().c_str()));
...
std::string vtkSlicerCLIModuleLogic::ConstructTemporarySceneFileName(vtkMRMLScene* scene)
{
  ...
  snprintf(tname, sizeof(tname), "%p", scene);
                                                                ...
                                                                CommandLineModuleType commandType)
{
  ...
  if (tag == "image")
  {
    ...
    else
    {
      ...
      snprintf(tname, name.size() + 100, "slicer:%p#%s", this->GetMRMLScene(), name.c_str());
...
void vtkSlicerCLIModuleLogic::SetMRMLApplicationLogic(vtkMRMLApplicationLogic* logic)
{
  ...
  if (logic)
  {
    vtkEventBroker::GetInstance()->AddObservation(logic, vtkSlicerApplicationLogic::RequestProcessedEvent, this, this->GetMRMLLogicsCallbackCommand());
  }
...
void vtkSlicerCLIModuleLogic::ApplyTask(void* clientdata)
{
  ...
  node0->GetModuleDescription().GetProcessInformation()->SetProgressCallback(vtkSlicerCLIModuleLogic::ProgressCallback, &lnp);
  ...
  for (id2fn0 = nodesToWrite.begin(); id2fn0 != nodesToWrite.end(); ++id2fn0)
  {
    ...
    if (sn)
    {
      ...
      else
      {
        ...
        if (extension == ".fcsv")
        {
          ...
          defaultOut.TakeReference(vtkMRMLStorageNode::SafeDownCast(this->GetMRMLScene()->CreateNodeByClass("vtkMRMLMarkupsFiducialStorageNode")));
        }
        else
        {
          defaultOut.TakeReference(sn->CreateDefaultStorageNode());
    ...
    if ((commandType == SharedObjectModule) && defaultOut)
    {
      ...
      if ((this->GetAllowInMemoryTransfer() == 0) || //
          std::find(MemoryTransferPossible.begin(), MemoryTransferPossible.end(), nd->GetClassName()) == MemoryTransferPossible.end())
      {
  ...
  for (id2fn0 = nodesToReload.begin(); id2fn0 != nodesToReload.end(); ++id2fn0)
  {
    ...
    else if (nd->IsA("vtkMRMLMarkupsNode"))
    {
      ...
      if (extension == ".fcsv")
      {
        ...
        {
          snode = vtkMRMLStorageNode::SafeDownCast(this->GetMRMLScene()->AddNewNodeByClass("vtkMRMLMarkupsFiducialStorageNode"));
  ...
  {
    vtkDebugMacro("Setting a location for a command line module: " << node0->GetModuleDescription().GetLocation() << ", target is '" << node0->GetModuleDescription().GetTarget()
                                                                   << "'");
  ...
  if (commandType == SharedObjectModule)
  {
    ...
    char tname[256];
    snprintf(tname, sizeof(tname), "%p", node0->GetModuleDescription().GetProcessInformation());
                ...
                || (*pit).GetMultiple() == "true")
            {
              for (int i = 0; i < regions->GetNumberOfROIs(); ++i)
              {
                ...
                if (regions->GetNthROISelected(i))
                {
                  ...
                  roiAsString << pt[0] << "," << pt[1] << "," << pt[2] << "," << Radius[0] << "," << Radius[1] << "," << Radius[2];
  ...
  if (commandType == CommandLineModule)
  {
    ...
    itksysProcess_SetOption(process, itksysProcess_Option_HideWindow, this->GetHideWindow());
    ...
    while ((pipe = itksysProcess_WaitForData(process, &tbuffer, &length, &timeout)) != 0)
    {
      ...
      {
        itksysProcess_Kill(process);
        this->Internal->Processes.erase(std::find(this->Internal->Processes.begin(), this->Internal->Processes.end(), process));
      ...
      else
      {
        ...
        if (result == itksysProcess_State_Exception)
        {
          ...
          switch (excResult)
          {
            ...
            case itksysProcess_Exception_Fault: message = "terminated with a fault"; break;
            case itksysProcess_Exception_Illegal: message = "terminated with an illegal instruction"; break;
            case itksysProcess_Exception_Interrupt: message = "terminated with an interrupt"; break;
            case itksysProcess_Exception_Numerical: message = "terminated with a numerical fault"; break;
            case itksysProcess_Exception_Other: message = "terminated with an unknown exception"; break;
          }
        }
  ...
  else if (commandType == SharedObjectModule)
  {
    ...
    if (returnValue)
    {
      vtkErrorMacro(<< node0->GetModuleDescription().GetTitle() << " returned " << returnValue << " which probably indicates an error.");
      node0->SetStatus(vtkMRMLCommandLineModuleNode::CompletedWithErrors, false);
      this->GetApplicationLogic()->RequestModified(node0);
      std::cout.rdbuf(origcoutrdbuf);
      std::cerr.rdbuf(origcerrrdbuf);
      ...
      if (d.HasParameter(p.GetReference()))
      {
        ...
        if (p.GetTag() == "table")
        {
          if (p.GetType() == "color")
          {
            ...
            // get its color node
            vtkMRMLDisplayableNode* rn = vtkMRMLDisplayableNode::SafeDownCast(this->GetMRMLScene()->GetNodeByID(reference.c_str()));
            if (rn)
            {
              ...
              {
                vtkErrorMacro(<< "Reference node \"" << reference.c_str() << "\" does not have a display node which is needed to find the color node.");
              }
            }
            else
            {
...
void vtkSlicerCLIModuleLogic::AutoRun(vtkMRMLCommandLineModuleNode* node)
{
  ...
  this->GetApplicationLogic()->InvokeEventWithDelay(node->GetAutoRunDelay() + extraDelay, node, vtkMRMLCommandLineModuleNode::AutoRunEvent, reinterpret_cast<void*>(requestTime));
}
