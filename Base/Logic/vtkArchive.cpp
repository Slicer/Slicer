...
bool extract_tar(const char* outFileName, bool verbose, bool extract)
{
  ...
  if(r)
    { 
    std::cerr << "Problem with archive_read_open_file(): "
  ...
  for (;;) 
    ...
    if (r != ARCHIVE_OK)
      {
      std::cerr << "Problem with archive_read_next_header(): "
                << archive_error_string(a) << std::endl;
      }
    if (verbose && extract)
      {
    ...
    if(extract)
      ...
      if (r != ARCHIVE_OK)
        {
        std::cerr << "Problem with archive_write_disk_set_options(): "
                  << archive_error_string(a) << std::endl;
        }
      ...
      if (r != ARCHIVE_OK)
        {
        std::cerr << "Problem with archive_write_header():"
                  << archive_error_string(a) << std::endl;
        std::cerr << "Current file:",
                  << archive_entry_pathname(entry) << std::endl;
        }
      else 
        {
        copy_data(a, ext);
        ...
        if (r != ARCHIVE_OK)
          {
          std::cerr << "Problem with archive_write_finish_entry(): ",
