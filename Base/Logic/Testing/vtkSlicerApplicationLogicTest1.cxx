...
int vtkSlicerApplicationLogicTest1(int, char*[])
{
  ...
  //-----------------------------------------------------------------------------
  std::string inputModulePath1(
    "/path/to/Slicer-Superbuild/Slicer-build/lib/Slicer-X.Y/qt-loadable-modules/libqSlicerVolumeRenderingModule.so");
  std::string inputModulePath2("c:\\path\\to\\Slicer-Superbuild\\Slicer-build\\lib\\Slicer-X.Y\\qt-loadable-"
                               "modules\\Release\\qSlicerVolumeRenderingModule.dll");
  ...
  {
    std::string expectedModuleShareDirectory =
      expectedModuleSlicerXYShareDirectory1 + "/qt-loadable-modules/VolumeRendering";
    std::string currentModuleShareDirectory =
      vtkSlicerApplicationLogic::GetModuleShareDirectory("VolumeRendering", inputModulePath1);
    ...
    currentModuleShareDirectory =
      vtkSlicerApplicationLogic::GetModuleShareDirectory("VolumeRendering", inputModulePath2);
    if (currentModuleShareDirectory != expectedModuleShareDirectory)
    {
      std::cerr << "Line " << __LINE__ << " - Failed to compute module share directory !\n"
                << "\texpected:" << expectedModuleShareDirectory << "\n"
    ...
    currentModuleShareDirectory =
      vtkSlicerApplicationLogic::GetModuleShareDirectory("VolumeRendering", inputModulePath3);
    if (currentModuleShareDirectory != expectedModuleShareDirectory)
    {
      std::cerr << "Line " << __LINE__ << " - Failed to compute module share directory !\n"
                << "\texpected:" << expectedModuleShareDirectory << "\n"
  ...
  //-----------------------------------------------------------------------------
  {
    ...
    std::string currentModuleShareDirectory =
      vtkSlicerApplicationLogic::GetModuleSlicerXYShareDirectory(inputModulePath1);
    if (currentModuleShareDirectory != expectedModuleShareDirectory)
    {
  ...
  //-----------------------------------------------------------------------------
  {
    ...
    TestDataType data;
    {
      TestRowType row;
      row.push_back(
        "/home/jchris/Projects/MRIChangeDetector-build/lib/Slicer-X.Y/qt-scripted-modules/MRIChangeDetector.pyc");
    ...
    {
      TestRowType row;
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build/lib/Slicer-X.Y/qt-scripted-modules/"
                    "MRIChangeDetector.pyc");
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build");
      row.push_back("4810");
      row.push_back("1");
      data.push_back(row);
    }
    {
      TestRowType row;
      row.push_back(
        "/home/jchris/Projects/Slicer.app/Contents/lib/Slicer-4.1/qt-loadable-modules/libqSlicerMarkupsModule.dylib");
    ...
    {
      TestRowType row;
      row.push_back("/home/jchris/Projects/Something.app/Contents/lib/Slicer-4.1/qt-loadable-modules/"
                    "libqSlicerMarkupsModule.dylib");
    ...
    }
    {
      ...
      row.push_back("/home/jchris/Projects/" Slicer_BUNDLE_LOCATION
                    "/lib/Slicer-4.1/qt-loadable-modules/libqSlicerMarkupsModule.dylib");
      row.push_back("/home/jchris/Projects/" Slicer_BUNDLE_LOCATION);
      row.push_back("4810");
      row.push_back("1");
      data.push_back(row);
    }
    {
      TestRowType row;
      row.push_back(
        "/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build/bin/Slicer.app/Contents/Extensions-4810/Reporting/"
        "lib/Slicer-4.1/qt-loadable-modules/Python/vtkSlicerReportingModuleLogic.py");
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build");
      row.push_back("4810");
      row.push_back("0");
      data.push_back(row);
    }
    {
      TestRowType row;
      row.push_back(
        "/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build/bin/" Slicer_BUNDLE_LOCATION
        "/Extensions-4810/Reporting/lib/Slicer-4.1/qt-loadable-modules/Python/vtkSlicerReportingModuleLogic.py");
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build");
      row.push_back("4810");
      row.push_back("0");
      data.push_back(row);
    ...
    {
      TestRowType row;
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build/bin/Foo.app/Contents/Extensions-4810/"
                    "Reporting/lib/Slicer-4.1/qt-loadable-modules/Python/vtkSlicerReportingModuleLogic.py");
      row.push_back("/home/jchris/Projects/Slicer4-Superbuild-Debug/Slicer-build");
      row.push_back("4810");
      row.push_back("0");
      data.push_back(row);
  ...
  //-----------------------------------------------------------------------------
  {
    ...
    std::cout << "After processing read data, app logic scene has " << numNodes << " model hierarchy nodes"
              << std::endl;
    ...
    for (int i = 0; i < numNodes; i++)
    {
      ...
      if (mrmlNode && mrmlNode->IsA("vtkMRMLModelHierarchyNode"))
      {
        ...
        std::cout << i << ": Model Hierarchy node named " << hnode->GetName() << " with id " << hnode->GetID()
                  << " has parent node id of " << (hnode->GetParentNodeID() ? hnode->GetParentNodeID() : "null")
                  << std::endl;
        // the second level clashed with the original hierarchy second level node, so below that, the parent node ids
        // have been shifted
        if (strcmp(hnode->GetName(), "1") == 0 && strcmp(hnode->GetParentNodeID(), "vtkMRMLModelHierarchyNode1") != 0)
        {
          std::cerr << "Hierarchy node has incorrect parent node id, expected vtkMRMLModelHierarchyNode1" << std::endl;
          return EXIT_FAILURE;
        }
        if (strcmp(hnode->GetName(), "2") == 0 && strcmp(hnode->GetParentNodeID(), "vtkMRMLModelHierarchyNode3") != 0)
        {
          std::cerr << "Hierarchy node has incorrect parent node id, expected vtkMRMLModelHierarchyNode3" << std::endl;
          return EXIT_FAILURE;
        }
        if (strcmp(hnode->GetName(), "3") == 0 && strcmp(hnode->GetParentNodeID(), "vtkMRMLModelHierarchyNode4") != 0)
        {
          std::cerr << "Hierarchy node has incorrect parent node id, expected vtkMRMLModelHierarchyNode4" << std::endl;
          return EXIT_FAILURE;
        }
        if (strcmp(hnode->GetName(), "4") == 0 && strcmp(hnode->GetParentNodeID(), "vtkMRMLModelHierarchyNode5") != 0)
        {
