...
void qSlicerViewersToolBarPrivate::init()
{
  ...
  this->CrosshairJumpSlicesOffsetAction->setToolTip(qSlicerViewersToolBar::tr("Slice view planes are shifted to match crosshair position (even if crosshair is not displayed)."));
  ...
  this->CrosshairJumpSlicesCenteredAction->setToolTip(qSlicerViewersToolBar::tr("Slice views are centered on crosshair position (even if crosshair is not displayed)."));
  ...
  this->CrosshairBasicAction->setToolTip(qSlicerViewersToolBar::tr("Basic crosshair extending across the field of view with a small gap at the crosshair position."));
  ...
  this->CrosshairToggleAction->setToolTip(qSlicerViewersToolBar::tr("Toggle crosshair visibility. Hold Shift key and move mouse in a view to set crosshair position."));
  ...
  QObject::connect(q, SIGNAL(toolButtonStyleChanged(Qt::ToolButtonStyle)), this->CrosshairToolButton, SLOT(setToolButtonStyle(Qt::ToolButtonStyle)));
  ...
  this->SliceIntersectionsFullIntersectionAction->setToolTip(qSlicerViewersToolBar::tr("Full slice intersection lines extending across the field of view."));
  ...
  this->SliceIntersectionsThicknessMapper->setMapping(this->SliceIntersectionsFineAction, vtkMRMLSliceDisplayNode::FineLines);
  ...
  QObject::connect(this->IntersectingSlicesInteractiveAction, SIGNAL(triggered(bool)), this, SLOT(setIntersectingSlicesInteractive(bool)));
  ...
  this->IntersectingSlicesTranslationEnabledAction->setToolTip(qSlicerViewersToolBar::tr("Control visibility of translation handles for slice intersection."));
  this->IntersectingSlicesTranslationEnabledAction->setCheckable(true);
  QObject::connect(this->IntersectingSlicesTranslationEnabledAction, SIGNAL(triggered(bool)), this, SLOT(setIntersectingSlicesTranslationEnabled(bool)));
  ...
  this->IntersectingSlicesRotationEnabledAction->setToolTip(qSlicerViewersToolBar::tr("Control visibility of rotation handles for slice intersection."));
  this->IntersectingSlicesRotationEnabledAction->setCheckable(true);
  QObject::connect(this->IntersectingSlicesRotationEnabledAction, SIGNAL(triggered(bool)), this, SLOT(setIntersectingSlicesRotationEnabled(bool)));
  ...
  QObject::connect(this->IntersectingSlicesVisibleAction, SIGNAL(triggered(bool)), this, SLOT(setIntersectingSlicesVisibility(bool)));

  q->addWidget(this->SliceIntersectionsToolButton);
  QObject::connect(q, SIGNAL(toolButtonStyleChanged(Qt::ToolButtonStyle)), this->SliceIntersectionsToolButton, SLOT(setToolButtonStyle(Qt::ToolButtonStyle)));
...
void qSlicerViewersToolBarPrivate::setCrosshairJumpSlicesMode(int jumpSlicesMode)
{
  ...
  {
    auto* node = static_cast<vtkMRMLCrosshairNode*>(nodes->GetItemAsObject(i));
    if (!node)
    {
      continue;
    }
...
void qSlicerViewersToolBarPrivate::updateWidgetFromMRML()
{
  ...
  for (int i = 0; i < crosshairs->GetNumberOfItems(); ++i)
  {
    crosshairNode = vtkMRMLCrosshairNode::SafeDownCast(crosshairs->GetItemAsObject(i));
    if (crosshairNode && crosshairNode->GetCrosshairName() == std::string("default"))
    {
      break;
    }
    crosshairNode = nullptr;
  }
  if (crosshairNode)
  {
  ...
  if (this->MRMLAppLogic)
  {
    ...
    this->IntersectingSlicesVisibleAction->setChecked(this->MRMLAppLogic->GetIntersectingSlicesEnabled(vtkMRMLApplicationLogic::IntersectingSlicesVisibility));
    // Slicer intersection interactive
    this->IntersectingSlicesInteractiveAction->setChecked(this->MRMLAppLogic->GetIntersectingSlicesEnabled(vtkMRMLApplicationLogic::IntersectingSlicesInteractive));
    ...
    this->IntersectingSlicesTranslationEnabledAction->setChecked(this->MRMLAppLogic->GetIntersectingSlicesEnabled(vtkMRMLApplicationLogic::IntersectingSlicesTranslation));
    this->IntersectingSlicesRotationEnabledAction->setChecked(this->MRMLAppLogic->GetIntersectingSlicesEnabled(vtkMRMLApplicationLogic::IntersectingSlicesRotation));
  }
}
