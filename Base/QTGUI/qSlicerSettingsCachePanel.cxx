...
void qSlicerSettingsCachePanel::setCacheManager(vtkCacheManager* cacheManager)
{
  ...
  qSlicerRelativePathMapper* relativePathMapper =
    new qSlicerRelativePathMapper(d->CachePathButton, "directory", SIGNAL(directoryChanged(QString)));
  ...
  this->registerProperty(
    "Cache/FreeBufferSize", d->CacheFreeBufferSpinBox, /*no tr*/ "value", SIGNAL(valueChanged(int)));
  this->registerProperty(
    "Cache/ForceRedownload", d->ForceRedownloadCheckBox, /*no tr*/ "checked", SIGNAL(toggled(bool)));
}
...
void qSlicerSettingsCachePanel::updateFromCacheManager()
{
  ...
  d->UsedCacheSizeLabel->setText(
    tr("%1MB used").arg(QString::number(qMax(d->CacheManager->GetCurrentCacheSize(), 0.f), 'f', 2)));
  d->CacheManager->FreeCacheBufferCheck();
  d->FreeCacheSizeLabel->setText(
    tr("%1MB free").arg(QString::number(d->CacheManager->GetFreeCacheSpaceRemaining(), 'f', 2)));
