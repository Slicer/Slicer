...
void qSlicerModulesListViewPrivate::init()
{
  ...
  q->connect(this->ModulesListModel, SIGNAL(itemChanged(QStandardItem*)), q, SLOT(onItemChanged(QStandardItem*)));
...
void qSlicerModulesListViewPrivate::updateItem(QStandardItem* item)
{
  ...
  // The module was registered, not ignored, initialized, but failed to be loaded
           ...
           !qobject_cast<qSlicerModuleFactoryManager*>(this->FactoryManager)->loadedModuleNames().contains(moduleName))
  {
  ...
  {
    if (this->FactoryManager == nullptr || this->FactoryManager->modulesToIgnore().contains(moduleName))
    {
  ...
  qSlicerAbstractCoreModule* coreModule =
    (this->FactoryManager ? this->FactoryManager->moduleInstance(moduleName) : nullptr);
  if (coreModule)
  {
...
// --------------------------------------------------------------------------
int qSlicerModulesListViewPrivate ::sortedInsertionIndex(const QString& moduleName) const
{
  int index = 0;
...
void qSlicerModulesListViewPrivate ::setModulesCheckState(const QStringList& moduleNames, Qt::CheckState checkState)
{
  ...
  foreach (const QString& moduleName, q->modules())
  {
    ...
    {
      moduleItem->setCheckState(checkState == Qt::Checked ? Qt::Unchecked : Qt::Checked);
    }
  }
}
...
void qSlicerModulesListView::setFactoryManager(qSlicerAbstractModuleFactoryManager* factoryManager)
{
  ...
  {
    disconnect(d->FactoryManager, SIGNAL(modulesRegistered(QStringList)), this, SLOT(updateModules(QStringList)));
    disconnect(d->FactoryManager, SIGNAL(moduleInstantiated(QString)), this, SLOT(updateModule(QString)));
    disconnect(d->FactoryManager, SIGNAL(modulesToIgnoreChanged(QStringList)), this, SLOT(updateModules()));
    disconnect(d->FactoryManager, SIGNAL(moduleIgnored(QString)), this, SLOT(updateModule(QString)));
    disconnect(d->FactoryManager, SIGNAL(moduleLoaded(QString)), this, SLOT(updateModule(QString)));
    d->removeModules();
  }
  d->FactoryManager = factoryManager;
  if (d->FactoryManager != nullptr)
  {
    connect(d->FactoryManager, SIGNAL(modulesRegistered(QStringList)), this, SLOT(updateModules(QStringList)));
    connect(d->FactoryManager, SIGNAL(moduleInstantiated(QString)), this, SLOT(updateModule(QString)));
    connect(d->FactoryManager, SIGNAL(modulesToIgnoreChanged(QStringList)), this, SLOT(updateModules()));
    connect(d->FactoryManager, SIGNAL(moduleIgnored(QString)), this, SLOT(updateModule(QString)));
    connect(d->FactoryManager, SIGNAL(moduleLoaded(QString)), this, SLOT(updateModule(QString)));
  }
