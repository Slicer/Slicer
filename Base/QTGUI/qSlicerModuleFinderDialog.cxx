...
void qSlicerModuleFinderDialog::onSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
  ...
  {
    moduleName = selectedIndexes.first().data(Qt::UserRole).toString();
...
void qSlicerModuleFinderDialog::onModuleTitleFilterTextChanged()
{
  ...
  // so we need to update the selection manually.
  // Note that other filter modifications (setFilterRole, setShowBuiltIn, etc.) change what data is considered
  // and so the entire filter gets invalidated, which updates the selection model. However, setFilterFixedString
  // only modifies a filter parameter and so the filter is not invalidated, therefore the selection requires
  // manual update. In the future, it may be considered changing the implementation to react to
  // rowsInserted, rowsRemoved, modelReset instead of relying on onSelectionChanged.

  // First check if the previously selected item is still shown (if yes, then we can keep that selection)
