/*=========================================================================
  ...
  Program:   3D Slicer
  Module:    $HeadURL: http://svn.slicer.org/Slicer3/trunk/Base/GUI/vtkSlicerModelDisplayManager.cxx $
...
#include "vtkSlicerManagerInteractorStyle.h"
...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkSlicerModelDisplayManager );
vtkCxxRevisionMacro ( vtkSlicerModelDisplayManager, "$Revision: 13525 $");

//---------------------------------------------------------------------------
vtkSlicerModelDisplayManager::vtkSlicerModelDisplayManager ( )
{
...
vtkSlicerModelDisplayManager::~vtkSlicerModelDisplayManager ( )
{
  ...
  if (this->MainViewer)
    {
    vtkSlicerManagerInteractorStyle *iStyle;
    iStyle = vtkSlicerManagerInteractorStyle::SafeDownCast (this->MainViewer->GetInteractor()->GetInteractorStyle());
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::PrintSelf ( ostream& os, vtkIndent indent )
{
  this->vtkObject::PrintSelf ( os, indent );

  os << indent << "vtkSlicerModelDisplayManager: " << this->GetClassName ( ) << "\n";
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::ProcessWidgetEvents(vtkObject *vtkNotUsed(caller),
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::CreateClipSlices()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::CreateAxis()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::ColorAxisLabelActors ( double r, double g, double b)
{
  for (unsigned int i=0; i<this->AxisLabelActors.size(); i++)
    {
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::AddAxisActors()
{
  if (this->MainViewer)
    {
    if (this->BoxAxisActor)
      {
      this->MainViewer->GetRenderers()->GetFirstRenderer()->AddViewProp( this->BoxAxisActor);
      }
    for (unsigned int i=0; i<this->AxisLabelActors.size(); i++)
      {
      vtkCamera *camera = this->MainViewer->GetRenderers()->GetFirstRenderer()->IsActiveCameraCreated() ? 
        this->MainViewer->GetRenderers()->GetFirstRenderer()->GetActiveCamera() : NULL;
      this->AxisLabelActors[i]->SetCamera(camera);
      this->MainViewer->GetRenderers()->GetFirstRenderer()->AddViewProp( this->AxisLabelActors[i]);
      }
    }
}

//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateAxis()
{
  this->UpdateViewNode();
  ...
  double bounds[6];
  this->MainViewer->GetRenderers()->GetFirstRenderer()->ComputeVisiblePropBounds(bounds);
...
//      vtkCamera *camera =
//        this->MainViewer->GetRenderers()->GetFirstRenderer()->IsActiveCameraCreated() ? 
//        this->MainViewer->GetRenderers()->GetFirstRenderer()->GetActiveCamera() : NULL;
//      this->AxisLabelActors[i]->SetCamera(camera);
      }
  ...
  for (unsigned int i=0; i<AxisLabelActors.size(); i++)
    {
    //WJPTEST
    if (this->MainViewer != NULL && this->MainViewer->GetRenderers()->GetFirstRenderer() != NULL )
      {
      this->AxisLabelActors[i]->SetCamera(this->MainViewer->GetRenderers()->GetFirstRenderer()->GetActiveCamera());
      }
    else
      {
      this->AxisLabelActors[i]->SetCamera(NULL );
  ...
  if (bBoxChanged)
    ...
    this->MainViewer->GetRenderers()->GetFirstRenderer()->GetActiveCamera()->Dolly(1.5);
    this->MainViewer->GetRenderers()->GetFirstRenderer()->ResetCameraClippingRange();
    }
#endif
...
//---------------------------------------------------------------------------
int vtkSlicerModelDisplayManager::UpdateClipSlicesFromMRML()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::SetClipPlaneFromMatrix(vtkMatrix4x4 *sliceMatrix, 
                                                   int planeDirection,
                                                   vtkPlane *plane)
{
  double normal[3];
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::ProcessMRMLEvents ( vtkObject *caller,
                                                unsigned long event, 
                                                void *callData )
{ 
  ...
  if (event == vtkMRMLScene::SceneClosingEvent )
    ...
    //this->MainViewer->GetRenderers()->GetFirstRenderer()->SetRenderModeToDisabled();
    }
  else if (event == vtkMRMLScene::SceneLoadEndEvent)
    {
    this->SetEnableRender(1);
    //this->MainViewer->GetRenderers()->GetFirstRenderer()->SetRenderModeToInteractive();
    ///WJPTEST
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateCameraNode()
{
...
*/
  ...
  this->MainViewer->GetRenderers()->GetFirstRenderer()->SetActiveCamera(cam);
  if (cam) 
    {
  ...
  vtkRenderWindowInteractor *rwi = 
    this->MainViewer->GetInteractor();
  if (rwi)
    {
    vtkInteractorObserver *iobs = rwi->GetInteractorStyle();
    vtkSlicerManagerInteractorStyle *istyle = 
      vtkSlicerManagerInteractorStyle::SafeDownCast(iobs);
    if (istyle)
      {
  ...
  this->InvokeEvent(vtkSlicerModelDisplayManager::ActiveCameraChangedEvent, NULL);
}

//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::AddCameraObservers()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveCameraObservers()
{
  if (!this->MRMLScene)
    {
    return;
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateViewNode()
{
#if 0
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveWidgetObservers ( ) 
{
}

//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::CreateWidget ( )
{
  if (this->IsCreated())
    {
  ...
  if (this->MainViewer == NULL) 
    {
    // TODO REMOVE THIS
    this->MainViewer = vtkRenderWindow::New ( ); 
    }
  ...
  //this->MainViewer->GetRenderers()->GetFirstRenderer()->SetRendererBackgroundColor(
  ...
  vtkRenderWindowInteractor *rwi = this->MainViewer->GetInteractor();
  if (rwi)
    {
    vtkSlicerManagerInteractorStyle *iStyle = vtkSlicerManagerInteractorStyle::New();
  ...
  if ( this->GetApplication() != NULL )
    ...
    this->MainViewer->GetRenderers()->GetFirstRenderer()->SetWidth ( app->GetDefaultGeometry()->GetSliceViewerMinDim() );
    this->MainViewer->GetRenderers()->GetFirstRenderer()->SetHeight ( app->GetDefaultGeometry()->GetSliceViewerMinDim() );
    }
    ***/
  ...
  this->CreateAxis();
  this->MainViewer->GetRenderers()->GetFirstRenderer()->ResetCamera ( );
}

//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateFromMRML()
{
  this->UpdateViewNode();
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateModelsFromMRML()
{
  ...
  if (clearDisplayedModels)
    ...
    for (iter = this->DisplayedActors.begin(); iter != this->DisplayedActors.end(); iter++)
      {
      this->MainViewer->GetRenderers()->GetFirstRenderer()->RemoveViewProp(iter->second);
      }
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateModifiedModel(vtkMRMLDisplayableNode *model)
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateModelPolyData(vtkMRMLDisplayableNode *model)
{
  ...
  for (unsigned int i=0; i<displayNodes.size(); i++)
    ...
    if (hasPolyData && ait == this->DisplayedActors.end())
      {
      this->MainViewer->GetRenderers()->GetFirstRenderer()->AddViewProp( prop );
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::UpdateModel(vtkMRMLDisplayableNode *model)
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::CheckModelHierarchies()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::AddHierarchiyObservers()
{
  if (this->MRMLScene == NULL)
    {
    return;
...
//----------------------------
void vtkSlicerModelDisplayManager::UpdateModelHierarchyVisibility(vtkMRMLModelHierarchyNode* mhnode, int visibility )
{
...
//----------------------------
void vtkSlicerModelDisplayManager::UpdateModelHierarchyDisplay(vtkMRMLDisplayableNode *model)
{
...
//----------------------------
std::vector< vtkMRMLDisplayNode* > vtkSlicerModelDisplayManager::GetDisplayNode(vtkMRMLDisplayableNode *model)
{
...
//----------------------------
vtkMRMLDisplayNode*  vtkSlicerModelDisplayManager::GetHierarchyDisplayNode(vtkMRMLDisplayableNode *model)
{
  vtkMRMLDisplayNode* dnode = NULL;
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RequestRender()
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::Render()
{
  ...
  if (this->GetEnableRender())
    ...
    if (this->MainViewer->GetRenderers()->GetFirstRenderer()->IsActiveCameraCreated())
      {
      this->IsRendering = 1;
      this->MainViewer->Render();
      this->IsRendering = 0;
      }
    vtkDebugMacro("vtkSlicerModelDisplayManager::Render called render" << endl);
    //this->MainViewer->GetRenderers()->GetFirstRenderer()->SetRenderState(currentRenderState);
    } 
}

//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveModelProps()
{
  std::map<std::string, vtkProp3D *>::iterator iter;
  ...
  for(iter=this->DisplayedActors.begin(); iter != this->DisplayedActors.end(); iter++) 
    ...
    if (modelDisplayNode == NULL)
      ...
      if (clipIter == this->DisplayedClipState.end())
        {
        vtkErrorMacro ("vtkSlicerModelDisplayManager::RemoveModelProps() Unknown clip state\n");
        }
      else 
        {
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveDisplayable(vtkMRMLDisplayableNode* model)
{
  if (!model)
    {
    return;
...
void vtkSlicerModelDisplayManager::RemoveDispalyedID(std::string &id)
{
...
int vtkSlicerModelDisplayManager::GetDisplayedModelsVisibility(vtkMRMLDisplayNode *model)
{
  int visibility = 1;
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveMRMLObservers()
{
  this->RemoveModelObservers(1);
  this->RemoveHierarchyObservers(1);
  this->RemoveCameraObservers();
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveModelObservers(int clearCache)
{
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveModelObservers( vtkMRMLDisplayableNode *model)
{
  vtkEventBroker *broker = vtkEventBroker::GetInstance();
  std::vector< vtkObservation *> observations;
  if (model != NULL)
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::RemoveHierarchyObservers(int clearCache)
{
  vtkEventBroker *broker = vtkEventBroker::GetInstance();
  std::vector< vtkObservation *> observations;
  std::map<std::string, int>::iterator iter;
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::SetModelDisplayProperty(vtkMRMLDisplayableNode *model)
{
  vtkMRMLTransformNode* tnode = model->GetParentTransformNode();
...
vtkProp3D *
vtkSlicerModelDisplayManager::GetActorByID (const char *id)
{
  if ( !id )
    {
    return (NULL);
...
const char *
vtkSlicerModelDisplayManager::GetIDByActor (vtkProp3D *actor)
{
  if ( !actor )
    {
    return (NULL);
...
//---------------------------------------------------------------------------
void vtkSlicerModelDisplayManager::ResetPick()
{
...
//---------------------------------------------------------------------------
int vtkSlicerModelDisplayManager::Pick(int x, int y)
{
  ...
  if (this->GetMainViewer() != NULL)
    {
    ren = this->GetMainViewer()->GetRenderers()->GetFirstRenderer();
    }
  else
    {
...
void vtkSlicerModelDisplayManager::SetBoxWidgetInteractor ()
{
  if ( this->GetMainViewer() &&  
       this->GetMainViewer()->GetInteractor() )
    {
    vtkRenderWindowInteractor *interactor = this->GetMainViewer()->GetInteractor();
    this->BoxWidget->SetInteractor(interactor);
    }
}

vtkClipPolyData* vtkSlicerModelDisplayManager::CreateTransformedClipper (vtkMRMLDisplayableNode *model)
{
