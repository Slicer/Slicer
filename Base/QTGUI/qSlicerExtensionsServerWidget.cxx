...
#ifdef Q_OS_MAC
      "          <li><b>"
    + qSlicerExtensionsServerWidget::tr("Extensions manager requires <em>3D Slicer</em> to be installed. "
...
// --------------------------------------------------------------------------
void qSlicerExtensionsServerWidgetPrivate::registerExtensionsManagerModel(qSlicerExtensionsManagerModel* oldModel, qSlicerExtensionsManagerModel* newModel)
{
  Q_Q(qSlicerExtensionsServerWidget);
...
void qSlicerExtensionsServerWidget::setExtensionsManagerModel(qSlicerExtensionsManagerModel* model)
{
  ...
  if (model)
  {
    ...
    QObject::connect(model, SIGNAL(extensionScheduledForUninstall(QString)), this, SLOT(onExtensionScheduledForUninstall(QString)));

    QObject::connect(model, SIGNAL(extensionCancelledScheduleForUninstall(QString)), this, SLOT(onExtensionCancelledScheduleForUninstall(QString)));

    QObject::connect(model, SIGNAL(slicerRequirementsChanged(QString, QString, QString)), this, SLOT(onSlicerRequirementsChanged()));

    QObject::connect(model, SIGNAL(messageLogged(QString, ctkErrorLogLevel::LogLevels)), this, SLOT(onMessageLogged(QString, ctkErrorLogLevel::LogLevels)));
...
void qSlicerExtensionsServerWidget::onLoadFinished(bool ok)
{
  ...
  {
    d->setFailurePage(
      QStringList() << QString("Failed to load extension page using this URL: <strong>%1</strong>").arg(this->extensionsManagerModel()->extensionsListUrl().toString()));
  }
  if (ok)
  {
    d->updateTheme();
...
void qSlicerExtensionsServerWidget::changeEvent(QEvent* e)
{
  ...
  switch (e->type())
  {
    case QEvent::StyleChange: d->updateTheme(); break;
    default: break;
  }
