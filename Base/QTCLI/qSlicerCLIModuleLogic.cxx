...
public:
  ...
  typedef std::pair<qSlicerCLIModuleLogic *, vtkMRMLCommandLineModuleNode *> LogicNodePair;
...
//----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::stringListToArray(const QStringList& list, std::vector<char*>& argv)
{
...
void qSlicerCLIModuleLogic::ProgressCallback ( void *who )
{
  ...
  qDebug() << "qSlicerCLIModuleLogic::ProgressCallback";
...
//----------------------------------------------------------------------------
std::string qSlicerCLIModuleLogic::constructTemporaryFileName(const std::string& tag,
...
// modify a MRML node.
//
void qSlicerCLIModuleLogic::applyTask(void *clientdata)
{
...
//
   ...
   std::string minisceneFilename = this->ConstructTemporarySceneFileName(miniscene);
...
void qSlicerCLIModuleLogic::generateInputDatasets(CommandLineModuleType commandType, vtkMRMLScene * miniscene,
  MRMLIDMap& sceneToMiniSceneMap, const MRMLIDToFileNameMap & nodesToWrite)
{
  qDebug() << "qSlicerCLIModuleLogic::generateInputDatasets";
  Q_ASSERT(miniscene);
  ...
  for (id2fn0 = nodesToWrite.begin();
       ...
       ++id2fn0)
    {
    vtkMRMLNode *nd = this->mrmlScene()->GetNodeByID( (*id2fn0).first.c_str() );
//
...
void qSlicerCLIModuleLogic::addOutputNodeToMiniScene(const std::string& minisceneFilename, vtkMRMLScene * miniscene, MRMLIDMap& sceneToMiniSceneMap,
  const MRMLIDToFileNameMap & nodesToReload)
{
  qDebug() << "qSlicerCLIModuleLogic::addOutputNodeToMiniScene";
  Q_ASSERT(miniscene);
...
void qSlicerCLIModuleLogic::buildCommandLine(const std::string& minisceneFilename,
...
{
  qDebug() << "qSlicerCLIModuleLogic::buildCommandLine";
  Q_ASSERT(node0);
    ...
    for (pit = pbeginit; pit != pendit; ++pit)
      ...
      if (hasFlag)
        ...
        if ((*pit).GetTag() == "region")
          {
          // get the region node
          vtkMRMLNode *node = this->mrmlScene()->GetNodeByID((*pit).GetDefault().c_str());
...
//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::processParametersWithIndices(const std::string& minisceneFilename,
  vtkMRMLCommandLineModuleNode * node0, MRMLIDMap& sceneToMiniSceneMap, QStringList& commandLineAsString)
{
  qDebug() << "qSlicerCLIModuleLogic::processParametersWithIndices";
  Q_ASSERT(node0);
  ...
  for (iit = indexmap.begin(); iit != indexmap.end(); ++iit)
    ...
    if ((*iit).second.GetTag() != "image"
      ...
      if ((*iit).second.GetDefault() != "")
        ...
        this->appLogic()->RequestModified( node0 );
        return;
        }
      }
...
// //-----------------------------------------------------------------------------
// std::string qSlicerCLIModuleLogic::generateCommand(vtkMRMLCommandLineModuleNode * node0,
//   const QStringList& commandLineAsString )
// {
//   qDebug() << "qSlicerCLIModuleLogic::generateCommand";
...
//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::runFilter(vtkMRMLCommandLineModuleNode * node0,
  const QStringList& argList, CommandLineModuleType commandType)
{
  qDebug() << "qSlicerCLIModuleLogic::runFilter";
  Q_ASSERT(node0);
  // run the filter
  //
  //
...
//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::onExecutionTerminated(vtkMRMLCommandLineModuleNode * node0,
  const MRMLIDMap& sceneToMiniSceneMap)
{
  qDebug() << "qSlicerCLIModuleLogic::onExecutionTerminated";
  Q_ASSERT(node0);
  MRMLIDToFileNameMap::const_iterator id2fn0;
          ...
          if (node0->GetModuleDescription().HasParameter((*pit).GetReference()))
            ...
            if (reference.size() > 0)
              ...
              vtkMRMLTransformableNode *t = vtkMRMLTransformableNode::SafeDownCast(
                this->mrmlScene()->GetNodeByID(reference.c_str()));
              if (t)
                {
                if ( (*pit).GetDefault() != "" )
                  {
                  ...
                  this->appLogic()->RequestModified( reqSTNID );
                  }
                }
              else
                {
...
//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::requestloadMinisceneLoading(const std::string& minisceneFilename,
  vtkMRMLCommandLineModuleNode * node0,
  vtkMRMLScene * miniscene, const MRMLIDMap& sceneToMiniSceneMap)
{
  qDebug() << "qSlicerCLIModuleLogic::requestloadMinisceneLoading";
  Q_ASSERT(node0);
  Q_ASSERT(miniscene);
...
//
    ...
    this->appLogic()->RequestReadScene( minisceneFilename, keys, values,
                                        displayData, this->deleteTemporaryFiles() );
    }
}

//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::cleanUp()
{
  qDebug() << "qSlicerCLIModuleLogic::cleanUp";
  // clean up
  //
  //
//   delete [] command;
...
//-----------------------------------------------------------------------------
void qSlicerCLIModuleLogic::runSharedObjectFilter(vtkMRMLCommandLineModuleNode * node0,
  const QStringList& argList)
{
  qDebug() << "qSlicerCLIModuleLogic::runSharedObjectFilter";
