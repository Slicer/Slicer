...
class DICOMDockWidget(qt.QDockWidget):
    ...
    def __init__(self, title, parent=None, browserWidget=None):
        ...
        self.fullScreenButton.setIcon(self.style().standardIcon(qt.QStyle.SP_TitleBarMaxButton))
        self.fullScreenButton.setToolTip(_("Toggle fullscreen"))
        self.fullScreenButton.setFlat(True)
        self.fullScreenButton.setIconSize(qt.QSize(10, 10))
        self.fullScreenButton.setFixedSize(20, 20)
    ...
    def toggleFullScreen(self):
        ...
        if not self.isDockFullScreen:
            # Enter fullscreen - set floating and resize to screen dimensions
            self.setFloating(True)

            # Get the screen geometry using modern QScreen API
            screen = qt.QApplication.screenAt(self.geometry.center())
            if not screen:
                screen = qt.QApplication.primaryScreen()
            screenGeometry = screen.geometry
            self.setGeometry(screenGeometry)

            # Optional: hide window frame for true fullscreen feel
            self.setWindowFlags(self.windowFlags() | qt.Qt.FramelessWindowHint)
            self.show()

            self.isDockFullScreen = True
            self.fullScreenButton.setIconSize(qt.QSize(20, 20))
            self.fullScreenButton.setIcon(self.style().standardIcon(qt.QStyle.SP_TitleBarNormalButton))
            self.fullScreenButton.setToolTip(_("Exit fullscreen"))
            ...
            # Exit fullscreen - restore window frame and dock
            self.setWindowFlags(self.windowFlags() & ~qt.Qt.FramelessWindowHint)
            self.show()
            self.showNormal()
    ...
    def closeEvent(self, event):
        ...
        if self.dicomInstance:
            ...
            # Clear the dock widget reference so it can be recreated if needed
            dicomInstance.dockWidget = None
            ...
            try:
                ...
                ui.showVisualBrowserButton.text = _("Show Visual DICOM Browser")
                ui.dockToRightPushButton.visible = False
                ui.dockToRightPushButton.checked = False
                ui.dockToRightPushButton.text = _("Show in Side Panel")
            except:
                pass
...
class DICOM(ScriptedLoadableModule):
    ...
    def createDockablePanel(self, browserWidget):
        """Create a dockable panel at main window level for the DICOM browser widget"""
        ...
        # Always set the browser widget as the dock widget's content (even if switching back)
...
class _ui_DICOMSettingsPanel:
    ...
    def onThumbnailsSizeChanged(self):
        ...
        if thumbnailsSize == "large":
            browserWidget.dicomVisualBrowser.thumbnailSizePreset = ctk.ctkDICOMVisualBrowserWidget.Large
        elif thumbnailsSize == "medium":
            browserWidget.dicomVisualBrowser.thumbnailSizePreset = ctk.ctkDICOMVisualBrowserWidget.Medium
        elif thumbnailsSize == "small":
            browserWidget.dicomVisualBrowser.thumbnailSizePreset = ctk.ctkDICOMVisualBrowserWidget.Small
...
class DICOMWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        if self.ui.showVisualBrowserButton.checked:
            self.ui.showVisualBrowserButton.text = _("Hide visual DICOM browser")
        else:
    ...
    def onLayoutChanged(self, viewArrangement):
        ...
        self.ui.showVisualBrowserButton.checked = settingsValue("DICOM/UseVisualDICOMBrowser", False, converter=toBool)
        self.ui.showVisualBrowserButton.text = (_("Hide visual DICOM browser") if self.ui.showVisualBrowserButton.checked
                                               else _("Show visual DICOM browser"))
    ...
    def switchToDockMode(self):
        ...
        dicomInstance.createDockablePanel(self.browserWidget)
        ...
        self.ui.dockToRightPushButton.text = _("Show in center view")
