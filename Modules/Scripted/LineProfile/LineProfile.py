...
class LineProfile(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.categories = [translate("Quantification")]
...
class LineProfileWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        #
        self.inputVolumeSelector = self.ui.inputVolumeSelector
        ...
        self.inputVolumeSelector.setToolTip(_("Pick the input to the algorithm which will be sampled along the line."))
        #
        # input line selector
        #
        self.inputLineWidget = self.ui.inputLineWidget
        ...
        self.inputLineWidget.setToolTip(_("Pick line or curve to take image samples along."))
        #
        # output table selector
        #
        self.outputTableSelector = self.ui.outputTableSelector
        ...
        self.outputTableSelector.setToolTip(_("Pick the table that will store the intensity and distance values." ))
        #
        # line resolution
        #
        self.lineResolutionSliderWidget = self.ui.lineResolutionSliderWidget
        ...
        self.lineResolutionSliderWidget.setToolTip(_("Number of points to sample along the line."))
        #
        # output plot selector
        #
        self.outputPlotSeriesSelector = self.ui.outputPlotSeriesSelector
        ...
        self.outputPlotSeriesSelector.setToolTip(_("Pick the output plot series to the algorithm." ))

        #
        # Proportional percent distance from start
        #
        self.plotProportionalDistanceCheckBox = self.ui.plotProportionalDistanceCheckBox
        self.plotProportionalDistanceCheckBox.checked = False
        self.plotProportionalDistanceCheckBox.setToolTip(_("If checked, then distance along the line in plot is not absolute, " \
                                                           "but the percent distance from the start of the line."))
        #
        # Peaks section
        #
        peaksCollapsibleButton = self.ui.peaksCollapsibleButton
        peaksCollapsibleButton.collapsed = True
        #
        # output peaks table selector
        #
        self.outputPeaksTableSelector = self.ui.outputPeaksTableSelector
        ...
        self.outputPeaksTableSelector.setToolTip(_("Pick the output table that will store information about each detected peak." ))
        #
        # peaks minimum width selector
        #
        self.peakMinimumWidthSpinBox = self.ui.peakMinimumWidthSpinBox
        ...
        self.peakMinimumWidthSpinBox.setToolTip(_("Minimum width of the peak. Use higher values to reject small peaks detected due to image noise."))
        #
        # peaks minimum height selector
        #
        self.heightPercentageForWidthMeasurementSpinBox = self.ui.heightPercentageForWidthMeasurementSpinBox
        ...
        self.heightPercentageForWidthMeasurementSpinBox.setToolTip(_("Height at which the peak width is measured. 50% computes full width " \
                                                                     "at half maximum. Larger value means that the height is measured near the" \
                                                                     " top of the peak."))
        #
        # peaks is maximum checkbox
        #
        self.peakIsMaximumCheckBox = self.ui.peakIsMaximumCheckBox
        self.peakIsMaximumCheckBox.checked = True
        self.peakIsMaximumCheckBox.setToolTip(_("If checked then peaks are local maximum values. If unchecked then peaks are local minimum values."))
        #
        # Apply Button
        #
