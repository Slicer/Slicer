...
public:
  ...
  /// Patient and Study subject hierarchy item ID.
  /// All exportables share the same patient and study information.
...
QString qSlicerDICOMTagEditorWidget::setExportables(QList<qSlicerDICOMExportable*> exportables)
{
  ...
  d->PatientItemID = vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID;
  ...
  foreach (qSlicerDICOMExportable* exportable, d->Exportables)
  {
    ...
    if (d->StudyItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID)
    {
      ...
      d->StudyItemID = parentItemID;
    }
    else if (d->StudyItemID != parentItemID)
    {
  ...
  // Get patient item
  d->PatientItemID = shNode->GetItemParent(d->StudyItemID);
  if (d->PatientItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID)
  {
  ...
  // At this line, because there must not be return statements between disconnect and connect (unless connect is called
  // before return!)
       ...
       ++patientTagIt)
  {
    ...
    if (std::find(patientItemAttributeNames.begin(), patientItemAttributeNames.end(), tagAttributeName)
        == patientItemAttributeNames.end())
    {
      shNode->SetItemAttribute(d->PatientItemID, tagAttributeName, "");
    }
  }
  // Get attribute names again in case some were missing
  patientItemAttributeNames = shNode->GetItemAttributeNames(d->PatientItemID);
  ...
  for (std::vector<std::string>::iterator it = patientItemAttributeNames.begin(); it != patientItemAttributeNames.end();
       ++it)
  {
    ...
    std::string attributePrefix =
      attributeName.substr(0, vtkMRMLSubjectHierarchyConstants::GetDICOMAttributePrefix().size());
  ...
  std::vector<std::string> studyItemAttributeNames = shNode->GetItemAttributeNames(d->StudyItemID);
       ...
       ++studyTagIt)
  {
    ...
    if (std::find(studyItemAttributeNames.begin(), studyItemAttributeNames.end(), tagAttributeName)
        == studyItemAttributeNames.end())
    {
  ...
  // Get attribute names again in case some were missing
  studyItemAttributeNames = shNode->GetItemAttributeNames(d->StudyItemID);
  // Create a row in table widget for each tag and populate exportables with study tags
  // (all tags are acquired from the exportable on export)
  for (std::vector<std::string>::iterator it = studyItemAttributeNames.begin(); it != studyItemAttributeNames.end();
       ++it)
  {
    std::string attributeName = (*it);
    if (attributeName.size() < vtkMRMLSubjectHierarchyConstants::GetDICOMAttributePrefix().size())
  ...
  foreach (qSlicerDICOMExportable* exportable, d->Exportables)
  {
    ...
    d->insertTagsTableRow(row);
    QTableWidgetItem* seriesHeaderNameItem =
      new QTableWidgetItem(QString("'%1' series tag name").arg(shNode->GetItemName(seriesItemID).c_str()), 1);
    ...
    foreach (QString tagName, exportableTagsMap.keys())
    {
      ...
      std::string tagAttributeName =
        vtkMRMLSubjectHierarchyConstants::GetDICOMAttributePrefix() + std::string(tagName.toUtf8().constData());
