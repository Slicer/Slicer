...
class DataProbe(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        parent.categories = [_("Quantification")]
...
class DataProbeInfoWidget:
    ...
    def getPixelString(self, volumeNode, ijk):
        ...
        if numberOfComponents > 3:
            return _("%d components") % numberOfComponents
    ...
    def processEvent(self, observee, event):
        ...
        if hasattr(self.frame.parent(), 'text'):
            ...
            if sceneName != "":
                self.frame.parent().text = _("Data Probe: %s") % self.fitName(sceneName, nameSize=2 * self.nameSize)
            else:
