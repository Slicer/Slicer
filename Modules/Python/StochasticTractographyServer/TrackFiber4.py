...
# Pre-calculate the scalar products with the gradient directions
  AnIsoExponent = tile(b.T, (1, vectors.shape[1]) )*dot(G.T, vectors)**2

# Pre-calculate matrix used for tensor estimation

  A = ones( (data.shape[3], 7) , 'float')
...
# Uniform prior distribution of the direction of the first step
  StartPrior = ones((1, vectors.shape[1]), 'float')
...
# Initialize
  ...
  for k in range(Npaths):
    ...
    for step in range(Nsteps):
      ...
      IsoExponenT = tile(IsoExponent, (1, vectors.shape[1])) 
...
# Initialize
  ...
  for k in range(Npaths):
    ...
    for step in range(Nsteps):
      ...
      # Get measurements
      #try:
      if 0<=coord[0]<data.shape[0] and 0<=coord[1]<data.shape[1] and 0<=coord[2]<data.shape[2]:
        y[:] = squeeze(data[coord[0], coord[1], coord[2], :])
      else:
        break
      #except:
...
# Gradients must be transformed in RAS!
def  TrackFiberT40(data, q, shpT, b, G, IJKstartpoints, R2I, I2R, lV, EV, xVTensor, dl=1, Nsteps=300, anisoT=0.2, useSpacing = False, seed=None):
# This function performs stochastic tracking of one fiber.
# The algorithm is described in the paper 
# O. Friman et al. 
# A Bayesian Approach for Stochastic White Matter Tractography
...
# Initialize
  ...
  for k in range(Npaths):
    ...
    for step in range(Nsteps):
      ...
      # Determine from which voxel to draw new direction  
      coord = floor(IJKpoint) + (ceil(IJKpoint+seps)-IJKpoint < rand(3,1).T)
      coord = coord-1 # Matlab
      coord = coord.squeeze()
      ...
      paths[k][0][:,step] = RASpoint
      paths[k][1][:,step] = IJKpoint
      paths[k][2][0,step] = log(Posterior[0][vindex[0][0]]) # previously vindex[0][0] 
      paths[k][3][0,step] = abs(beta/(alpha+beta))
      paths[k][4][0,0] = paths[k][4][0,0] + 1
...
# Gradients must be transformed in RAS!
def  TrackFiberX40(data, mask, q, shpT, b, G, IJKstartpoints, R2I, I2R, lV, EV, xVTensor, dl=1, Nsteps=300, anisoT=0.2, useSpacing = False, seed=None):
# This function performs stochastic tracking of one fiber.
# The algorithm is described in the paper 
# O. Friman et al. 
# A Bayesian Approach for Stochastic White Matter Tractography
