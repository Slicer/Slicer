...
Module:    $RCSfile: vtkMimxUGrid.cxx,v $
Language:  C++
...
vtkMimxUGrid::vtkMimxUGrid()
{
        ...
        this->Links = 0;
        this->MeshSeedValues = vtkIntArray::New();
}
...
vtkMimxUGrid::~vtkMimxUGrid()
{
        ...
        this->MeshSeedValues->Delete();
}

vtkActor* vtkMimxUGrid::GetActor()
{
        return this->Actor;
}

void vtkMimxUGrid::SetPrevColor(double color[3])
{
        for(int i=0; i<3; i++)  this->PrevColor[i] = color[i];
}

void vtkMimxUGrid::GetPrevColor(double color[3])
{
        for(int i=0; i<3; i++)  color[i] = this->PrevColor[i];
}

void vtkMimxUGrid::CreateBoundingBox(vtkMimxSTLSource* STL)
{
        double bounds[6];
        STL->GetPolyData()->GetBounds(bounds);
        vtkPoints* points = vtkPoints::New();
        points->SetNumberOfPoints(8);
        // set all the 8 vertices of the hexahedron
        points->SetPoint(0, bounds[0], bounds[2], bounds[4]);
        points->SetPoint(1, bounds[0], bounds[2], bounds[5]);
        points->SetPoint(2, bounds[1], bounds[2], bounds[5]);
        points->SetPoint(3, bounds[1], bounds[2], bounds[4]);
        points->SetPoint(4, bounds[0], bounds[3], bounds[4]);
        points->SetPoint(5, bounds[0], bounds[3], bounds[5]);
        points->SetPoint(6, bounds[1], bounds[3], bounds[5]);
        points->SetPoint(7, bounds[1], bounds[3], bounds[4]);

        vtkHexahedron* aHexahedron = vtkHexahedron::New();
        for(int i=0; i < 8; i++)        aHexahedron->GetPointIds()->SetId(i, i);


        this->UGrid->Allocate(1, 1);
        this->UGrid->InsertNextCell(aHexahedron->GetCellType(),
...
void vtkMimxUGrid::ComputeMeshSeed(double AvElLength)
{

        this->MeshSeedValues->Initialize();     
        this->MeshSeedFromAverageElementLength(AvElLength);
}

int vtkMimxUGrid::MeshSeedFromAverageElementLength(double ElLength)
{
...
void vtkMimxUGrid::BuildMeshSeedLinks()
{
        this->Links = 1;
        ...
        for (int i=0; i<this->UGrid->GetNumberOfCells(); i++)
        {
                for (int j=1; j<=3; j++)
                {
                        ...
                        meshseedtree->Delete();
                        /*              for (int k=0; k < this->UGrid->GetNumberOfCells(); k++)
                        {
...
void vtkMimxUGrid::SetUnstructuredGrid(vtkUnstructuredGrid* UnsGrid)
{
        this->UGrid = UnsGrid;
        this->Links = 0;
}

void vtkMimxUGrid::SetMeshSeedValues(vtkIntArray* IntArray)
{
        this->UGrid->GetCellData()->SetVectors(IntArray);
}

void vtkMimxUGrid::GetCellNeighbors(vtkIdType cellnum, vtkIdList *cellids)
{
        cellids->Initialize();
...
int vtkMimxUGrid::CheckIfCellsShareEdgeX(int cellnum1, int cellnum2, int parcell, int edgenum)
{
        bool status1 = false;
        bool status2 = false;
...
int vtkMimxUGrid::CheckIfCellsShareEdgeY(int cellnum1, int cellnum2, int parcell, int edgenum)
{
        bool status1 = false;
        bool status2 = false;
        vtkGenericCell *cell = vtkGenericCell::New();
...
// bounding boxes.
void vtkMimxUGrid::EstimateMeshSeedsBasedOnAverageElementLength(double ElLength)
{
        if(!this->Links)        
        {
...
int vtkMimxUGrid::CheckIfCellsShareEdgeZ(int cellnum1, int cellnum2, int parcell, int edgenum)
{
        bool status1 = false;
        bool status2 = false;
        vtkGenericCell *cell = vtkGenericCell::New();
...
int vtkMimxUGrid::WhichEdgeOfCellBeingCompared(vtkIdType vertex1, vtkIdType vertex2)
{
...
void vtkMimxUGrid::SetHigherNumberedBBoxMeshSeed(int boxnum, int axis)
{
        int dim[3], dimmod[3];
...
void vtkMimxUGrid::ChangeMeshSeed(int BoxNum, int Axis, int SeedNum)
{
        if(!this->Links)        
        {
                this->BuildMeshSeedLinks();
        ...
        if(this->UGrid->GetCellData()->GetVectors())
        {
        this->MeshSeedValues = (vtkIntArray*)
                this->UGrid->GetCellData()->GetVectors();
        int dim[3], dimmod[3];
        this->MeshSeedValues->GetTupleValue(BoxNum, dim);
        dim[Axis] = SeedNum;
