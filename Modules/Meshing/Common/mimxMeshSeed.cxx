...
Module:    $RCSfile: mimxMeshSeed.cxx,v $
Language:  C++
...
int MeshSeed::MeshSeedFromAverageElementLength(double ElLength)
{
...
void MeshSeed::BuildMeshSeedLinks()
{
  this->Links = 1;
...
void MeshSeed::SetUnstructuredGrid(vtkUnstructuredGrid* UnsGrid)
{
  this->UGrid = UnsGrid;
  this->Links = 0;
}

void MeshSeed::SetMeshSeedValues(vtkIntArray* IntArray)
{
  this->MeshSeedValues = IntArray;
  this->Links = 0;
}

void MeshSeed::GetCellNeighbors(vtkIdType cellnum, vtkIdList *cellids)
{
  cellids->Initialize();
...
int MeshSeed::CheckIfCellsShareEdgeX(int cellnum1, int cellnum2, int parcell, int edgenum)
{
  bool status1 = false;
  bool status2 = false;
...
int MeshSeed::CheckIfCellsShareEdgeY(int cellnum1, int cellnum2, int parcell, int edgenum)
{
  bool status1 = false;
  bool status2 = false;
  vtkGenericCell *cell = vtkGenericCell::New();
...
// bounding boxes.
void MeshSeed::EstimateMeshSeedsBasedOnAverageElementLength(double ElLength)
{
  if(!this->Links)  
  {
...
int MeshSeed::CheckIfCellsShareEdgeZ(int cellnum1, int cellnum2, int parcell, int edgenum)
{
  bool status1 = false;
  bool status2 = false;
  vtkGenericCell *cell = vtkGenericCell::New();
...
int MeshSeed::WhichEdgeOfCellBeingCompared(vtkIdType vertex1, vtkIdType vertex2)
{
...
void MeshSeed::SetHigherNumberedBBoxMeshSeed(int boxnum, int axis)
{
  int dim[3], dimmod[3];
...
void MeshSeed::ChangeMeshSeed(int BoxNum, int Axis, int SeedNum)
{
  if(!this->Links)  
  {
    this->BuildMeshSeedLinks();
