...
Module:    $RCSfile: vtkMimxViewMergePointsWidget.cxx,v $
Language:  C++
...
vtkCxxRevisionMacro(vtkMimxViewMergePointsWidget, "$Revision: 1.3 $");
vtkStandardNewMacro(vtkMimxViewMergePointsWidget);
//----------------------------------------------------------------------
...
public:
        ...
        vtkMimxViewMergePointsWidget  *UnstructuredGridWidget;
        vtkMimxModPointWidget *PointWidget;
        int HandleNumber;
};
//----------------------------------------------------------------------
vtkMimxViewMergePointsWidget::vtkMimxViewMergePointsWidget()
{
        this->CallbackMapper->SetCallbackMethod(
                vtkCommand::LeftButtonPressEvent,
                vtkEvent::ShiftModifier, 0, 1, NULL,
                vtkMimxViewMergePointsWidget::ShiftLeftMouseButtonDown,
                this, vtkMimxViewMergePointsWidget::ShiftLeftButtonDownCallback);

        this->CallbackMapper->SetCallbackMethod(
                vtkCommand::LeftButtonReleaseEvent,
                vtkEvent::ShiftModifier, 0, 1, NULL,
                vtkMimxViewMergePointsWidget::ShiftLeftMouseButtonUp,
                this, vtkMimxViewMergePointsWidget::ShiftLeftButtonUpCallback);

        this->CallbackMapper->SetCallbackMethod(vtkCommand::LeftButtonPressEvent,
                vtkMimxViewMergePointsWidget::CrtlLeftMouseButtonDown,
                this, vtkMimxViewMergePointsWidget::CrtlLeftButtonDownCallback);

        this->CallbackMapper->SetCallbackMethod(vtkCommand::MouseMoveEvent,
                vtkMimxViewMergePointsWidget::CrtlLeftMouseButtonMove,
                this, vtkMimxViewMergePointsWidget::CrtlMouseMoveCallback);

        this->CallbackMapper->SetCallbackMethod(vtkCommand::LeftButtonReleaseEvent,
                vtkMimxViewMergePointsWidget::CrtlLeftMouseButtonUp,
                this, vtkMimxViewMergePointsWidget::CrtlLeftButtonUpCallback);

        this->CallbackMapper->SetCallbackMethod(vtkCommand::MiddleButtonPressEvent,
                vtkMimxViewMergePointsWidget::CrtlMiddleMouseButtonDown,
                this, vtkMimxViewMergePointsWidget::CrtlMiddleButtonDownCallback);

        this->CallbackMapper->SetCallbackMethod(vtkCommand::MiddleButtonReleaseEvent,
                vtkMimxViewMergePointsWidget::CrtlMiddleMouseButtonUp,
                this, vtkMimxViewMergePointsWidget::CrtlMiddleButtonUpCallback);

  this->State = vtkMimxViewMergePointsWidget::Start;
  ...
  this->AppendFilter = vtkAppendFilter::New();
  this->Tolerance = 1.0;
 }
...
//              this->ComputeAverageHandleSize(this);
                ...
                this->ComputeMergePointsAndFaces(this);
        }
...
//----------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::SetEnabled(int enabling, vtkMimxViewMergePointsWidget *Self)
{ 
        if ( ! Self->Interactor )
        {
                vtkErrorMacro(<<"The interactor must be set prior to enabling/disabling widget");
...
//--------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CrtlLeftButtonDownCallback(vtkAbstractWidget *w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
        ...
        {
                Self->State = vtkMimxViewMergePointsWidget::Outside;
        ...
        if ( path != NULL )
        {
                ...
                Self->State = vtkMimxViewMergePointsWidget::MovingHandle;
        ...
        else
        {
                ...
                if(path !=NULL)
                {
                        Self->State = vtkMimxViewMergePointsWidget::MovingMultipleFace;
...
//                              Self->State = vtkMimxViewMergePointsWidget::PickMultipleFace;
...
//----------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ShiftLeftButtonDownCallback(vtkAbstractWidget *w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
...
//-------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ShiftLeftButtonUpCallback(vtkAbstractWidget *w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
        //if ( Self->State == vtkMimxViewMergePointsWidget::Outside ||
        //      Self->State == vtkMimxViewMergePointsWidget::Start )
        //{
        ...
        //if(Self->State == vtkMimxViewMergePointsWidget::MovingHandle)
        //{
...
//----------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CrtlLeftButtonUpCallback(vtkAbstractWidget *w)
{
  vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
  //if ( Self->State == vtkMimxViewMergePointsWidget::Outside ||
         // Self->State == vtkMimxViewMergePointsWidget::Start)
...
//---------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CrtlMiddleButtonDownCallback(vtkAbstractWidget* w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
...
//      int i;
        ...
        if(path !=NULL)
        {
                ...
                //Self->State = vtkMimxViewMergePointsWidget::MovingEdge;
                //// hide handles so that you can place a quad at the face picked
                //Self->HideHandles(Self);
...
//---------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CrtlMiddleButtonUpCallback(vtkAbstractWidget* w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
        ...
        //if(Self->State == vtkMimxViewMergePointsWidget::MovingEdge)
        //{
                Self->ShowHandles(Self);
        //}
...
//----------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CrtlMouseMoveCallback(vtkAbstractWidget *w)
{
        vtkMimxViewMergePointsWidget *Self = reinterpret_cast<vtkMimxViewMergePointsWidget*>(w);
        ...
        if (Self->State == vtkMimxViewMergePointsWidget::MovingMultipleFace)
        {
                Self->HideHandles(Self);
        }
        int X = Self->Interactor->GetEventPosition()[0];
...
//---------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
//-----------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::PositionHandles(vtkMimxViewMergePointsWidget *Self)
{
...
//-----------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::SizeHandles(vtkMimxViewMergePointsWidget *Self)
{
...
//------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::SizeHandles()
{
        vtkIntArray *constrain = vtkIntArray::SafeDownCast(
...
//-----------------------------------------------------------------------------
int vtkMimxViewMergePointsWidget::HighlightHandle(vtkProp *prop, vtkMimxViewMergePointsWidget *Self)
{
...
//------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::CreateDefaultProperties()
{
        // Handle properties
        ...
        this->WarningHandleProperty = vtkProperty::New();
        this->WarningHandleProperty->SetColor(1,1,0);   
        // Outline properties
...
//----------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::EnablePointWidget(vtkMimxViewMergePointsWidget *Self)
{
...
//--------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::DisablePointWidget(
...
//-----------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::SetPoint(int i, double x[3], vtkMimxViewMergePointsWidget *Self)
{
        //      double x1[3];
...
void vtkMimxViewMergePointsWidget::Initialize(vtkMimxViewMergePointsWidget *Self)
{
        ...
        Self->AppendFilter->RemoveAllInputs();
        Self->DeleteInteriorHighlightedFaces();
}

void vtkMimxViewMergePointsWidget::TranslateProp(double p1[3], double p2[3], vtkMimxViewMergePointsWidget *Self)
{
        double dirvec[3];
        int i, j;
...
//----------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::HideHandles(vtkMimxViewMergePointsWidget *Self)
{
        // turn off the handles
        for (int i=0; i<Self->Handle->GetNumberOfItems(); i++)
        {
...
//-----------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ShowHandles(vtkMimxViewMergePointsWidget *Self)
{
        // turn off the handles
        for (int i=0; i<Self->Handle->GetNumberOfItems(); i++)
        {
...
//--------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::TranslatePropFace(double p1[3], double p2[3], vtkMimxViewMergePointsWidget *Self)
{
        double dirvec[3];
        int i, j;
        for (i=0; i<3; i++)     dirvec[i] = p2[i] - p1[i];
...
//-----------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::TranslatePropFaceParallel(double p1[3], double p2[3], vtkMimxViewMergePointsWidget *Self)
{
...
//-------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ExtractEdge(vtkMimxViewMergePointsWidget *Self)
{
...
//------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ComputeSelectedEdgePointIds(vtkMimxViewMergePointsWidget *Self)
{
        int i, j;
        Self->SelectedPointIds->Initialize();
        for(i=0; i<Self->SelectedCellIds->GetNumberOfIds(); i++)
...
//-------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::RemoveHighlightedFaces(vtkMimxViewMergePointsWidget *Self)
{
...
//-------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::RemoveHighlightedEdges(vtkMimxViewMergePointsWidget *Self)
{
        if(Self->EdgeGeometry)
                Self->CurrentRenderer->RemoveActor(Self->EdgeGeometry);
}
//-------------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ShowInteriorHighlightedFaces()
{
...
//-----------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::HideInteriorHighlightedFaces()
{
        int numitems = this->InteriorFaceGeometry->GetNumberOfItems();
        int i;
...
//-----------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::DeleteInteriorHighlightedFaces()
{
        this->HideInteriorHighlightedFaces();
        int numitems = this->InteriorFaceGeometry->GetNumberOfItems();
        if(numitems)
...
//-----------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ComputeInteriorHighlightedFaces()
{
        //int i,CellFace;
...
//------------------------------------------------------------------------------------------------
int vtkMimxViewMergePointsWidget::GetInputPickedCellAndFace(
...
//-----------------------------------------------------------------------------------------------
int vtkMimxViewMergePointsWidget::GetInputPickedCompleteFace(
...
//-------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::ShowInteriorHighlightedFace(int FaceNum)
{
...
//--------------------------------------------------------------------------------------------
void vtkMimxViewMergePointsWidget::HideInteriorHighlightedFace(int FaceNum)
{
        int location = this->InteriorFaceList->IsId(FaceNum);
        if(location == -1)      return;
...
void vtkMimxViewMergePointsWidget::SetInput(vtkDataSetCollection *InputColl)
{
        ...
        this->InputCollection = InputColl;
        this->AppendFilter = vtkAppendFilter::New();
        int i;
        for (i=0; i<this->InputCollection->GetNumberOfItems(); i++)
        {
                this->AppendFilter->AddInput(this->InputCollection->GetItem(i));
        }
        this->AppendFilter->Update();
        this->UGrid = this->AppendFilter->GetOutput();
        this->Execute(this);
}
//-------------------------------------------------------------------------------------------------
...
{
        Self->DeleteInteriorHighlightedFaces();
        Self->UGrid->BuildLinks();
        vtkPointLocator *locator = vtkPointLocator::New();
        locator->SetDataSet(Self->UGrid);
        vtkPoints *locatorpoints = vtkPoints::New();
        locatorpoints->Allocate(Self->UGrid->GetNumberOfPoints());
        locator->InitPointInsertion(locatorpoints, Self->UGrid->GetBounds());
        int i, j;
        vtkIdList *PointIdList = vtkIdList::New();
        for (i=0; i<Self->UGrid->GetNumberOfPoints(); i++)
        {
                locator->InsertNextPoint(Self->UGrid->GetPoint(i));
        }
        for (i=0; i<Self->UGrid->GetNumberOfPoints(); i++)
        {
                        ...
                        double x[3];
                        Self->UGrid->GetPoint(i, x);
                        locator->FindPointsWithinRadius(Self->Tolerance, x, LocalList);
                        for(j=0; j< LocalList->GetNumberOfIds(); j++)
                        {
                                if(LocalList->GetId(j) != i)
                                {
                                        vtkIdList *edgeList = vtkIdList::New();
                                        edgeList->InsertNextId(i);      
                                        edgeList->InsertNextId(LocalList->GetId(j));
                                        //if(!IsEdge(Self, edgeList))
                                        //{
                                                int point = LocalList->GetId(j);
                                                PointIdList->InsertUniqueId(LocalList->GetId(j));
                                        //}
                                        edgeList->Delete();
                                }
                        }
        }
        int numIds = PointIdList->GetNumberOfIds();
        for (i=0; i<Self->UGrid->GetNumberOfPoints(); i++)
        {
                if(PointIdList->IsId(i) != -1)
                {
                        vtkActor::SafeDownCast(Self->Handle->GetItemAsObject(i))->SetProperty(this->WarningHandleProperty);
                        ((vtkMimxModPointWidget*) (Self->PointWidget->GetItemAsObject(i)))
                                ->GetProperty()->SetColor(1.0,1.0,0);
                }
                else
                {
                        vtkActor::SafeDownCast(Self->Handle->GetItemAsObject(i))->SetProperty(this->HandleProperty);
                        ((vtkMimxModPointWidget*) (Self->PointWidget->GetItemAsObject(i)))
                                ->GetProperty()->SetColor(1,0,0);
                }
        }
...
//-------------------------------------------------------------------------------------------------
int vtkMimxViewMergePointsWidget::IsEdge(vtkMimxViewMergePointsWidget *Self, vtkIdList *IdList)
{
        int i,j,k;
        for (i=0; i<Self->UGrid->GetNumberOfCells(); i++)
        {
                ...
                {
                        vtkCell *edgeCell = Cell->GetEdge(j);
                        vtkIdList *ptIds = edgeCell->GetPointIds();
                        bool status = false;
                        for (k=0; k<IdList->GetNumberOfIds(); k++)
                        {
                                if(ptIds->IsId(IdList->GetId(k)) == -1)
                                {
                                        status = true;
                                        break;
                                }
...
{
        if(this->AppendFilter->GetOutput()->GetNumberOfCells())
        {
