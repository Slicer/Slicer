...
vtkCxxRevisionMacro(vtkMimxExtractCellWidget, "$Revision: 1.2 $");
...
vtkMimxExtractCellWidget::vtkMimxExtractCellWidget()
{
  ...
  this->EventCallbackCommand->SetCallback(vtkMimxExtractCellWidget::ProcessEvents);
...
void vtkMimxExtractCellWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                 unsigned long event,
                                 void* clientdata, 
                                 void* vtkNotUsed(calldata))
{
  vtkMimxExtractCellWidget* self = reinterpret_cast<vtkMimxExtractCellWidget *>( clientdata );
...
void vtkMimxExtractCellWidget::OnLeftButtonDown()
{
  ...
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
    {
    this->State = vtkMimxExtractCellWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
  {
    this->State = vtkMimxExtractCellWidget::Moving;
    vtkIdType PickedCell = this->CellPicker->GetCellId();
...
void vtkMimxExtractCellWidget::OnLeftButtonUp()
{
  if ( this->State == vtkMimxExtractCellWidget::Outside ||
       this->State == vtkMimxExtractCellWidget::Start )
    {
    return;
    }
...
void vtkMimxExtractCellWidget::OnMiddleButtonDown()
{
  //int X = this->Interactor->GetEventPosition()[0];
  //int Y = this->Interactor->GetEventPosition()[1];
...
void vtkMimxExtractCellWidget::OnMiddleButtonUp()
{
  /*if ( this->State == vtkMimxExtractCellWidget::Outside ||
       this->State == vtkMimxExtractCellWidget::Start )
    {
...
void vtkMimxExtractCellWidget::OnRightButtonDown()
{
//  int X = this->Interactor->GetEventPosition()[0];
//  int Y = this->Interactor->GetEventPosition()[1];
//
...
//  if ( path != NULL )
//    {
//    this->State = vtkMimxExtractCellWidget::Scaling;
//    this->HighlightOutline(1);
//    this->HandlePicker->GetPickPosition(this->LastPickPosition);
//    this->ValidPick = 1;
//    }
...
//}
//
//void vtkMimxExtractCellWidget::OnRightButtonUp()
//{
//  if ( this->State == vtkMimxExtractCellWidget::Outside )
//    {
//    return;
...
void vtkMimxExtractCellWidget::OnMouseMove()
{
  ...
  //// Process the motion
  //if ( this->State == vtkMimxExtractCellWidget::Moving )
  //  {
  //  // Okay to process
  ...
  //    }
  //  }
  //else if ( this->ScalingEnabled && this->State == vtkMimxExtractCellWidget::Scaling )
  //  {
...
void vtkMimxExtractCellWidget::PlaceWidget(double bounds[6])
{
