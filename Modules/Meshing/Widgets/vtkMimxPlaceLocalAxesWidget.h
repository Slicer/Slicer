...
Module:    $RCSfile: vtkMimxPlaceLocalAxesWidget.h,v $
Language:  C++

Date:      $Date: 2008/07/26 20:30:55 $
...
=========================================================================*/
// .NAME vtkMimxPlaceLocalAxesWidget - orthogonal hexahedron 3D widget
// .SECTION Description
...
// nice feature of the object is that the vtkMimxPlaceLocalAxesWidget, like any 3D widget,
// will work with the current interactor style. That is, if vtkMimxPlaceLocalAxesWidget does
...
// vtkMimxPlaceLocalAxesWidget will terminate the processing of the event that it handles.
//
// To use this object, just invoke SetInteractor() with the argument of the
// method a vtkRenderWindowInteractor.  You may also wish to invoke
// "PlaceWidget()" to initially position the widget. The interactor will act
// normally until the "i" key (for "interactor") is pressed, at which point the
// vtkMimxPlaceLocalAxesWidget will appear. (See superclass documentation for information
...
// rotate vtkMimxPlaceLocalAxesWidget, pick a face (but not a face handle) and move the left
...
//
// The vtkMimxPlaceLocalAxesWidget is very flexible. It can be used to select, cut, clip, or
...
#ifndef __vtkMimxPlaceLocalAxesWidget_h
#define __vtkMimxPlaceLocalAxesWidget_h
...
public:
  ...
  static vtkMimxPlaceLocalAxesWidget *New();

  vtkTypeRevisionMacro(vtkMimxPlaceLocalAxesWidget,vtk3DWidget);
  ...
  // set.
  vtkGetObjectMacro(LocalAxesActor,vtkLocalAxesActor);
 ...
 /* void SetOutlineCursorWires(int);
  ...
  vtkGetMacro(PickedCell, int);
  
  int GetMeshSeedX();
  int GetMeshSeedY();
  int GetMeshSeedZ();

  vtkSetObjectMacro(XMeshSeed, vtkKWEntryWithLabel);
  vtkSetObjectMacro(YMeshSeed, vtkKWEntryWithLabel);
  vtkSetObjectMacro(ZMeshSeed, vtkKWEntryWithLabel);
  vtkSetObjectMacro(SelectionTypeWidget, vtkKWMenuButtonWithLabel);
...
protected:
  ...
  vtkActor          *CellActor;
  vtkActor                      *InputActor;
  vtkDataSetMapper *CellMapper;
  vtkUnstructuredGrid       *CellDataSet;
 ...
 /* virtual void Translate(double *p1, double *p2);
  ...
  vtkKWMenuButtonWithLabel *SelectionTypeWidget;
  
  
private:
  vtkMimxPlaceLocalAxesWidget(const vtkMimxPlaceLocalAxesWidget&);  //Not implemented
  void operator=(const vtkMimxPlaceLocalAxesWidget&);  //Not implemented
};
