...
Module:    $RCSfile: vtkMimxModPointWidget.cxx,v $
Language:  C++
Date:      $Date: 2008/02/27 22:16:30 $
...
vtkCxxRevisionMacro(vtkMimxModPointWidget, "$Revision: 1.6 $");
vtkStandardNewMacro(vtkMimxModPointWidget);

vtkMimxModPointWidget::vtkMimxModPointWidget()
{
        this->State = vtkMimxModPointWidget::Start;
        this->EventCallbackCommand->SetCallback(vtkMimxModPointWidget::ProcessEvents);
...
void vtkMimxModPointWidget::SetEnabled(int enabling)
{
        if ( ! this->Interactor )
        {
...
void vtkMimxModPointWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                                                   unsigned long event,
                                                                   void* clientdata, 
                                                                   void* vtkNotUsed(calldata))
{
        vtkMimxModPointWidget* self = reinterpret_cast<vtkMimxModPointWidget *>( clientdata );
...
void vtkMimxModPointWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkMimxModPointWidget::Highlight(int highlight)
{
        if ( highlight )
        {
...
int vtkMimxModPointWidget::DetermineConstraintAxis(int constraint, double *x)
{
...
void vtkMimxModPointWidget::OnLeftButtonDown()
{
        ...
        {
                this->State = vtkMimxModPointWidget::Outside;
                return;
        }
        ...
        if ( path != NULL )
        {
                this->State = vtkMimxModPointWidget::Moving;
                this->Highlight(1);
...
void vtkMimxModPointWidget::OnLeftButtonUp()
{
        if ( this->State == vtkMimxModPointWidget::Outside ||
                this->State == vtkMimxModPointWidget::Start )
        {
                return;
        }
...
void vtkMimxModPointWidget::OnMiddleButtonDown()
{
        int X = this->Interactor->GetEventPosition()[0];
        int Y = this->Interactor->GetEventPosition()[1];
        ...
        if ( path != NULL )
        {
                this->State = vtkMimxModPointWidget::Translating;
                this->Highlight(1);
                this->ConstraintAxis = this->DetermineConstraintAxis(-1,NULL);
        }
        else
...
void vtkMimxModPointWidget::OnMiddleButtonUp()
{
        if ( this->State == vtkMimxModPointWidget::Outside ||
                this->State == vtkMimxModPointWidget::Start )
        {
...
void vtkMimxModPointWidget::OnRightButtonDown()
{
        int X = this->Interactor->GetEventPosition()[0];
        int Y = this->Interactor->GetEventPosition()[1];
        ...
        if ( path != NULL )
        {
                this->State = vtkMimxModPointWidget::Scaling;
...
void vtkMimxModPointWidget::OnRightButtonUp()
{
        if ( this->State == vtkMimxModPointWidget::Outside ||
                this->State == vtkMimxModPointWidget::Start )
        {
...
void vtkMimxModPointWidget::OnMouseMove()
{
        ...
        // Process the motion
        if ( this->State == vtkMimxModPointWidget::Moving )
        {
        ...
        else if ( this->State == vtkMimxModPointWidget::Scaling )
        {
                this->Scale(prevPickPoint, pickPoint, X, Y);
        }

        else if ( this->State == vtkMimxModPointWidget::Translating )
        {
                if ( !this->WaitingForMotion || this->WaitCount++ > 3 )
                {
                        this->ConstraintAxis = 
...
void vtkMimxModPointWidget::MoveFocus(double *p1, double *p2)
{
...
void vtkMimxModPointWidget::Translate(double *p1, double *p2)
{
        //Get the motion vector
        double v[3];
        v[0] = p2[0] - p1[0];
...
void vtkMimxModPointWidget::Scale(double *p1, double *p2, int vtkNotUsed(X), int Y)
{
        //Get the motion vector
        double v[3];
        v[0] = p2[0] - p1[0];
...
void vtkMimxModPointWidget::CreateDefaultProperties()
{
        ...
        this->SelectedProperty->SetAmbientColor(1.0,0.0,0.0);
        this->SelectedProperty->SetLineWidth(2.0);
}

void vtkMimxModPointWidget::PlaceWidget(double bds[6])
{
        int i;
...
void vtkMimxModPointWidget::GetPolyData(vtkPolyData *pd)
{
        this->Cursor3D->Update(); 
