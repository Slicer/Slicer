...
vtkCxxRevisionMacro(vtkMimxExtractFaceWidget, "$Revision: 1.3 $");
...
vtkMimxExtractFaceWidget::vtkMimxExtractFaceWidget()
{
  ...
  this->EventCallbackCommand->SetCallback(vtkMimxExtractFaceWidget::ProcessEvents);
...
void vtkMimxExtractFaceWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                 unsigned long event,
                                 void* clientdata, 
                                 void* vtkNotUsed(calldata))
{
  vtkMimxExtractFaceWidget* self = reinterpret_cast<vtkMimxExtractFaceWidget *>( clientdata );
...
void vtkMimxExtractFaceWidget::OnLeftButtonDown()
{
  ...
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
    {
    this->State = vtkMimxExtractFaceWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
  {
    this->State = vtkMimxExtractFaceWidget::Moving;
    vtkIdType PickedCell = this->FacePicker->GetCellId();
    if(PickedCell != -1)
    {
 /*     if(this->FacePolyData->GetCellData()->GetScalars())
      {
        this->FacePolyData->GetCellData()->GetScalars()->Delete();
      }*/
      vtkIntArray *intarray = vtkIntArray::New();
      intarray->SetNumberOfValues(this->FacePolyData->GetNumberOfCells());
      this->FacePolyData->GetCellData()->SetScalars(intarray);
      for(int i=0; i < this->FacePolyData->GetNumberOfCells(); i++)  intarray->SetValue(i,0.0);
       intarray->SetValue(PickedCell,1.0);
      intarray->Delete();
    vtkIdType t;
    this->FacePolyData->GetCellPoints(PickedCell,t,this->FacePoints);
    }
  }
  else
  {
...
void vtkMimxExtractFaceWidget::OnLeftButtonUp()
{
  if ( this->State == vtkMimxExtractFaceWidget::Outside ||
       this->State == vtkMimxExtractFaceWidget::Start )
    {
    return;
    }
...
void vtkMimxExtractFaceWidget::OnMiddleButtonDown()
{
  //int X = this->Interactor->GetEventPosition()[0];
  //int Y = this->Interactor->GetEventPosition()[1];
...
void vtkMimxExtractFaceWidget::OnMiddleButtonUp()
{
  /*if ( this->State == vtkMimxExtractFaceWidget::Outside ||
       this->State == vtkMimxExtractFaceWidget::Start )
    {
...
void vtkMimxExtractFaceWidget::OnRightButtonDown()
{
//  int X = this->Interactor->GetEventPosition()[0];
//  int Y = this->Interactor->GetEventPosition()[1];
//
...
//  if ( path != NULL )
//    {
//    this->State = vtkMimxExtractFaceWidget::Scaling;
//    this->HighlightOutline(1);
//    this->HandlePicker->GetPickPosition(this->LastPickPosition);
//    this->ValidPick = 1;
//    }
...
//}
//
//void vtkMimxExtractFaceWidget::OnRightButtonUp()
//{
//  if ( this->State == vtkMimxExtractFaceWidget::Outside )
//    {
//    return;
...
void vtkMimxExtractFaceWidget::OnMouseMove()
{
  ...
  //// Process the motion
  //if ( this->State == vtkMimxExtractFaceWidget::Moving )
  //  {
  //  // Okay to process
  ...
  //    }
  //  }
  //else if ( this->ScalingEnabled && this->State == vtkMimxExtractFaceWidget::Scaling )
  //  {
...
void vtkMimxExtractFaceWidget::PlaceWidget(double bounds[6])
{
