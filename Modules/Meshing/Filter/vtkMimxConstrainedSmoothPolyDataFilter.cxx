...
Module:    $RCSfile: vtkMimxConstrainedSmoothPolyDataFilter.cxx,v $
Language:  C++
...
vtkCxxRevisionMacro(vtkMimxConstrainedSmoothPolyDataFilter, "$Revision: 1.4 $");
vtkStandardNewMacro(vtkMimxConstrainedSmoothPolyDataFilter);
...
// The following code defines methods for the vtkConstrainedSmoothPolyDataFilter class
//
...
vtkMimxConstrainedSmoothPolyDataFilter::vtkMimxConstrainedSmoothPolyDataFilter()
{
...
void vtkMimxConstrainedSmoothPolyDataFilter::SetSource(vtkPolyData *source)
{
  this->SetInput(1, source);
}

vtkPolyData *vtkMimxConstrainedSmoothPolyDataFilter::GetSource()
{
...
int vtkMimxConstrainedSmoothPolyDataFilter::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if ( source )
    ...
    cellLocator->BuildLocator();
        ...
        obbtree = vtkOBBTree::New();
        //obbtree->SetDataSet(source);
        //obbtree->SetNumberOfCellsPerBucket(99999);
        //obbtree->SetMaxLevel(99999);
        //obbtree->SetTolerance(1e-7);
    ...
    for (i=0; i<numPts; i++) 
      {
                  ...
                  {
                        input->GetPointData()->GetScalars()->GetTuple(i,status);
                  }
                  else{ status[0] = 1;
                  }
                        if(status[0] != 0)
        ...
        if ( source ) 
          ...
          sPtr->subId, sPtr->p, dist2, w) == 0)
            ...
            cellLocator->FindClosestPoint(xNew, closestPt, sPtr->cellId, 
                                          sPtr->subId, dist2);
                        //////////////////////////////////////////////////////////
                        // all the calculations pertaining to OBBtree are done here
                        // get all the cells connecting the point 'i'
//                      vtkIdList *cellidlist = vtkIdList::New();
//                      
//                      input->GetPointCells(i, cellidlist);
//                      double normal[3], normalstore[3];
//                      for(k=0; k < 3; k++)    normalstore[k] = 0.0;
//                      for(k=0; k < cellidlist->GetNumberOfIds(); i++)
//                      {
//                                                              
////                            vtkMeshQuality::QuadMaxAngle(input->Get) to be implemented later
//                      }
//                      cellidlist->Delete();
            }
          for (k=0; k<3; k++)
...
int vtkMimxConstrainedSmoothPolyDataFilter::FillInputPortInformation(int port,
                                                      vtkInformation *info)
{
...
void vtkMimxConstrainedSmoothPolyDataFilter::PrintSelf(ostream& os, vtkIndent indent)
{
