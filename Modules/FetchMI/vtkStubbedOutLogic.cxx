...
//----------------------------------------------------------------------------
void vtkFetchMILogic::QueryServerWithTerms ( )
{
}
      ...
      if ( !(strcmp(attName, "Label")) ||
        ...
        if ( !(strcmp ("XND", svctype )) )
          ...
          // no-op
                ...
                !(strcmp(attName, "uri")) )
        {
        //--- we are filling up the resource table here.
        // only need to add the resource
        value = element->GetAttributeValue ( i );
        this->ResourceDescription->AddOrUpdateTag ( "uri", value );
        }
      }
    }
}
  ...
  if ( this->GetFetchMINode() != NULL )
    ...
    if ( !(strcmp ("HID", svctype )) )
      ...
      if ( this->ResourceDescription != NULL )
        {
          {
          //--- if parsing fails for some reason, empty out the response table.
          }
...
//--- Nicole: check for appropriateness and code up? requires touching vtkmrmlscene
//----------------------------------------------------------------------------
...
//--- Nicole: check for appropriateness and code up?
//----------------------------------------------------------------------------
