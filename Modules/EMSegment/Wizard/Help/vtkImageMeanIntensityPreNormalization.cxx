...
//------------------------------------------------------------------------------
                                                                   vtkImageMeanIntensityPreNormalization* vtkImageMeanIntensityPreNormalization::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkImageMeanIntensityPreNormalization");
  if(ret) return (vtkImageMeanIntensityPreNormalization*)ret;
  // If the factory was unable to create the object, then create it here.
  return new vtkImageMeanIntensityPreNormalization;
}

                                                                 //----------------------------------------------------------------------------
                                                                   vtkImageMeanIntensityPreNormalization::vtkImageMeanIntensityPreNormalization()
{
  this->NormValue = -1; 
...
//----------------------------------------------------------------------------
void vtkImageMeanIntensityPreNormalization::ComputeInputUpdateExtent(int inExt[6], int outExt[6])
{
  this->GetInput()->GetWholeExtent(inExt);
}

//----------------------------------------------------------------------------
void vtkImageMeanIntensityPreNormalization::PrintSelf(ostream& os, vtkIndent indent)
{
}

int* vtkImageMeanIntensityPreNormalization::InitializeHistogram(vtkImageAccumulate *HIST, vtkImageData *Input, int &HistMin, int &HistMax)
{
  // 1. Detect extrema 
  HIST->SetInput(Input);
...
int vtkImageMeanIntensityPreNormalization::DetermineFilterMax(int *HIST_Ptr, const int HIST_Length, const int FilterNumMaxVoxels)
{
  int VoxelCount =0;
...
void vtkImageMeanIntensityPreNormalization::SmoothHistogram(const int* Input , const int InputLength, const int SmoothWidth, int &MaxIndex, int* Output)
{
...
int vtkImageMeanIntensityPreNormalization::DetermineFilterMin(const int* HIST_PTR, const int HIST_Length)
{
...
int vtkImageMeanIntensityPreNormalization::DetermineFirstValey(const int *aSmoothHistogram,const int aSmoothHistogramLength)
{
...
void vtkImageMeanIntensityPreNormalization::MeanMRI(vtkImageData *Input, vtkImageData *Output)
{
...
*/
  ...
  if (this->PrintInfo)
    {
    std::cerr << "vtkImageMeanIntensityPreNormalization::MeanMRI " << endl;
    ...
    std::cerr << "  Minimum: " << this->InitialHistogramSmoothingWidth << endl;
    }
  ...
  FilterHistogramMin = this->InitialHistogramSmoothingWidth;//
  //this->DetermineFilterMin(HIST_PTR,FilterHistogramMax+1);
 ...
 //ImageIntensityCorrectionRatio = NormValue/ImageIntensityMean;
  ...
  //std::cout<<"ImageIntensityCorrectionRatio: "<< ImageIntensityCorrectionRatio << std::endl;
 ...
 /*
  ...
  if (this->PrintInfo)
    ...
    std::cerr << "  Min Intensity: " << FilterHistogramMin << endl;
    std::cerr << "  Max Intensity: " << FilterHistogramMax << endl;
...
//----------------------------------------------------------------------------
void vtkImageMeanIntensityPreNormalization::ExecuteInformation(vtkImageData *inData, vtkImageData *outData) 
{
...
void vtkImageMeanIntensityPreNormalization::ExecuteData(vtkDataObject *)
{
  int inExt[6];
  int outExt[6];
  // Necessary  for VTK
