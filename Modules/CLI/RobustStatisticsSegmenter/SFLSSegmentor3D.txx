    ...
    for (long iy = 0; iy < m_ny; ++iy)
    {
      for (long ix = 0; ix < m_nx; ++ix)
      {
        ...
        else
        {
          ...
          if ((iy + 1 < m_ny && mp_mask->GetPixel(idx4) == 0) || (iy - 1 >= 0 && mp_mask->GetPixel(idx3) == 0)
              || (ix + 1 < m_nx && mp_mask->GetPixel(idx2) == 0) || (ix - 1 >= 0 && mp_mask->GetPixel(idx1) == 0)
              || (iz + 1 < m_nz && mp_mask->GetPixel(idx6) == 0) || (iz - 1 >= 0 && mp_mask->GetPixel(idx5) == 0))
          {
...
double CSFLSSegmentor3D<TPixel>::computeKappa(long ix, long iy, long iz)
{
  ...
  return (dxx * (dy2 + dz2) + dyy * (dx2 + dz2) + dzz * (dx2 + dy2) - 2 * dx * dy * dxy - 2 * dx * dz * dxz
          - 2 * dy * dz * dyz)
