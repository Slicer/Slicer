...
typename itk::DiffusionTensor3DInterpolateImageFunction<PixelType>::Pointer InterpolationTypeFct(
  std::string interpolationType,
  std::string windowFunction,
  unsigned int splineOrder,
  int numberOfThreads)
{
  ...
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType,
                                                                     RADIUS,
  ...
  {
    typename BSplineInterpolateImageFunctionType::Pointer bSplineInterpolator =
      BSplineInterpolateImageFunctionType::New();
...
template <class PixelType>
void SetListFromTransform(const typename itk::MatrixOffsetTransformBase<PixelType, 3, 3>::Pointer& transform,
                          parameters& list)
{
  for (int i = 0; i < 3; i++)
  {
...
template <class PixelType>
itk::Matrix<double, 4, 4> ComputeTransformMatrix(
...
template <class PixelType>
typename itk::DiffusionTensor3DAffineTransform<PixelType>::Pointer FSOrPPD(const std::string& ppd,
                                                                           itk::Matrix<double, 4, 4>* matrix = nullptr)
{
...
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetUpTransform(
  const parameters& list,
  const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
...
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetTransformAndOrder(
  parameters& list,
  const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
  ...
  if (list.transformationFile.compare(""))
  {
    ...
    if (transformClassName.find("AffineTransform") != std::string::npos) // if affine transform
    {
      matrixOffsetTransform =
        static_cast<typename FSAffineTransformType::Superclass::AffineTransformType*>(transform.GetPointer());
      list.transformType.assign("a");
      SetListFromTransform<double>(matrixOffsetTransform, list);
    }
    else
    {
      if (transformClassName == "Rigid3DTransform" || transformClassName == "Euler3DTransform"
          || transformClassName == "CenteredEuler3DTransform" || transformClassName == "QuaternionRigidTransform"
          || transformClassName == "VersorTransform" || transformClassName == "ScaleSkewVersor3DTransform"
...
// Set the transformation
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetTransform(
  parameters& list,
  const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
...
template <class PixelType>
itk::Matrix<double, 3, 3> SetOutputParameters(
  const parameters& list,
                              ...
                              const itk::Matrix<double, 3, 3>& direction)
{
  ...
  if (fieldSize == size && fieldSpacing == spacing && fieldDirection == direction && fieldOrigin == origin)
  {
    return;
  }
...
int Do(parameters list)
{
  ...
  interpol =
    InterpolationTypeFct<PixelType>(list.interpolationType, list.windowFunction, list.splineOrder, list.numberOfThread);
  ...
  TransformTypePointer transform;
  if ((list.transformationFile.compare("") && transformFile->GetTransformList()->size() > 1 && nonRigidTransforms > 0)
      || list.deffield.compare(""))
  {
    ...
    while (list.transformationFile.compare("") && transformFile->GetTransformList()->size())
    {
      ...
      if (!list.notbulk
          && transform->GetTransform()->GetTransformTypeAsString() == "BSplineDeformableTransform_double_3_3"
...
int main(int argc, char* argv[])
{
  ...
  if (list.outputImageSpacing.size() != 3 || list.outputImageSize.size() != 3
      || (list.outputImageOrigin.size() != 3 && list.outputImageOrigin.size() != 0) || list.directionMatrix.size() != 9
      || rotationPoint.size() != 3 || list.transformMatrix.size() != 12)
  {
