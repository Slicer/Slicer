...
typename itk::DiffusionTensor3DInterpolateImageFunction<PixelType>::Pointer InterpolationTypeFct(std::string interpolationType,
                                                                                                 std::string windowFunction,
                                                                                                 unsigned int splineOrder,
                                                                                                 int numberOfThreads)
{
  typedef itk::DiffusionTensor3DInterpolateImageFunction<PixelType> InterpolatorType;
  typedef itk::DiffusionTensor3DNearestNeighborInterpolateFunction<PixelType> NearestNeighborhoodInterpolatorType;
  typedef itk::DiffusionTensor3DLinearInterpolateFunction<PixelType> LinearInterpolatorType;
  typedef itk::ConstantBoundaryCondition<itk::Image<PixelType, 3>> BoundaryConditionType;
  typedef itk::Function::HammingWindowFunction<RADIUS> HammingwindowFunctionType;
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType, RADIUS, HammingwindowFunctionType, BoundaryConditionType>
    HammingWindowedSincInterpolateFunctionType;
  typedef itk::Function::CosineWindowFunction<RADIUS> CosinewindowFunctionType;
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType, RADIUS, CosinewindowFunctionType, BoundaryConditionType> CosineWindowedSincInterpolateFunctionType;
  typedef itk::Function::WelchWindowFunction<RADIUS> WelchwindowFunctionType;
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType, RADIUS, WelchwindowFunctionType, BoundaryConditionType> WelchWindowedSincInterpolateFunctionType;
  typedef itk::Function::LanczosWindowFunction<RADIUS> LanczoswindowFunctionType;
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType, RADIUS, LanczoswindowFunctionType, BoundaryConditionType>
    LanczosWindowedSincInterpolateFunctionType;
  typedef itk::Function::BlackmanWindowFunction<RADIUS> BlackmanwindowFunctionType;
  typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction<PixelType, RADIUS, BlackmanwindowFunctionType, BoundaryConditionType>
    BlackmanWindowedSincInterpolateFunctionType;
  typedef itk::DiffusionTensor3DBSplineInterpolateImageFunction<PixelType> BSplineInterpolateImageFunctionType;
  ...
  {
    typedef itk::DiffusionTensor3DInterpolateImageFunctionReimplementation<PixelType> DiffusionTensor3DInterpolateImageFunctionReimplementation;
...
template <class PixelType>
void SetListFromTransform(const typename itk::MatrixOffsetTransformBase<PixelType, 3, 3>::Pointer& transform, parameters& list)
{
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
  ...
  for (int i = 0; i < 3; i++)
  {
    list.transformMatrix.push_back(transform->GetTranslation()[i]);
    list.rotationPoint.push_back(transform->GetCenter()[i]);
  }
}

template <class PixelType>
itk::Point<double> ImageCenter(const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image)
{
...
template <class PixelType>
itk::Matrix<itk::SpacePrecisionType, 4, 4> ComputeTransformMatrix(const parameters& list,
...
template <class PixelType>
typename itk::DiffusionTensor3DAffineTransform<PixelType>::Pointer FSOrPPD(const std::string& ppd, itk::Matrix<itk::SpacePrecisionType, 4, 4>* matrix = nullptr)
{
  typedef itk::DiffusionTensor3DFSAffineTransform<PixelType> FSAffineTransformType;
...
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetUpTransform(const parameters& list,
                                                                            const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
                                                                            const typename itk::DiffusionTensor3DNonRigidTransform<PixelType>::TransformType::Pointer& nonRigidFile,
...
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetTransformAndOrder(parameters& list,
                                                                                  const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
                                                                                  typename itk::DiffusionTensor3DNonRigidTransform<PixelType>::TransformType::Pointer& transform,
                                                                                  const itk::Point<double>& outputImageCenter)
{
  typedef itk::DiffusionTensor3DNonRigidTransform<PixelType> NonRigidTransformType;
  typedef itk::DiffusionTensor3DRigidTransform<PixelType> RigidTransformType;
  ...
  if (list.transformationFile.compare(""))
  {
    ...
    {
      matrixOffsetTransform = static_cast<typename FSAffineTransformType::Superclass::AffineTransformType*>(transform.GetPointer());
    ...
    else
    {
          ...
          transformClassName == "Similarity3DTransform")        // if rigid3D transform
      {
        ...
        matrixOffsetTransform = static_cast<typename RigidTransformType::Rigid3DTransformType*>(transform.GetPointer());
        SetListFromTransform<itk::SpacePrecisionType>(matrixOffsetTransform, list);
      }
      else // if non-rigid
      {
        ...
        { // if non rigid Transform loaded
          ...
          nonRigidFile = static_cast<typename NonRigidTransformType::TransformType*>(transform.GetPointer());
        }
        else // something else
        {
          std::cerr << "Transformation type not yet implemented for tensors" << std::endl;
          return nullptr;
        }
      }
    }
    if (list.transformType.compare("nr")) // if rigid or affine transform
    {
      ...
      {
        std::cerr << "Error in the file containing the transformation" << std::endl;
        return nullptr;
      }
    }
  }
...
// Set the transformation
template <class PixelType>
typename itk::DiffusionTensor3DTransform<PixelType>::Pointer SetTransform(parameters& list,
                                                                          const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image,
                                                                          ...
                                                                          const itk::Point<double>& outputImageCenter)
{
  ...
  if (list.transformationFile.compare(""))
  {
    ...
    {
      transform = static_cast<TransformType*>(transformFile->GetTransformList()->back().GetPointer());
    }
    else
    {
      transform = static_cast<TransformType*>(transformFile->GetTransformList()->front().GetPointer());
    }
  }
...
template <class PixelType>
itk::Matrix<itk::SpacePrecisionType, 3, 3> SetOutputParameters(const parameters& list,
                                                               typename itk::DiffusionTensor3DResample<PixelType, PixelType>::Pointer& resampler,
                                                               const typename itk::Image<itk::DiffusionTensor3D<PixelType>, 3>::Pointer& image)
{
  typedef itk::Image<unsigned char, 3> ImageType;
                              ...
                              const itk::Matrix<itk::SpacePrecisionType, 3, 3>& direction)
{
  ...
  typedef itk::ResampleImageFilter<DeformationImageType, DeformationImageType, itk::SpacePrecisionType> ResampleImageFilter;
  ...
  // resampleFieldFilter->SetInterpolator(linearVectorInterpolator); // linear by default
...
{
  typedef itk::Image<itk::DiffusionTensor3D<PixelType>, 3> InputImageType;
  ...
  {
    std::cerr << "Reading input image: Exception caught!" << std::endl;
  ...
  interpol = InterpolationTypeFct<PixelType>(list.interpolationType, list.windowFunction, list.splineOrder, list.numberOfThread);
      ...
      || list.deffield.compare(""))
  {
    ...
    if (list.deffield.compare(""))
    {
      ...
      ResampleDeformationField(
        field, dummyOutputImage->GetOrigin(), dummyOutputImage->GetSpacing(), dummyOutputImage->GetLargestPossibleRegion().GetSize(), dummyOutputImage->GetDirection());
    }
    ...
    while (list.transformationFile.compare("") && transformFile->GetTransformList()->size())
    {
      ...
      if (!list.notbulk && transform->GetTransform()->GetTransformTypeAsString() == transformClassName // Check if transform file contains a BSpline
  ...
  { // local for memory management: the input image should not stay in
    ...
    itk::Matrix<itk::SpacePrecisionType, 3, 3> outputImageDirection;
    outputImageDirection = SetOutputParameters<PixelType>(list, resampler, image);
    if (!hasMeasurementFrame || list.noMeasurementFrame)
    {
      itk::Matrix<itk::SpacePrecisionType, 3, 3> transposeOutputDirection(outputImageDirection.GetTranspose());
  ...
  {
    std::cerr << "Writing output image: Exception caught!" << std::endl;
    std::cerr << Except << std::endl;
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
...
int main(int argc, char* argv[])
{
  ...
  {
    case itk::IOComponentEnum::FLOAT: return Do<float>(list); break;
    case itk::IOComponentEnum::DOUBLE: return Do<double>(list); break;
