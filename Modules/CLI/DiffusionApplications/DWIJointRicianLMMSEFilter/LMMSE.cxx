...
int main( int ac, char* av[] )
{
  if( ac < 3 )
    {
    std::cerr << "Usage: " << av[0] << " Input Output [radiusLMMSEfilt=2 radiusZ=1 noise=20.0f neighbours=1 ]\n";
    return EXIT_FAILURE;
    }

  typedef unsigned short                                    PixelType;
  typedef itk::VectorImage<PixelType>                       ImageType;
  ...
  if( ac > 6 )
    {
    neighbours = ::atoi( av[6] );
    }

  filter->SetNeighbours( neighbours );
  filter->SetSigma( noise );
