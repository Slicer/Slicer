...
itk::Point<double> ImageCenter(const typename ImageType::Pointer& image)
{
...
template <class ImageType>
typename itk::Transform<double, 3, 3>::Pointer SetTransformAndOrder(
  parameters& list,
  const typename ImageType::Pointer& image,
  typename itk::Transform<double, 3, 3>::Pointer transform,
  const itk::Point<double>& outputImageCenter)
  ...
  if (list.transformationFile.compare("")) // Get transformation matrix from command line if no file given
  {
    ...
    else
    {
      ...
      else // if non-rigid
      {
        ...
        {
          std::cerr << "Transformation type not yet implemented" << std::endl;
          return nullptr;
        }
      }
    }
    if (list.transformType.compare("nr")) // if rigid or affine transform
    {
      ...
      {
        std::cerr << "Error in the file containing the matrix transformation" << std::endl;
        return nullptr;
      }
    }
  }
...
template <class ImageType>
itk::Transform<double, 3, 3>::Pointer SetAllTransform(
  parameters& list,
  ...
  else if (list.transformationFile.compare("") && transformFile->GetTransformList()->size() > 1)
  {
    ...
    do
    {
      ...
      if (!(transformClassName.find("AffineTransform") != std::string::npos
            || transformClassName == "MatrixOffsetTransformBase" || transformClassName == "Rigid3DTransform"
            || transformClassName == "Euler3DTransform" || transformClassName == "CenteredEuler3DTransform"
            || transformClassName == "QuaternionRigidTransform" || transformClassName == "VersorTransform"
            || transformClassName == "VersorRigid3DTransform" || transformClassName == "ScaleSkewVersor3DTransform"
            || transformClassName == "ScaleVersor3DTransform" || transformClassName == "Similarity3DTransform"
...
template <class PixelType>
int AddImage(typename itk::VectorImage<PixelType, 3>::Pointer& imagePile,
...
template <class PixelType>
int SeparateImages(const typename itk::VectorImage<PixelType, 3>::Pointer& imagePile,
...
Transform3DPointer InverseTransform(const Transform3DPointer& transform)
{
  ...
  try
  {
    ...
    {
      if (transformClassName == "MatrixOffsetTransformBase" || transformClassName == "Rigid3DTransform"
          || transformClassName == "Euler3DTransform" || transformClassName == "CenteredEuler3DTransform"
          || transformClassName == "QuaternionRigidTransform" || transformClassName == "VersorTransform"
          || transformClassName == "VersorRigid3DTransform" || transformClassName == "ScaleSkewVersor3DTransform"
          || transformClassName == "ScaleVersor3DTransform" || transformClassName == "Similarity3DTransform"
...
itk::Matrix<double, 3, 3> ReadMeasurementFrame(itk::MetaDataDictionary& dico,
    ...
    if (entryvalue)
    {
      ...
      if (pos != -1)
      {
        ...
        if (inverseTransform)
        {
          ...
          oss << transformedVector[0] << "  " << transformedVector[1] << "  " << transformedVector[2];
...
typename itk::InterpolateImageFunction<ImageType, double>::Pointer SetInterpolator(const parameters& list)
{
  ...
  else if (!list.interpolationType.compare("ws"))
  {
    if (!list.windowFunction.compare("h"))
    {
      ...
      typedef itk::WindowedSincInterpolateImageFunction<ImageType, RADIUS, windowFunction, BoundaryCondition, double>
