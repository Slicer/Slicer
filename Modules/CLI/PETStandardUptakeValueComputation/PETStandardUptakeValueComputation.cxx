...
Standardized uptake value, SUV, (also referred to as the dose uptake ratio, DUR) is a widely used, simple PET
quantifier, calculated as a ratio of tissue radioactivity concentration (e.g. in units kBq/ml) at time T, CPET(T) and
injected dose (e.g. in units MBq) at the time of injection divided by body weight (e.g. in units kg).

SUVbw = CPET(T) / (Injected dose / Patient's weight)

Instead of body weight, the injected dose may also be corrected by the lean body mass, or body surface area (BSA) (Kim
et al., 1994). Verbraecken et al. (2006) review the different formulas for calculating the BSA.
...
Most PET systems record their pixels in units of activity concentration (MBq/ml) (once Rescale Slope has been applied,
and the units are specified in the Units attribute).

To compute SUVbw, for example, it is necessary to apply the decay formula and account for the patient's weight. For that
to be possible, during de-identification, the Patient's Weight must not have been removed, and even though dates may
have been removed, it is important not to remove the times, since the difference between the time of injection and the
acquisition time is what is important.

In particular, DO NOT REMOVE THE FOLLOWING DICOM TAGS:
Radiopharmaceutical Start Time (0018,1072) Decay Correction (0054,1102) Decay Factor (0054,1321) Frame Reference Time
(0054,1300) Radionuclide Half Life (0018,1075) Series Time (0008,0031) Patient's Weight (0010,1030)
...
If there is a strong need to remove times from an identity leakage perspective, then one can normalize all times to some
epoch, but it has to be done consistently across all images in the entire study (preferably including the CT reference
images); note though, that the time of injection may be EARLIER than the Study Time, which you might assume would be the
earliest, so it takes a lot of effort to get this right.

For Philips images, none of this applies, and the images are in COUNTS and the private tag (7053,1000) SUV Factor must
be used. To calculate the SUV of a particular pixel, you just have to calculate [pixel _value * tag 7053|1000 ]
  ...
  */
  ...
  if (fileReader.GetElementSQ(0x0054, 0x0016, seq, false) == EXIT_SUCCESS)
  {
    ...
    if (fileReader.GetElementCS(0x0054, 0x1001, tag, false) == EXIT_SUCCESS)
    {
      ...
      else if ((units.find("MBq") != std::string::npos) || (units.find("MBQ") != std::string::npos))
      {
        list.radioactivityUnits = "MBq";
        list.tissueRadioactivityUnits = "MBq";
      }
      else if ((units.find("kBq") != std::string::npos) || (units.find("kBQ") != std::string::npos)
      ...
      else if ((units.find("uBq") != std::string::npos) || (units.find("uBQ") != std::string::npos))
      {
        list.radioactivityUnits = "uBq";
        list.tissueRadioactivityUnits = "uBq";
      }
      else if ((units.find("Bq") != std::string::npos) || (units.find("BQ") != std::string::npos))
      {
        list.radioactivityUnits = "Bq";
        list.tissueRadioactivityUnits = "Bq";
      }
      else if ((units.find("MCi") != std::string::npos) || (units.find("MCI") != std::string::npos))
      {
        list.radioactivityUnits = "MCi";
        list.tissueRadioactivityUnits = "MCi";
      }
      else if ((units.find("kCi") != std::string::npos) || (units.find("kCI") != std::string::npos)
      ...
      else if ((units.find("uCi") != std::string::npos) || (units.find("uCI") != std::string::npos))
      {
        list.radioactivityUnits = "uCi";
        list.tissueRadioactivityUnits = "uCi";
      }
      else if ((units.find("Ci") != std::string::npos) || (units.find("CI") != std::string::npos))
      {
    ...
    //---
    //--- CalibrationFactor
    if (fileReader.GetElementDS(0x7053, 0x1009, 1, &list.calibrationFactor, false) != EXIT_SUCCESS)
    {
  ...
  if ((parsingDICOM) && (list.injectedDose != 0.0) && (list.patientWeight != 0.0)
  ...
  else
  {
    ...
    std::cerr << "\tseriesReferenceTime " << list.seriesReferenceTime << " should not be MODULE_INIT_NO_VALUE"
              << std::endl;
    std::cerr << "\tinjectionTime " << list.injectionTime << " should not be MODULE_INIT_NO_VALUE" << std::endl;
    std::cerr << "\tradionuclideHalfLife " << list.radionuclideHalfLife << " should not be MODULE_INIT_NO_VALUE"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (int i = lo; i <= hi; i++)
  {
    ...
    if (voxNumber > 0)
    {
      ...
      //      outputStringStream << list.patientName << ", " << list.studyDate << ", " << list.injectedDose  << ", "  <<
      //      i << ", " << suvmax << ", " << suvmean << ", " << labelName.c_str() << ", " << ", " << ", " << ", " <<
      //      std::endl;
      ...
      if (outputFile.compare("") != 0)
      {
        ...
        if (!ofile.is_open())
        {
          ...
          std::cerr << "ERROR: cannot open nuclear medicine output csv parameter file '" << outputFile.c_str()
                    << "', see return strings for values" << std::endl;
        }
        else
        {
          ss.str("");
          ...
          ss << list.patientName << ", " << list.studyDate << ", " << list.injectedDose << ", " << i << ", " << suvmin
             << ", " << suvmax << ", " << suvmean << ", " << labelName.c_str() << std::endl;
          ofile << ss.str();
          ofile.close();
  ...
  if (outputStringFile.compare("") != 0)
  {
    ...
    if (!stringFile.is_open())
    {
      ...
      std::cerr << "ERROR: cannot open nuclear medicine output string parameter file '" << outputStringFile.c_str()
                << "', output string was:\n"
    ...
    stringFile.close();
    std::cout << "Wrote return string to " << outputStringFile.c_str() << ": " << std::endl
              << stringOutput.c_str() << std::endl;
  }
  else
  {
    ...
    if (outputFile.compare("") == 0)
    {
      std::cerr << "Neither the nuclear medicine output csv parameter file nor the string parameter file were "
                   "specified, please specify one or the other to obtain the calculated output."
                << std::endl;
      return EXIT_FAILURE;
    }
  }
