...
// onto the stack of C
    ...
    while( Cpt != nextPt )
      ...
      for( ::size_t k = 0; k < neigh_pts.size(); k++ )
        ...
        double dist = pow(thatpt[0] - thispt[0], 2.0) + pow(thatpt[1] - thispt[1], 2.0) + pow(thatpt[2] - thispt[2],
                                                                                              2.0);
...
void ComputeNormals( MeshData* meshdata )
{
  ...
  if( bTextInputNormals )
    ...
    if( !bTextInputNormals )
      ...
      {
      std::cerr << "Normals not computed for mesh data, numverts = " << numverts
                << ", normal array sizes were allocated to be x = " << meshdata->nx.size() << ", y = "
                << meshdata->ny.size()
                << ", z = " << meshdata->nz.size() << "\n";
      }
    }
...
//    int vert2 = pts[2];
    for( int k = 0; k < 3; k++ )
      ...
      for( int kk = 0; kk < 3; kk++ )
        {
        if( 0 ==
            std::count( meshdata->adjimm[pts[kk]].myNeighbs.begin(), meshdata->adjimm[pts[kk]].myNeighbs.end(), pts[k] ) )
          {
