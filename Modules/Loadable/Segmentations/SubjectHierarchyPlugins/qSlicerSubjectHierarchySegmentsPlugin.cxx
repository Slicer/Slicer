...
qSlicerSubjectHierarchySegmentsPluginPrivate::qSlicerSubjectHierarchySegmentsPluginPrivate(
  qSlicerSubjectHierarchySegmentsPlugin& object)
  : q_ptr(&object)
...
void qSlicerSubjectHierarchySegmentsPluginPrivate::init()
{
  ...
  this->ShowOnlyCurrentSegmentAction =
    new QAction(qSlicerSubjectHierarchySegmentsPlugin::tr("Show only this segment"), q);
...
//----------------------------------------------------------------------------
double qSlicerSubjectHierarchySegmentsPlugin::canReparentItemInsideSubjectHierarchy(vtkIdType itemID,
...
bool qSlicerSubjectHierarchySegmentsPlugin::reparentItemInsideSubjectHierarchy(vtkIdType itemID, vtkIdType parentItemID)
{
  ...
  vtkMRMLSegmentationNode* fromSegmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(shNode->GetItemDataNode(shNode->GetItemParent(itemID)));
  vtkMRMLSegmentationNode* toSegmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(shNode->GetItemDataNode(parentItemID));
  ...
  if (!success)
  {
    ...
    if (fromSegmentationNode->GetSegmentation()->GetSourceRepresentationName()
        == toSegmentationNode->GetSegmentation()->GetSourceRepresentationName())
    {
    ...
    QMessageBox::StandardButton answer = QMessageBox::question(nullptr,
                                                               tr("Failed to move segment between segmentations"),
                                                               message,
...
QString qSlicerSubjectHierarchySegmentsPlugin::tooltip(vtkIdType itemID) const
{
  ...
  if (!segmentationNode)
  {
    qCritical() << Q_FUNC_INFO << ": Unable to find segmentation node for segment subject hierarchy item "
  ...
  if (!segment)
  {
    qCritical() << Q_FUNC_INFO << ": Unable to get segment for segment subject hierarchy item "
                << shNode->GetItemName(itemID).c_str();
    return tooltipString;
  }
  ...
  tooltipString.append(
    tr("Segment - ID: %1, Representations: %2, Color: (%3, %4, %5)\nTags: %6")
...
//-----------------------------------------------------------------------------
void qSlicerSubjectHierarchySegmentsPlugin::setDisplayColor(vtkIdType itemID,
                                                            QColor color,
...
//-----------------------------------------------------------------------------
QColor qSlicerSubjectHierarchySegmentsPlugin::getDisplayColor(vtkIdType itemID,
                                                              QMap<int, QVariant>& terminologyMetaData) const
{
  ...
  std::string tagValue;
  terminologyMetaData[qSlicerTerminologyItemDelegate::TerminologyRole] = QVariant(
    segment->GetTag(vtkSegment::GetTerminologyEntryTagName(), tagValue) ? QString(tagValue.c_str()) : QString());
...
void qSlicerSubjectHierarchySegmentsPlugin::showContextMenuActionsForItem(vtkIdType itemID)
{
  ...
  qSlicerSubjectHierarchySegmentationsPlugin* segmentationsPlugin =
    qobject_cast<qSlicerSubjectHierarchySegmentationsPlugin*>(
...
void qSlicerSubjectHierarchySegmentsPlugin::showOnlyCurrentSegment()
{
  ...
  {
    qCritical() << Q_FUNC_INFO << ": Failed to find segmentation subject hierarchy item for segment item "
                << shNode->GetItemName(currentItemID).c_str();
    return;
  }
  vtkMRMLSegmentationNode* segmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(shNode->GetItemDataNode(segmentationShItemID));
  if (!segmentationNode)
  {
    qCritical() << Q_FUNC_INFO << ": Unable to find segmentation node for segment subject hierarchy item "
                << shNode->GetItemName(currentItemID).c_str();
  ...
  for (segmentIt = segmentShItemIDs.begin(); segmentIt != segmentShItemIDs.end(); ++segmentIt)
  {
    ...
    std::string segmentId =
      shNode->GetItemAttribute(segmentItemID, vtkMRMLSegmentationNode::GetSegmentIDAttributeName());
    // Set visibility
...
//---------------------------------------------------------------------------
bool qSlicerSubjectHierarchySegmentsPlugin::showItemInView(vtkIdType itemID,
...
void qSlicerSubjectHierarchySegmentsPlugin::setOpacityForCurrentItem(double opacity)
{
  ...
  vtkMRMLSegmentationNode* segmentationNode =
    vtkSlicerSegmentationsModuleLogic::GetSegmentationNodeForSegmentSubjectHierarchyItem(currentItemID,
                                                                                         shNode->GetScene());
  if (!segmentationNode)
  {
    qCritical() << Q_FUNC_INFO << ": Unable to find segmentation node for segment subject hierarchy item "
