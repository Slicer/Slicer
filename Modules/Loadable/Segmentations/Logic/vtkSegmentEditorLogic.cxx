...
std::string vtkSegmentEditorLogic::AddEmptySegment(const std::string& segmentId, int segmentStatus) const
{
  ...
  std::string addedSegmentID = segmentationNode->GetSegmentation()->AddEmptySegment(segmentId.c_str());
...
bool vtkSegmentEditorLogic::CanAddSegments() const
{
  ...
  return (GetSegmentation() != nullptr) && (GetSourceVolumeNode() != nullptr) && (!GetReferenceImageGeometryString().empty());
}

//-----------------------------------------------------------------------------
bool vtkSegmentEditorLogic::CanRedo() const
{
  return SegmentationHistory && SegmentationHistory->IsRestoreNextStateAvailable();
}
...
bool vtkSegmentEditorLogic::CanRemoveSegments() const
{
  ...
  return IsSegmentIdValid(GetCurrentSegmentID());
}
...
bool vtkSegmentEditorLogic::CanTriviallyConvertSourceRepresentationToBinaryLabelMap() const
{
  ...
  if (GetSegmentation()->GetNumberOfSegments() < 1)
  {
    return true;
  }

  if (GetSegmentation()->GetSourceRepresentationName() == vtkSegmentationConverter::GetSegmentationBinaryLabelmapRepresentationName())
  {
    return true;
  }
  return false;
...
{
  return SegmentationHistory && SegmentationHistory->IsRestorePreviousStateAvailable();
}
...
bool vtkSegmentEditorLogic::ContainsClosedSurfaceRepresentation() const
{
  ...
  return GetSegmentation()->ContainsRepresentation(vtkSegmentationConverter::GetSegmentationClosedSurfaceRepresentationName());
}

//-----------------------------------------------------------------------------
void vtkSegmentEditorLogic::CreateAndSetBlankSourceVolumeFromSegmentationGeometry() const
{
  if (!IsSegmentationNodeValid() || !MRMLScene)
    return;

  std::string referenceImageGeometry = GetReferenceImageGeometryStringFromSegmentation(GetSegmentation());
      ...
      && referenceExtent[4] <= referenceExtent[5])
  {
    ...
    vtkMRMLSegmentationNode* segmentationNode = GetSegmentationNode();
    std::string sourceVolumeNodeName = (segmentationNode->GetName() ? segmentationNode->GetName() : "Volume") + std::string(" source volume");
    vtkMRMLScalarVolumeNode* sourceVolumeNode = vtkMRMLScalarVolumeNode::SafeDownCast(MRMLScene->AddNewNodeByClass("vtkMRMLScalarVolumeNode", sourceVolumeNodeName.c_str()));
    ...
    SetSourceVolumeNode(sourceVolumeNode);
  }
}
...
void vtkSegmentEditorLogic::CreateAndSetBlankSourceVolumeIfNeeded() const
{
  ...
  if (GetSourceVolumeNode() != nullptr)
    return;
...
{
  vtkMRMLSegmentEditorNode* editorNode = GetSegmentEditorNode();
  if (!editorNode)
  {
    return {};
  }
...
{
  return DefaultTerminologyEntry;
}
...
std::string vtkSegmentEditorLogic::GetReferenceImageGeometryString() const
{
  ...
  std::string referenceImageGeometry = this->GetReferenceImageGeometryStringFromSegmentation(segmentation);
  if (referenceImageGeometry.empty())
  {
    ...
    referenceImageGeometry = this->GetReferenceImageGeometryStringFromSegmentation(segmentation);
  }
...
vtkSegmentation* vtkSegmentEditorLogic::GetSegmentation() const
{
  if (!IsSegmentationNodeValid())
    return nullptr;
  return GetSegmentationNode()->GetSegmentation();
}
...
{
  return SegmentEditorNode ? SegmentEditorNode->GetSegmentationNode() : nullptr;
}
...
{
  return GetSegmentationNode() != nullptr;
}
...
bool vtkSegmentEditorLogic::IsSegmentIdValid(const std::string& segmentId) const
{
  ...
  vtkSegmentation* segmentation = GetSegmentation();
  if (!segmentation)
  {
    return false;
  }
...
{
  if (!IsSegmentationNodeValid() || !SegmentationHistory)
  {
    return;
  }

  MRMLNodeModifyBlocker blocker(GetSegmentationNode());
  SegmentationHistory->RestoreNextState();
  GetSegmentationNode()->InvokeCustomModifiedEvent(vtkMRMLDisplayableNode::DisplayModifiedEvent, GetSegmentationNode()->GetDisplayNode());
}
...
std::string vtkSegmentEditorLogic::RemoveSelectedSegment() const
{
  ...
  std::string newId = GetNextSegmentID(1, false);
  if (newId.empty())
  {
    newId = GetNextSegmentID(-1, false);
  }
  if (!newId.empty())
  {
...
{
  if (SegmentationHistory && SegmentationHistory->GetMaximumNumberOfStates() > 0)
  {
    return SegmentationHistory->SaveState();
  }
  return false;
}
...
void vtkSegmentEditorLogic::SelectFirstSegment(bool visibleOnly) const
{
  ...
  std::vector<std::string> filterIds = visibleOnly ? GetVisibleSegmentIDs() : segmentIds;
  for (const auto& segmentId : segmentIds)
  {
    if (IsSegmentIdInList(segmentId, filterIds))
    {
...
void vtkSegmentEditorLogic::SetSegmentEditorNode(vtkMRMLSegmentEditorNode* newSegmentEditorNode)
{
  if (SegmentEditorNode)
  {
    SegmentEditorNode->RemoveObserver(SegmentEditorNodeObs);
  }
  ...
  if (newSegmentEditorNode)
  {
    ...
    SegmentEditorNodeObs = newSegmentEditorNode->AddObserver(vtkCommand::ModifiedEvent, updateCommand);
  }
}
...
void vtkSegmentEditorLogic::SetSegmentationNodeID(const std::string& nodeID) const
{
  ...
  this->SetSegmentationNode(vtkMRMLSegmentationNode::SafeDownCast(MRMLScene->GetNodeByID(nodeID.c_str())));
}

//-----------------------------------------------------------------------------
void vtkSegmentEditorLogic::SetSegmentationHistory(const vtkSmartPointer<vtkSegmentationHistory>& segmentationHistory)
{
  if (segmentationHistory == SegmentationHistory)
  {
    return;
  }
  if (SegmentationHistory)
  {
    SegmentationHistory->RemoveObserver(SegmentHistoryObs);
  }

  SegmentationHistory = segmentationHistory;
  if (!SegmentationHistory)
  {
    return;
  }
  ...
  SynchronizeSegmentationHistorySegmentation();
}

//------------------------------------------------------------------------------
void vtkSegmentEditorLogic::SetSourceVolumeNode(vtkMRMLNode* node) const
{
  if (!SegmentEditorNode || !IsSegmentationNodeValid())
  {
    if (node)
    {
...
void vtkSegmentEditorLogic::SetSourceVolumeNodeID(const std::string& nodeID) const
{
  ...
  this->SetSourceVolumeNode(MRMLScene->GetNodeByID(nodeID.c_str()));
}
...
bool vtkSegmentEditorLogic::SetSourceRepresentationToBinaryLabelMap() const
{
  ...
  bool closedSurfacePresent = ContainsClosedSurfaceRepresentation();
  bool createBinaryLabelmapRepresentationSuccess = GetSegmentation()->CreateRepresentation(vtkSegmentationConverter::GetSegmentationBinaryLabelmapRepresentationName());
  if (!createBinaryLabelmapRepresentationSuccess)
    return false;

  GetSegmentation()->SetSourceRepresentationName(vtkSegmentationConverter::GetSegmentationBinaryLabelmapRepresentationName());

  if (closedSurfacePresent)
  {
    GetSegmentation()->CreateRepresentation(vtkSegmentationConverter::GetSegmentationClosedSurfaceRepresentationName());
  }
...
bool vtkSegmentEditorLogic::TrivialSetSourceRepresentationToBinaryLabelmap() const
{
  ...
  {
    MRMLNodeModifyBlocker blocker(GetSegmentationNode());
    GetSegmentationNode()->GetSegmentation()->SetSourceRepresentationName(vtkSegmentationConverter::GetSegmentationBinaryLabelmapRepresentationName());
    return true;
  }
  return false;
}
...
{
  if (!GetSegmentationNode() || !SegmentationHistory)
  {
    return;
  }
                                           ...
                                           vtkMRMLSegmentationNode* segmentationNode /*=nullptr*/)
{
  ...
  CreateMaskImageFromPolyData(polyData, polyMaskImage, sliceWidget);
  ...
  AppendImage(input, polyMaskImage);
}
...
void vtkSegmentEditorLogic::CreateMaskImageFromPolyData(vtkPolyData* polyData, vtkOrientedImageData* outputMask, vtkMRMLSliceNode* sliceNode)
{
  ...
  XyzToRas(originXYZ, originRAS, sliceNode);
...
{
  std::vector<std::string> segmentIds = GetSegmentIDs();
  std::string currentId = GetCurrentSegmentID();
...
std::string vtkSegmentEditorLogic::GetNextSegmentID(int offset, bool visibleOnly) const
{
  ...
  int iCurrent = GetCurrentSegmentIndex();
  if (offset == 0 || iCurrent < 0)
    return GetCurrentSegmentID();
...
{
  vtkMRMLSliceLogic* sliceLogic = ApplicationLogic ? ApplicationLogic->GetSliceLogic(sliceNode) : nullptr;
  return GetSliceSpacing(sliceNode, sliceLogic);
}
...
{
  return IsSegmentIdInList(segmentID, GetVisibleSegmentIDs());
}

//-----------------------------------------------------------------------------
bool vtkSegmentEditorLogic::IsSelectedSegmentVisible() const
{
  vtkSegment* selectedSegment = GetSelectedSegment();
                                                    ...
                                                    bool bypassMasking)
{
  ...
  if (vtkMRMLScalarVolumeNode* sourceVolumeNode = SegmentEditorNode->GetSourceVolumeNode())
  {
    if (vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(SegmentEditorNode->GetScene()))
    {
...
{
  std::string newId = GetNextSegmentID(offset, visibleOnly);
...
void vtkSegmentEditorLogic::XyToRas(int xy[2], double outputRas[3], vtkMRMLSliceNode* sliceNode)
{
  ...
  XyzToRas(xyz, outputRas, sliceNode);
}
...
{
  std::array<double, 3> outputRas = { 0.0, 0.0, 0.0 };
  XyToRas(xy, outputRas.data(), sliceNode);
  return outputRas;
}
                                     ...
                                     vtkMRMLTransformNode* parentTransformNode /*=nullptr*/)
{
  ...
  XyzToRas(inputXyz, ras, sliceNode);
                                                   ...
                                                   vtkMRMLTransformNode* parentTransformNode /*=nullptr*/)
{
  ...
  XyzToIjk(inputXyz, outputIjk.data(), sliceNode, image, parentTransformNode);
  return outputIjk;
}
...
{
  double xyz[3] = { static_cast<double>(xy[0]), static_cast<double>(xy[1]), 0.0 };
  XyzToIjk(xyz, outputIjk, sliceNode, image, parentTransformNode);
}
...
{
  std::array<int, 3> outputIjk = { 0, 0, 0 };
  XyToIjk(xy, outputIjk.data(), sliceNode, image, parentTransformNode);
  return outputIjk;
}
...
{
  vtkMRMLSegmentationNode* prevSegmentationNode = std::get<1>(SegmentationObs);
  vtkMRMLSegmentationNode* newSegmentationNode = GetSegmentationNode();
  ...
  {
    prevSegmentationNode->RemoveObserver(std::get<0>(SegmentationObs));
  }
  ...
  if (newSegmentationNode)
  {
    ...
    SegmentationObs = std::make_tuple(obs, newSegmentationNode);
  }
}
...
void vtkSegmentEditorLogic::SynchronizeSegmentationHistorySegmentation() const
{
  if (SegmentationHistory)
  {
    SegmentationHistory->SetSegmentation(GetSegmentation());
  }
}
