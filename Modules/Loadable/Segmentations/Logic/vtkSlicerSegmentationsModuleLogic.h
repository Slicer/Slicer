...
public:
  ...
  static vtkMRMLSegmentationNode* GetSegmentationNodeForSegmentation(vtkMRMLScene* scene,
  ...
  static vtkMRMLSegmentationNode* GetSegmentationNodeForSegment(vtkMRMLScene* scene,
                                                                vtkSegment* segment,
                                                                std::string& segmentId);

  /// Load segmentation from file
  /// \param filename Path and name of file containing segmentation (nrrd, vtm, etc.)
  /// \param autoOpacities Optional flag determining whether segment opacities are calculated automatically based on
  /// containment. True by default. \param nodeName Optional string to use for the segmentation node name. \param
  /// colorTableNode Optional color node used to name the segments and set segment color. \return Loaded segmentation
  /// node
  ...
  /// \param labelmapVolumeNode Labelmap volume to be populated with the oriented image data. The volume node must exist
  /// and be added to the MRML scene. \return Success flag
  static bool CreateLabelmapVolumeFromOrientedImageData(vtkOrientedImageData* orientedImageData,
  ...
  /// \param shiftImageDataExtentToZeroStart: Adjust image origin to make image extents start from zero. May be
  /// necessary for compatibility with some algorithms
  ...
  static vtkOrientedImageData* CreateOrientedImageDataFromVolumeNode(
  ...
  /// Create segment from labelmap volume MRML node. The contents are set as binary labelmap representation in the
  /// segment. Returns nullptr if labelmap contains more than one label. In that case \sa
  /// ImportLabelmapToSegmentationNode needs to be used. NOTE: Need to take ownership of the created object! For example
  /// using vtkSmartPointer<vtkSegment>::Take \param labelmapVolumeNode Model node containing image data that will be
  /// the binary labelmap representation in the created segment \param segmentationNode Segmentation node that will be
  /// the container of the segment. It is used to get parent transform to
  ///   make sure the created segment will be located the same place the image was, considering all transforms involved.
  ///   nullptr value means that this consideration is not needed. Default value is nullptr.
  /// \return Created segment that then can be added to the segmentation if needed. Need to take ownership of the
  /// created
  ...
  /// \param modelNode Model node containing poly data that will be the closed surface representation in the created
  /// segment \param segmentationNode Segmentation node that will be the container of the segment. It is used to get
  /// parent transform to
  ///   make sure the created segment will be located the same place the model was, considering all transforms involved.
  ///   nullptr value means that this consideration is not needed. Default value is nullptr.
  /// \return Created segment that then can be added to the segmentation if needed. Need to take ownership of the
  /// created
  ///   object! For example using vtkSmartPointer<vtkSegment>::Take
  ...
  static vtkMRMLSegmentationNode* GetSegmentationNodeForSegmentSubjectHierarchyItem(vtkIdType segmentShItemID,
                                                                                    vtkMRMLScene* scene);
  ...
  /// \param referenceVolumeNode If specified, then the merged labelmap node will match the geometry of
  /// referenceVolumeNode \param extentComputationMode If referenceVolumeNode is not specified then labelmap extents
  /// will be determined based on this value.
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use
  ///   reference geometry extent.
  /// \param colorTableNode If specified then it is used for setting label values in the output labelmapNode (by
  /// matching segment name to color name).
  ...
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use
  ///   reference geometry extent.
  static bool ExportVisibleSegmentsToLabelmapNode(
    vtkMRMLSegmentationNode* segmentationNode,
    vtkMRMLLabelMapVolumeNode* labelmapNode,
    vtkMRMLVolumeNode* referenceVolumeNode = nullptr,
    int extentComputationMode = vtkSegmentation::EXTENT_UNION_OF_EFFECTIVE_SEGMENTS);
  ...
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use
  ///   reference geometry extent.
  static bool ExportAllSegmentsToLabelmapNode(
    vtkMRMLSegmentationNode* segmentationNode,
    vtkMRMLLabelMapVolumeNode* labelmapNode,
    int extentComputationMode = vtkSegmentation::EXTENT_UNION_OF_EFFECTIVE_SEGMENTS);
  ...
  /// The colors of the new segments are randomly generated, unless terminology context is specified, in which case the
  /// terminology
  ...
  /// \param baseSegmentName Prefix for the names of the new segments. Empty by default, in which case the prefix will
  /// be "Label"
  ...
  /// \param terminologyContextName Terminology context the entries of which are mapped to the labels imported from the
  /// labelmap node \param insertBeforeSegmentId New segments will be inserted before this segment.
  ...
  static bool ImportModelToSegmentationNode(vtkMRMLModelNode* modelNode,
                                            vtkMRMLSegmentationNode* segmentationNode,
                                            std::string insertBeforeSegmentId = "");

  /// Import models in a folder into the segmentation as segments.
  static bool ImportModelsToSegmentationNode(vtkIdType folderItemId,
                                             vtkMRMLSegmentationNode* segmentationNode,
                                             std::string insertBeforeSegmentId = "");

  /// Export closed surface representation of multiple segments to files. Typically used for writing 3D printable model
  /// files. \param segmentationNode Segmentation node from which the the segments are exported \param destinationFolder
  /// Folder name where segments will be exported to \param fileFormat Output file format (STL or OBJ). \param merge
  /// Merge all models into a single mesh. Only applicable to STL format. \param lps Save files in LPS coordinate
  /// system. If set to false then RAS coordinate system is used. \param segmentIds List of segment IDs to export
  ...
  /// Label values found by matching color + segment name. If a segment name is not found in the table, then the label
  /// value returned for the segment is the first label value outside the table range. \param segmentationNode
  /// Segmentation node that has the export color node reference. \param colorTableNode Color table used to get the
  /// label values for the segments. \param segmentIds List of segment ids to get values for. The order of segmentIds
  /// dictates the order of the returned label values. \param labelValues Output label values from the color node.
  /// Length of the array will be the same as the number of segmentIds.
  static void GetLabelValuesFromColorNode(vtkMRMLSegmentationNode* segmentationNode,
  ...
  /// \param segmentIds List of segment ids to get values for. The order of segmentIds dictates the order of the
  /// returned label values. \param extension The file extension used for the output file. "nrrd" by default. \param
  /// useCompression If compression should be applied to the output file. \param referenceVolumeNode If specified, then
  /// the saved segmentation will match the geometry of referenceVolumeNode \param extentComputationMode If
  /// referenceVolumeNode is not specified then the saved segmentation extents will be determined based on this value.
  ...
  ///   If a segment name is not found in the color node, then the first unused value outside of the color table range
  ///   will be used.
  ...
  static vtkDataObject* CreateRepresentationForOneSegment(vtkSegmentation* segmentation,
                                                          std::string segmentID,
  ...
  /// Useful if we want to get a surface or contours representation of a segmentation in the proper geometry for
  /// processing. \return Success flag
  ...
  /// Useful if we want to add a representation to a segment, and we want to make sure that the segment will be located
  /// the same place the representation node was. The output transform is the representation node's parent transform
  /// concatenated with the inverse of the segmentation's parent transform. It needs to be applied on the
  /// representation. \param representationNode Transformable node which contains the representation we want to add to
  /// the segment \param segmentationNode Segmentation node that will contain the segment to which the representation is
  /// added. It is the
  ///   representation node's parent transform concatenated with the inverse of the segmentation's parent transform.
  /// \param representationToSegmentationTransform General transform between the representation node and the
  /// segmentation node. \return Success flag
  ...
  /// A duplicate of the representation data object is copied into the argument output object, with the segmentation's
  /// parent transform applied if requested (on by default). \param segmentationNode Input segmentation node containing
  /// the segment to extract \param segmentID Segment identifier of the segment to extract \param representationName
  /// Name of the requested representation \param segmentRepresentation Output representation data object into which the
  /// given representation in the segment is copied \param applyParentTransform Flag determining whether to apply parent
  /// transform of the segmentation node. On by default \return Success flag
  ...
  /// Convenience function to get binary labelmap representation of a segment in a segmentation. Uses \sa
  /// GetSegmentRepresentation A duplicate of the oriented image data is copied into the argument image data, with the
  /// segmentation's parent transform applied if requested (on by default). The oriented image data can be used directly
  /// for processing, or to create a labelmap volume using \sa CreateLabelmapVolumeFromOrientedImageData. \param
  /// segmentationNode Input segmentation node containing the segment to extract \param segmentID Segment identifier of
  /// the segment to extract \param imageData Output oriented image data into which the segment binary labelmap is
  /// copied \param applyParentTransform Flag determining whether to apply parent transform of the segmentation node.
  ///   If on, then the oriented image data is in RAS, otherwise in the segmentation node's coordinate frame. On by
  ///   default
  /// \return Success flag
  ...
  /// Convenience function to get closed surface representation of a segment in a segmentation. Uses \sa
  /// GetSegmentRepresentation A duplicate of the closed surface data is copied into the argument image data, with the
  /// segmentation's parent transform applied if requested (on by default). \param segmentationNode Input segmentation
  /// node containing the segment to extract \param segmentID Segment identifier of the segment to extract \param
  /// polyData Output polydata into which the segment polydata is copied \param applyParentTransform Flag determining
  /// whether to apply parent transform of the segmentation node.
  ///   If on, then the oriented image data is in RAS, otherwise in the segmentation node's coordinate frame. On by
  ///   default
  /// \return Success flag
  ...
  /// Assign terminology to segments in a segmentation node based on the labels of a labelmap node. Match is made based
  /// on the 3dSlicerLabel terminology type attribute. If the terminology context does not contain that attribute, match
  /// cannot be made. \param terminologyContextName Terminology context the entries of which are mapped to the labels
  /// imported from the labelmap node
  ...
  /// \param segmentationNode Node containing the segmentation
  /// \param referenceVolumeNode Determines geometry of merged labelmap if not nullptr, automatically determined
  /// otherwise \param segmentIDs Segment IDs to be converted. If empty, all segments will be converted. \param
  /// extentComputationMode If referenceVolumeNode is not specified then the saved segmentation extents will be
  /// determined based on this value. \param mergedLabelmap_Reference Output merged labelmap in the reference volume
  /// coordinate system \param labelValues Output label values from the color node. Length of the array must be the same
  /// as the number of segmentIds.
  ...
  static bool IsEffectiveExentOutsideReferenceVolume(vtkMRMLVolumeNode* referenceVolumeNode,
                                                     vtkMRMLSegmentationNode* segmentationNode,
                                                     vtkStringArray* segmentIDs = nullptr);
  ...
  static bool IsSegmentationExentOutsideReferenceGeometry(vtkOrientedImageData* referenceGeometry,
...
protected:
  ...
  /// (there is an implementation already in qSlicerSaveDataDialogPrivate::nodeFileInfo and it would be good to keep a
  /// central one)
