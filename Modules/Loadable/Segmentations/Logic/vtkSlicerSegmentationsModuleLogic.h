...
public:
  ...
  /// using vtkSmartPointer<vtkSegment>::Take
  /// \param labelmapVolumeNode Model node containing image data that will be the binary labelmap representation in the
  /// created segment
  /// \param segmentationNode Segmentation node that will be the container of the segment. It is used to get parent
  /// transform to
  ...
  /// Add a new empty color table node to the scene that can be used to store segment color and terminology information
  /// in each color entry. ExportSegmentsToColorTableNode() can be used to fill the table with colors from a
  /// segmentation node.
  ...
  /// referenceVolumeNode
  /// \param extentComputationMode If referenceVolumeNode is not specified then labelmap extents will be determined
  /// based on this value.
  ...
  ///   Segment names are matched based on terminology and if there is no match based on terminology then based on
  ///   segment name. If a segment name is not found in the color node, then the first label value outside of the input
  ///   color table range will be used. Segment color, name, and terminology stored in the color table of the exported
  ///   labelmap are set from the segmentation.
  ...
  /// If there is still no match then a new label value generated for the segment. The label values are all outside of
  /// the range of the input color table to avoid any ambiguities.
  ...
  /// \param labelValues Output label values from the color node. Length of the array will be the same as the number of
  /// segmentIds.
  ...
  /// \param referenceVolumeNode If specified, then the saved segmentation will match the geometry of
  /// referenceVolumeNode
  /// \param extentComputationMode If referenceVolumeNode is not specified then the saved segmentation extents will be
  /// determined based on this value.
  /// \param colorTableNode Optional color table node used to set the exported label values for the segments.
  ///   Segment names are matched based on terminology and if there is no match based on terminology then based on
  ///   segment name. If a segment name is not found in the color node, then the first label value outside of the input
  ///   color table range will be used.
  ...
  /// \param segmentationNode Segmentation node that will contain the segment to which the representation is added. It
  /// is the
  ...
  /// parent transform applied if requested (on by default).
  ...
  /// \param segmentRepresentation Output representation data object into which the given representation in the segment
  /// is copied
  /// \param applyParentTransform Flag determining whether to apply parent transform of the segmentation node. On by
  /// default
  /// \return Success flag
  ...
  /// for processing, or to create a labelmap volume using \sa CreateLabelmapVolumeFromOrientedImageData.
  /// \param segmentationNode Input segmentation node containing the segment to extract
  /// \param segmentID Segment identifier of the segment to extract
  ...
  /// segmentation's parent transform applied if requested (on by default).
  /// \param segmentationNode Input segmentation node containing the segment to extract
  /// \param segmentID Segment identifier of the segment to extract
  ...
  /// triggered.
  /// \param mergeMode Determines if the labelmap should replace the segment, combined with a maximum or minimum
  /// operation, or set under the mask.
  ...
  /// \param labelValues Output label values from the color node. Length of the array must be the same as the number of
  /// segmentIds.
