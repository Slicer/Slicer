...
std::string vtkSlicerSegmentationGeometryLogic::CalculateOutputGeometry()
{
  ...
  if (sourceVolumeNode || (sourceSegmentationNode && this->IsSourceSegmentationWithBinaryLabelmapMaster()))
  {
    // TODO: Fractional labelmaps cannot be used yet as source, as DetermineCommonLabelmapGeometry only supports binary
    // labelmaps
...
void vtkSlicerSegmentationGeometryLogic::CalculatePaddedOutputGeometry()
{
  ...
  std::string segmentationGeometryString =
    this->InputSegmentationNode->GetSegmentation()->DetermineCommonLabelmapGeometry(
  ...
  if (!segmentationGeometryString.empty())
  {
    ...
    vtkSegmentationConverter::DeserializeImageGeometry(
      segmentationGeometryString, segmentationGeometryImageData, false /*don't allocate*/);
    ...
    vtkOrientedImageDataResample::GetTransformBetweenOrientedImages(
      segmentationGeometryImageData, this->OutputGeometryImageData, segmentationGeometryToReferenceGeometryTransform);
    ...
    vtkOrientedImageDataResample::TransformExtent(segmentationGeometryImageData->GetExtent(),
                                                  segmentationGeometryToReferenceGeometryTransform,
                                                  transformedSegmentationExtent);

    if (outputGeometryExtent[0] > outputGeometryExtent[1] || outputGeometryExtent[2] > outputGeometryExtent[3] ||
    ...
    else
    {
      ...
      for (int i = 0; i < 3; ++i)
      {
        ...
        outputGeometryExtent[2 * i + 1] =
          std::max(outputGeometryExtent[2 * i + 1], transformedSegmentationExtent[2 * i + 1]);
      }
    }
  }
...
std::string vtkSlicerSegmentationGeometryLogic::CalculateOutputGeometryFromImage()
{
  ...
  if (!vtkMRMLTransformNode::GetMatrixTransformBetweenNodes(this->SourceGeometryNode->GetParentTransformNode(),
                                                            this->InputSegmentationNode->GetParentTransformNode(),
                                                            sourceToInputSegmentationMatrix))
  {
    vtkWarningMacro(
      "CalculateOutputGeometry: Ignoring parent transforms because non-linear components have been found");
  }

  vtkNew<vtkMatrix4x4> outputGeometryImageToSegmentationMatrix;
  vtkMatrix4x4::Multiply4x4(
    sourceToInputSegmentationMatrix, sourceImageToSourceMatrix, outputGeometryImageToSegmentationMatrix);
  ...
  {
    vtkCalculateOversamplingFactor::ApplyOversamplingOnImageGeometry(this->OutputGeometryImageData,
...
std::string vtkSlicerSegmentationGeometryLogic::CalculateOutputGeometryFromBounds(bool keepCurrentAxisDirections)
{
  ...
  if (!vtkMRMLTransformNode::GetMatrixTransformBetweenNodes(this->InputSegmentationNode->GetParentTransformNode(),
                                                            this->SourceGeometryNode->GetParentTransformNode(),
                                                            segmentationToSourceMatrix))
  {
    vtkWarningMacro(
      "CalculateOutputGeometry: Ignoring parent transforms because non-linear components have been found");
  }
  ...
  if (keepCurrentAxisDirections && this->InputSegmentationCanBeResampled())
  {
    ...
    vtkMatrix4x4::Multiply4x4(
      segmentationToSourceMatrix, inputLabelmapIJKToInputSegmentation, inputLabelmapIJKToSource);
    ...
    vtkMatrix4x4::Multiply4x4(
      segmentationToSourceMatrix, outputGeometryImageToWorld, outputGeometryImageToSourceMatrix);
  }
  else
  {
    ...
    vtkNew<vtkMatrix4x4> outputGeometryImageToSegmentationMatrix;
    vtkMatrix4x4::Multiply4x4(
      sourceToSegmentationMatrix, outputGeometryImageToSourceMatrix, outputGeometryImageToSegmentationMatrix);
    this->OutputGeometryImageData->SetImageToWorldMatrix(outputGeometryImageToSegmentationMatrix);
  }

  // Calculate extent
  const double corner1_Source[4] = { sourceBounds[0] + outputSpacing[0] / 2.0,
                                     sourceBounds[2] + outputSpacing[1] / 2.0,
                                     sourceBounds[4] + outputSpacing[2] / 2.0,
                                     1.0 };
  const double corner2_Source[4] = { sourceBounds[1] - outputSpacing[0] / 2.0,
...
bool vtkSlicerSegmentationGeometryLogic::IsSourceSegmentationWithBinaryLabelmapMaster()
{
  ...
  if (sourceSegmentationNode && sourceSegmentationNode->GetSegmentation() &&
...
void vtkSlicerSegmentationGeometryLogic::ComputeSourceAxisIndexForInputAxis()
{
  ...
  {
    vtkErrorMacro("ComputeSourceAxisIndexForInputAxis: MRML scene of the given source node and the widget are "
                  "different, cannot set node");
    return;
  }
  ...
  if ((transformableSourceNode->IsA("vtkMRMLScalarVolumeNode") ||
       this->IsSourceSegmentationWithBinaryLabelmapMaster()) &&
      this->InputSegmentationCanBeResampled())
  {
    ...
    vtkMRMLTransformNode::GetTransformBetweenNodes(this->InputSegmentationNode->GetParentTransformNode(),
                                                   transformableSourceNode->GetParentTransformNode(),
                                                   segmentationToSourceTransform);
    vtkNew<vtkMatrix4x4> segmentationToSourceMatrix;
    if (vtkMRMLTransformNode::IsGeneralTransformLinear(segmentationToSourceTransform,
                                                       segmentationToSourceTransformLinear))
    {
    ...
    {
      vtkWarningMacro(
        "ComputeSourceAxisIndexForInputAxis: Ignoring parent transforms because non-linear components have been found");
    ...
    for (int sourceAxisIndex = 0; sourceAxisIndex < 3; sourceAxisIndex++)
    {
      ...
      {
        double currentComponentValue =
          fabs(inputLabelmapIJKToSource->GetElement(sourceAxisIndex, labelmapIJKAxisIndex));
...
bool vtkSlicerSegmentationGeometryLogic::ResampleLabelmapsInSegmentationNode()
{
  ...
  {
    vtkErrorMacro(
      "vtkSlicerSegmentationGeometryLogic::ResampleLabelmapsInSegmentationNode: invalid input segmentation node");
    return false;
  }

  // Check if source representation is binary or fractional labelmap (those are the only supported representations in
  // segment editor)
      ...
      sourceRepresentationName != vtkSegmentationConverter::GetFractionalLabelmapRepresentationName())
  {
    vtkErrorMacro("vtkSlicerSegmentationGeometryLogic::ResampleLabelmapsInSegmentationNode: "
                  << "Source representation needs to be a labelmap type, but '" << sourceRepresentationName.c_str()
                  << "' found");
    return false;
  }
       ...
       ++segmentIdIt)
  {
    ...
    vtkOrientedImageData* currentLabelmap =
      vtkOrientedImageData::SafeDownCast(currentSegment->GetRepresentation(sourceRepresentationName));
    if (!currentLabelmap)
    {
      vtkErrorMacro("vtkSlicerSegmentationGeometryLogic::ResampleLabelmapsInSegmentationNode: "
    ...
    {
      vtkErrorMacro("vtkSlicerSegmentationGeometryLogic::ResampleLabelmapsInSegmentationNode: "
                    << "Segment " << this->InputSegmentationNode->GetName() << "/" << currentSegmentID.c_str()
                    << " failed to be resampled");
      success = false;
      continue;
    }
  }
...
bool vtkSlicerSegmentationGeometryLogic::SetReferenceImageGeometryInSegmentationNode()
{
  if (!this->InputSegmentationNode || !this->InputSegmentationNode->GetSegmentation())
  {
    vtkErrorMacro("vtkSlicerSegmentationGeometryLogic::SetReferenceImageGeometryInSegmentationNode: invalid input "
                  "segmentation node");
    return false;
  }
  ...
  {
    vtkErrorMacro(
      "vtkSlicerSegmentationGeometryLogic::SetReferenceImageGeometryInSegmentationNode: invalid output geometry");
    return false;
  }
  ...
  if (this->SourceGeometryNode && this->SourceGeometryNode->GetID() &&
      vtkMRMLSegmentationNode::SafeDownCast(this->SourceGeometryNode) != this->InputSegmentationNode)
  {
    this->InputSegmentationNode->SetNodeReferenceID(
      vtkMRMLSegmentationNode::GetReferenceImageGeometryReferenceRole().c_str(), this->SourceGeometryNode->GetID());
  }
