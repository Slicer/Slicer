...
{
  return displayNode && displayNode->GetVisibility(this->External->GetMRMLViewNode()->GetID()) &&
...
//---------------------------------------------------------------------------
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::RemoveDisplayNode(
...
//---------------------------------------------------------------------------
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::AddDisplayNode(vtkMRMLSegmentationNode* mNode,
                                                                           vtkMRMLSegmentationDisplayNode* displayNode)
{
...
vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::CreateSegmentPipeline(std::string vtkNotUsed(segmentID))
{
...
//---------------------------------------------------------------------------
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::UpdateDisplayNode(
  vtkMRMLSegmentationDisplayNode* displayNode)
{
...
//---------------------------------------------------------------------------
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::UpdateAllDisplayNodesForSegment(
...
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::UpdateSegmentPipelines(
  vtkMRMLSegmentationDisplayNode* displayNode,
  PipelineMapType& segmentPipelines)
{
  // Get segmentation
       ...
       ++segmentIdIt)
  {
    ...
    if (pipelineIt == segmentPipelines.end())
    {
      ...
      // It is important to only update the transform if the transform chain is actually changed,
      // because recomputing a non-linear transformation on a complex model may be very time-consuming.
      if (!vtkMRMLTransformNode::AreTransformsEqual(nodeToWorld.GetPointer(),
                                                    segmentPipelines[*segmentIdIt]->NodeToWorldTransform))
      {
  ...
  PipelineMapType& segmentPipelines)
{
  ...
  if (shownRepresentationName.empty())
  {
    ...
    for (PipelineMapType::iterator pipelineIt = segmentPipelines.begin(); pipelineIt != segmentPipelines.end();
         ++pipelineIt)
    {
       ...
       ++pipelineIt)
  {
    ...
    vtkPolyData* polyData =
      vtkPolyData::SafeDownCast(segmentation->GetSegmentRepresentation(pipelineIt->first, shownRepresentationName));
    ...
    lut->SetBelowRangeColor(
      color[0], color[1], color[2], capSurface ? opacity * displayNode->GetClippingCapOpacity() : 0.0);
...
void vtkMRMLSegmentationsDisplayableManager3D::vtkInternal::FindPickedDisplayNodeFromMesh(vtkPointSet* mesh)
{
  ...
  for (pipelinesIt = this->DisplayPipelines.begin(); pipelinesIt != this->DisplayPipelines.end(); ++pipelinesIt)
  {
    ...
    for (PipelineMapType::iterator pipelineIt = pipelinesIt->second.begin(); pipelineIt != pipelinesIt->second.end();
         ++pipelineIt)
    {
                                                                      ...
                                                                      void* callData)
{
    ...
    if (event == vtkMRMLDisplayableNode::DisplayModifiedEvent)
    {
      ...
      if (displayNode)
      {
        ...
        this->RequestRender();
             ...
             (event == vtkSegmentation::RepresentationModified) || (event == vtkSegmentation::SegmentModified))
    {
  ...
  else if (vtkMRMLClipNode::SafeDownCast(caller))
  {
    ...
    for (auto displayNodeIt = this->Internal->DisplayPipelines.begin();
