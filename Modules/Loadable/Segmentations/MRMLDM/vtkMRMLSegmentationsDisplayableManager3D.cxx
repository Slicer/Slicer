  ...
  vtkMRMLSegmentationDisplayNode* displayNode, PipelineMapType &segmentPipelines)
{
  ...
  for (PipelineMapType::iterator pipelineIt=segmentPipelines.begin(); pipelineIt!=segmentPipelines.end(); ++pipelineIt)
  {
    ...
    if (clipNode && displayNode->GetClipping())
    {
      pipeline->Clipper->SetClipFunction(clipNode->GetImplicitFunctionWorld());
      pipeline->Capper->SetClipFunction(clipNode->GetImplicitFunctionWorld());

      vtkEventBroker* broker = vtkEventBroker::GetInstance();
      if (!broker->GetObservationExist(clipNode, vtkMRMLClipNode::ClipNodeModifiedEvent, this->External, this->External->GetMRMLNodesCallbackCommand()))
      {
        broker->AddObservation(clipNode, vtkMRMLClipNode::ClipNodeModifiedEvent, this->External, this->External->GetMRMLNodesCallbackCommand());
      }
