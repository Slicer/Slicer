...
qSlicerSegmentationsModuleWidgetPrivate::qSlicerSegmentationsModuleWidgetPrivate(
  qSlicerSegmentationsModuleWidget& object)
  : q_ptr(&object)
...
vtkMRMLSegmentationDisplayNode* qSlicerSegmentationsModuleWidget::segmentationDisplayNode(bool create /*=false*/)
{
  ...
  vtkMRMLSegmentationNode* segmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(d->MRMLNodeComboBox_Segmentation->currentNode());
  if (!segmentationNode)
  {
    return nullptr;
  }
...
void qSlicerSegmentationsModuleWidget::updateCopyMoveButtonStates()
{
  ...
  vtkMRMLSegmentationNode* otherSegmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(d->SegmentsTableView_Other->segmentationNode());
  ...
  vtkMRMLSegmentationNode* currentSegmentationNode =
    vtkMRMLSegmentationNode::SafeDownCast(d->MRMLNodeComboBox_Segmentation->currentNode());
...
void qSlicerSegmentationsModuleWidget::init()
{
  ...
  // Make connections
  connect(d->MRMLNodeComboBox_Segmentation,
  ...
  connect(d->MRMLNodeComboBox_Segmentation,
          SIGNAL(currentNodeChanged(vtkMRMLNode*)),
          d->SegmentsTableView,
          SLOT(setSegmentationNode(vtkMRMLNode*)));
  connect(d->MRMLNodeComboBox_Segmentation,
          SIGNAL(currentNodeChanged(vtkMRMLNode*)),
          d->SegmentsTableView_Current,
          SLOT(setSegmentationNode(vtkMRMLNode*)));
  connect(d->MRMLNodeComboBox_Segmentation,
          SIGNAL(currentNodeChanged(vtkMRMLNode*)),
          d->RepresentationsListView,
          SLOT(setSegmentationNode(vtkMRMLNode*)));

  connect(d->SegmentsTableView,
  ...
  connect(d->MRMLNodeComboBox_OtherSegmentationOrRepresentationNode,
          SIGNAL(currentNodeChanged(vtkMRMLNode*)),
          this,
          SLOT(setOtherSegmentationOrRepresentationNode(vtkMRMLNode*)));

  connect(d->ImportExportOperationButtonGroup,
          SIGNAL(buttonClicked(QAbstractButton*)),
          this,
          SLOT(updateImportExportWidgets()));

  connect(
    d->ImportExportTypeButtonGroup, SIGNAL(buttonClicked(QAbstractButton*)), this, SLOT(updateImportExportWidgets()));
  ...
  connect(
    d->ColorTableNodeSelector, SIGNAL(currentNodeIDChanged(const QString&)), this, SLOT(onExportColorTableChanged()));

  d->ExportToFilesWidget->setSettingsKey("ExportSegmentsToFiles");
  connect(d->MRMLNodeComboBox_Segmentation,
          SIGNAL(currentNodeChanged(vtkMRMLNode*)),
          d->ExportToFilesWidget,
          SLOT(setSegmentationNode(vtkMRMLNode*)));
...
void qSlicerSegmentationsModuleWidget::onSegmentationNodeChanged(vtkMRMLNode* node)
{
  ...
  qvtkReconnect(d->SegmentationNode,
                ...
                this,
                SLOT(onSegmentationNodeReferenceChanged()));
  qvtkReconnect(d->SegmentationNode,
                segmentationNode,
                ...
                SLOT(onSegmentationNodeReferenceChanged()));
  qvtkReconnect(
    d->SegmentationNode, segmentationNode, vtkSegmentation::SegmentModified, this, SLOT(updateLayerWidgets()));
...
void qSlicerSegmentationsModuleWidget::onAddSegment()
{
  ...
  std::string addedSegmentID =
    currentSegmentationNode->GetSegmentation()->AddEmptySegment(d->SegmentsTableView->textFilter().toStdString());
  int status = 0;
  ...
  vtkSlicerSegmentationsModuleLogic::SetSegmentStatus(
    currentSegmentationNode->GetSegmentation()->GetSegment(addedSegmentID), status);
  ...
  if (currentSegmentationNode->GetSegmentation()->GetNumberOfSegments() > 1)
  {
    ...
    currentSegmentationNode->GetSegmentation()
...
void qSlicerSegmentationsModuleWidget::updateLayerWidgets()
{
  ...
  {
    layerCountSS << d->SegmentationNode->GetSegmentation()->GetNumberOfLayers(
...
void qSlicerSegmentationsModuleWidget::updateImportExportWidgets()
{
  ...
  d->MRMLNodeComboBox_ExportLabelmapReferenceVolume->setEnabled(d->radioButton_Labelmap->isChecked()
                                                                && d->radioButton_Export->isChecked());
                                                                       ...
                                                                       bool removeFromSource /*=false*/)
{
  ...
  if (!toSegmentation->CanAcceptSegment(segment))
  {
    ...
    vtkMRMLSegmentationNode* fromNode =
      vtkSlicerSegmentationsModuleLogic::GetSegmentationNodeForSegmentation(this->mrmlScene(), fromSegmentation);
    vtkMRMLSegmentationNode* toNode =
      vtkSlicerSegmentationsModuleLogic::GetSegmentationNodeForSegmentation(this->mrmlScene(), toSegmentation);
    ...
    QString message = tr("Cannot convert source representation '%1' into target source '%2', "
                         ...
                         "the source representation of '%5' to '%1'?\n\n"
    ...
    QMessageBox::StandardButton answer = QMessageBox::question(
      nullptr, tr("Failed to copy segment"), message, QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
                                                                        ...
                                                                        bool removeFromSource /*=false*/)
{
  ...
  foreach (QString segmentId, selectedSegmentIds)
  {
    success =
      success
      && this->copySegmentBetweenSegmentations(sourceSegmentation, targetSegmentation, segmentId, removeFromSource);
  }
...
bool qSlicerSegmentationsModuleWidget::exportFromCurrentSegmentation()
{
  ...
  else
  {
    // Visible segments
    vtkMRMLSegmentationDisplayNode* displayNode =
      vtkMRMLSegmentationDisplayNode::SafeDownCast(currentSegmentationNode->GetDisplayNode());
  ...
  vtkMRMLLabelMapVolumeNode* labelmapNode =
    vtkMRMLLabelMapVolumeNode::SafeDownCast(shNode->GetItemDataNode(selectedItem));
  if (d->radioButton_Labelmap->isChecked() && !labelmapNode)
  {
    ...
    {
      exportedNodeName +=
        "-" + std::string(currentSegmentationNode->GetSegmentation()->GetSegment(segmentIDs[0])->GetName());
    }
    ...
    vtkMRMLLabelMapVolumeNode* newLabelmapNode =
      vtkMRMLLabelMapVolumeNode::SafeDownCast(currentSegmentationNode->GetScene()->AddNode(newNode));
  ...
  else if (folderItem)
  {
    ...
    QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
    bool success =
      vtkSlicerSegmentationsModuleLogic::ExportSegmentsToModels(currentSegmentationNode, segmentIDs, folderItem);
    QApplication::restoreOverrideCursor();
    if (!success)
    {
...
bool qSlicerSegmentationsModuleWidget::importToCurrentSegmentation()
{
  ...
  if (labelmapNode)
  {
    std::string currentTerminologyContextName(d->ComboBox_TerminologyContext->currentText()
                                                  == d->ComboBox_TerminologyContext->defaultText()
                                                ? ""
  ...
  else if (folderItem)
  {
    ...
    {
      QString message =
        tr("Failed to copy polydata from models under folder %1!").arg(shNode->GetItemName(folderItem).c_str());
      qCritical() << Q_FUNC_INFO << ": " << message;
