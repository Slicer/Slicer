...
qSlicerSegmentEditorAbstractEffectPrivate::qSlicerSegmentEditorAbstractEffectPrivate(
  qSlicerSegmentEditorAbstractEffect& object)
  : q_ptr(&object)
  , Scene(nullptr)
...
//-----------------------------------------------------------------------------
void qSlicerSegmentEditorAbstractEffect::applyImageMask(vtkOrientedImageData* input,
                                                                         ...
                                                                         bool bypassMasking /*=false*/)
{
  this->modifySegmentByLabelmap(
    this->parameterSetNode()->GetSegmentationNode(),
                                                                 ...
                                                                 bool bypassMasking /*=false*/)
{
  int modificationExtent[6] = { 0, -1, 0, -1, 0, -1 };
  this->modifySegmentByLabelmap(
    segmentationNode, segmentID, modifierLabelmap, modificationMode, modificationExtent, bypassMasking);
}
                                                                 ...
                                                                 bool bypassMasking /*=false*/)
{
      ...
      parameterSetNode->GetSourceVolumeIntensityMask())
  {
    ...
    if (!bypassMasking && parameterSetNode->GetMaskMode() != vtkMRMLSegmentationNode::EditAllowedEverywhere)
    {
      vtkOrientedImageDataResample::ModifyImage(
        maskImage, this->maskLabelmap(), vtkOrientedImageDataResample::OPERATION_MAXIMUM);
    }
    ...
    if (parameterSetNode->GetSourceVolumeIntensityMask())
    {
      ...
      threshold->ThresholdBetween(parameterSetNode->GetSourceVolumeIntensityMaskRange()[0],
                                  parameterSetNode->GetSourceVolumeIntensityMaskRange()[1]);
      ...
      vtkOrientedImageDataResample::ModifyImage(
        maskImage, thresholdMask, vtkOrientedImageDataResample::OPERATION_MAXIMUM);
    }

    vtkSmartPointer<vtkOrientedImageData> segmentLayerLabelmap = vtkOrientedImageData::SafeDownCast(
      segment->GetRepresentation(segmentationNode->GetSegmentation()->GetSourceRepresentationName()));
    ...
    {
      // If we are painting inside a segment, the erase effect can modify the current segment outside the masking region
      // by adding back regions in the current segment. Add the current segment to the editable area
      ...
      vtkOrientedImageDataResample::ModifyImage(
        maskImage, invertedSegment, vtkOrientedImageDataResample::OPERATION_MINIMUM);
    }
    ...
    if (segmentLayerLabelmap && modificationMode == qSlicerSegmentEditorAbstractEffect::ModificationModeSet)
    {
          ...
          segmentThresholdExtent[4] <= segmentThresholdExtent[5])
      {
        ...
        vtkOrientedImageDataResample::ModifyImage(
          segmentOutsideMask, maskImage, vtkOrientedImageDataResample::OPERATION_MINIMUM);
        vtkOrientedImageDataResample::ModifyImage(
          modifierLabelmap, segmentOutsideMask, vtkOrientedImageDataResample::OPERATION_MAXIMUM);
      }
    }
  }
  ...
  if (modificationMode == qSlicerSegmentEditorAbstractEffect::ModificationModeSet)
  {
    ...
    if (!vtkSlicerSegmentationsModuleLogic::SetBinaryLabelmapToSegment(
          invertedModifierLabelmap.GetPointer(),
          segmentationNode,
          segmentID,
  ...
  segmentationNode->GetSegmentation()->GetSegmentIDsSharingBinaryLabelmapRepresentation(
    segmentID, sharedSegmentIDs, false);

  std::vector<std::string> segmentsToErase;
  for (std::string segmentIDToOverwrite : segmentIDsToOverwrite)
  {
    std::vector<std::string>::iterator foundSegmentIDIt =
      std::find(sharedSegmentIDs.begin(), sharedSegmentIDs.end(), segmentIDToOverwrite);
  ...
  if (!segmentsToErase.empty() && (modificationMode == qSlicerSegmentEditorAbstractEffect::ModificationModeSet ||
                                   ...
                                   modificationMode == qSlicerSegmentEditorAbstractEffect::ModificationModeRemoveAll))
  {
    ...
    for (std::string eraseSegmentID : segmentsToErase)
    {
      ...
      vtkDataObject* dataObject =
        currentSegment->GetRepresentation(vtkSegmentationConverter::GetBinaryLabelmapRepresentationName());
      ...
      segmentationNode->GetSegmentation()->GetSegmentIDsSharingBinaryLabelmapRepresentation(
        eraseSegmentID, currentSharedIDs, true);
      ...
      if (dontOverwriteIDs.size() > 0)
      {
        ...
        for (std::string dontOverwriteID : dontOverwriteIDs)
        {
          ...
          vtkOrientedImageDataResample::ApplyImageMask(
            invertedModifierLabelmap2, maskImage, VTK_UNSIGNED_CHAR_MAX, true);
        }
      }

      if (!vtkSlicerSegmentationsModuleLogic::SetBinaryLabelmapToSegment(
            invertedModifierLabelmap2,
            segmentationNode,
            eraseSegmentID,
            vtkSlicerSegmentationsModuleLogic::MODE_MERGE_MIN,
            extent,
           ...
           strcmp(this->parameterSetNode()->GetMaskSegmentID(), segmentID) != 0)
  {
    ...
    // The only exception is when we draw inside one specific segment. In that case erasing adds to the mask segment. It
    // is useful for splitting a segment into two by painting.
    if (!vtkSlicerSegmentationsModuleLogic::SetBinaryLabelmapToSegment(
          modifierLabelmap,
          segmentationNode,
          this->parameterSetNode()->GetMaskSegmentID(),
          ...
          segmentIDsToOverwrite))
    {
      qCritical() << Q_FUNC_INFO << ": Failed to add back modifier labelmap to segment "
                  << this->parameterSetNode()->GetMaskSegmentID();
    }
  }
  ...
  if (sourceVolumeNode)
  {
    vtkMRMLSubjectHierarchyNode* shNode =
      vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(d->ParameterSetNode->GetScene());
    if (shNode)
    {
...
int qSlicerSegmentEditorAbstractEffect::confirmCurrentSegmentVisible()
{
  ...
  confirmCurrentSegmentVisibleMsgBox->setText(
    tr("The currently selected segment is hidden. Would you like to make it visible?"));
  ...
  QSettings settings;
  int savedButtonSelection =
    settings
      .value(confirmCurrentSegmentVisibleMsgBox->dontShowAgainSettingsKey(), static_cast<int>(QMessageBox::InvalidRole))
      .toInt();
...
void qSlicerSegmentEditorAbstractEffect::setCommonParameter(QString name, QString value)
{
  ...
  // Don't pass parameter name as char pointer, as custom modified events may be compressed and invoked after
  // EndModify() and by that time the pointer may not be valid anymore.
...
//-----------------------------------------------------------------------------
void qSlicerSegmentEditorAbstractEffect::xyzToRas(double inputXyz[3],
                                                  double outputRas[3],
...
//-----------------------------------------------------------------------------
QVector3D qSlicerSegmentEditorAbstractEffect::xyToIjk(QPoint xy,
                                                      qMRMLSliceWidget* sliceWidget,
                                                      vtkOrientedImageData* image,
                                                      vtkMRMLTransformNode* parentTransformNode /*=nullptr*/)
{
