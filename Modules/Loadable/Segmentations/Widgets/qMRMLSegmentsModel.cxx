...
QString qMRMLSegmentsModel::segmentIDFromItem(QStandardItem* item) const
{
  ...
  const QVariant segmentID = item->data(qMRMLSegmentsModel::SegmentIDRole);
...
{
  const QModelIndex index = this->indexFromSegmentID(segmentID, column);
...
QModelIndex qMRMLSegmentsModel::indexFromSegmentID(QString segmentID, int column /*=0*/) const
{
  ...
  const QModelIndex startIndex = this->index(0, 0);
...
void qMRMLSegmentsModel::updateItemFromSegment(QStandardItem* item, QString segmentID, int column)
{
  ...
  const bool wasUpdating = d->UpdatingItemFromSegment;
  d->UpdatingItemFromSegment = true;

  const Qt::ItemFlags flags = this->segmentFlags(segmentID, column);
  item->setFlags(flags);
...
void qMRMLSegmentsModel::updateItemDataFromSegment(QStandardItem* item, QString segmentID, int column)
{
  ...
  const int index = segmentation->GetSegmentIndex(segmentID.toStdString());
  ...
  if (column == this->nameColumn())
  {
    ...
    double* const colorArray = segment->GetColor();
    const QColor color = QColor::fromRgbF(colorArray[0], colorArray[1], colorArray[2]);
    ...
    const QString segmentTerminologyTagValue(d->getTerminologyUserDataForSegment(segment));
    ...
    const QString defaultSegmentTerminology = QString::fromStdString(vtkSlicerTerminologiesModuleLogic::GetDefaultTerminologyEntryAsString(d->SegmentationNode));
  ...
  {
    const int layer = segmentation->GetLayerIndex(segmentID.toStdString(), vtkSegmentationConverter::GetBinaryLabelmapRepresentationName());
    std::stringstream ss;
    ss << layer;
  ...
  else
  {
    ...
    else if (column == this->visibilityColumn())
    {
      ...
      const bool visible = properties.Visible && (properties.Visible3D || properties.Visible2DFill || properties.Visible2DOutline);
    ...
    {
      const QString displayedOpacityStr = QString::number(properties.Opacity3D, 'f', 2);
...
void qMRMLSegmentsModel::updateSegmentFromItemData(QString segmentID, QStandardItem* item)
{
  ...
  if (item->column() == this->nameColumn())
  {
    ...
    const QColor color = item->data(qSlicerTerminologyItemDelegate::ColorRole).value<QColor>();
    double* const oldColorArray = segment->GetColor();
    ...
    const QString nameFromItem = item->data(qSlicerTerminologyItemDelegate::NameRole).toString();
  ...
  else if (item->column() == this->statusColumn())
  {
    ...
    const int status = item->data(StatusRole).toInt();
  ...
  else
  {
    ...
    {
      vtkMRMLSegmentationDisplayNode* displayNode = vtkMRMLSegmentationDisplayNode::SafeDownCast(d->SegmentationNode->GetDisplayNode());
      const bool visible = item->data(VisibilityRole).toBool();
    ...
    {
      const QString opacity = item->data(Qt::EditRole).toString();
      const QString currentOpacity = QString::number(properties.Opacity3D, 'f', 2);
...
void qMRMLSegmentsModel::updateItemsFromColumnIndex(int column)
{
  ...
  for (const std::string segmentID : segmentIDs)
  {
    QStandardItem* const item = this->itemFromSegmentID(segmentID.c_str(), column);
    if (item)
    {
...
void qMRMLSegmentsModel::updateItemsFromSegmentID(QString segmentID)
{
  ...
  const QModelIndexList itemIndexes = this->indexes(segmentID);
  for (const QModelIndex itemIndex : itemIndexes)
  {
    QStandardItem* const item = this->itemFromIndex(itemIndex);
...
void qMRMLSegmentsModel::onEvent(vtkObject* caller, unsigned long event, void* clientData, void* callData)
{
  ...
  {
    const char* const segmentIDPtr = reinterpret_cast<const char*>(callData);
    if (segmentIDPtr)
    {
...
void qMRMLSegmentsModel::onSegmentAdded(QString segmentID)
{
  ...
  std::vector<std::string> segmentIDs;
  d->SegmentationNode->GetSegmentation()->GetSegmentIDs(segmentIDs);
  for (const std::string currentSegmentID : segmentIDs)
  {
    const QModelIndex index = this->indexFromSegmentID(currentSegmentID.c_str());
    if (index.isValid())
    {
      continue;
    }
...
void qMRMLSegmentsModel::onSegmentRemoved(QString removedSegmentID)
{
  ...
  {
    const QModelIndex index = this->indexFromSegmentID(removedSegmentID);
  ...
  for (int i = this->rowCount() - 1; i >= 0; --i)
  {
    ...
    const std::string currentSegmentID = this->segmentIDFromIndex(index).toStdString();
    const std::vector<std::string>::iterator currentSegmentIt = std::find(segmentIDs.begin(), segmentIDs.end(), currentSegmentID);
...
void qMRMLSegmentsModel::reorderItems()
{
  ...
  {
    const std::string segmentID = segmentation->GetNthSegmentID(i);
    const QModelIndex index = this->indexFromSegmentID(segmentID.c_str());
    if (index.row() == i)
    {
      continue;
    }
    const QList<QStandardItem*> items = this->takeRow(index.row());
...
QString qMRMLSegmentsModel::terminologyTooltipForSegment(vtkSegment* segment)
{
  ...
  const vtkSmartPointer<vtkSlicerTerminologyEntry> terminologyEntry = vtkSmartPointer<vtkSlicerTerminologyEntry>::New();
