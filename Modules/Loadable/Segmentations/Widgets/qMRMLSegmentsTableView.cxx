...
// Delegate for the name column: mirrors the edited text into NameRole so that
// the model's itemChanged handler picks up the rename and writes it to MRML.
class qMRMLSegmentsNameDelegate : public QStyledItemDelegate
{
public:
  ...
  void setModelData(QWidget* editor, QAbstractItemModel* model, const QModelIndex& index) const override
  {
    ...
    model->setData(index, name, qMRMLSegmentsModel::NameRole);
    // Even if the user leaves the same name in place (e.g., starts editing using F2 and then hits Enter without changing the name)
    // we mark the name as not auto-generated, as the user explicitly approved. If the user cancels editing (e.g., hits Escape during editing)
    // then this method is not called and auto-generated flag does not change.
    model->setData(index, false, qMRMLSegmentsModel::NameAutoGeneratedRole);
  }
};
...
public:
  ...
  /// Creates a QToolButton for the visibility cell at the given proxy row.
  void setupVisibilityButton(int row);
  /// Creates a QToolButton for the status cell at the given proxy row.
  void setupStatusButton(int row);
  /// Creates a flat colored QToolButton for the color cell at the given proxy row.
  void setupColorButton(int row);

  int segmentDropRowForPosition(const QPoint& viewportPosition) const;
  bool isSegmentsTableReorderDrag(QDropEvent* event) const;
  QStringList segmentIDsAfterDisplayedRowMove(const QStringList& currentSegmentIDs, const QStringList& displayedSegmentIDs, const QStringList& movedSegmentIDs, int row) const;
...
{
  return QIcon(qMRMLUtils::createColorPixmap(qApp->style(), color));
}
...
{
  return this->SegmentsTable && event && event->source() == this->SegmentsTable && event->mimeData() && event->mimeData()->hasFormat(SEGMENTS_TABLE_REORDER_MIME_TYPE);
}

//-----------------------------------------------------------------------------
QStringList qMRMLSegmentsTableViewPrivate::segmentIDsAfterDisplayedRowMove(const QStringList& currentSegmentIDs,
                                                                           const QStringList& displayedSegmentIDs,
                                                                           const QStringList& movedSegmentIDs,
                                                                           int row) const
{
...
void qMRMLSegmentsTableViewPrivate::init()
{
  ...
  // Set item delegates: name column mirrors edits into NameRole; opacity column uses the slider delegate
  this->SegmentsTable->setItemDelegateForColumn(this->Model->nameColumn(), new qMRMLSegmentsNameDelegate(this->SegmentsTable));
  ...
  // Visibility and status buttons: create on row insert, update icons on data change.
  QObject::connect(this->SortFilterModel,
                   &QAbstractItemModel::rowsInserted,
                   q,
                   [this, q](const QModelIndex&, int first, int last)
                   {
                     // Defer until after the view has completed its layout so setIndexWidget
                     // places the button at the correct cell rect.
                     QTimer::singleShot(0,
                                        q,
                                        [this, first, last]()
                                        {
                                          ...
                                          {
                                            this->setupVisibilityButton(row);
                                            this->setupStatusButton(row);
                                            this->setupColorButton(row);
                                          }
                                        });
                   });
  QObject::connect(this->SortFilterModel,
                   ...
                   [this](const QModelIndex& topLeft, const QModelIndex& bottomRight, const QVector<int>&)
                   {
                     ...
                     int statusCol = this->Model->statusColumn();
                     for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
                     {
                       if (topLeft.column() <= visCol && visCol <= bottomRight.column())
                       {
                         QModelIndex index = this->SortFilterModel->index(row, visCol);
                         auto* button = qobject_cast<QToolButton*>(this->SegmentsTable->indexWidget(index));
                         if (button)
                         {
                           button->setIcon(index.data(qMRMLSegmentsModel::VisibilityIconRole).value<QIcon>());
                         }
                       }
                       if (topLeft.column() <= statusCol && statusCol <= bottomRight.column())
                       {
                         QModelIndex index = this->SortFilterModel->index(row, statusCol);
                         auto* button = qobject_cast<QToolButton*>(this->SegmentsTable->indexWidget(index));
                         if (button)
                         {
                           button->setIcon(index.data(qMRMLSegmentsModel::StatusIconRole).value<QIcon>());
                         }
                       }
                       int colorCol = this->Model->colorColumn();
                       if (topLeft.column() <= colorCol && colorCol <= bottomRight.column())
                       {
                         QModelIndex index = this->SortFilterModel->index(row, colorCol);
                         auto* button = qobject_cast<QToolButton*>(this->SegmentsTable->indexWidget(index));
                         if (button)
                         {
                           button->setIcon(colorSwatchIcon(index.data(qMRMLSegmentsModel::ColorRole).value<QColor>()));
                         }
                       }
                     }
                   });
...
//-----------------------------------------------------------------------------
void qMRMLSegmentsTableViewPrivate::setupVisibilityButton(int row)
{
  Q_Q(qMRMLSegmentsTableView);
  QModelIndex proxyIndex = this->SortFilterModel->index(row, this->Model->visibilityColumn());
  if (!proxyIndex.isValid())
  {
    return;
  }
  QString segmentId = this->SortFilterModel->segmentIDFromIndex(proxyIndex);
  ...
  button->setIcon(proxyIndex.data(qMRMLSegmentsModel::VisibilityIconRole).value<QIcon>());
  ...
  QObject::connect(button,
                   ...
                   [this, q, segmentId]()
                   {
                     QStandardItem* item = this->Model->itemFromSegmentID(segmentId);
                     if (item && this->SegmentationNode)
                     {
                       bool visible = (item->data(qMRMLSegmentsModel::VisibilityRole).toInt() != 0);
                       q->setSegmentVisibility(segmentId, !visible, -1, -1, -1);
                     }
                   });

  this->SegmentsTable->setIndexWidget(proxyIndex, button);
}

//-----------------------------------------------------------------------------
void qMRMLSegmentsTableViewPrivate::setupStatusButton(int row)
{
  Q_Q(qMRMLSegmentsTableView);
  QModelIndex proxyIndex = this->SortFilterModel->index(row, this->Model->statusColumn());
  if (!proxyIndex.isValid())
  {
    return;
  }
  ...
  button->setIconSize(QSize(iconSize, iconSize));
  button->setStyleSheet("QToolButton { padding: 0px; }");
  button->setIcon(proxyIndex.data(qMRMLSegmentsModel::StatusIconRole).value<QIcon>());
  ...
  QObject::connect(button,
                   ...
                   [this, segmentId]()
                   {
                     vtkSegment* segment = this->SegmentationNode ? this->SegmentationNode->GetSegmentation()->GetSegment(segmentId.toStdString()) : nullptr;
                     if (!segment)
                     {
                       return;
                     }
                     int status = vtkSlicerSegmentationsModuleLogic::GetSegmentStatus(segment);
                     if (status == vtkSlicerSegmentationsModuleLogic::SegmentStatus::Flagged)
                     {
...
//-----------------------------------------------------------------------------
void qMRMLSegmentsTableViewPrivate::setupColorButton(int row)
{
  Q_Q(qMRMLSegmentsTableView);
  QModelIndex proxyIndex = this->SortFilterModel->index(row, this->Model->colorColumn());
  if (!proxyIndex.isValid())
  {
    return;
  }
  ...
  button->setIconSize(QSize(iconSize, iconSize));
  button->setStyleSheet("QToolButton { padding: 0px; }");
  button->setIcon(colorSwatchIcon(proxyIndex.data(qMRMLSegmentsModel::ColorRole).value<QColor>()));
  ...
  QObject::connect(button,
                   ...
                   [q, persistentIndex]()
                   {
                     ...
                     {
                       q->onSegmentsTableDoubleClicked(persistentIndex);
                     }
                   });
...
void qMRMLSegmentsTableView::onSegmentsTableDoubleClicked(const QModelIndex& modelIndex)
{
  ...
  if (modelIndex.column() == d->Model->colorColumn() || modelIndex.column() == d->Model->nameColumn())
  {
    if (this->useTerminologySelector())
    {
      ...
      QString name = modelIndex.model()->data(nameIndex, qMRMLSegmentsModel::NameRole).toString();
      bool nameAutoGenerated = modelIndex.model()->data(nameIndex, qMRMLSegmentsModel::NameAutoGeneratedRole).toBool();
      QColor color = modelIndex.model()->data(nameIndex, qMRMLSegmentsModel::ColorRole).value<QColor>();
      bool colorAutoGenerated = modelIndex.model()->data(nameIndex, qMRMLSegmentsModel::ColorAutoGeneratedRole).toBool();
      QColor generatedColor = modelIndex.model()->data(nameIndex, qMRMLSegmentsModel::GeneratedColorRole).value<QColor>();
    ...
    else
    {
      ...
      {
        QColor color = modelIndex.model()->data(modelIndex, qMRMLSegmentsModel::ColorRole).value<QColor>();
      ...
      else // name column: start inline rename
      {
...
void qMRMLSegmentsTableView::setReadOnly(bool aReadOnly)
{
  ...
  {
    d->SegmentsTable->setEditTriggers(QAbstractItemView::EditKeyPressed);
...
bool qMRMLSegmentsTableView::moveSegmentsToRow(QStringList segmentIDs, int row)
{
  ...
  QStringList reorderedSegmentIDs = d->segmentIDsAfterDisplayedRowMove(currentSegmentIDsQt, displayedSegmentIDs, movedSegmentIDs, row);
...
void qMRMLSegmentsTableView::modelReset()
{
  ...
  // Model reset invalidates all persistent indices; recreate visibility and status buttons.
  for (int row = 0; row < d->SortFilterModel->rowCount(); ++row)
  {
    d->setupVisibilityButton(row);
    d->setupStatusButton(row);
    d->setupColorButton(row);
  }
}
