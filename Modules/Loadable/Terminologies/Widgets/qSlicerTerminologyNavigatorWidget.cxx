...
void qSlicerTerminologyNavigatorWidgetPrivate::init()
{
  ...
  QObject::connect(
    this->ComboBox_AnatomicContext, SIGNAL(currentIndexChanged(int)), q, SLOT(onAnatomicContextSelectionChanged(int)));
  QObject::connect(this->tableWidget_AnatomicRegion,
                   SIGNAL(currentItemChanged(QTableWidgetItem*, QTableWidgetItem*)),
                   q,
                   SLOT(onRegionSelected(QTableWidgetItem*, QTableWidgetItem*)));
  QObject::connect(this->ComboBox_AnatomicRegionModifier,
                   SIGNAL(currentIndexChanged(int)),
                   q,
                   SLOT(onRegionModifierSelectionChanged(int)));
  QObject::connect(
    this->SearchBox_AnatomicRegion, SIGNAL(textChanged(QString)), q, SLOT(onRegionSearchTextChanged(QString)));
  ...
  if (qSlicerApplication::application())
  {
    ...
    this->AnatomicalRegionExpandButton->setChecked(
      settings->value("Terminology/ShowAnatomicalRegionSelector", false).toBool());
  }
  ...
  QObject::connect(this->pushButton_LoadAnatomicContext, SIGNAL(clicked()), q, SLOT(onLoadAnatomicContextClicked()));
                                                                         ...
                                                                         vtkSlicerTerminologyType* type)
{
  ...
  vtkSlicerTerminologiesModuleLogic::CodeIdentifier typeId =
    vtkSlicerTerminologiesModuleLogic::CodeIdentifierFromTerminologyType(type);
...
bool qSlicerTerminologyNavigatorWidget::setTerminologyEntry(vtkSlicerTerminologyEntry* entry)
{
  ...
  {
    qCritical() << Q_FUNC_INFO << ": Failed to find category with name "
                << (categoryObject->GetCodeMeaning() ? categoryObject->GetCodeMeaning() : "NULL");
    returnValue = false;
  }
  ...
  {
    qCritical() << Q_FUNC_INFO << ": Failed to find type with name "
                << (typeObject->GetCodeMeaning() ? typeObject->GetCodeMeaning() : "NULL");
    returnValue = false;
  }
...
void qSlicerTerminologyNavigatorWidget::populateCategoryTable()
{
  ...
  for (idIt = categories.begin(); idIt != categories.end(); ++idIt, ++index)
  {
    ...
    if (d->CurrentCategoryObject->GetCodingSchemeDesignator()
        && !addedCategoryId.CodingSchemeDesignator.compare(d->CurrentCategoryObject->GetCodingSchemeDesignator())
        && d->CurrentCategoryObject->GetCodeValue()
        && !addedCategoryId.CodeValue.compare(d->CurrentCategoryObject->GetCodeValue()))
    {
...
void qSlicerTerminologyNavigatorWidget::populateTypeTable()
{
  ...
  for (idIt = types.begin(); idIt != types.end(); ++idIt, ++typeIndex)
  {
    ...
    if (d->CurrentTypeObject->GetCodingSchemeDesignator()
        && !addedTypeId.CodingSchemeDesignator.compare(d->CurrentTypeObject->GetCodingSchemeDesignator())
...
void qSlicerTerminologyNavigatorWidget::populateTypeModifierComboBox()
{
  ...
  if (d->CurrentTerminologyName.isEmpty() || !d->CurrentCategoryObject || !d->CurrentTypeObject
      || !d->CurrentTypeObject->GetCodeValue())
  {
...
{
  Q_D(qSlicerTerminologyNavigatorWidget);
  const QString& anatomicContextFileName = QFileDialog::getOpenFileName(
    this, "Select anatomic context json file...", QString(), "Json files (*.json);; All files (*)");
  if (!anatomicContextFileName.isEmpty())
  {
    ...
    QString loadedContextName =
      logic->LoadAnatomicContextFromFile(anatomicContextFileName.toUtf8().constData()).c_str();
    if (!loadedContextName.isEmpty())
    {
      QMessageBox::information(
        this, "Load succeeded", QString("Loading anatomic context named %1 succeeded").arg(loadedContextName));

      this->copyContextToUserDirectory(anatomicContextFileName);
    }
    else
    {
      QString errorMessage = "Loading anatomic context from file %1 failed!<br><br>"
                             "Please check validity of the file using the <a "
                             "href=\"https://qiicr.org/dcmqi/#/validators\">online validator tool</a>.";
...
void qSlicerTerminologyNavigatorWidget::copyContextToUserDirectory(QString filePath)
{
  ...
  if (QFile::exists(targetFilePath))
  {
    ...
    QMessageBox::StandardButton answer = QMessageBox::question(
      nullptr, tr("Context file exists"), message, QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
...
void qSlicerTerminologyNavigatorWidget::populateAnatomicContextComboBox()
{
  ...
  for (std::vector<std::string>::iterator anIt = anatomicRegionContextNames.begin();
       anIt != anatomicRegionContextNames.end();
       ++anIt)
  {
...
void qSlicerTerminologyNavigatorWidget::populateRegionTable()
{
  ...
  logic->FindRegionsInAnatomicContext(d->CurrentAnatomicContextName.toUtf8().constData(),
                                      regions,
                                      d->SearchBox_AnatomicRegion->text().toUtf8().constData());
  ...
  for (idIt = regions.begin(); idIt != regions.end(); ++idIt, ++index)
  {
    ...
    if (d->CurrentRegionObject->GetCodingSchemeDesignator()
        && !addedRegionId.CodingSchemeDesignator.compare(d->CurrentRegionObject->GetCodingSchemeDesignator())
        && d->CurrentRegionObject->GetCodeValue()
        && !addedRegionId.CodeValue.compare(d->CurrentRegionObject->GetCodeValue()))
    {
...
void qSlicerTerminologyNavigatorWidget::populateRegionModifierComboBox()
{
  ...
  for (idIt = regionModifiers.begin(); idIt != regionModifiers.end(); ++idIt, ++index)
  {
    ...
    QMap<QString, QVariant> userData;
    userData[QString::number(CodingSchemeDesignatorRole)] =
...
void qSlicerTerminologyNavigatorWidget::onRegionSelected(QTableWidgetItem* currentItem, QTableWidgetItem* previousItem)
{
  ...
  if (!logic->GetRegionInAnatomicContext(d->CurrentAnatomicContextName.toUtf8().constData(), regionId, region))
  {
