...
{
  Q_D(qSlicerTerminologySelectorButton);
  const bool usedCustomNameOrColor = (d->TerminologyInfo.NameAutoGenerated == false || d->TerminologyInfo.ColorAutoGenerated == false);
  if (qSlicerTerminologySelectorDialog::getTerminology(d->TerminologyInfo, this))
  {
    this->update();
    emit terminologyChanged();

    const bool nowUsingCustomNameOrColor = (d->TerminologyInfo.NameAutoGenerated == false || d->TerminologyInfo.ColorAutoGenerated == false);
    if (nowUsingCustomNameOrColor && !usedCustomNameOrColor)
    {
      ...
      {
        const char* const contextName = d->TerminologyInfo.GetTerminologyEntry()->GetTerminologyContextName();
        if (contextName)
        {
