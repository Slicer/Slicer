...
//---------------------------------------------------------------------------
int vtkSlicerSubjectHierarchyModuleLogicTest(int vtkNotUsed(argc), char* vtkNotUsed(argv)[])
{
  if (!TestExpand())
  {
...
bool PopulateScene(vtkMRMLScene* scene)
{
  ...
  if (expectedNodeCount != currentNodeCount)
  {
    std::cerr << "Line " << __LINE__ << " - Problem with PopulateScene\n"
              ...
              << "\texpectedNodeCount: " << expectedNodeCount << std::endl;
    return false;
  }
...
bool TestAssociations()
{
  ...
  vtkMRMLScalarVolumeNode* volume1Node =
    vtkMRMLScalarVolumeNode::SafeDownCast(shNode->GetItemDataNode(volume1ShItemID));
  if (!volume1Node)
  {
...
bool TestTreeOperations()
{
  ...
  if (patientShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
      ...
      || volume2ShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
  ...
  if (currentChildCount != expectedChildCount)
  {
    std::cerr << "Line " << __LINE__ << " - Failed to find associated children nodes:\n"
              ...
              << "\texpected count: " << expectedChildCount << std::endl;
    return false;
  }
  ...
  std::string attributeValueStudyLevel = shNode->GetAttributeFromItemAncestor(
    volume2ShItemID, STUDY_ATTRIBUTE_NAME, vtkMRMLSubjectHierarchyConstants::GetDICOMLevelStudy());
  ...
  std::string attributeValueWrongLevel = shNode->GetAttributeFromItemAncestor(
    volume2ShItemID, STUDY_ATTRIBUTE_NAME, vtkMRMLSubjectHierarchyConstants::GetDICOMLevelPatient());
...
bool TestVisibilityOperations()
{
  ...
  if (patientShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
      ...
      || volume1ShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
      || model1ShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
      || volume2ShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
      || model21ShItemID == vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID
  ...
  vtkMRMLScalarVolumeNode* volume2Node =
    vtkMRMLScalarVolumeNode::SafeDownCast(shNode->GetItemDataNode(volume2ShItemID));
  ...
  vtkMRMLScalarVolumeDisplayNode* volume1DisplayNode =
    vtkMRMLScalarVolumeDisplayNode::SafeDownCast(volume1Node->GetDisplayNode());
  vtkMRMLModelDisplayNode* model1DisplayNode = vtkMRMLModelDisplayNode::SafeDownCast(model1Node->GetDisplayNode());
  vtkMRMLScalarVolumeDisplayNode* volume2DisplayNode =
    vtkMRMLScalarVolumeDisplayNode::SafeDownCast(volume2Node->GetDisplayNode());
...
bool TestTransformBranch()
{
  ...
  if (currentTransformNode != expectedTransformNode)
  {
    std::cerr << "Line " << __LINE__ << " - Failed to apply transform1 on model1:\n"
              << "\tcurrentTransformNode: " << (currentTransformNode ? currentTransformNode->GetID() : "<null>") << "\n"
              << "\texpectedTransformNode: " << (expectedTransformNode ? expectedTransformNode->GetID() : "<null>")
              << std::endl;
    return false;
  }
  if (!shNode->IsAnyNodeInBranchTransformed(study2ShItemID))
  ...
  {
    std::cerr << transformNode2->GetName() << " was not hardened on model21 when applying " << transformNode2->GetName()
              << " on " << shNode->GetItemName(study2ShItemID) << std::endl;
    return false;
  }
