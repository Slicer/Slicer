...
void qSlicerSubjectHierarchyVisibilityPluginPrivate::init()
{
  ...
  this->ShowInAllViewsAction = new QAction(qSlicerSubjectHierarchyVisibilityPlugin::tr("Show in all views"), q);
  QObject::connect(this->ShowInAllViewsAction, SIGNAL(triggered()), q, SLOT(showInAllViews()));

  this->ToggleVisibility2DAction = new QAction(qSlicerSubjectHierarchyVisibilityPlugin::tr("2D visibility"), q);
...
void qSlicerSubjectHierarchyVisibilityPlugin::showVisibilityContextMenuActionsForItem(vtkIdType itemID)
{
  ...
  const vtkSmartPointer<vtkCollection> childDisplayableNodes = vtkSmartPointer<vtkCollection>::New();
  ...
  for (int i = 0; i < childDisplayableNodes->GetNumberOfItems(); ++i)
  {
    ...
    vtkMRMLScalarVolumeNode* const volumeNode = vtkMRMLScalarVolumeNode::SafeDownCast(childDisplayableNodes->GetItemAsObject(i));
