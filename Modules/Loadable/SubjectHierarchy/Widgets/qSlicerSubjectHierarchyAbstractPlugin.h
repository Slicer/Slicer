...
/// subject hierarchy items. To exercise the default implementations, a Default plugin \sa
...
///   \sa canOwnSubjectHierarchyItem - in case of the latter two the node mentioned below is the data node associated to
///   the item):
...
/// 0.3 = It is likely that other plugins will be able to handle the node in question better (typical value for plugins
/// for generic types, such as Volumes) 0.5 = The plugin has equal chance to handle this node as others (an example can
/// be color table node) 0.7 = The plugin is likely be the only one that can handle the node in question, but there is a
/// chance that other plugins can do that too 1.0 = The node in question can only be handled by the plugin (by node type
...
public:
  ...
  Q_INVOKABLE virtual bool showItemInView(vtkIdType itemID,
...
public:
  ...
  Q_INVOKABLE static void setActionPosition(QAction* action,
                                            int section,
                                            int weight = 0,
