...
void qSlicerCamerasModuleWidget::setup()
{
  ...
  connect(d->CameraNodeSelector, SIGNAL(currentNodeChanged(vtkMRMLNode*)), this, SLOT(setCameraToCurrentView(vtkMRMLNode*)));
  connect(d->CameraNodeSelector, SIGNAL(nodeAdded(vtkMRMLNode*)), this, SLOT(onCameraNodeAdded(vtkMRMLNode*)));
  connect(d->CameraNodeSelector, SIGNAL(nodeAboutToBeRemoved(vtkMRMLNode*)), this, SLOT(onCameraNodeRemoved(vtkMRMLNode*)));
}

//-----------------------------------------------------------------------------
void qSlicerCamerasModuleWidget::onCurrentViewNodeChanged(vtkMRMLNode* mrmlNode)
{
  vtkMRMLViewNode* const currentViewNode = vtkMRMLViewNode::SafeDownCast(mrmlNode);
...
{
  Q_D(qSlicerCamerasModuleWidget);
  vtkMRMLViewNode* const currentViewNode = vtkMRMLViewNode::SafeDownCast(d->ViewNodeSelector->currentNode());
  this->synchronizeCameraWithView(currentViewNode);
}
...
void qSlicerCamerasModuleWidget::synchronizeCameraWithView(vtkMRMLViewNode* currentViewNode)
{
  ...
  vtkSlicerCamerasModuleLogic* camerasLogic = vtkSlicerCamerasModuleLogic::SafeDownCast(this->logic());
  vtkMRMLCameraNode* const found_camera_node = camerasLogic->GetViewActiveCameraNode(currentViewNode);
...
{
  Q_D(qSlicerCamerasModuleWidget);
  vtkMRMLCameraNode* currentCameraNode = vtkMRMLCameraNode::SafeDownCast(mrmlNode);
  if (!currentCameraNode)
  { // if the camera list is empty, there is no current camera
    return;
  }
  vtkMRMLViewNode* currentViewNode = vtkMRMLViewNode::SafeDownCast(d->ViewNodeSelector->currentNode());
...
{
  vtkMRMLCameraNode* const cameraNode = vtkMRMLCameraNode::SafeDownCast(mrmlNode);
  if (!cameraNode)
  {
    // Q_ASSERT(cameraNode);
    return;
  }
  this->qvtkConnect(cameraNode, vtkMRMLCameraNode::LayoutNameModifiedEvent, this, SLOT(synchronizeCameraWithView()));
}
...
void qSlicerCamerasModuleWidget::onCameraNodeRemoved(vtkMRMLNode* mrmlNode)
{
  ...
  this->qvtkDisconnect(cameraNode, vtkMRMLCameraNode::LayoutNameModifiedEvent, this, SLOT(synchronizeCameraWithView()));
}
...
bool qSlicerCamerasModuleWidget::setEditedNode(vtkMRMLNode* node, QString role /* = QString()*/, QString context /* = QString()*/)
{
  ...
  if (vtkMRMLCameraNode::SafeDownCast(node))
  {
    ...
    vtkMRMLViewNode* const viewNode = vtkMRMLViewNode::SafeDownCast(this->mrmlScene()->GetSingletonNode(cameraNode->GetLayoutName(), "vtkMRMLViewNode"));
    if (!viewNode)
    {
      return false;
    }
