...
void vtkSlicerAngleRepresentation2D::BuildArc()
{
  ...
  bool longArc = (angle > 180.0 || angle < 0.0) && (markupsNode->GetAngleMode() == vtkMRMLMarkupsAngleNode::OrientedPositive);
  ...
  const double anglePlacementRatio = 0.5 * (longArc ? 0.5 : 1.0);
  ...
  length = l1 < l2 ? l1 : l2;
  const double angleTextPlacementRatio = 0.7 * (longArc ? 0.5 : 1.0);
