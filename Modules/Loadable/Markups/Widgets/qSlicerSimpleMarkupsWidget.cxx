...
void qSlicerSimpleMarkupsWidget::setup()
{
  ...
  connect(d->MarkupsPlaceWidget,
          SIGNAL(activeMarkupsPlaceModeChanged(bool)),
          this,
          SIGNAL(activeMarkupsPlaceModeChanged(bool)));

  d->MarkupsControlPointsTableWidget->setColumnCount(CONTROL_POINT_COLUMNS);
  d->MarkupsControlPointsTableWidget->setHorizontalHeaderLabels(QStringList() << tr("Label") << tr("R") //: right
                                                                              << tr("A")                //: anterior
                                                                              << tr("S")                //: superior
  );
  ...
  d->MarkupsControlPointsTableWidget->setSelectionBehavior(
    QAbstractItemView::SelectRows); // only select rows rather than cells

  connect(d->MarkupsControlPointsTableWidget,
          SIGNAL(customContextMenuRequested(const QPoint&)),
          this,
          SLOT(onMarkupsControlPointsTableContextMenu(const QPoint&)));
  connect(d->MarkupsControlPointsTableWidget,
          SIGNAL(cellChanged(int, int)),
          this,
          SLOT(onMarkupsControlPointEdited(int, int)));
  // listen for click on a markup
  connect(d->MarkupsControlPointsTableWidget,
          SIGNAL(cellClicked(int, int)),
          this,
          SLOT(onMarkupsControlPointSelected(int, int)));
...
{
  qWarning("qSlicerSimpleMarkupsWidget::getCurrentNode() method is deprecated. Use "
           "qSlicerSimpleMarkupsWidget::currentNode() method instead");
...
void qSlicerSimpleMarkupsWidget::setCurrentNode(vtkMRMLNode* currentNode)
{
  ...
  this->qvtkReconnect(
    d->CurrentMarkupsNode, currentMarkupsNode, vtkMRMLMarkupsNode::PointAddedEvent, this, SLOT(onPointAdded()));
  this->qvtkReconnect(
    d->CurrentMarkupsNode, currentMarkupsNode, vtkMRMLMarkupsNode::PointRemovedEvent, this, SLOT(updateWidget()));
  this->qvtkReconnect(
    d->CurrentMarkupsNode, currentMarkupsNode, vtkMRMLMarkupsNode::PointModifiedEvent, this, SLOT(updateWidget()));
...
void qSlicerSimpleMarkupsWidget::onMarkupsControlPointsTableContextMenu(const QPoint& position)
{
  ...
  {
    qCritical(
      "qSlicerSimpleMarkupsWidget::onMarkupsControlPointsTableContextMenu failed: Markups module logic is invalid");
    return;
  }
  ...
  {
    d->MarkupsLogic->JumpSlicesToNthPointInMarkup(
      this->currentNode()->GetID(), currentControlPoint, true /* centered */, d->ViewGroup);
  }
...
void qSlicerSimpleMarkupsWidget::updateWidget()
{
  ...
  if (d->MarkupsControlPointsTableWidget->rowCount() == currentMarkupsNode->GetNumberOfControlPoints())
  {
    ...
    for (int i = 0; i < currentMarkupsNode->GetNumberOfControlPoints(); i++)
    {
      ...
      d->MarkupsControlPointsTableWidget->item(i, CONTROL_POINT_LABEL_COLUMN)
        ->setText(QString::fromStdString(controlPointLabel));
      d->MarkupsControlPointsTableWidget->item(i, CONTROL_POINT_X_COLUMN)
        ->setText(QString::number(controlPointPosition[0], 'f', 3));
      d->MarkupsControlPointsTableWidget->item(i, CONTROL_POINT_Y_COLUMN)
        ->setText(QString::number(controlPointPosition[1], 'f', 3));
      d->MarkupsControlPointsTableWidget->item(i, CONTROL_POINT_Z_COLUMN)
        ->setText(QString::number(controlPointPosition[2], 'f', 3));
    }
  }
  else
  {
