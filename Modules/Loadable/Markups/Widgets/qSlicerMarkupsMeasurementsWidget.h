...
class Q_SLICER_MODULE_MARKUPS_WIDGETS_EXPORT qSlicerMarkupsMeasurementsWidget : public qSlicerWidget
{
  Q_OBJECT
...
signals:
  /// Emitted when the user successfully renames a measurement via double-click or Enter key.
  /// \param markupsNode The renamed markups node.
  /// \param newName The new name assigned to the node.
  void measurementRenamed(vtkMRMLMarkupsNode* markupsNode, const QString& newName);
...
public slots:
  ...
  void onActiveMarkupItemClicked(QModelIndex);
  void onActiveMarkupItemChanged(vtkIdType);
  /// Trigger rename on double-click
  void onActiveMarkupItemDoubleClicked(QModelIndex);
