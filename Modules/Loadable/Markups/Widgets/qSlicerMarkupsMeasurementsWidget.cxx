...
void qSlicerMarkupsMeasurementsWidgetPrivate::setupUi(qSlicerMarkupsMeasurementsWidget* widget)
{
  ...
  QObject::connect(this->measurementsListView, SIGNAL(clicked(QModelIndex)),
    q, SLOT(onActiveMarkupItemClicked(QModelIndex)));
...
void qSlicerMarkupsMeasurementsWidgetPrivate::jumpAllSlices(vtkMRMLSliceNode* exclude /* =nullptr */)
{
  ...
  vtkMRMLSliceNode::JumpAllSlices(q->mrmlScene(), boundsCenter[0], boundsCenter[1], boundsCenter[2],
    vtkMRMLSliceNode::CenteredJumpSlice, this->ViewGroup, exclude);
  ...
  for (crosshairNodes->InitTraversal(it);(crosshairNode = static_cast<vtkMRMLCrosshairNode*>(
                                   crosshairNodes->GetNextItemAsObject(it)));)
  {
