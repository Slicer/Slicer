...
void qSlicerMarkupsMeasurementsWidgetPrivate::setupUi(qSlicerMarkupsMeasurementsWidget* widget)
{
  ...
  // double-click or Enter triggers rename
  QObject::connect(this->measurementsListView, SIGNAL(doubleClicked(QModelIndex)), q, SLOT(onActiveMarkupItemDoubleClicked(QModelIndex)));
  this->measurementsListView->installEventFilter(q);
...
//-----------------------------------------------------------------------------
void qSlicerMarkupsMeasurementsWidget::onActiveMarkupItemDoubleClicked(QModelIndex)
{
  this->onRenameMeasurement();
}

//-----------------------------------------------------------------------------
bool qSlicerMarkupsMeasurementsWidget::eventFilter(QObject* obj, QEvent* event)
{
  Q_D(qSlicerMarkupsMeasurementsWidget);
  if (obj == d->measurementsListView && event->type() == QEvent::KeyPress)
  {
...
void qSlicerMarkupsMeasurementsWidget::onRenameMeasurement()
{
  ...
  if (d->CurrentMarkupsNode->IsA("vtkMRMLMarkupsFiducialNode"))
  {
    d->CurrentMarkupsNode->SetNthControlPointLabel(0, newName.toStdString());
  }

  emit measurementRenamed(d->CurrentMarkupsNode, newName);
}
