...
//-----------------------------------------------------------------------------
class qSlicerMarkupsPlaceWidgetPrivate : public Ui_qSlicerMarkupsPlaceWidget
{
...
public:
  ...
  bool LastSignaledPlaceModeEnabled; // if placeModeEnabled changes compared to this value then a
                                     // activeMarkupsPlaceModeChanged signal will be emitted
...
//-----------------------------------------------------------------------------
qSlicerMarkupsPlaceWidget::qSlicerMarkupsPlaceWidget(QWidget* parentWidget)
  : Superclass(parentWidget)
  , d_ptr(new qSlicerMarkupsPlaceWidgetPrivate(*this))
{
  this->setup();
}
...
void qSlicerMarkupsPlaceWidget::setup()
{
  ...
  if (!d->MarkupsLogic)
  {
    qCritical() << Q_FUNC_INFO
                << ": Markups module is not found, some markup manipulation features will not be available";
  }
  ...
  QObject::connect(
    d->ActionFixedNumberOfControlPoints, SIGNAL(triggered()), this, SLOT(onFixedNumberOfControlPointsButtonClicked()));
...
void qSlicerMarkupsPlaceWidget::setSelectionNode(vtkMRMLSelectionNode* selectionNode)
{
  ...
  this->qvtkReconnect(d->SelectionNode,
                      selectionNode,
                      vtkMRMLSelectionNode::ActivePlaceNodePlacementValidEvent,
                      this,
                      SLOT(updateWidget()));
...
void qSlicerMarkupsPlaceWidget::setCurrentNode(vtkMRMLNode* currentNode)
{
  ...
  this->qvtkReconnect(
    d->CurrentMarkupsNode, currentMarkupsNode, vtkMRMLMarkupsNode::PointAddedEvent, this, SLOT(updateWidget()));
  this->qvtkReconnect(
    d->CurrentMarkupsNode, currentMarkupsNode, vtkMRMLMarkupsNode::PointRemovedEvent, this, SLOT(updateWidget()));
  this->qvtkReconnect(d->CurrentMarkupsNode,
                      currentMarkupsNode,
...
bool qSlicerMarkupsPlaceWidget::currentNodeActive() const
{
  ...
  vtkMRMLMarkupsNode* currentMarkupsNode = this->currentMarkupsNode();
  if (d->MarkupsLogic == nullptr || this->mrmlScene() == nullptr || currentMarkupsNode == nullptr
      || d->InteractionNode == nullptr || d->SelectionNode == nullptr)
  {
    return false;
  }
  ...
  bool placeNodeClassNameMatches =
    activePlaceNodeClassName && std::string(activePlaceNodeClassName).compare(currentMarkupsNode->GetClassName()) == 0;
...
bool qSlicerMarkupsPlaceWidget::placeModeEnabled() const
{
  ...
  vtkMRMLMarkupsNode* currentMarkupsNode = this->currentMarkupsNode();
  if (d->SelectionNode == nullptr || d->InteractionNode == nullptr || this->mrmlScene() == nullptr
      || currentMarkupsNode == nullptr)
  {
    return false;
  }
...
void qSlicerMarkupsPlaceWidget::setPlaceModeEnabled(bool placeEnable)
{
  ...
  if (d->MarkupsLogic == nullptr || this->mrmlScene() == nullptr || d->InteractionNode == nullptr
      || this->currentMarkupsNode() == nullptr)
  {
    if (placeEnable)
    {
...
void qSlicerMarkupsPlaceWidget::updateWidget()
{
  ...
  if (fixedNumberControlPoints)
  {
    ...
    d->DeleteButton->setToolTip(qSlicerMarkupsPlaceWidget::tr(
      "Unset position of the last control point placed (the control point will not be deleted)."));
  }
  else
  {
  ...
  d->ActionUnsetLast->setVisible(
    !fixedNumberControlPoints
    && d->UnsetLastControlPointOptionVisible); // QToolButton button action does this so don't also have in menu
...
void qSlicerMarkupsPlaceWidget::updateDeleteButton()
{
  ...
  {
    bool showMenu =
      (d->ActionDeleteAll->isVisible() || d->ActionUnsetLast->isVisible() || d->ActionUnsetAll->isVisible());
    ...
    d->DeleteButton->setVisible(
      showMenu || !currentMarkupsNode->GetFixedNumberOfControlPoints()); // hide when no options to show
  }
}
...
void qSlicerMarkupsPlaceWidget::setMRMLScene(vtkMRMLScene* scene)
{
  ...
  {
    selectionNode =
      vtkMRMLSelectionNode::SafeDownCast(d->MarkupsLogic->GetMRMLScene()->GetNodeByID("vtkMRMLSelectionNodeSingleton"));
    interactionNode = vtkMRMLInteractionNode::SafeDownCast(
      d->MarkupsLogic->GetMRMLScene()->GetNodeByID("vtkMRMLInteractionNodeSingleton"));
  }
...
void qSlicerMarkupsPlaceWidget::setPlaceMultipleMarkups(PlaceMultipleMarkupsType option)
{
  ...
  if (d->PlaceButton)
  {
    ...
    d->PlaceButton->setPopupMode(d->PlaceMultipleMarkups == ShowPlaceMultipleMarkupsOption
...
QColor qSlicerMarkupsPlaceWidget::nodeColor() const
{
  ...
  color.setRgb(
    static_cast<int>(rgbDoubleVector[0]), static_cast<int>(rgbDoubleVector[1]), static_cast<int>(rgbDoubleVector[2]));
  return color;
}
