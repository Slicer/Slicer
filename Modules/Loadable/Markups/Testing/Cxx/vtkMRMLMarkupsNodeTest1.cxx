...
int vtkMRMLMarkupsNodeTest1(int, char*[])
{
  ...
  for (int n = 0; n < node1->GetNumberOfControlPoints(); ++n)
  {
    ...
    std::cout << n << ": orientation = " << orientation[0] << "," << orientation[1] << "," << orientation[2] << "," << orientation[3] << std::endl;
    // test for default
  ...
  node1->SetNthControlPointOrientation(0, testOrientation[0], testOrientation[1], testOrientation[2], testOrientation[3]);
  ...
  for (int r = 0; r < 4; r++)
  {
    if (newOrientation[r] != testOrientation[r])
    {
      std::cerr << "Failed to set orientation! "
                << "Expected: " << testOrientation[0] << ", " << testOrientation[1] << ", " << testOrientation[2] << ", " << testOrientation[3] << " but got: " << newOrientation[0]
                << ", " << newOrientation[1] << ", " << newOrientation[2] << ", " << newOrientation[3] << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  if (p1.GetX() != p0[0] || //
      ...
      p1.GetZ() != p0[2])
  {
    std::cerr << "Failed to get back markup 2 point 1 via vector, expected " << p0[0] << "," << p0[1] << "," << p0[2] << ", but got " << p1.GetX() << "," << p1.GetY() << ","
              << p1.GetZ() << std::endl;
    return EXIT_FAILURE;
  }
  else
  ...
  if (p3[0] != p0[0] || //
      ...
      p3[2] != p0[2])
  {
    std::cerr << "Failed to get back markup 2 point 1 via array, expected " << p0[0] << "," << p0[1] << "," << p0[2] << ", but got " << p3[0] << "," << p3[1] << "," << p3[2]
              << std::endl;
    return EXIT_FAILURE;
  }
  else
