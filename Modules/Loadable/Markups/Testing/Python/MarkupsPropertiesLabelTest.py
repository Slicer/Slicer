...
class MarkupsPropertiesLabelTest(unittest.TestCase):
    ...
    def testLabelClick(self):
        ...
        self.assertEqual(self.display.GetActiveComponentType(), self.display.ComponentPropertiesLabel)
        initialPoints = [tuple(self.angle.GetNthControlPointPositionVector(i)) for i in range(3)]
        # Label is not draggable: the button press is not consumed (same as on a markup line)
