...
void qSlicerSubjectHierarchyMarkupsPluginPrivate::jumpToPoint(int controlPointIndex)
{
  ...
  vtkSlicerMarkupsLogic* markupsLogic =
    vtkSlicerMarkupsLogic::SafeDownCast(appLogic->GetModuleLogic(/*no tr*/ "Markups"));
  if (!markupsLogic)
  {
...
void qSlicerSubjectHierarchyMarkupsPlugin::showVisibilityContextMenuActionsForItem(vtkIdType itemID)
{
  ...
  if (associatedNode)
  {
    ...
    d->ToggleCurrentItemScaleHandleVisible->setVisible(
      vtkMRMLMarkupsROIDisplayNode::SafeDownCast(displayNode) != nullptr ||
...
void qSlicerSubjectHierarchyMarkupsPlugin::renamePoint()
{
  ...
  bool ok = false;
  QString newName =
    QInputDialog::getText(nullptr, tr("Rename %1").arg(oldName), tr("New name:"), QLineEdit::Normal, oldName, &ok);
  if (!ok)
  {
    return;
  }
...
void qSlicerSubjectHierarchyMarkupsPlugin::editNodeTerminology()
{
  ...
  {
    vtkSlicerTerminologiesModuleLogic::GetDefaultTerminologyEntry(markupsNode, terminologyEntry);
  }
  ...
  if (oldColor != terminologyInfo.Color || newTerminologyString.compare(oldTerminologyString.c_str()))
  {
    ...
    // DefaultTerminologyRole is skipped, as the terminology selector does not change it
