...
//-----------------------------------------------------------------------------
QWidget* qSlicerColorTableTerminologyDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
void qSlicerColorTableTerminologyDelegate::setEditorData(QWidget* editor, const QModelIndex& index) const
{
  ...
  if (editorButton)
  {
    ...
    const int colorIndex = index.data(qMRMLItemDelegate::ColorEntryRole).toInt();

    const QVariant nodePointerVar = index.data(qMRMLItemDelegate::PointerRole);
    ...
    const QString terminologyString(colorNode->GetTerminologyAsString(colorIndex).c_str());

    // Convert string to VTK terminology entry. Do not check success, as an empty terminology is also a valid starting point
    ...
    if (logic)
    {
      ...
      {
        const QStringList lastTerminologyContextNames = settings->value("Terminology/LastTerminologyContexts").toStringList();
        for (auto& name : lastTerminologyContextNames)
        {
          preferredTerminologyNames.push_back(name.toStdString().c_str());
        }
      }
      const std::vector<std::string> preferredAnatomicalContextNames; // use default order for now
    ...
    const bool colorAndNameAutoGenerated = !colorNode->GetColorDefined(colorIndex);
    ...
    qSlicerTerminologyNavigatorWidget::TerminologyInfoBundle terminologyInfo(
      terminologyEntry, colorNode->GetColorName(colorIndex), colorAndNameAutoGenerated, color, colorAndNameAutoGenerated, color);
...
{
  vtkSlicerTerminologiesModuleLogic* const logic = terminologiesLogic();
  if (!logic)
  {
    return;
  }
  ...
  if (colorTableNode != nullptr)
  {
    ...
    colorTableNode->SetColor(colorIndex, terminologyInfo.Color.redF(), terminologyInfo.Color.greenF(), terminologyInfo.Color.blueF(), terminologyInfo.Color.alphaF());
  }
