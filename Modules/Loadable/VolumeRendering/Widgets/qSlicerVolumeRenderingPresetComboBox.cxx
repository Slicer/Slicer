...
void qSlicerVolumeRenderingPresetComboBoxPrivate::init()
{
  ...
  QObject::connect(
    this->PresetComboBox, SIGNAL(currentNodeChanged(vtkMRMLNode*)), q, SIGNAL(currentNodeChanged(vtkMRMLNode*)));
  QObject::connect(
    this->PresetComboBox, SIGNAL(currentNodeIDChanged(QString)), q, SIGNAL(currentNodeIDChanged(QString)));
  ...
  for (int i = 0; i < this->PresetComboBox->nodeCount(); ++i)
  {
    ...
    if (!presetIcon.isNull())
    {
      qMRMLSceneModel* sceneModel =
        qobject_cast<qMRMLSceneModel*>(this->PresetComboBox->sortFilterProxyModel()->sourceModel());
...
void qSlicerVolumeRenderingPresetComboBox::applyPreset(vtkMRMLNode* node)
{
  ...
  if (!presetNode->GetVolumeProperty() || !presetNode->GetVolumeProperty()->GetRGBTransferFunction()
