...
int ComparePiecewiseFunction(vtkPiecewiseFunction* actualFunction, vtkPiecewiseFunction* expectedFunction)
{
  ...
  for (int i = 0; i < expectedFunction->GetSize(); ++i)
  {
    ...
    CHECK_DOUBLE_TOLERANCE(actualValues[0], expectedValues[0], DOUBLE_TOLERANCE);
    CHECK_DOUBLE_TOLERANCE(actualValues[1], expectedValues[1], DOUBLE_TOLERANCE);
    CHECK_DOUBLE_TOLERANCE(actualValues[2], expectedValues[2], DOUBLE_TOLERANCE);
    CHECK_DOUBLE_TOLERANCE(actualValues[3], expectedValues[3], DOUBLE_TOLERANCE);
  }
...
int CompareVolumeProperty(vtkVolumeProperty* actualVolumeProperty, vtkVolumeProperty* expectedVolumeProperty)
{
  ...
  {
    CHECK_DOUBLE_TOLERANCE(actualVolumeProperty->GetIsoSurfaceValues()->GetValue(i),
      expectedVolumeProperty->GetIsoSurfaceValues()->GetValue(i), DOUBLE_TOLERANCE);
  }
  ...
  for (int i = 0; i < VTK_MAX_VRCOMP; ++i)
  {
    ...
    CHECK_EXIT_SUCCESS(ComparePiecewiseFunction(actualVolumeProperty->GetGrayTransferFunction(i), expectedVolumeProperty->GetGrayTransferFunction(i)));
...
int vtkMRMLVolumePropertyJsonStorageNodeTest1(int argc, char* argv[])
{
  ...
  jsonStorageNode->WriteData(expectedVolumePropertyNode);

  vtkNew<vtkMRMLVolumePropertyNode> actualVolumePropertyNode;
  jsonStorageNode->ReadData(actualVolumePropertyNode);
  ...
  CHECK_EXIT_SUCCESS(CompareVolumeProperty(
    actualVolumePropertyNode->GetVolumeProperty(),
    expectedVolumePropertyNode->GetVolumeProperty()));
