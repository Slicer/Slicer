...
public:
  ...
  /// Using vtkVolume::SetProperty(vtkVolumeProperty*) directly would call Modified() on the volume property (due to a
  /// VTK bug), which would make this node "modified since read". Therefore, the .vp file would be also suggested to be
  /// saved (even though it is not actually modified). This method sets the property in the volume with suppressing the
  /// effect of the superfluous modification events.
