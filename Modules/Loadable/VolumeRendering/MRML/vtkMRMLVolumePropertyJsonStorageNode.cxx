...
int vtkMRMLVolumePropertyJsonStorageNode::ReadDataInternal(vtkMRMLNode* refNode)
{
  ...
  {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkMRMLVolumePropertyJsonStorageNode::ReadDataInternal",
      "Reading volume property node file failed: 'volumeProperties' array is empty.");
    return 0;
  }
  ...
  vtkMRMLVolumePropertyNode* volumePropertyNode, vtkMRMLJsonElement* volumePropertyElement)
{
  ...
  vtkSmartPointer<vtkDoubleArray> isoSurfaceValues = vtkSmartPointer<vtkDoubleArray>::Take(
  ...
  if (componentsElement)
  {
    ...
    if (numberOfComponents > VTK_MAX_VRCOMP)
    {
      vtkWarningMacro("vtkMRMLVolumePropertyJsonStorageNode::ReadVolumeProperty: "
        "Number of components exceeds maximum (" << VTK_MAX_VRCOMP << "). "
        "Only the first " << VTK_MAX_VRCOMP << " components will be read.");
    }
