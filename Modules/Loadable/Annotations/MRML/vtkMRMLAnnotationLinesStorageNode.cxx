...
//----------------------------------------------------------------------------
int vtkMRMLAnnotationLinesStorageNode::ReadAnnotationLineDisplayProperties(vtkMRMLAnnotationLineDisplayNode* refNode,
...
//----------------------------------------------------------------------------
int vtkMRMLAnnotationLinesStorageNode::ReadAnnotationLinesData(vtkMRMLAnnotationLinesNode* refNode,
                                                               char line[1024],
                                                               int typeColumn,
                                                               int startIDColumn,
                                                               int endIDColumn,
...
//----------------------------------------------------------------------------
int vtkMRMLAnnotationLinesStorageNode::ReadAnnotationLinesProperties(vtkMRMLAnnotationLinesNode* vtkNotUsed(refNode),
                                                                     char line[1024],
                                                                     int& typeColumn,
                                                                     int& startIDColumn,
                                                                     int& endIDColumn,
                                                                     ...
                                                                     int& numColumns)
{
  ...
  if (lineString.find(preposition + "Columns = ") != std::string::npos)
  {
    ...
    vtkDebugMacro("Got " << numColumns << " columns, type = " << typeColumn << ", startPointID = " << startIDColumn
                         << ",  endPointID = " << endIDColumn << ", sel = " << selColumn << ", vis = " << visColumn);
    return 1;
  }
  return 0;
}
...
int vtkMRMLAnnotationLinesStorageNode::ReadAnnotation(vtkMRMLAnnotationLinesNode* refNode)
{
  ...
  while (fstr.good())
  {
    ...
    if (line[0] == '#')
    {
      if (line[1] == ' ')
      {
        if (!this->ReadAnnotationLinesProperties(refNode,
                                                 line,
                                                 typePointColumn,
                                                 startIDColumn,
                                                 endIDColumn,
    ...
    {
      if (this->ReadAnnotationLinesData(
            refNode, line, typePointColumn, startIDColumn, endIDColumn, selPointColumn, visPointColumn, numPointColumns)
          < 0)
      {
        return 0;
      }
...
//----------------------------------------------------------------------------
int vtkMRMLAnnotationLinesStorageNode::WriteAnnotationLineDisplayProperties(fstream& of,
...
int vtkMRMLAnnotationLinesStorageNode::WriteAnnotationLinesData(fstream& of, vtkMRMLAnnotationLinesNode* refNode)
{
  ...
  for (int i = 0; i < refNode->GetNumberOfLines(); i++)
  {
    ...
    of << this->GetAnnotationStorageType() << "|" << pointIDs[0] << "|" << pointIDs[1] << "|" << sel << "|" << vis
       << endl;
  }
...
int vtkMRMLAnnotationLinesStorageNode::WriteAnnotationDataInternal(vtkMRMLNode* refNode, fstream& of)
{
  ...
  {
    vtkWarningMacro(
      "WriteAnnotationDataInternal: with stream: can't call WriteAnnotationDataInternal on superclass, retval = "
      << retval);
    return 0;
  }
  ...
  if (aNode == nullptr)
  {
    vtkErrorMacro("WriteAnnotationDataInternal: unable to cast input node " << refNode->GetID()
                                                                            << " to a known annotation line node");
    return 0;
  }
