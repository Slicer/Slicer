...
//----------------------------------------------------------------------------
int vtkMRMLAnnotationFiducialsStorageNode::ReadAnnotationFiducialsData(vtkMRMLAnnotationFiducialNode* refNode,
                                                                       char line[1024],
                                                                       int labelColumn,
                                                                       int xColumn,
                                                                       int yColumn,
                                                                       ...
                                                                       int numColumns)
{
  ...
  // newNode->InvokeEvent(vtkMRMLScene::NodeAddedEvent,
  // newCPDisplayNode);//vtkMRMLAnnotationNode::DisplayModifiedEvent);

  return 1;
}

//----------------------------------------------------------------------------
int vtkMRMLAnnotationFiducialsStorageNode::ReadAnnotationFiducialsProperties(vtkMRMLAnnotationFiducialNode* refNode,
                                                                             char line[1024],
                                                                             int& labelColumn,
                                                                             int& xColumn,
                                                                             int& yColumn,
                                                                             ...
                                                                             int& numColumns)
{
  ...
  if (lineString.find(preposition + "Columns = ") != std::string::npos)
  {
    ...
    vtkDebugMacro("Got " << numColumns << " columns, label = " << labelColumn << ", x = " << xColumn << ",  y = "
  ...
  while (fstr.good())
  {
    ...
    if (line[0] == '#')
    {
      if (line[1] == ' ')
      {
        ...
        int retval = this->ReadAnnotationTextProperties(
          refNode, line, typeColumn, annotationColumn, selColumn, visColumn, numColumns);
        if (retval <= 0)
        {
          // it could be defining the fiducial properties
          if (!this->ReadAnnotationFiducialsProperties(refNode,
                                                       line,
                                                       labelColumn,
                                                       xPointColumn,
                                                       yPointColumn,
                                                       ...
                                                       numPointColumns))
          {
            ...
            {
              vtkDebugMacro("ReadAnnotation: read point disp properties: '" << line << "'");
            }
          }
          else
          {
            vtkDebugMacro("ReadAnnotation: read fids properties: '" << line << "'");
          }
        }
        else
        {
          vtkDebugMacro("ReadAnnotation: read annot text properties returned " << retval << " for line: '" << line
                                                                               << "'");
        }
      }
    }
    else
    {
      ...
      {
        // could be point data
        if (this->ReadAnnotationFiducialsData(refNode,
                                              line,
                                              labelColumn,
                                              xPointColumn,
                                              yPointColumn,
      ...
      if (line[1] == ' ')
      {
        ...
        if (retval <= 0)
        {
                                                       ...
                                                       numPointColumns))
          {
            ...
            {
              vtkDebugMacro("ReadOneFiducial: read point disp properties: '" << line << "'");
            }
          }
          else
          {
            vtkDebugMacro("ReadOneFiducial: read fids properties: '" << line << "'");
          }
        }
        else
        {
          vtkDebugMacro("ReadOneFiducial: read annot text properties returned " << retval << " for line: '" << line
                                                                                << "'");
        }
      }
    }
