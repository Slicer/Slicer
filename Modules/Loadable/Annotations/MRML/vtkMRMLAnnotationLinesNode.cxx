...
//---------------------------------------------------------------------------
void vtkMRMLAnnotationLinesNode::ProcessMRMLEvents(vtkObject* caller, unsigned long event, void* callData)
{
  ...
  for (int j = NUM_CP_ATTRIBUTE_TYPES; j < NUM_LINE_ATTRIBUTE_TYPES; j++)
  {
    ...
    if (!dataArray || dataArray->GetSize() != n)
    {
      vtkErrorMacro("Annotation " << this->GetName() << " Attribute " << this->GetAttributeTypesEnumAsString(j)
                                  << " is out of sync with number of lines");
    }
    else
    {
...
vtkMRMLStorageNode* vtkMRMLAnnotationLinesNode::CreateDefaultStorageNode()
{
  ...
  return vtkMRMLStorageNode::SafeDownCast(scene->CreateNodeByClass("vtkMRMLAnnotationLinesStorageNode"));
}
