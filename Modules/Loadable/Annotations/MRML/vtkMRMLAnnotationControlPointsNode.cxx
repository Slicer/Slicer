...
//----------------------------------------------------------------------------
void vtkMRMLAnnotationControlPointsNode::WriteCLI(std::vector<std::string>& commandLine, std::string prefix, int coordinateSystem, int multipleFlag)
{
  ...
  if (this->GetPoints())
  {
    ...
    if (multipleFlag == false && //
        n > 1)
    {
      vtkWarningMacro("WriteCLI - Ignoring 'multipleFlag' and writing all " << n << " points for annotation " << this->GetID() << ". For more details see "
  ...
  for (int j = NUM_TEXT_ATTRIBUTE_TYPES; j < NUM_CP_ATTRIBUTE_TYPES; j++)
  {
    ...
    if (!dataArray || dataArray->GetSize() != n)
    {
      vtkErrorMacro("Annotation " << this->GetName() << " Attribute " << this->GetAttributeTypesEnumAsString(j)
                                  << " is out of sync with PolyData->GetPoints()! Size of Attribute:" << dataArray->GetSize() << " Number of Points: " << n);
    }
    else
    {
...
int vtkMRMLAnnotationControlPointsNode::SetControlPoint(int id, double newControl[3], int selectedFlag, int visibleFlag)
{
  ...
  // cout << "New ControlPoints Coord:" << this->GetControlPointCoordinates(id)[0] << " " << this->GetControlPointCoordinates(id)[1] << " " <<
  // this->GetControlPointCoordinates(id)[2] << endl;
  // vtkIndent blub;
