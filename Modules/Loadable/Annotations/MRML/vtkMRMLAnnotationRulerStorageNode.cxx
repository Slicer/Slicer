                                                                     ...
                                                                     int& numColumns)
{
  ...
  else if (lineString.find(preposition + "Columns = ") != std::string::npos)
  {
    ...
    vtkDebugMacro("Got " << numColumns << " columns, type = " << typeColumn << ", line1ID = " << line1IDColumn
                         << ", sel = " << selColumn << ", vis = " << visColumn);
    // this->DebugOff();
    return 1;
  }
  // this->DebugOff();
...
int vtkMRMLAnnotationRulerStorageNode::ReadAnnotation(vtkMRMLAnnotationRulerNode* refNode)
{
  ...
  while (fstr.good())
  {
    ...
    if ((line[0] == '#') && (line[1] == ' '))
    {
      this->ReadAnnotationRulerProperties(
        refNode, line, typePointColumn, line1IDColumn, selPointColumn, visPointColumn, numPointColumns);
    }
    else
    {
      if (this->ReadAnnotationRulerData(
            refNode, line, typePointColumn, line1IDColumn, selPointColumn, visPointColumn, numPointColumns) < 0)
      {
        return 0;
      }
    }
...
{
  // cast the input node
  vtkMRMLAnnotationRulerNode* aNode = vtkMRMLAnnotationRulerNode::SafeDownCast(refNode);
...
int vtkMRMLAnnotationRulerStorageNode::WriteAnnotationRulerProperties(fstream& of, vtkMRMLAnnotationRulerNode* refNode)
{
  ...
  of << "# " << this->GetAnnotationStorageType()
     << "DistanceAnnotationFormat = " << refNode->GetDistanceAnnotationFormat() << endl;
