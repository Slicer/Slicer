                                                            ...
                                                            vtkMRMLMessageCollection* userMessages /*=nullptr*/)
{
  ...
  if (!storageNode->SupportedFileType(name.c_str()))
  {
    vtkErrorToMessageCollectionMacro(
      userMessages, "vtkSlicerTransformLogic::AddTransform", "Unsupported transform file format: " << filename);
    return nullptr;
  }
  ...
  if (!storageNode->ReadData(generalTransform.GetPointer()))
  {
    ...
    vtkErrorToMessageCollectionMacro(
      userMessages, "vtkSlicerTransformLogic::AddTransform", "Failed to read transform from file: " << filename);
    return nullptr;
  }
                                                         ...
                                                         bool transformToWorld /* = true */)
{
  ...
  vtkSlicerTransformLogic::GetTransformedPointSamples(
    outputPointSet, inputTransformNode, samplePositions_RAS.GetPointer(), transformToWorld);
}
                                                         ...
                                                         bool transformToWorld /* = true */)
{
  ...
  for (int sampleIndex = 0; sampleIndex < numOfSamples; sampleIndex++)
  {
    ...
    sampleVectors_RAS->SetTuple3(
      sampleIndex, pointDislocationVector_RAS[0], pointDislocationVector_RAS[1], pointDislocationVector_RAS[2]);
  }
...
/// pointGroupSize: the number of points will be N*pointGroupSize (the actual number will be returned in
/// numGridPoints[3])
//----------------------------------------------------------------------------
                                                                ...
                                                                vtkPoints* samplePositions_RAS /*=nullptr*/)
{
  ...
  else
  {
    ...
    double xOfs =
      (fieldOfViewSize[0] - (numOfPointsX * pointSpacing)) / 2 - fieldOfViewSize[0] / 2 + fieldOfViewOrigin[0];
    double yOfs =
      (fieldOfViewSize[1] - (numOfPointsY * pointSpacing)) / 2 - fieldOfViewSize[1] / 2 + fieldOfViewOrigin[1];
  ...
  for (int i = 0; i < numOfTuples; i++)
  {
    ...
    dot = chosenVector[0] * sliceNormal_RAS[0] + chosenVector[1] * sliceNormal_RAS[1]
          + chosenVector[2] * sliceNormal_RAS[2];
  ...
  outputPointSet->GetPointData()->SetActiveAttribute(GetVisualizationDisplacementMagnitudeScalarName(),
  ...
  for (point_IJK[2] = extent[4]; point_IJK[2] <= extent[5]; point_IJK[2]++)
  {
    for (point_IJK[1] = extent[2]; point_IJK[1] <= extent[3]; point_IJK[1]++)
    {
      for (point_IJK[0] = extent[0]; point_IJK[0] <= extent[1]; point_IJK[0]++)
      {
        ...
        float mag = sqrt(pointDislocationVector_RAS[0] * pointDislocationVector_RAS[0]
...
//----------------------------------------------------------------------------
vtkMRMLVolumeNode* vtkSlicerTransformLogic::CreateDisplacementVolumeFromTransform(
  vtkMRMLTransformNode* inputTransformNode,
  ...
  vtkMRMLVolumeNode* existingOutputVolumeNode /* = nullptr */)
{
  ...
  if ((referenceVolumeNode == nullptr || referenceVolumeNode->GetImageData() == nullptr)
      && (existingOutputVolumeNode == nullptr))
  {
  ...
  if (referenceVolumeNode != nullptr)
  {
    ...
    if (vtkMRMLTransformNode::GetMatrixTransformBetweenNodes(
          referenceVolumeNode->GetParentTransformNode(), nullptr /* world */, rasToWorld.GetPointer()))
    {
      vtkMatrix4x4::Multiply4x4(rasToWorld.GetPointer(), ijkToRas.GetPointer(), ijkToRas.GetPointer());
    }
    else
    {
      vtkErrorMacro("vtkSlicerTransformLogic::CreateDisplacementVolumeFromTransform: non-linearly transformed "
                    "reference volume is not supported."
  ...
  if (magnitude)
  {
    vtkSlicerTransformLogic::GetTransformedPointSamplesAsMagnitudeImage(
      outputVolume, inputTransformNode, ijkToRas.GetPointer());
    outputVolumeNode->SetVoxelVectorType(vtkMRMLVolumeNode::VoxelVectorTypeUndefined);
  }
  else
  {
    vtkSlicerTransformLogic::GetTransformedPointSamplesAsVectorImage(
      outputVolume, inputTransformNode, ijkToRas.GetPointer());
...
//----------------------------------------------------------------------------
vtkMRMLTransformNode* vtkSlicerTransformLogic::ConvertToGridTransform(
  vtkMRMLTransformNode* inputTransformNode,
  ...
  vtkMRMLTransformNode* existingOutputTransformNode /* = nullptr */)
{
  ...
  if ((referenceVolumeNode == nullptr || referenceVolumeNode->GetImageData() == nullptr)
      && (existingOutputTransformNode == nullptr))
  {
  ...
  if (referenceVolumeNode != nullptr)
  {
    ...
    for (int c = 0; c < 3; c++)
    {
      ...
      spacing[c] =
        sqrt(ijkToRas->Element[0][c] * ijkToRas->Element[0][c] + ijkToRas->Element[1][c] * ijkToRas->Element[1][c]
  ...
  vtkSlicerTransformLogic::GetTransformedPointSamplesAsVectorImage(
    outputVolume, inputTransformNode, ijkToRas.GetPointer(), transformToWorld);
...
{
  double roiSpacing[3] = {
    sqrt(roiToRAS->Element[0][0] * roiToRAS->Element[0][0] + roiToRAS->Element[1][0] * roiToRAS->Element[1][0]
         + roiToRAS->Element[2][0] * roiToRAS->Element[2][0]),
    sqrt(roiToRAS->Element[0][1] * roiToRAS->Element[0][1] + roiToRAS->Element[1][1] * roiToRAS->Element[1][1]
         + roiToRAS->Element[2][1] * roiToRAS->Element[2][1]),
    sqrt(roiToRAS->Element[0][2] * roiToRAS->Element[0][2] + roiToRAS->Element[1][2] * roiToRAS->Element[1][2]
         + roiToRAS->Element[2][2] * roiToRAS->Element[2][2])
  };
                                                      ...
                                                      vtkPoints* samplePositions_RAS)
{
  ...
  else
  {
    GetTransformedPointSamplesOnRoi(
      pointSet.GetPointer(), inputTransformNode, roiToRAS, roiSize, displayNode->GetGlyphSpacingMm());
  }
                                                      ...
                                                      vtkPoints* samplePositions_RAS)
{
  ...
  vtkMRMLTransformNode* inputTransformNode = vtkMRMLTransformNode::SafeDownCast(displayNode->GetDisplayableNode());
  vtkSlicerTransformLogic::GetTransformedPointSamplesOnSlice(pointSet.GetPointer(),
                                                             inputTransformNode,
                                                             sliceToRAS,
                                                             fieldOfViewOrigin,
                                                             fieldOfViewSize,
                                                             displayNode->GetGlyphSpacingMm(),
                                                             1,
                                                             nullptr,
                                                             samplePositions_RAS);
  ...
  switch (displayNode->GetGlyphType())
  {
    ...
    case vtkMRMLTransformDisplayNode::GLYPH_TYPE_SPHERE:
      ...
      break;
    default:
      glyph2DSource->SetGlyphTypeToNone();
  }
...
void vtkSlicerTransformLogic::CreateGrid(vtkPolyData* gridPolyData,
                                         vtkMRMLTransformDisplayNode* displayNode,
                                         int numGridPoints[3],
                                         vtkPolyData* warpedGrid /*=nullptr*/)
{
  ...
  if (warpedGrid)
  {
    ...
    int idx = polyoutput->GetPointData()->AddArray(
      gridPolyData->GetPointData()->GetArray(GetVisualizationDisplacementMagnitudeScalarName()));
                                                     ...
                                                     double* fieldOfViewSize)
{
  ...
  GetTransformedPointSamplesOnSlice(gridPolyData.GetPointer(),
                                    transformNode,
                                    sliceToRAS,
                                    fieldOfViewOrigin,
                                    fieldOfViewSize,
                                    pointSpacing,
                                    GetGridSubdivision(displayNode),
                                    numGridPoints);
  ...
  if (displayNode->GetGridShowNonWarped())
  {
    ...
    vtkDataArray* displacementMagnitudeScalars =
      gridPolyData->GetPointData()->GetArray(GetVisualizationDisplacementMagnitudeScalarName());
    vtkSmartPointer<vtkDataArray> zeroDisplacementMagnitudeScalars =
      vtkSmartPointer<vtkDataArray>::Take(displacementMagnitudeScalars->NewInstance());
    ...
    output->GetPointData()->SetActiveAttribute(GetVisualizationDisplacementMagnitudeScalarName(),
                                               vtkDataSetAttributes::SCALARS);
  }
  else
  {
...
//----------------------------------------------------------------------------
void vtkSlicerTransformLogic::GetGridVisualization3d(vtkPolyData* output,
                                                     vtkMRMLTransformDisplayNode* displayNode,
                                                     vtkMatrix4x4* roiToRAS,
                                                     int* roiSize)
{
  ...
  vtkNew<vtkPolyData> gridPolyData;
  vtkMRMLTransformNode* inputTransformNode = vtkMRMLTransformNode::SafeDownCast(displayNode->GetDisplayableNode());
  GetTransformedPointSamplesOnRoi(gridPolyData.GetPointer(),
                                  inputTransformNode,
                                  roiToRAS,
                                  roiSize,
                                  pointSpacing,
...
//----------------------------------------------------------------------------
void vtkSlicerTransformLogic::GetContourVisualization3d(vtkPolyData* output,
                                                        vtkMRMLTransformDisplayNode* displayNode,
                                                        vtkMatrix4x4* roiToRAS,
                                                        int* roiSize)
{
...
bool vtkSlicerTransformLogic::GetVisualization2d(vtkPolyData* output,
                                                 ...
                                                 vtkPoints* samplePositions_RAS /*=nullptr*/)
{
  if (displayNode == nullptr || output == nullptr || sliceToRAS == nullptr || fieldOfViewOrigin == nullptr
      || fieldOfViewSize == nullptr)
  {
    return false;
  }
                                                 ...
                                                 vtkMRMLMarkupsNode* glyphPointsNode /*=nullptr*/)
{
  ...
  else if (markupsRoiNode != nullptr)
  {
    ...
    double roiCorner_Object[4] = {
      -roiDiameter_Object[0] / 2.0, -roiDiameter_Object[1] / 2.0, -roiDiameter_Object[2] / 2.0, 1.0
    };
    double roiCorner_World[4] = { 0.0, 0.0, 0.0, 1.0 };
  ...
  vtkSlicerTransformLogic::GetVisualization3d(
    output, displayNode, ijkToRAS.GetPointer(), regionSize_IJK, samplePoints_RAS);
  return true;
}
...
//----------------------------------------------------------------------------
void vtkSlicerTransformLogic::GetTransformedNodes(vtkMRMLScene* scene,
                                                  ...
                                                  bool recursive)
{
  ...
  for (size_t i = 0; i < nodes.size(); ++i)
  {
    ...
    if (node->GetTransformNodeID() && strcmp(transformNode->GetID(), node->GetTransformNodeID()) == 0)
    {
      transformedNodes.push_back(node);

      vtkMRMLTransformNode* childTransformNode = vtkMRMLTransformNode::SafeDownCast(node);
...
void vtkSlicerTransformLogic::GetNodesRASBounds(const std::vector<vtkMRMLDisplayableNode*>& nodes, double bounds[6])
{
  ...
  for (size_t i = 0; i < nodes.size(); ++i)
  {
    ...
    if (nodeBounds[0] <= nodeBounds[1] || nodeBounds[2] <= nodeBounds[3] || nodeBounds[4] <= nodeBounds[5])
    {
