...
const std::string VOLUME_DISPLAY_PRESETS_SCHEMA =
  ...
  "volumes-display-presets-schema-v1.0.1.json#";
...
void vtkSlicerVolumesLogic::InitializeDefaultVolumeDisplayPresets()
{
  ...
  {
    vtkErrorMacro(<< errorPrefix << " File is expected to contain $schema: " << VOLUME_DISPLAY_PRESETS_SCHEMA
       ...
       ++presetIt)
  {
    ...
    if (!preset["name"].IsString() || !preset["id"].IsString() || !preset["window"].IsNumber()
        || !preset["level"].IsNumber())
    {
      vtkErrorMacro(<< errorPrefix << " Error reading preset " << presetIndex
    ...
    if (!preset["name"].IsString() || !preset["id"].IsString() || !preset["window"].IsNumber()
        || !preset["level"].IsNumber() || !preset["color"].IsString())
    {
      vtkErrorMacro(<< errorPrefix << " Error reading preset " << presetIndex
                    << ". Wrong type for property name, id, window, or level.");
      continue;
