...
void AddVoxels(T* Ptr, vtkIdType inInc[3], int Dim[3], int CentroidFlag, double & result, double *centroid) {
  ...
  for (int z = 0; z < Dim[2]; z++) {
    for (int y = 0; y < Dim[1]; y++) {
      ...
      double centroidSlice = 0.0;
      for (int x = 0; x < Dim[0]; x++) {
    if (*Ptr) {
      resultSlice += *Ptr;
      if (CentroidFlag) centroidSlice += x* (*Ptr);
    }
    Ptr ++;
      ...
      result += resultSlice;
      if (CentroidFlag) {
    centroid[0] += centroidSlice;
    centroid[1] += resultSlice*y; 
    centroid[2] += resultSlice*z; 
      }
  ...
  if (CentroidFlag) {
    centroid[0] = centroid[0] / result;
    centroid[1] = centroid[1] / result;
    centroid[2] = centroid[2] / result;
  }
