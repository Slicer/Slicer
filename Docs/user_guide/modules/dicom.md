...
**Definition**
> A **reverse proxy** is an HTTPS front end (e.g., NGINX, Apache, Kong) placed **in front of** the PACS/VNA that terminates TLS, can perform **OIDC/OAuth2** with the hospital IdP, and forwards authorized **DICOMweb** requests to the internal service—so **Slicer (on-prem or in cloud) connects over HTTPS to a single URL** while the **PACS/VNA stays isolated on its private network/VPC** (reachable via private connectivity or a tightly restricted public endpoint).
