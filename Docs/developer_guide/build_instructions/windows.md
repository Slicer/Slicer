...
- [Qt5](https://www.qt.io/download-open-source): Download the Qt universal installer and run the tool. On the "Customize" page of the Qt Maintenance Tool, expand the "Qt" tree. Then expand the tree matching the latest version of Qt 6.9 (e.g. Qt 6.9.3). Select `MSVC 2022 64-bit`. Selecting `Sources` and `Qt Debug Information Files` are recommended for debugging (they allow stepping into Qt files with the debugger in debug-mode builds). Expand the "Additional Libraries" tree and select `Qt 5 Compatibility module`, `Qt Multimedia`, `Qt Positioning`, `Qt Shader Tools`, `Qt State Machines`, `Qt WebChannel`. Collapse the "Qt" tree. Expand the "Extensions" tree, then expand "Qt WebEngine" and finally expand the corresponding tree matching the latest version of Qt 6.9 (e.g. 6.9.3). Select `MSVC 2022 x64`.
