...
- Due to maximum path length limitations during build the build process, build folders must be located in a folder with very short total path length. This is expecially critical on Windows and macOS. For example, `/opt/s` has been confirmed to work on macOS.
